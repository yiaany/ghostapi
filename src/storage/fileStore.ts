...
export async function ensurePrivateDirectory(directoryPath: string): Promise<void> {
  ...
  if (info.isSymbolicLink()) {
    throw new Error(`Refusing to use symbolic-link data directory: ${directoryPath}`);
  }
  if (process.platform !== "win32") await chmod(directoryPath, 0o700);
}

export async function readJsonFile<T>(filePath: string, fallback: T, sanitize: (value: unknown) => T): Promise<T> {
  try {
...
export async function withFileLock<T>(filePath: string, operation: () => Promise<T>, options: FileStoreOptions = {}): Promise<T> {
...
async function reclaimAbandonedLock(lockPath: string, staleLockMs: number): Promise<void> {
  try {
    ...
    try {
      ...
      pid = typeof metadata.pid === "number" && Number.isInteger(metadata.pid) ? metadata.pid : null;
    } catch (error) {
...
function isTransientLockError(error: unknown): boolean {
  return isErrorCode(error, "EPERM") || isErrorCode(error, "EACCES") || isErrorCode(error, "EBUSY");
}
