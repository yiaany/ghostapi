...
const SCENARIOS: ScenarioPreset[] = [
  {
    ...
    provider: "stripe",
    description:
      "Returns a stable customer object for local onboarding and billing flows.",
    steps: [
      {
        ...
        body: {
          ...
          livemode: false,
          metadata: { scenario: "stripe-customer-create" },
        },
      },
    ],
  },
  {
    ...
    provider: "stripe",
    description:
      "Forces a card-decline shaped Stripe error so agents must implement failure handling.",
    steps: [
      {
  ...
  ...getProviderScenarios(),
  {
    ...
    provider: "github",
    description:
      "Returns a deterministic issue payload for GitHub automation tests.",
    steps: [
      {
        name: "Create issue",
        method: "POST",
        ...
        body: {
          id: 10001,
          number: 42,
          title: "GhostAPI local issue",
          state: "open",
          html_url: "https://github.local/ghostapi/demo/issues/42",
        },
      },
    ],
  },
...
export async function getScenarioPreset(
  id: string,
): Promise<ScenarioPreset | null> {
...
export async function shareScenario(
  id: string,
): Promise<{ scenario: ScenarioPreset; shareText: string }> {
  const scenario = await exportScenario(id);
  const encoded = Buffer.from(JSON.stringify(scenario), "utf8").toString(
    "base64url",
  );
  return {
    scenario,
...
export async function saveEventsAsScenario(
  events: ProxyEvent[],
  input: { title?: string; id?: string } = {},
): Promise<ScenarioPreset> {
  const title =
    input.title?.trim() || `Traffic scenario ${new Date().toISOString()}`;
  ...
  const scenario = normalizeScenario({
    ...
    steps: events
      .filter((event) => event.source !== "fault")
      .slice(-20)
      .map((event, index) => ({
  ...
  try {
    ...
    for (const entry of entries) {
      if (entry.isFile() && entry.name.endsWith(".json")) {
        scenarios.push(
          normalizeScenario(
            JSON.parse(await readFile(join(scenariosDir, entry.name), "utf8")),
          ),
        );
      }
    }
...
async function readCustomScenario(id: string): Promise<ScenarioPreset | null> {
  try {
    return normalizeScenario(
      JSON.parse(
        await readFile(
          join(getDataPaths().scenarios, `${slugify(id)}.json`),
          "utf8",
        ),
      ),
    );
...
async function writeScenario(scenario: ScenarioPreset): Promise<void> {
  ...
  if (bytes > MAX_SCENARIO_BYTES)
    throw new Error("Scenario exceeds its size limit.");
  const directory = getDataPaths().scenarios;
  await ensurePrivateDirectory(directory);
  await withFileLock(
    join(directory, ".quota"),
    async () => {
      ...
      if (
        existing.length >= MAX_CUSTOM_SCENARIOS ||
        existing.reduce((total, entry) => total + entry.size, bytes) >
          MAX_SCENARIO_STORE_BYTES
      )
        throw new Error("Custom scenario persistence quota was reached.");
...
function normalizeScenario(input: unknown): ScenarioPreset {
  ...
  if (
    ...
    throw new Error("Scenario title is invalid.");
  if (
    !Array.isArray(record.steps) ||
    record.steps.length === 0 ||
    record.steps.length > 100
  )
    throw new Error("Scenario steps must contain 1-100 entries.");
  const id =
...
function normalizeStep(input: unknown): ScenarioStep {
  ...
  if (!isScenarioMethod(method))
    throw new Error("Scenario step method is invalid.");
  const path = String(record.path ?? "");
  if (!path.startsWith("/") || path.length > 2048 || /[\r\n\0]/.test(path))
    throw new Error("Scenario step path is invalid.");
  const status = Number(record.status);
  if (!Number.isInteger(status) || status < 100 || status > 599)
    throw new Error("Scenario step status must be 100-599.");
  const name =
    typeof record.name === "string" ? record.name : `${method} ${path}`;
  if (name.length === 0 || name.length > 200 || /[\r\n\0]/.test(name))
    throw new Error("Scenario step name is invalid.");
  return {
    name,
    method,
    path,
...
function readHeaders(value: unknown): Record<string, string> | undefined {
  ...
  if (Object.keys(raw).length !== Object.keys(safe).length)
    throw new Error(
      "Scenario response headers contain an unsafe or unsupported header.",
    );
  return safe;
}
