...
export async function authenticateIngestKey(
  ...
  if (
    record === undefined ||
    !constantTimeHexEqual(record.secret_sha256, await sha256(token))
  )
    throw new ReportAuthenticationError("Invalid ingest credential.");
  return record.id;
}

export async function markIngestKeyUsed(
  primary: Pick<Pool, "query">,
  projectId: string,
  keyId: string,
): Promise<void> {
  await primary.query(
    "update app.ci_ingest_keys set last_used_at = now() where id = $1 and project_id = $2",
    [keyId, projectId],
  );
}
...
export async function acceptReport(
  ...
  if (ledger.rows[0] === undefined) {
    ...
    if (!constantTimeHexEqual(record.request_sha256, requestHash))
  ...
  if (organizationId === null)
    throw new ReportInputError("Project is unavailable.");
  ...
  await client.query(
    ...
    [
      ...
      input.report.runId,
      requestHash,
      JSON.stringify(input.report.payload),
    ],
  );
  ...
  await client.query(
    ...
    [
      eventId,
      reportId,
      JSON.stringify({ schemaVersion: 1, eventId, reportId }),
    ],
  );
...
export function validateReport(value: ReportPayload): void {
  if (
    ...
    !isIdentifier(value.runId, 128) ||
    !isPlainObject(value.payload)
  )
    throw new ReportInputError("Report payload is invalid.");
  rejectSensitiveValue(value.payload, 0);
  const encoded = stableJson(value);
  if (Buffer.byteLength(encoded, "utf8") > MAX_REPORT_BYTES)
    throw new ReportInputError(
      `Report payload exceeds ${MAX_REPORT_BYTES} bytes.`,
    );
}
...
function validateIdempotencyKey(value: string): void {
  if (!/^[A-Za-z0-9._:-]{16,256}$/.test(value))
    throw new ReportInputError(
      "Idempotency-Key must contain 16-256 safe characters.",
    );
}
...
function rejectSensitiveValue(
  value: unknown,
  depth: number,
  key?: string,
): void {
  if (depth > 8)
    throw new ReportInputError("Report payload is too deeply nested.");
  if (
    key !== undefined &&
    [
      "authorization",
      ...
      "secret",
      "token",
    ].some((name) => key.toLowerCase().includes(name))
  )
    throw new ReportInputError("Report payload contains an unsafe field.");
  if (typeof value === "string") {
    if (
      value.length > 64 * 1024 ||
      /(?:sk_live_|sk_test_|ghp_|xox[baprs]-|Bearer\s+[A-Za-z0-9._~+/-]{12,})/i.test(
        value,
      )
    )
      throw new ReportInputError(
        "Report payload contains a secret-shaped value.",
      );
    return;
  }
  ...
  if (Array.isArray(value)) {
    if (value.length > 1_000)
      throw new ReportInputError("Report payload contains too many entries.");
    for (const entry of value) rejectSensitiveValue(entry, depth + 1, key);
    return;
  }
  if (!isPlainObject(value))
    throw new ReportInputError("Report payload must contain JSON values only.");
  const entries = Object.entries(value);
  if (entries.length > 1_000)
    throw new ReportInputError("Report payload contains too many fields.");
  for (const [childKey, child] of entries)
    rejectSensitiveValue(child, depth + 1, childKey);
}
