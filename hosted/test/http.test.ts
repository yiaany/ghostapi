    ...
    auth: {
      ...
      api: {
        getSession: async ({ headers }: { headers: Headers }) =>
          headers.get("authorization") === "Session member"
            ? { user: { id: "member", email: "member@example.test" } }
            : null,
      },
    } as never,
    queue: {
      receiver: { verify: async () => true },
      client: {},
      configured: true,
    } as never,
    rateLimiter: {
...
test("rejects a chunked oversized report before authentication or database access", async () => {
  ...
  const response = await app.handle(
    new Request("https://api.example.test/v1/projects/project/reports", {
      method: "POST",
...
test("returns tenant-safe not-found for unauthenticated and cross-tenant report reads", async () => {
  ...
  const app = createHostedApp(
    config,
    dependencies(async (_text, values) => {
      calls.push(values ?? []);
      return { rows: [] };
    }),
  );
  const unauthenticated = await app.handle(
    new Request("https://api.example.test/v1/reports/report-a"),
  );
...
test("enforces CSRF origin and developer role on scenario writes", async () => {
  ...
  const badOrigin = await app.handle(
    new Request("https://api.example.test/v1/projects/project/scenarios/test", {
      method: "POST",
      headers: {
...
test("rejects an oversized chunked schema route before JSON parsing", async () => {
  ...
  const stream = new ReadableStream<Uint8Array>({
    start(controller) {
      controller.enqueue(
        new TextEncoder().encode('{"version":1,"definition":{"value":"'),
      );
...
test("sets security headers on health responses", async () => {
  const response = await createHostedApp(config, dependencies()).handle(
    new Request("https://api.example.test/healthz"),
  );
...
test("passes a materialized bounded request to auth handlers that clone requests", async () => {
  ...
  (
    deps.auth as unknown as { handler: (request: Request) => Promise<Response> }
  ).handler = async (request) => Response.json(await request.clone().json());
  const response = await createHostedApp(config, deps).handle(
    new Request("https://api.example.test/api/auth/test", {
      method: "POST",
      headers: {
        "content-type": "application/json",
        origin: "https://app.example.test",
...
test("rate limits organization creation before authentication or database access", async () => {
  ...
  (
    deps.auth as unknown as { api: { getSession: () => Promise<null> } }
  ).api.getSession = async () => {
    sessions += 1;
    return null;
  };
  deps.rateLimiter.consume = async () => {
    throw new (await import("../src/rateLimit.js")).RateLimitError(60);
  };
  const response = await createHostedApp(config, deps).handle(
