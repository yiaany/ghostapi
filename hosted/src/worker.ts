...
import { isObject, ScenarioDefinitionError, validatePath, validateScenarioDefinition } from "./scenario.js";
...
export async function dispatchOutbox(database: Database, queue: ReturnType<typeof createQueue>, callbackUrl: string, maxAttempts: number, workerMaxAttempts: number, limit = 100): Promise<number> {
  const events = await database.transaction(async (client) => {
    ...
    const claimed = await client.query<{ id: string; payload: ReportEvent; attempts: number }>(
      `with picked as (
    ...
    try {
      ...
      await database.primary.query(
        `update app.outbox_events
          ...
          where id = $1`,
        [event.id, exhausted ? "dead_letter" : "pending", safeFailureCode(error)]
      );
      if (exhausted) {
        await database.primary.query(
...
export async function processReportEvent(database: Database, event: ReportEvent, maxAttempts = 5): Promise<"completed" | "duplicate" | "dead_letter"> {
  if (event.schemaVersion !== 1 || !isUuid(event.eventId) || !isUuid(event.reportId)) throw new Error("Invalid queue event.");
  const claimed = await database.transaction(async (client) => claimReport(client, event, maxAttempts));
  if (claimed !== "claimed") return claimed;

  try {
    await database.transaction(async (client) => {
      const report = await client.query<{ project_id: string; payload: Record<string, unknown> }>(
      ...
      for (const scenario of scenarios.rows) {
        ...
        await client.query(
          `insert into app.scenario_run_results (report_id, scenario_version_id, status, result)
           ...
           on conflict (report_id, scenario_version_id) do update set status = excluded.status, result = excluded.result, completed_at = now()`,
          [event.reportId, scenario.id, result.passed ? "passed" : "failed", JSON.stringify(result)]
        );
      }
      await client.query("update app.reports set status = 'completed', completed_at = now(), failure_code = null where id = $1", [event.reportId]);
      await client.query(
...
async function claimReport(client: PoolClient, event: ReportEvent, maxAttempts: number): Promise<"claimed" | "duplicate" | "dead_letter"> {
  ...
  if (outbox.rows[0]?.aggregate_id !== event.reportId) throw new Error("queue_event_mismatch");
  ...
  if (receipt.rows[0] === undefined) {
    const existing = await client.query<{ status: "processing" | "completed" | "failed" | "dead_letter"; attempts: number }>(
      "select status, attempts from app.job_receipts where event_id = $1 and handler = 'process-report-v1' for update",
      [event.eventId]
    );
    if (existing.rows[0]?.status === "completed" || existing.rows[0]?.status === "dead_letter") return "duplicate";
    if (existing.rows[0]?.status === "failed" && existing.rows[0].attempts >= maxAttempts) {
      await client.query(
      ...
      await client.query("update app.reports set status = 'failed', completed_at = now(), failure_code = 'worker_attempts_exhausted' where id = $1", [event.reportId]);
      return "dead_letter";
    }
    throw new JobLeaseActiveError();
  }
  await client.query("update app.reports set status = 'processing', failure_code = null where id = $1 and status in ('accepted', 'failed')", [event.reportId]);
  return "claimed";
}

export function evaluateScenario(definition: unknown, payload: Record<string, unknown>): { passed: boolean; assertions: Array<{ path: string; passed: boolean }> } | null {
  validateScenarioDefinition(definition);
  const when = definition.when ?? {};
  for (const [path, expected] of Object.entries(when)) if (!deepEqual(readPath(payload, path), expected)) return null;
  const assertions = definition.assertions ?? [];
  const results = assertions.map((assertion) => {
    const actual = readPath(payload, assertion.path);
    const passed = "equals" in assertion ? deepEqual(actual, assertion.equals) : assertion.exists === true ? actual !== undefined : false;
...
async function receiptAttempts(database: Database, eventId: string): Promise<number> {
...
function isRetryableFailure(error: unknown): boolean {
  ...
  return error instanceof Error && !["queue_event_mismatch", "report_not_processable"].includes(error.message);
}

function safeFailureCode(error: unknown): string {
  const value = error instanceof Error ? error.message : "unknown_error";
  return /^[a-z0-9_:-]{1,120}$/i.test(value) ? value.toLowerCase() : "processing_error";
}
