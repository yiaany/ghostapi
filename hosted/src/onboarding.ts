...
export async function createOrganization(
  ...
  await client.query(
    "insert into app.organization_memberships (organization_id, user_id, role) values ($1, $2, 'owner')",
...
export async function createInvitation(
  ...
  if (!(await lockOrganization(client, input.organizationId)))
...
export async function acceptInvitation(
  ...
  if (
    record === undefined ||
    record.email.toLowerCase() !== input.userEmail.toLowerCase()
  )
    return null;
  ...
  await client.query(
    "update app.organization_invitations set accepted_at = now(), accepted_by = $2 where id = $1",
    [record.id, input.userId],
  );
  await audit(
    client,
