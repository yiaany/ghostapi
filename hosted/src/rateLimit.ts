...
export function createRateLimiter(config: HostedConfig) {
  ...
  return {
    ...
    async consume(key: string, limit: number, windowMs: number): Promise<void> {
      if (!/^[a-z0-9:_-]{1,160}$/.test(key) || !Number.isInteger(limit) || limit < 1 || !Number.isInteger(windowMs) || windowMs < 1_000) throw new Error("Rate limit input is invalid.");
      try {
        const [count, ttl] = await increment.exec([key], [String(windowMs)]);
        if (count > limit) throw new RateLimitError(Math.max(1, Math.ceil(Math.max(0, ttl) / 1_000)));
      } catch (error) {
