...
const resendConformanceFixtures: readonly ProviderConformanceFixture[] = [
  {
    ...
    assertResponse(response) {
      if (
        ...
        !response.body.id.startsWith("email_mock_")
      ) {
    ...
    assertStateTransition(transition, response) {
      const id =
        isJsonObject(response.body) && typeof response.body.id === "string"
          ? response.body.id
          : "";
      if (
        transition === null ||
        transition.key !== `resend:${id}` ||
...
export const resendPack: ProviderPack = {
  ...
  manifest: {
    ...
    packVersion: "1.0.0",
    apiVersions: {
      default: RESEND_API_VERSION,
      supported: [RESEND_API_VERSION],
    },
    capabilities: {
      detection: true,
      requestParsing: true,
  ...
  selectApiVersion(request) {
    const requested =
      firstHeaderValue(request.headers["x-ghostapi-api-version"]) ??
      RESEND_API_VERSION;
  ...
  validate(parsedRequest, request) {
    if (
      request.method !== "POST" ||
      (request.path !== "/emails" && !request.path.startsWith("/emails/"))
    )
      return null;
