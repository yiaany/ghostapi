...
describe("local action ledger and incident replay", () => {
  it("chains action intent, approval, outcome, and verification while exporting only the tenant scope", async () => {
    ...
    const tenantB = access.issue({
      tenantId: "tenant-b",
      principalId: "auditor-b",
    ...
    await ledger.recordAction(
      tenantB,
      await gateway.inspect("ledger-action-b"),
    );
    ...
    expect(
      timeline.find((entry) => entry.stage === "verification")?.data.status,
    ).toBe("verified");
    ...
    expect(JSON.stringify(exported)).not.toMatch(
      /authorization|cookie|token|secret|email|phone|address|sk_live_/i,
    );
  });

  it("detects modified or deleted ledger entries", async () => {
    await createWorld({
      id: "corrupt-ledger-world",
      seed: "corrupt-ledger-seed",
    });
    const gateway = createGateway();
    const action = actionEnvelope(
      "corrupt-ledger-action",
      "corrupt-ledger-world",
    );
    await gateway.submit(action, approvalFor(action), policy());
    const access = createTestLedgerAccessAuthorizer();
    ...
    await ledger.recordAction(
      capability,
      await gateway.inspect("corrupt-ledger-action"),
    );
    ...
    expect(await ledger.verifyTenant(capability)).toMatchObject({
      valid: false,
    });

    await writeFile(
      path,
      JSON.stringify({ ...modified, entries: modified.entries.slice(1) }),
      "utf8",
    );
    expect(await ledger.verifyTenant(capability)).toMatchObject({
      valid: false,
  ...
  it("turns an ambiguous action into a local deterministic regression fixture without network or credentials", async () => {
    ...
    const capability = access.issue({
      ...
      principalId: "investigator",
      permissions: ["append", "read"],
    });
    const ledger = createLedger(access.authorizer, "incident-ledger.json");
    await ledger.recordAction(
      capability,
      await gateway.inspect("incident-action"),
    );

    const incident = await ledger.createIncidentFixture(
      capability,
      "incident-action",
    );
    const replay = await ledger.replayIncidentFixture(
      capability,
      incident.fixture,
    );
    expect(incident.fixture.expected).toEqual({
      status: 409,
    ...
    expect(JSON.stringify(incident.fixture)).not.toMatch(
      /authorization|cookie|token|secret|email|phone|address|sk_live_/i,
    );
  });
  ...
  it("keeps deletion a recorded request, blocks it under a local legal hold, and does not silently prune history", async () => {
    ...
    await expect(ledger.requestDeletion(capability)).rejects.toThrow(
      "legal hold",
    );
    ...
    expect(
      (await ledger.exportTenant(capability)).entries.map(
        (entry) => entry.stage,
      ),
    ).toEqual(["retention", "legal_hold", "legal_hold", "deletion_request"]);
  });
  ...
  it("marks policy and approval echoes as caller-claimed and reports untracked tenants as not tracked", async () => {
    ...
    await ledger.recordAction(
      capability,
      await gateway.inspect("basis-action"),
    );
    const timeline = await ledger.timeline(capability, "basis-action");
    expect(
      timeline.find((entry) => entry.stage === "policy_decision")?.data.basis,
    ).toBe("caller_claimed");
    expect(
      timeline.find((entry) => entry.stage === "approval")?.data.basis,
    ).toBe("caller_claimed");
    expect(await ledger.verifyTenant(capability)).toMatchObject({
      valid: true,
      tracked: true,
    ...
    expect(
      ...
      valid: true,
      entryCount: 0,
      headHash: sha256("ghostapi.action-ledger.v1:tenant-unknown"),
      tracked: false,
    });
  });
  ...
  it("rotates expired tenant history at the entry limit and relinks the surviving chain without breaking integrity", async () => {
    ...
    await ledger.recordAction(
      capability,
      await gateway.inspect("rotation-action"),
    );
    ...
    expect(
      exported.entries.some((entry) => entry.actionId === "seed-action-1"),
    ).toBe(false);
    expect(
      exported.entries.some((entry) => entry.actionId === "seed-action-2000"),
    ).toBe(true);
  });
});
...
function seedLedgerState(tenantId: string, count: number, oldCount: number) {
  ...
  for (let index = 1; index <= count; index += 1) {
    const base = {
      ...
      actionId: `seed-action-${index}`,
      actionHash: "c".repeat(64),
      stage: "intent" as const,
      data: { operation: "synthetic.op" },
      previousHash,
    };
