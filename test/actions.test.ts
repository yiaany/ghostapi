...
describe("synthetic production action gateway", () => {
  it("binds approval to canonical arguments, executes synthetic state exactly once, and writes a receipt chain", async () => {
    ...
    expect(stored.receipts.map((entry) => entry.status)).toEqual([
      "requested",
      "attempted",
      "committed",
      "verified",
    ...
    expect(
      stored.receipts.every(
        (entry, index) =>
          index === 0 ||
          entry.previousReceiptHash === stored.receipts[index - 1]?.receiptHash,
      ),
    ).toBe(true);
  ...
  it("blocks mutated arguments after approval and rejects a self-approval", async () => {
    ...
    const mutated = {
      ...action,
      resource: { ...action.resource, id: "other-world" },
      arguments: { worldId: "other-world" },
    };
    await expect(
      gateway.execute(
        mutated,
    ...
    await expect(
      gateway.submit(
        actionEnvelope("action-self", "mutated-world"),
        approvalFor(
          actionEnvelope("action-self", "mutated-world"),
          "agent-one",
        ),
    ...
    await expect(
      gateway.submit(
        actionEnvelope("action-expired", "mutated-world"),
        approvalFor(
          actionEnvelope("action-expired", "mutated-world"),
          "reviewer-one",
    ...
    await expect(
      gateway.submit(
        {
          ...actionEnvelope("action-expired-envelope", "mutated-world"),
          expiresAt: "2028-12-31T23:59:59.000Z",
        },
        approvalFor({
          ...actionEnvelope("action-expired-envelope", "mutated-world"),
  ...
  it("fails closed for unsupported operations, policy/identity drift, and unknown outcomes without retry", async () => {
    ...
    await expect(
      gateway.execute(
        action,
        { actorId: "agent-one", workloadId: "checkout-worker" },
        { ...policy(), hash: "c".repeat(64) },
      ),
    ).rejects.toThrow("does not match");
    await expect(
      gateway.execute(
        action,
        { actorId: "other-agent", workloadId: "checkout-worker" },
        policy(),
      ),
    ...
    expect(
      (await gateway.inspect("action-three")).receipts.map(
        (entry) => entry.status,
      ),
    ).toEqual(["requested", "attempted", "failed"]);
    ...
    ).rejects.toThrow("does not support action operation");
    await expect(gateway.compensate("action-three")).rejects.toThrow(
  ...
  it("rejects tampered persisted receipts", async () => {
    ...
    await expect(gateway.inspect("action-tampered")).rejects.toThrow(
      ActionGatewayError,
    );
  });
  ...
  it("checks the persisted kill switch in the synthetic world's final commit section", async () => {
    ...
    const controller = createLocalSafetyController({
      path: join(process.env.GHOSTAPI_DATA_DIR!, "safety-final-check.json"),
    ...
    const adapter = {
      ...baseAdapter,
      async execute(
        action: Parameters<typeof baseAdapter.execute>[0],
        controls: Parameters<typeof baseAdapter.execute>[1],
      ) {
        await controller.stop({
          identity: stopOperator,
          scope: { kind: "global" },
          reason: "race before synthetic commit",
        });
    ...
    ).rejects.toThrow("not automatically retried");
    expect((await inspectWorld("stopped-world")).state.receipts).toEqual([]);
    expect(
      (await gateway.inspect("action-stopped")).receipts.map(
        (entry) => entry.status,
      ),
    ).toEqual(["requested", "attempted", "failed"]);
  });
  ...
  it("accepts a serialized signed approval and rejects forged provenance", async () => {
    await createWorld({
      id: "forged-approval-world",
      seed: "forged-approval-seed",
    });
    ...
    await expect(
      gateway.submit(action, serialized, policy()),
    ).resolves.toMatchObject({ actionHash: actionHash(action) });
    const forgedAction = actionEnvelope(
      "action-forged-two",
      "forged-approval-world",
    );
    await expect(
      gateway.submit(
        forgedAction,
        {
          ...approvalFor(forgedAction),
          provenance: {
  ...
  it("rejects a trusted approval object that was modified after issuance", async () => {
    await createWorld({
      id: "tampered-approval-world",
      seed: "tampered-approval-seed",
    });
    const action = actionEnvelope(
      "action-tampered-approval",
      "tampered-approval-world",
    );
    const approval = approvalFor(action);
    approval.approvedBy = "attacker";

    await expect(
      createGateway().submit(action, approval, policy()),
    ).rejects.toThrow("authenticity could not be verified");
  });
...
function createGateway(
  adapter = createSyntheticActionAdapter(),
  safetyController?: ReturnType<typeof createLocalSafetyController>,
) {
...
function approvalFor(
  action: ReturnType<typeof actionEnvelope> | { [key: string]: unknown },
