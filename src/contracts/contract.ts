...
import { prepareScenarioRecording, type ScenarioBundle, type ScenarioRecordingOptions } from "../scenarios/scenarioBundle.js";
...
export function importOpenApiContract(input: unknown, options: OpenApiImportOptions = {}): GhostApiContract {
  ...
  if (document.servers !== undefined) throw new ContractError("OpenAPI servers are unsupported because GhostAPI imports contracts locally and never resolves remote URLs.");
  const paths = objectAt(document.paths, "OpenAPI paths must be an object.");
  const pathEntries = Object.entries(paths);
  if (pathEntries.length === 0 || pathEntries.length > MAX_PATHS) throw new ContractError(`OpenAPI paths must contain 1-${MAX_PATHS} entries.`);
  ...
  for (const [path, pathItem] of pathEntries) {
    ...
    for (const [methodName, operationValue] of Object.entries(item)) {
      ...
      if (operations.length > MAX_OPERATIONS) throw new ContractError(`OpenAPI document exceeds ${MAX_OPERATIONS} operations.`);
    }
  }
  if (operations.length === 0) throw new ContractError("OpenAPI document does not contain any supported operations.");
...
export function importHarContract(input: unknown, options: HarContractImportOptions): { bundle: ScenarioBundle; contract: GhostApiContract } {
...
export function contractFromScenarioBundle(bundle: ScenarioBundle, title = bundle.metadata.title, importedAt = bundle.metadata.recordedAt): GhostApiContract {
...
export async function importOpenApiContractFromFile(path: string, options: OpenApiImportOptions = {}, projectRoot = process.cwd()): Promise<GhostApiContract> {
  return importOpenApiContract(await readBoundedJson(path, projectRoot, "OpenAPI input"), options);
}

export async function importHarContractFromFile(path: string, options: HarContractImportOptions, projectRoot = process.cwd()): Promise<{ bundle: ScenarioBundle; contract: GhostApiContract }> {
  return importHarContract(await readBoundedJson(path, projectRoot, "HAR input"), options);
}

export async function writeContract(contract: GhostApiContract, outPath?: string, projectRoot = process.cwd()): Promise<string> {
  ...
  if (Buffer.byteLength(serialized, "utf8") > MAX_CONTRACT_BYTES) throw new ContractError(`Contract exceeds ${MAX_CONTRACT_BYTES} bytes.`);
  await atomicWriteJson(target, valid);
  return target;
}

export async function loadContract(path: string, projectRoot = process.cwd()): Promise<GhostApiContract> {
...
export function validateContract(input: unknown): GhostApiContract {
  ...
  if (root.schemaVersion !== 1 || root.kind !== "ghostapi.contract") throw new ContractError("Unsupported contract schema or kind.");
  ...
  if (metadata.source !== "openapi" && metadata.source !== "har") throw new ContractError("Contract metadata source must be openapi or har.");
  if (!isTimestamp(metadata.importedAt)) throw new ContractError("Contract metadata importedAt must be an ISO timestamp.");
  if (!Array.isArray(root.operations) || root.operations.length === 0 || root.operations.length > MAX_OPERATIONS) throw new ContractError(`Contract operations must contain 1-${MAX_OPERATIONS} entries.`);
  const operations = root.operations.map((operation) => validateOperation(operation)).sort(compareOperations);
  const keys = new Set<string>();
  for (const operation of operations) {
    const key = `${operation.method} ${operation.path}`;
    if (keys.has(key)) throw new ContractError(`Contract operation is duplicated: ${key}`);
    keys.add(key);
  }
  if (!Array.isArray(root.providerCapabilities) || root.providerCapabilities.length > 20) throw new ContractError("Contract provider capabilities are invalid.");
...
export function diffContracts(baselineInput: GhostApiContract, candidateInput: GhostApiContract): ContractDiff {
  ...
  for (const key of [...baselineOperations.keys()].sort()) {
    if (!candidateOperations.has(key)) findings.push(finding("endpoint-removed", key, "breaking", `Endpoint removed: ${key}. Existing callers can no longer rely on this operation.`));
  }
  for (const key of [...candidateOperations.keys()].sort()) {
    if (!baselineOperations.has(key)) findings.push(finding("endpoint-added", key, "non-breaking", `Endpoint added: ${key}.`));
  }
  for (const key of [...baselineOperations.keys()].filter((entry) => candidateOperations.has(entry)).sort()) {
    ...
    for (const status of [...beforeStatuses].filter((value) => !afterStatuses.has(value)).sort()) findings.push(finding("response-status-removed", `${key} ${status}`, "breaking", `Response status ${status} removed from ${key}.`));
    for (const status of [...afterStatuses].filter((value) => !beforeStatuses.has(value)).sort()) findings.push(finding("response-status-added", `${key} ${status}`, "uncertain", `Response status ${status} added to ${key}; client handling is unknown.`));
    for (const status of [...beforeStatuses].filter((value) => afterStatuses.has(value)).sort()) diffSchema(before.responses[status] ?? undefined, after.responses[status] ?? undefined, `${key} response ${status}`, "response", findings);
  }
...
function parseOpenApiOperation(path: string, method: string, value: unknown): ContractOperation {
  ...
  if (responseEntries.length === 0 || responseEntries.length > 20) throw new ContractError(`OpenAPI ${method} ${path} responses must contain 1-20 entries.`);
  const responses: Record<string, ContractSchema | null> = {};
  for (const [status, response] of responseEntries) {
    if (!/^(?:[1-5][0-9]{2}|default)$/.test(status)) throw new ContractError(`OpenAPI ${method} ${path} has unsupported response status: ${status}.`);
...
function parseRequestBody(value: unknown, label: string): ContractSchema | undefined {
...
function parseResponse(value: unknown, label: string): ContractSchema | null {
  ...
  if (response.headers !== undefined) throw new ContractError(`${label} headers are unsupported; import JSON response schemas only.`);
  return response.content === undefined ? null : parseContent(response.content, label) ?? null;
}

function parseContent(value: unknown, label: string): ContractSchema | undefined {
  const content = objectAt(value, `${label} content must be an object.`);
  const entries = Object.entries(content);
  if (entries.length !== 1 || entries[0]![0].toLowerCase() !== "application/json") throw new ContractError(`${label} supports exactly one application/json content entry.`);
...
function parseOpenApiSchema(value: unknown, label: string, depth = 0): ContractSchema {
  if (depth > MAX_SCHEMA_DEPTH) throw new ContractError(`${label} exceeds schema recursion depth ${MAX_SCHEMA_DEPTH}.`);
  ...
  if (schema.nullable === true) throw new ContractError(`${label} nullable is unsupported; use an explicit oneOf outside this supported subset.`);
  const result: ContractSchema = {};
  if (schema.type !== undefined) {
    if (schema.type !== "string" && schema.type !== "number" && schema.type !== "integer" && schema.type !== "boolean" && schema.type !== "array" && schema.type !== "object") throw new ContractError(`${label} has unsupported schema type.`);
    result.type = schema.type;
  }
  if (schema.enum !== undefined) {
    if (!Array.isArray(schema.enum) || schema.enum.length === 0 || schema.enum.length > 100 || schema.enum.some((entry) => !["string", "number", "boolean"].includes(typeof entry) && entry !== null)) throw new ContractError(`${label} enum must contain 1-100 scalar JSON values.`);
    result.enum = [...schema.enum] as ContractSchema["enum"];
  }
  if (schema.required !== undefined) {
    if (!Array.isArray(schema.required) || schema.required.some((entry) => typeof entry !== "string" || !isFieldName(entry)) || new Set(schema.required).size !== schema.required.length) throw new ContractError(`${label} required must be unique field names.`);
    result.required = [...schema.required].sort();
  }
  if (schema.properties !== undefined) {
    if (result.type !== "object") throw new ContractError(`${label} properties requires type object.`);
    const properties = objectAt(schema.properties, `${label} properties must be an object.`);
    const entries = Object.entries(properties);
    if (entries.length > MAX_PROPERTIES || entries.some(([name]) => !isFieldName(name))) throw new ContractError(`${label} properties exceed supported limits.`);
  ...
  if (schema.items !== undefined) {
    if (result.type !== "array") throw new ContractError(`${label} items requires type array.`);
    result.items = parseOpenApiSchema(schema.items, `${label}.items`, depth + 1);
  }
  if (result.type === undefined && result.enum === undefined) throw new ContractError(`${label} must declare type or enum in the supported subset.`);
  if (result.required !== undefined && result.type !== "object") throw new ContractError(`${label} required requires type object.`);
  return result;
}

function assertNoUnsupportedOpenApiFeatures(value: Record<string, unknown>, label: string): void {
  for (const key of ["$ref", "$dynamicRef", "$recursiveRef", "allOf", "anyOf", "oneOf", "not", "discriminator", "callbacks", "links", "security", "externalDocs", "webhooks"]) {
    if (value[key] !== undefined) throw new ContractError(`${label} uses unsupported OpenAPI feature ${key}. External or recursive references are never fetched or resolved.`);
  }
}

function inferSchema(value: unknown, depth = 0): ContractSchema {
  if (depth > MAX_SCHEMA_DEPTH) throw new ContractError(`HAR body exceeds schema recursion depth ${MAX_SCHEMA_DEPTH}.`);
  if (value === null) return { type: "null" };
  if (Array.isArray(value)) {
    const items = value.length === 0 ? undefined : value.slice(1).reduce<ContractSchema>((current, entry) => mergeSchemas(current, inferSchema(entry, depth + 1)) ?? {}, inferSchema(value[0], depth + 1));
  ...
  if (entries.length > MAX_PROPERTIES) throw new ContractError(`HAR body object exceeds ${MAX_PROPERTIES} properties.`);
...
function mergeSchemas(left: ContractSchema | null | undefined, right: ContractSchema | null | undefined): ContractSchema | undefined {
  ...
  const result: ContractSchema = { ...(left.type === undefined ? {} : { type: left.type }) };
  ...
  if (left.type === "array") result.items = mergeSchemas(left.items, right.items);
  return result;
}

function diffSchema(before: ContractSchema | null | undefined, after: ContractSchema | null | undefined, location: string, context: "request" | "response", findings: ContractDiffFinding[]): void {
  if (before === undefined || before === null || after === undefined || after === null) {
    if (before !== after) findings.push(finding("schema-presence-changed", location, "uncertain", `${location} schema presence changed; compatibility cannot be proven.`));
    return;
  }
  ...
  for (const field of Object.keys(beforeProperties).filter((field) => !Object.hasOwn(afterProperties, field)).sort()) findings.push(finding("property-removed", `${location}.${field}`, context === "response" ? "breaking" : "uncertain", `${location} property removed: ${field}.${context === "request" ? " Acceptance of this field is not modeled by the supported subset." : ""}`));
  for (const field of Object.keys(afterProperties).filter((field) => !Object.hasOwn(beforeProperties, field)).sort()) findings.push(finding("property-added", `${location}.${field}`, context === "request" ? "non-breaking" : "uncertain", `${location} property added: ${field}.${context === "response" ? " Client response tolerance is unknown." : ""}`));
  for (const field of Object.keys(beforeProperties).filter((field) => Object.hasOwn(afterProperties, field)).sort()) diffSchema(beforeProperties[field], afterProperties[field], `${location}.${field}`, context, findings);
  if (before.items !== undefined || after.items !== undefined) diffSchema(before.items, after.items, `${location}[]`, context, findings);
}

function diffEnum(before: ContractSchema["enum"], after: ContractSchema["enum"], location: string, context: "request" | "response", findings: ContractDiffFinding[]): void {
  if (before === undefined || after === undefined) {
    if (before !== after) findings.push(finding("enum-presence-changed", location, "uncertain", `${location} enum constraint changed; compatibility cannot be proven.`));
    return;
  }
  const beforeValues = new Set(before.map(stableStringify));
  const afterValues = new Set(after.map(stableStringify));
  for (const value of [...beforeValues].filter((value) => !afterValues.has(value)).sort()) findings.push(finding("enum-value-removed", `${location}.${value}`, "breaking", `${location} enum value removed: ${value}.`));
  for (const value of [...afterValues].filter((value) => !beforeValues.has(value)).sort()) findings.push(finding("enum-value-added", `${location}.${value}`, context === "request" ? "non-breaking" : "uncertain", `${location} enum value added: ${value}.${context === "response" ? " Client handling is unknown." : ""}`));
}

function diffProviderCapabilities(before: ContractProviderCapability[], after: ContractProviderCapability[], findings: ContractDiffFinding[]): void {
  ...
  for (const name of [...baseline.keys()].sort()) {
    if (!candidate.has(name)) findings.push(finding("provider-pack-removed", name, "breaking", `Provider pack capability snapshot removed: ${name}.`));
  }
  for (const name of [...candidate.keys()].sort()) {
    if (!baseline.has(name)) findings.push(finding("provider-pack-added", name, "non-breaking", `Provider pack capability snapshot added: ${name}.`));
  }
  for (const name of [...baseline.keys()].filter((name) => candidate.has(name)).sort()) {
    ...
    if (left.packVersion !== right.packVersion) findings.push(finding("provider-pack-version-changed", name, "uncertain", `Provider pack ${name} version changed from ${left.packVersion ?? "legacy"} to ${right.packVersion ?? "legacy"}; semantic compatibility is not inferred from version text.`));
...
function validateOperation(value: unknown): ContractOperation {
  ...
  if (method === null) throw new ContractError("Contract operation method is invalid.");
  ...
  if (responseEntries.length === 0 || responseEntries.length > 20) throw new ContractError("Contract operation responses are invalid.");
  return {
    method,
    ...
    responses: Object.fromEntries(responseEntries.sort(([left], [right]) => left.localeCompare(right)).map(([status, schema]) => {
      if (!/^(?:[1-5][0-9]{2}|default)$/.test(status)) throw new ContractError("Contract response status is invalid.");
      return [status, schema === null ? null : validateSchema(schema, `Contract response ${status} schema`)];
    }))
  };
}

function validateSchema(value: unknown, label: string, depth = 0): ContractSchema {
  if (depth > MAX_SCHEMA_DEPTH) throw new ContractError(`${label} exceeds schema recursion depth ${MAX_SCHEMA_DEPTH}.`);
  const schema = objectAt(value, `${label} is invalid.`);
  assertExactKeys(schema, ["type", "enum", "required", "properties", "items"], label);
  const result: ContractSchema = {};
  if (schema.type !== undefined) {
    if (!["string", "number", "integer", "boolean", "array", "object", "null"].includes(schema.type as string)) throw new ContractError(`${label} type is invalid.`);
    result.type = schema.type as ContractSchema["type"];
  }
  if (schema.enum !== undefined) {
    if (!Array.isArray(schema.enum) || schema.enum.length === 0 || schema.enum.length > 100 || schema.enum.some((entry) => !["string", "number", "boolean"].includes(typeof entry) && entry !== null)) throw new ContractError(`${label} enum is invalid.`);
    result.enum = [...schema.enum] as ContractSchema["enum"];
  }
  if (schema.required !== undefined) {
    if (!Array.isArray(schema.required) || schema.required.some((entry) => typeof entry !== "string" || !isFieldName(entry)) || new Set(schema.required).size !== schema.required.length) throw new ContractError(`${label} required is invalid.`);
    result.required = [...schema.required].sort();
  }
  if (schema.properties !== undefined) {
    if (result.type !== "object") throw new ContractError(`${label} properties requires type object.`);
    ...
    const entries = Object.entries(properties);
    if (entries.length > MAX_PROPERTIES || entries.some(([key]) => !isFieldName(key))) throw new ContractError(`${label} properties are invalid.`);
  ...
  if (result.type === undefined && result.enum === undefined) throw new ContractError(`${label} must declare type or enum.`);
  if (result.required !== undefined && result.type !== "object") throw new ContractError(`${label} required requires type object.`);
  return result;
}
...
function validateCapability(value: unknown): ContractProviderCapability {
  ...
  if (typeof capability.name !== "string" || !/^[a-z][a-z0-9-]{0,63}$/.test(capability.name)) throw new ContractError("Contract provider capability name is invalid.");
  if (capability.packVersion !== null && typeof capability.packVersion !== "string") throw new ContractError("Contract provider pack version is invalid.");
  const flags = objectAt(capability.capabilities, "Contract provider capabilities are invalid.");
  if (Object.keys(flags).length > 30 || Object.values(flags).some((value) => typeof value !== "boolean")) throw new ContractError("Contract provider capabilities are invalid.");
  return { name: capability.name, packVersion: capability.packVersion, capabilities: Object.fromEntries(Object.entries(flags).sort(([left], [right]) => left.localeCompare(right))) as Record<string, boolean> };
}
...
function validateOpenApiPath(value: unknown): asserts value is string {
  if (typeof value !== "string" || !value.startsWith("/") || value.startsWith("//") || value.length > 1_000 || /[\r\n?#{}]/.test(value)) throw new ContractError("OpenAPI path must be a bounded relative path without query, fragment, path parameters, or line breaks.");
}
...
function compareOperations(left: ContractOperation, right: ContractOperation): number {
  return operationKey(left).localeCompare(operationKey(right));
}

function finding(kind: string, subject: string, severity: ContractDiffSeverity, message: string): ContractDiffFinding {
  return { id: `${kind}:${subject}`, severity, message };
}

function normalizeTitle(value: unknown): string {
  if (typeof value !== "string" || value.trim() === "" || value.trim().length > 120) throw new ContractError("Contract title must be 1-120 characters.");
  const title = value.trim();
  if (sanitizeSecretString(title) !== title) throw new ContractError("Contract title must not contain a secret-shaped value.");
  return title;
}
...
function objectAt(value: unknown, message: string): Record<string, unknown> {
  if (value === null || typeof value !== "object" || Array.isArray(value)) throw new ContractError(message);
  return value as Record<string, unknown>;
}

function assertExactKeys(record: Record<string, unknown>, allowed: readonly string[], label: string): void {
  const unknown = Object.keys(record).find((key) => !allowed.includes(key));
  if (unknown !== undefined) throw new ContractError(`${label} contains unsupported field: ${unknown}.`);
}

function assertSerializedSize(value: unknown, label: string, limit: number): void {
  try {
    if (Buffer.byteLength(JSON.stringify(value), "utf8") > limit) throw new ContractError(`${label} exceeds ${limit} bytes.`);
  } catch (error) {
...
async function readBoundedJson(path: string, projectRoot: string, label: string, limit = MAX_INPUT_BYTES): Promise<unknown> {
  const target = await resolveExistingPath(path, projectRoot, label);
  const source = await readFile(target);
  if (source.byteLength > limit) throw new ContractError(`${label} exceeds ${limit} bytes.`);
  if (isArchive(source)) throw new ContractError(`${label} archive input is unsupported; extract one bounded JSON OpenAPI or HAR file first. GhostAPI never decompresses untrusted archives.`);
  try {
...
async function resolveExistingPath(path: string, projectRoot: string, label: string): Promise<string> {
  ...
  if (!info.isFile() || info.isSymbolicLink()) throw new ContractError(`${label} must be a regular non-symlink file.`);
  if (!await isRealPathInsideAllowedRoots(target, projectRoot)) throw new ContractError(`${label} resolves outside the project root or GHOSTAPI_DATA_DIR through a symlink.`);
  return target;
}

async function resolveOutputPath(path: string, projectRoot: string): Promise<string> {
  const target = isAbsolute(path) ? resolve(path) : resolve(projectRoot, path);
  assertPathInsideAllowedRoots(projectRoot, target, "Contract output");
  await mkdir(dirname(target), { recursive: true, mode: 0o700 });
  const parent = await lstat(dirname(target));
  if (!parent.isDirectory() || parent.isSymbolicLink()) throw new ContractError("Contract output parent must be a real directory, not a symlink.");
  if (!await isRealPathInsideAllowedRoots(dirname(target), projectRoot)) throw new ContractError("Contract output parent resolves outside the project root or GHOSTAPI_DATA_DIR through a symlink.");
  const existing = await lstat(target).catch((error: unknown) => isErrorCode(error, "ENOENT") ? null : Promise.reject(error));
  if (existing?.isSymbolicLink()) throw new ContractError("Contract output path must not be a symlink.");
  return target;
}

function assertPathInsideAllowedRoots(projectRoot: string, target: string, label: string): void {
  if (!isInside(projectRoot, target) && !isInside(getDataPaths().root, target)) throw new ContractError(`${label} path traversal outside the project root or GHOSTAPI_DATA_DIR is not allowed.`);
  if (basename(target).trim() === "") throw new ContractError(`${label} path must include a file name.`);
}
...
async function isRealPathInsideAllowedRoots(target: string, projectRoot: string): Promise<boolean> {
...
function stableStringify(value: unknown): string {
  ...
  return `{${Object.entries(value as Record<string, unknown>).filter(([, entry]) => entry !== undefined).sort(([left], [right]) => left.localeCompare(right)).map(([key, entry]) => `${JSON.stringify(key)}:${stableStringify(entry)}`).join(",")}}`;
}
