...
import {
  createLocalActionLedger,
  createTestLedgerAccessAuthorizer,
} from "../src/ledger/index.js";
import {
  createLocalReconciliationService,
  createLocalSloController,
  createTestReconciliationOperatorAuthorizer,
...
describe("local reconciliation service", () => {
  it("classifies committed, unknown, not_committed, and drifted outcomes without retrying", async () => {
    ...
    const ambiguousGateway = createGateway({
      ...createSyntheticActionAdapter(),
      execute: async () => ({ outcome: "unknown" as const }),
    });
    await expect(
      ambiguousGateway.execute(
        ambiguous,
        { actorId: "agent-one", workloadId: "checkout-worker" },
        policy(),
      ),
    ...
    const capability = access.issue({
      tenantId: "recon-tenant",
    ...
    const operator = issue({
      id: "recon-ops",
      principalId: "recon-ops-one",
      permissions: ["reconciliation.manage", "reconciliation.inspect"],
    });
    const { authorizer: sloAuthorizer, issue: issueSlo } =
      createTestSloOperatorAuthorizer();
    const sloOperator = issueSlo({
      id: "slo-ops",
      principalId: "slo-ops-one",
      permissions: ["slo.inspect"],
    });
    const sloController = createLocalSloController({
      path: sloPath("recon-slo.json"),
      now: () => new Date(FIXED_NOW),
      operatorAuthorizer: sloAuthorizer,
    });
    ...
    const provider = createWorldStateReconciliationProvider(
      async (actionId) => worldByAction[actionId] ?? null,
    );
    const service = createLocalReconciliationService({
      path: reconPath("recon-state.json"),
      now: () => new Date(FIXED_NOW),
      ledger,
      capability,
      provider,
    ...
    const byId = Object.fromEntries(
      report.actions.map((action) => [action.actionId, action]),
    );
    expect(byId["recon-ok"]).toMatchObject({
      outcome: "committed",
      retrySafe: false,
      ledgerIntent: "committed",
    });
    expect(byId["recon-ambig"]).toMatchObject({
      outcome: "unknown",
      retrySafe: false,
      ledgerIntent: "ambiguous",
    });
    expect(byId["recon-none"]).toMatchObject({
      outcome: "not_committed",
      retrySafe: true,
      ledgerIntent: "not_committed",
    });
    expect(byId["recon-drift"]).toMatchObject({
      outcome: "drifted",
      retrySafe: false,
      ledgerIntent: "committed",
    });

    expect(report.sli.duplicatePrevention).toEqual({
      measured: 3,
      ok: 3,
      okRateBps: 10_000,
    });
    expect(report.sli.receiptVerification).toEqual({
      measured: 2,
      ok: 2,
      okRateBps: 10_000,
    });
    expect(report.sli.availability).toEqual({
      measured: 3,
      ok: 1,
      okRateBps: 3_333,
    });
    ...
    const driftFinding = findings.find(
      (finding) => finding.classification === "drifted",
    )!;
    const unknownFinding = findings.find(
    ...
    const inspectOnly = issue({
      id: "recon-ro",
      principalId: "recon-ro-one",
      permissions: ["reconciliation.inspect"],
    });
    await expect(
      service.resolveDrift({
        identity: inspectOnly,
        findingId: driftFinding.findingId,
    ...
    await expect(
      service.resolveUnknown({
        ...
        findingId: driftFinding.findingId,
        reason: "wrong kind",
        evidenceRef: "world:recon-world-drift",
      }),
    ).rejects.toThrow("Only unknown findings");
    await expect(
      service.resolveDrift({
        identity: operator,
        findingId: unknownFinding.findingId,
        reason: "wrong kind",
      }),
    ).rejects.toThrow("Only drifted findings");

    const resolvedDrift = await service.resolveDrift({
      identity: operator,
      findingId: driftFinding.findingId,
      reason: "provider receipt restored after replay",
    });
    expect(resolvedDrift).toMatchObject({
      status: "resolved",
      resolution: { resolvedBy: "recon-ops-one" },
    });
    const resolvedUnknown = await service.resolveUnknown({
      identity: operator,
      findingId: unknownFinding.findingId,
      reason: "confirmed not committed",
      evidenceRef: "world:recon-world-ambig receipts=0",
    });
    expect(resolvedUnknown).toMatchObject({
      status: "resolved",
      resolution: { evidenceRef: "world:recon-world-ambig receipts=0" },
    });
    expect(
      (await service.listFindings({ identity: operator })).every(
  ...
  it("does not open findings for actions that never reached the provider and blocks on ledger corruption", async () => {
    ...
    const operator = issue({
      id: "recon-clean-ops",
      principalId: "recon-clean-ops-one",
      permissions: ["reconciliation.manage"],
    });
    const provider = createWorldStateReconciliationProvider(async (actionId) =>
      actionId === "recon-clean" ? "recon-clean-world" : null,
    );
    const service = createLocalReconciliationService({
      path: reconPath("recon-clean-state.json"),
      now: () => new Date(FIXED_NOW),
      ledger,
      capability,
      provider,
    ...
    await expect(
      service.runReconciliation({ identity: readOnly }),
    ).rejects.toThrow("permission");

    const path = join(
      process.env.GHOSTAPI_DATA_DIR!,
      "recon-clean-ledger.json",
    );
    const modified = JSON.parse(await readFile(path, "utf8")) as {
    ...
    await expect(
      service.runReconciliation({ identity: operator }),
...
function createGateway(adapter = createSyntheticActionAdapter()) {
  ...
  return createLocalActionGateway({
    now: () => new Date(FIXED_NOW),
    adapter,
    approvalVerifier: approvalAuthority.verifier,
    pathForAction: (actionId) =>
      join(root, "actions", `${actionId}.action.json`),
  });
}

function createLedger(
  authorizer: ReturnType<typeof createTestLedgerAccessAuthorizer>["authorizer"],
  fileName: string,
) {
  return createLocalActionLedger({
    path: join(process.env.GHOSTAPI_DATA_DIR!, fileName),
    incidentsPath: join(
      process.env.GHOSTAPI_DATA_DIR!,
      `incidents-${fileName}`,
    ),
...
function approvalFor(action: ReturnType<typeof actionEnvelope>) {
  return approvalAuthority.issue({
    ...
    expiresAt: "2030-01-01T00:00:00.000Z",
    nonce: `approval-nonce-${action.actionId}`,
  } as const);
}
