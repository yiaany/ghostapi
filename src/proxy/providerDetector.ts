...
export function detectProvider(
  inputOrPath: ProviderDetectionInput | string,
  headers: IncomingHttpHeaders = {},
): Provider {
  const input =
    typeof inputOrPath === "string"
      ? { path: inputOrPath, headers }
      : inputOrPath;
  ...
  if (
    isStripePath(normalizedPath) ||
    normalizedHeaders["stripe-version"] !== undefined ||
    hasStripeAuthorization(normalizedHeaders)
  ) {
    return "stripe";
  }

  if (
    normalizedPath.includes("/2010-04-01/") ||
  ...
  if (
    normalizedPath.startsWith("/repos/") ||
    normalizedPath.startsWith("/user/") ||
    normalizedPath === "/user" ||
    normalizedHeaders["x-github-api-version"] !== undefined
  ) {
    return "github";
  }

  if (
    isOpenAiPath(normalizedPath) ||
    normalizedHeaders["openai-organization"] !== undefined ||
    normalizedHeaders["openai-project"] !== undefined
  ) {
    return "openai";
  }

  if (
    /^\/api\/v\d+\/(?:channels|guilds|webhooks|interactions|applications)(?:\/|$)/.test(
      normalizedPath,
    ) ||
...
function isStripePath(path: string): boolean {
  return /^\/v1\/(?:customers|charges|payment_intents|setup_intents|checkout|prices|products|subscriptions|invoices|payment_methods|refunds|accounts|balance|payouts|transfers|events|webhook_endpoints)(?:\/|$)/.test(
    path,
  );
}

function isOpenAiPath(path: string): boolean {
  return /^\/v1\/(?:chat\/completions|responses|embeddings|models|images|audio|files|fine_tuning|assistants|threads|vector_stores)(?:\/|$)/.test(
    path,
  );
}
...
function getProviderOverride(
  ...
  const queryOverride =
    typeof query.ghost_provider === "string" ? query.ghost_provider : undefined;
  const override = (headerOverride ?? queryOverride)?.toLowerCase();

  return override !== undefined && isRegisteredProvider(override)
    ? override
    : undefined;
}
...
function hasStripeAuthorization(
  headers: Record<string, string | string[]>,
): boolean {
  const authorization = firstValue(headers.authorization);
  return (
    authorization !== undefined &&
    /(?:bearer\s+)?[sr]k_(?:live|test)_/i.test(authorization)
  );
}
