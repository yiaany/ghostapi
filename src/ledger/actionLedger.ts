...
export type LedgerAccessAuthorizer = (
  access: unknown,
...
export type LedgerVerification =
  | { valid: true; entryCount: number; headHash: string; tracked: boolean }
...
export class LocalActionLedger {
  ...
  async recordAction(
    ...
    return this.mutate(async (state) => {
      ...
      if (existing.length > 0) {
        if (existing[0]!.actionHash !== actionHash(record.envelope))
          throw new ActionLedgerError(
            "Action id already has a different ledger timeline in this tenant.",
          );
      ...
      append("intent", {
        ...
        expectedEffectsHash: sha256(
          canonicalJson(record.envelope.expectedSideEffects),
        ),
      ...
      append(
        ...
        record.credentialGrant === undefined
          ...
          : {
              status: "granted",
              grantIdHash: hash(
                record.credentialGrant.grantIdHash,
                "Credential grant reference",
              ),
              credentialVersion: positiveInteger(
                record.credentialGrant.credentialVersion,
                "Credential version",
              ),
            },
      );
      ...
      append("compensation", {
        status:
          record.envelope.reversibility === "compensatable"
            ? "not_attempted"
            : "not_supported",
      });
  ...
  async exportTenant(accessValue: unknown): Promise<LedgerExport> {
    ...
    if (!integrity.valid)
      throw new ActionLedgerError(
        `Ledger integrity verification failed: ${integrity.error}`,
      );
    return {
      schemaVersion: 1,
      kind: "ghostapi.action-ledger-export",
  ...
  async configureRetention(
    ...
    return this.mutate(async (state) => {
      ...
      appendEntry(
        state,
        access.tenantId,
        governanceActionId("retention", state, access.tenantId),
        sha256(`retention:${access.tenantId}`),
        "retention",
        { retentionDays },
        this.now().toISOString(),
      );
  ...
  async setLegalHold(
    ...
    return this.mutate(async (state) => {
      ...
      appendEntry(
        state,
        access.tenantId,
        governanceActionId("hold", state, access.tenantId),
        sha256(`legal-hold:${access.tenantId}`),
        "legal_hold",
        { active },
        this.now().toISOString(),
      );
  ...
  async requestDeletion(accessValue: unknown): Promise<LedgerTenantState> {
    ...
    return this.mutate(async (state) => {
      ...
      if (tenant.legalHold)
        throw new ActionLedgerError(
          "Deletion request is blocked by the active local legal hold.",
        );
      if (tenant.deletionRequestedAt === undefined) {
        ...
        appendEntry(
          state,
          access.tenantId,
          governanceActionId("deletion", state, access.tenantId),
          sha256(`deletion:${access.tenantId}`),
          "deletion_request",
          { status: "requested" },
  ...
  async createIncidentFixture(
    ...
    if (intent === undefined)
      throw new ActionLedgerError(
        "Incident replay requires an action intent in the tenant-scoped ledger timeline.",
      );
    const verification = timeline
      .filter((entry) => entry.stage === "verification")
      .at(-1);
    const outcome =
      verification?.data.status === "verified"
        ? "verified"
    ...
    try {
      ...
      if (world.manifest.seed !== seed)
        throw new ActionLedgerError(
          "Incident synthetic world id collides with a different deterministic seed.",
        );
    } catch (error) {
      if (
    ...
    const request: ScenarioReplayRequest = {
      method: "POST",
    ...
    const bundlePath = await writeScenarioBundle(
      bundle,
      join(this.incidentsPath, `${prefix}.bundle.json`),
      getDataPaths().root,
    );
  ...
  async replayIncidentFixture(
    ...
    if (fixture.tenantId !== access.tenantId)
      throw new ActionLedgerError(
        "Incident fixture belongs to a different tenant.",
      );
    ...
    if (
      ...
      throw new ActionLedgerError(
        "Incident fixture did not reproduce its expected local outcome.",
      );
  ...
  private authorize(
    ...
    const authorized = this.accessAuthorizer(
      access,
      permission,
      accessTenantId(access),
    );
    if (!authorized.permissions.includes(permission))
      throw new ActionLedgerError(
        "Ledger access authorizer returned an insufficient permission set.",
      );
    return {
  ...
  private async readState(): Promise<ActionLedgerState> {
    ...
    if (!info.isFile() || info.isSymbolicLink())
      throw new ActionLedgerError(
        "Action ledger must be a regular non-symlink file.",
      );
    const source = await readFile(this.path, "utf8");
    if (Buffer.byteLength(source, "utf8") > MAX_LEDGER_BYTES)
      throw new ActionLedgerError(
        `Action ledger exceeds ${MAX_LEDGER_BYTES} bytes.`,
      );
    try {
  ...
  private async mutate<T>(
    ...
    ) => Promise<{ state: ActionLedgerState; result: T }>,
  ): Promise<T> {
...
export function createLocalActionLedger(
  options: ActionLedgerOptions = {},
): LocalActionLedger {
  ...
  return {
    ...
    authorizer(access, permission, tenantId) {
      if (access === null || typeof access !== "object")
        throw new ActionLedgerError(
          "Ledger access is not a verified capability.",
        );
...
export function validateIncidentFixture(value: unknown): IncidentFixture {
  ...
  const world = object(
    fixture.syntheticWorld,
    "Incident fixture world is invalid.",
  );
  ...
  const expected = object(
    fixture.expected,
    "Incident fixture expected outcome is invalid.",
  );
  exactKeys(
    expected,
    ["status", "outcome"],
    "Incident fixture expected outcome",
  );
  if (
    (expected.status !== 200 && expected.status !== 409) ||
  ...
  const request = object(
    fixture.request,
    "Incident fixture request is invalid.",
  );
  return {
    schemaVersion: 1,
    kind: "ghostapi.incident-fixture",
    ...
    sourceLedgerHeadHash: hash(
      fixture.sourceLedgerHeadHash,
      "Incident fixture ledger head hash",
    ),
    syntheticWorld: {
...
function prepareBundle(value: unknown): ScenarioBundle {
  ...
  if (
    bundle.interactions.length !== 1 ||
    bundle.metadata.sandboxHosts.length !== 1 ||
    bundle.metadata.sandboxHosts[0] !== "incident.sandbox.localhost"
  )
    throw new ActionLedgerError(
      "Incident fixture must contain one local-only replay interaction.",
    );
  return bundle;
}

function appendReceiptEvidence(
  append: (stage: LedgerStage, data: LedgerEntry["data"]) => void,
...
function validateState(value: unknown): ActionLedgerState {
  ...
  exactKeys(
    state,
    ["schemaVersion", "kind", "entries", "tenants"],
    "Action ledger state",
  );
  if (
  ...
  if (
    ...
    throw new ActionLedgerError(
      "Action ledger state exceeds its bounded limits.",
    );
  ...
  for (const tenant of tenants)
    assertTenantIntegrity(
      { schemaVersion: 1, kind: "ghostapi.action-ledger", entries, tenants },
      tenant.tenantId,
    );
  if (
    new Set(entries.map((entry) => entry.tenantId)).size !== tenants.length &&
    entries.some(
      (entry) => !tenants.some((tenant) => tenant.tenantId === entry.tenantId),
    )
  )
...
function validateTenant(value: unknown): LedgerTenantState {
  ...
  const deletionRequestedAt =
    tenant.deletionRequestedAt === undefined
      ? undefined
      : timestamp(
          tenant.deletionRequestedAt,
          "Ledger deletion request timestamp",
        );
  if (
  ...
  return {
    ...
    entryCount: nonNegativeInteger(
      tenant.entryCount,
      "Ledger tenant entry count",
    ),
    headHash: hash(tenant.headHash, "Ledger tenant head hash"),
    retentionDays: tenant.retentionDays as number | null,
...
function appendEntry(
  ...
  if (state.entries.length >= MAX_ENTRIES) {
    ...
    if (state.entries.length >= MAX_ENTRIES)
      throw new ActionLedgerError(
        `Action ledger entry limit of ${MAX_ENTRIES} was reached; export and controlled retention review are required.`,
      );
  }
  const entry: Omit<LedgerEntry, "entryHash"> = {
    ...
    previousHash:
      tenant.entryCount === 0 ? genesisHash(tenantId) : tenant.headHash,
  };
...
function relinkTenantChain(
  tenantId: string,
  surviving: LedgerEntry[],
): LedgerEntry[] {
...
function getOrCreateTenant(
  ...
  if (state.tenants.length >= MAX_TENANTS)
    throw new ActionLedgerError(
      `Action ledger tenant limit of ${MAX_TENANTS} was reached.`,
    );
  const tenant: LedgerTenantState = {
    tenantId,
    entryCount: 0,
...
function assertTenantIntegrity(
  state: ActionLedgerState,
  tenantId: string,
): void {
...
function entriesForAction(
  ...
  return state.entries
    .filter(
      (entry) => entry.tenantId === tenantId && entry.actionId === actionId,
    )
    .map((entry) => structuredClone(entry));
}
...
function normalizeData(
  ...
  for (const [key, entry] of entries) {
    if (
      ...
      /(?:authorization|cookie|token|secret|password|email|phone|address|card|payload|body)/i.test(
        key,
      )
    )
      throw new ActionLedgerError(
        `Ledger structured field is forbidden: ${key}`,
      );
    if (typeof entry === "string")
      normalized[key] = safeString(
        entry,
        `Ledger structured field ${key}`,
        512,
      );
    else if (typeof entry === "number") {
      if (!Number.isFinite(entry))
        throw new ActionLedgerError(
          `Ledger structured field ${key} must be finite.`,
        );
      ...
      throw new ActionLedgerError(
        `Ledger structured field ${key} must be scalar.`,
      );
  }
  return normalized;
}
...
function normalizeFixtureHeaders(value: unknown): Record<string, string> {
  const headers = object(
    value ?? {},
    "Incident fixture request headers are invalid.",
  );
  const normalized: Record<string, string> = {};
  for (const [key, entry] of Object.entries(headers)) {
    if (key.toLowerCase() !== "content-type" || typeof entry !== "string")
      throw new ActionLedgerError(
        "Incident fixture contains an unsafe request header.",
      );
    normalized["content-type"] = safeString(
      entry,
      "Incident fixture content type",
      128,
    );
  }
  return normalized;
...
function sanitizeStructured(value: unknown, label: string): unknown {
  ...
  if (
    ...
    throw new ActionLedgerError(
      `${label} contains a secret-shaped value or prohibited PII.`,
    );
...
function denyLedgerAccess(): never {
  throw new ActionLedgerError(
    "Ledger access is disabled until a verified tenant access authorizer is configured.",
  );
}

function governanceActionId(
  kind: string,
  state: ActionLedgerState,
  tenantId: string,
): string {
  return `governance-${kind}-${tenantId}-${state.entries.filter((entry) => entry.tenantId === tenantId).length + 1}`.slice(
    0,
    128,
  );
}
...
function readOutcome(value: unknown): "verified" | "requires_reconciliation" {
  ...
  return body.outcome === "verified" ||
    ...
    : (() => {
        throw new ActionLedgerError(
          "Incident replay response outcome is invalid.",
        );
      })();
}
...
function normalizePermissions(
  value: readonly LedgerPermission[],
): LedgerPermission[] {
  if (
    !Array.isArray(value) ||
    value.length === 0 ||
...
function safeString(value: unknown, label: string, maximum: number): string {
  if (
    ...
    throw new ActionLedgerError(
      `${label} must be a bounded non-secret string.`,
    );
  return value;
}
