...
describe("policy parser", () => {
  it("parses the versioned schema and rejects unknown fields, versions, aliases, and interpolation", () => {
    expect(parsePolicyYaml(POLICY)).toMatchObject({
      version: 1,
      network: { default: "deny" },
    });
    expect(() =>
      parsePolicyYaml(POLICY.replace("version: 1", "version: 2")),
    ).toThrow("policy.version");
    expect(() =>
      parsePolicyYaml(
        POLICY.replace("network:\n", "network:\n  unexpected: true\n"),
      ),
    ...
    expect(() =>
      parsePolicyYaml(
        POLICY.replace(
          "host: localhost",
          "host: &loopback localhost\n    - host: *loopback",
        ),
      ),
    ).toThrow("anchors and aliases");
    expect(() =>
      parsePolicyYaml(POLICY.replace("localhost", "${GHOSTAPI_HOST}")),
    ).toThrow("interpolation");
  });

  it("limits input size and reports path-aware validation errors", () => {
    expect(() =>
      parsePolicyYaml(`${POLICY}\n#${"x".repeat(128 * 1024)}`),
    ).toThrow("policy: exceeds");
    expect(() =>
      parsePolicyYaml(
        POLICY.replace(
          "maxProductionEgressAttempts: 0",
          "maxProductionEgressAttempts: -1",
        ),
      ),
    ).toThrow("policy.reports.maxProductionEgressAttempts");
  });

  it("rejects policy path traversal and symlink-free missing files predictably", async () => {
    await expect(
      loadPolicyFile("../outside.yaml", process.cwd(), true),
    ).rejects.toThrow("path traversal");
    await expect(
      loadPolicyFile("missing-policy-for-test.yaml", process.cwd()),
    ).resolves.toBeNull();
  });
});
...
describe("policy evaluation", () => {
  ...
  it("evaluates credentials, scenarios, enforcement modes, and report thresholds without side effects", () => {
    const policy = parsePolicyYaml(POLICY);
    expect(
      evaluatePolicy(policy, { type: "credential", value: "sk_live_secret" })
        .allowed,
    ).toBe(false);
    expect(
      evaluatePolicy(policy, { type: "credential", value: "sk_test_local" })
        .allowed,
    ).toBe(true);
    expect(
    ...
    expect(
      evaluatePolicy(policy, {
        type: "scenario",
        scenarioId: "stripe.card_declined",
        completedScenarioIds: ["stripe.card_declined"],
      }).allowed,
    ).toBe(true);
    expect(
      evaluatePolicy(policy, { type: "enforcement", mode: "proxy-guidance" })
        .allowed,
    ).toBe(false);
    expect(
      evaluatePolicy(policy, {
    ...
    expect(
      evaluatePolicy(policy, {
        ...
        forbiddenCredentialMatches: 0,
        breakingContractChanges: 1,
      }).allowed,
    ).toBe(false);
  });

  it("loads a policy once from a bounded local file", async () => {
    const loaded = await loadPolicyFile(
      "test/fixtures/strict.policy.yaml",
      process.cwd(),
      true,
    );
