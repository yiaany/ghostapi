...
export async function saveToStateStore(
  id: string,
  obj: unknown,
): Promise<void> {
  if (
  ...
  await mutateJsonFile(getDataPaths().state, {}, sanitizeState, (state) =>
    boundState({ ...state, [id]: sanitizeSecrets(obj) }),
  );
}

export async function transactState<T>(
  operation: (state: Readonly<Record<string, unknown>>) => {
  ...
  if (result === undefined)
    throw new Error("Local state transaction did not return a result.");
  return result;
}

function boundState(state: Record<string, unknown>): Record<string, unknown> {
  const sanitized = Object.fromEntries(
    Object.entries(state).map(([key, value]) => [key, sanitizeSecrets(value)]),
  );
  const entries = Object.entries(sanitized);
  if (entries.length > MAX_STATE_ENTRIES)
    throw new Error(`State store is limited to ${MAX_STATE_ENTRIES} entries.`);
  for (const [key, value] of entries) {
    if (
      ...
      throw new Error("State key is invalid.");
    if (
      Buffer.byteLength(JSON.stringify(value), "utf8") > MAX_STATE_ENTRY_BYTES
    )
      throw new Error("State entry exceeds its size limit.");
  }
  if (Buffer.byteLength(JSON.stringify(sanitized), "utf8") > MAX_STATE_BYTES)
    throw new Error("State store exceeds its aggregate size limit.");
  return sanitized;
}
