...
export function registerRoutes(app: Express, config: ServerConfig): void {
    ...
    if (!addSseClient(response)) {
      response.status(503).json({
        error: {
          ...
          message: "Too many event stream clients.",
        },
      });
      return;
    }
  ...
  app.post("/api/fault-lab", async (req: Request, res: Response) => {
    try {
      ...
      const message =
        error instanceof Error ? error.message : "Invalid Fault Lab config.";
      res
        .status(400)
        .json({ error: { code: "invalid_fault_lab_config", message } });
    }
  });
  ...
  app.post("/api/ai-rules", async (_req: Request, res: Response) => {
    try {
      ...
      const message =
        error instanceof Error ? error.message : "Unable to generate AI rules.";
      res
        .status(500)
        .json({ error: { code: "ai_rules_generation_failed", message } });
    }
  });
    ...
    try {
      ...
      const message =
        error instanceof Error
          ? error.message
          : "Unable to generate repo setup.";
      res
        .status(500)
        .json({ error: { code: "setup_generation_failed", message } });
    }
  });
  ...
  app.post("/api/scenarios", async (req: Request, res: Response) => {
    try {
      ...
      const message =
        error instanceof Error ? error.message : "Unable to import scenario.";
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        res
          .status(201)
          .json(await saveEventsAsScenario(getEventsHistory(), req.body));
      } catch (error) {
        const message =
          error instanceof Error
            ? error.message
            : "Unable to save traffic as scenario.";
        res
          .status(400)
          .json({ error: { code: "scenario_save_failed", message } });
      }
    },
  );
  ...
  app.post("/api/scenarios/:id/replay", async (req: Request, res: Response) => {
    try {
      ...
      const message =
        error instanceof Error ? error.message : "Unable to replay scenario.";
  ...
  app.get("/api/scenarios/:id/export", async (req: Request, res: Response) => {
    try {
      ...
      const message =
        error instanceof Error ? error.message : "Unable to export scenario.";
      res.status(404).json({ error: { code: "scenario_not_found", message } });
    }
  });
  ...
  app.post("/api/scenarios/:id/share", async (req: Request, res: Response) => {
    try {
      ...
      const message =
        error instanceof Error ? error.message : "Unable to share scenario.";
      res.status(404).json({ error: { code: "scenario_not_found", message } });
    }
  });
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        if (!isClearTarget(target)) {
          res.status(400).json({
            error: {
              code: "invalid_clear_target",
              message: "Use target cache, state, events, or all.",
            },
          });
          return;
        }
  ...
  app.get("/api/safety-report", async (_req: Request, res: Response) => {
    try {
      ...
      const message =
        error instanceof Error
          ? error.message
          : "Unable to generate safety report.";
      res
        .status(500)
        .json({ error: { code: "safety_report_failed", message } });
    }
  });
  ...
  app.use((err: unknown, _req: Request, res: Response, _next: NextFunction) => {
    const message =
      err instanceof Error ? err.message : "Unknown local proxy error";
    res.status(500).json({
...
function isClearTarget(value: unknown): value is ClearTarget {
  return (
    typeof value === "string" && CLEAR_TARGETS.includes(value as ClearTarget)
  );
}
