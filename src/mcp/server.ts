...
import { updateFaultLabConfig } from "../fault/faultLab.js";
...
export async function startMcpServer(): Promise<void> {
  ...
  server.registerTool(
    "inspect_state",
    {
      ...
      description:
        "Return the current local object state from .ghostapi/state.json.",
    },
  ...
  server.registerTool(
    "set_api_behavior",
    {
      ...
      description:
        "Configure a deterministic local response for a method/path pair.",
      inputSchema: {
        path: z.string().min(1),
        method: z.enum([
          "GET",
  ...
  server.registerTool(
    "get_traffic_logs",
    {
      ...
      description:
        "Return the latest 50 captured HTTP requests from the in-memory dashboard buffer.",
    },
  ...
  server.registerTool(
    "toggle_chaos_mode",
    {
      ...
      description:
        "Enable or disable local latency/errors for resilience testing.",
      inputSchema: {
        enabled: z.boolean(),
