...
describe("Stripe core provider pack", () => {
  ...
  it("supports official SDK-style form requests, CRUD state, and cursor pagination", async () => {
    await withServer(async (baseUrl) => {
      const create = async (email: string) =>
        requestJson(baseUrl, "/v1/customers", {
          method: "POST",
          headers: {
      ...
      const page = await requestJson(
        baseUrl,
        `/v1/customers?limit=1&starting_after=${secondId}`,
      );
      expect(page.body).toMatchObject({
        object: "list",
        data: [expect.objectContaining({ id: firstId })],
        has_more: false,
        url: "/v1/customers",
      });
    });
  ...
  it("creates payment methods, payment intents, checkout sessions, and refunds from JSON", async () => {
    await withServer(async (baseUrl) => {
      ...
      const retrievedIntent = await requestJson(
        baseUrl,
        `/v1/payment_intents/${String(intent.body.id)}`,
      );
      expect(retrievedIntent.body).toMatchObject({
        id: intent.body.id,
        client_secret: intent.body.client_secret,
      });
      ...
      const checkout = await requestJson(baseUrl, "/v1/checkout/sessions", {
        ...
        body: JSON.stringify({
          ...
          line_items: [
            { price_data: { currency: "usd", unit_amount: 2500 }, quantity: 1 },
          ],
        }),
      });
  ...
  it("replays idempotent requests atomically without retaining the caller key", async () => {
    await withServer(async (baseUrl) => {
      ...
      const create = () =>
        requestJson(baseUrl, "/v1/payment_intents", {
          method: "POST",
          headers: {
      ...
      expect([
        first.response.headers.get("x-ghostapi-idempotency"),
        second.response.headers.get("x-ghostapi-idempotency"),
      ]).toContain("REPLAY");
  ...
  it("fails closed for unsupported versions and endpoints, malformed values, missing resources, and decline scenarios", async () => {
    await withServer(async (baseUrl) => {
      const unsupportedVersion = await requestJson(baseUrl, "/v1/customers", {
        method: "POST",
        headers: {
          "content-type": "application/json",
      ...
      expect(unsupportedVersion.response.headers.get("request-id")).toMatch(
        /^req_/,
      );
      expect(unsupportedVersion.body).toMatchObject({
        error: { code: "invalid_api_version", param: "Stripe-Version" },
      });

      const invalid = await requestJson(baseUrl, "/v1/payment_intents", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ amount: "wrong", currency: "usd" }),
      });
      expect(invalid.response.status).toBe(400);
      expect(invalid.body).toMatchObject({
        error: { code: "parameter_invalid_integer", param: "amount" },
      });

      const decline = await requestJson(baseUrl, "/v1/payment_intents", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          amount: 1000,
      ...
      expect(decline.body).toMatchObject({
      ...
      expect(missing.body).toMatchObject({
        error: { code: "resource_missing", param: "id" },
      });

      const endpoint = await requestJson(baseUrl, "/v1/events", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: "{}",
      });
  ...
  it("runs a deterministic trial, activation, failed renewal, invoice recovery, cancellation, and refund lifecycle", async () => {
    await withServer(async (baseUrl) => {
      ...
      const activated = await requestJson(
        baseUrl,
        `/v1/subscriptions/${String(trial.body.id)}/renew`,
        {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: "{}",
      ...
      expect(activated.body).toMatchObject({
        id: trial.body.id,
        status: "active",
        latest_invoice: expect.stringMatching(/^in_/),
      });

      const failedRenewal = await requestJson(
        baseUrl,
        `/v1/subscriptions/${String(trial.body.id)}/renew`,
        {
          method: "POST",
      ...
      const canceled = await requestJson(
        baseUrl,
        `/v1/subscriptions/${String(trial.body.id)}`,
        { method: "DELETE" },
      );
  ...
  it("delivers signed local webhook payloads with reproducible duplicate, delay, ordering, and invalid-signature modes", async () => {
    await withServer(async (baseUrl) => {
      ...
      const events = await requestJson(
        baseUrl,
        "/v1/events?delivery_mode=out_of_order",
      );
      const event = (events.body.data as Array<Record<string, unknown>>).find(
        (candidate) => candidate.type === "payment_intent.succeeded",
      );
      expect(event).toBeDefined();
      expect(event!.data).toMatchObject({
        object: { object: "payment_intent", id: intent.body.id },
      });

      const first = await fetch(
        `${baseUrl}/v1/events/${String(event!.id)}/deliver?delivery_mode=duplicate`,
      );
      const firstPayload = await first.text();
      expect(first.headers.get("x-ghostapi-webhook-delivery")).toBe(
        "duplicate",
      );
      expect(
        verifyStripeWebhookSignature(
          firstPayload,
          String(first.headers.get("stripe-signature")),
        ),
      ).toBe(true);
      ...
      const delayed = await fetch(
        `${baseUrl}/v1/events/${String(event!.id)}/deliver?delivery_mode=delayed&delay_ms=1`,
      );
      expect(delayed.headers.get("x-ghostapi-webhook-delivery")).toBe(
        "delayed",
      );
      const invalid = await fetch(
        `${baseUrl}/v1/events/${String(event!.id)}/deliver?delivery_mode=invalid_signature`,
      );
      expect(
        verifyStripeWebhookSignature(
          await invalid.text(),
          String(invalid.headers.get("stripe-signature")),
        ),
      ).toBe(false);
      expect(JSON.stringify(await getStateStore())).not.toContain(
  ...
  it("expires idempotency receipts after 24 hours instead of replaying stale responses", () => {
        ...
        snapshot: () => ({
          "stripe:idempotency_test": {
            ...
            response: {
              status: 200,
              headers: {},
              body: { id: "pi_stale", object: "payment_intent" },
            },
          },
        }),
      },
    ...
    const request = {
      ...
      query: {},
      headers: {
        "idempotency-key": "idempotency_test",
        "content-type": "application/json",
      },
    ...
    const response = stripePack.handleDeterministic({
      request,
