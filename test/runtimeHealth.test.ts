...
describe("runtime health and backup/restore", () => {
  ...
  it("reports ready on a fresh data directory and degraded on a corrupt store", async () => {
    ...
    expect(healthy.stores.every((store) => store.status === "healthy")).toBe(
      true,
    );
    ...
    await writeFile(
      join(root, "reliability", "costs.json"),
      "{ broken",
      "utf8",
    );
    const degraded = await checkRuntimeHealth();
    expect(degraded.ready).toBe(false);
    expect(degraded.stores.find((store) => store.id === "costs")?.status).toBe(
      "degraded",
    );
  ...
  it("backs up and restores a populated runtime for a disaster-recovery drill", async () => {
    ...
    const capability = access.issue({
      tenantId: "dr-tenant",
      principalId: "auditor",
      permissions: ["append", "read", "export", "manage_retention"],
    });
    const ledger = createLocalActionLedger({
      path: join(root, "action-ledger.json"),
      now: () => new Date(FIXED_NOW),
      accessAuthorizer: access.authorizer,
    });
    await ledger.configureRetention(capability, 30);

    const backup = await backupRuntime({
      destinationDir: join(root, "reliability", "backups", "dr-backup"),
    });
    ...
    const restoredCapability = restoredAccess.issue({
      tenantId: "dr-tenant",
      principalId: "auditor",
      permissions: ["append", "read"],
    });
    const restoredLedger = createLocalActionLedger({
      now: () => new Date(FIXED_NOW),
      accessAuthorizer: restoredAccess.authorizer,
    });
    expect(await restoredLedger.verifyTenant(restoredCapability)).toMatchObject(
      { valid: true },
    );
  });
  ...
  it("refuses to overwrite an existing backup destination", async () => {
    ...
    await backupRuntime({
      destinationDir: join(root, "reliability", "backups", "dr-backup-two"),
    });
    await expect(
      backupRuntime({
        destinationDir: join(root, "reliability", "backups", "dr-backup-two"),
  ...
  it("rejects a tampered backup during restore", async () => {
    ...
    const backup = await backupRuntime({
      destinationDir: join(root, "reliability", "backups", "dr-backup-three"),
    });
    ...
    await expect(
      restoreRuntimeBackup({
        sourceDir: backup.path,
        targetDir: join(root, "reliability", "backups", "dr-restored-three"),
      }),
  ...
  it("rejects a restore manifest that escapes the backup root", async () => {
    ...
    const backup = await backupRuntime({
      destinationDir: join(root, "reliability", "backups", "dr-backup-four"),
    });
    const manifest = JSON.parse(
      await readFile(join(backup.path, "manifest.json"), "utf8"),
    ...
    await writeFile(
      join(backup.path, "manifest.json"),
    ...
    await expect(
      restoreRuntimeBackup({
        sourceDir: backup.path,
        targetDir: join(root, "reliability", "backups", "dr-restored-four"),
      }),
  ...
  it("reports degraded when the inventory store is corrupt", async () => {
    ...
    expect(degraded.ready).toBe(false);
    expect(
      degraded.stores.find((store) => store.id === "inventory")?.status,
    ).toBe("degraded");
  ...
  it("refuses to restore into a non-empty target directory", async () => {
    ...
    const backup = await backupRuntime({
      destinationDir: join(root, "reliability", "backups", "dr-backup-six"),
    });
    const occupied = join(
      root,
      "reliability",
      "backups",
      "dr-restored-occupied",
    );
    await mkdir(occupied, { recursive: true });
    await writeFile(join(occupied, "existing.json"), "{}", "utf8");
    await expect(
      restoreRuntimeBackup({ sourceDir: backup.path, targetDir: occupied }),
  ...
  it("excludes only the canonical cache, runs, and backups locations", async () => {
    ...
    await writeFile(
      join(root, "contracts", "sub", "cache", "keep.txt"),
      "keep",
      "utf8",
    );
    const backup = await backupRuntime({
    ...
    expect(
      paths.every((path) => !path.startsWith("reliability/backups/")),
    ).toBe(true);
  });
  ...
  it("verifies the backup left the source data directory untouched", async () => {
    ...
    await backupRuntime({
      destinationDir: join(root, "reliability", "backups", "dr-backup-five"),
    });
