...
const workflowPath = fileURLToPath(
  new URL("../.github/workflows/ghostapi-pr-safety.yml", import.meta.url),
);
const ciWorkflowPath = fileURLToPath(
  new URL("../.github/workflows/ci.yml", import.meta.url),
);
const gameDayWorkflowPath = fileURLToPath(
  new URL(
    "../.github/workflows/ghostapi-kill-switch-game-day.yml",
    import.meta.url,
  ),
);
const safeFixturePath = fileURLToPath(
  new URL("../examples/ci-smoke/safe.mjs", import.meta.url),
);
const egressTestPath = fileURLToPath(
  new URL("./egressRun.test.ts", import.meta.url),
);
...
describe("GhostAPI PR safety workflow", () => {
  it("uses immutable actions, enforced execution, sanitized evidence, and safe comment boundaries", async () => {
    ...
    expect(workflow).toContain(
      "EVIDENCE_DIR: ${{ github.workspace }}/.ghostapi-ci/evidence",
    );
    expect(workflow).toContain(
      "GHOSTAPI_DATA_DIR: ${{ github.workspace }}/.ghostapi-ci/data",
    );
    expect(workflow).toContain(
      "ghostapi run --policy examples/ci-smoke/ghostapi.policy.yaml -- npm --prefix examples/ci-smoke run test:safe",
    );
    expect(workflow).toContain(
      "ghostapi run --policy examples/ci-smoke/ghostapi.policy.yaml -- npm --prefix examples/ci-smoke run test:production-egress",
    );
    expect(workflow).toContain(
      "ghostapi evidence generate --policy examples/ci-smoke/ghostapi.policy.yaml",
    );
    expect(workflow).toContain(
...
describe("standard CI workflow", () => {
  it("uses immutable actions and a read-only repository token", async () => {
    ...
    expect(workflow).toContain("shell: bash");
    expect(workflow).toContain(
      "apt-get install --yes util-linux iproute2 curl",
    );
    expect(workflow).toContain(
    ...
    expect(workflow).toContain(
      "npm test -- --run test/egressRun.test.ts --reporter=verbose",
    );
    expect(workflow).toContain(
      "Linux egress enforcement tests must execute without skips.",
    );
    expect(workflow).toContain("Hosted Docker build");
    expect(workflow).toContain(
      "docker build --file hosted/Dockerfile --tag ghostapi-hosted-ci hosted",
    );
...
describe("kill-switch game-day workflow", () => {
  it("schedules a local synthetic drill with immutable actions and no provider side effect", async () => {
    ...
    expect(workflow).toContain(
      "npm test -- --run test/safetyController.test.ts",
    );
    expect(workflow).not.toMatch(
      /provider|vault|credential|webhook|curl|fetch/i,
    );
  });
});
