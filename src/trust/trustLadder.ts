...
export type TrustCapabilities = {
  ...
  provider: "ghostapi-synthetic";
  environment: "synthetic";
  levels: TrustLevelCapability[];
};
export type TrustCanaryScope = {
  tenantIds: string[];
  resourceIds: string[];
...
export interface TrustOwnerVerifier {
  authenticate(identity: unknown): Promise<TrustOwner>;
}
export type TrustCanaryDecision = {
  target: TrustTarget;
  assigned: boolean;
  bucket: number;
  reason: string;
...
export type TrustOutcomeObservation = {
  target: TrustTarget;
  actionHash: string;
  outcomeHash: string;
...
export type TrustLadderState = {
  schemaVersion: 1;
  targets: TrustTargetState[];
  auditAnchor: string;
  audit: TrustAuditRecord[];
};
export type TrustLadderOptions = {
  path?: string;
  now?: () => Date;
  ownerVerifier: TrustOwnerVerifier;
};
...
export function createLocalTrustLadder(
  options: TrustLadderOptions,
): LocalTrustLadder {
...
export class LocalTrustLadder {
  ...
  constructor(options: TrustLadderOptions) {
    ...
    this.capabilities = validateCapabilities(
  ...
  async inspect(targetValue: unknown): Promise<TrustTargetState> {
    ...
    return this.mutate((state, now) =>
      clone(findOrCreateTarget(state, target, now)),
    );
  }
  ...
  async assignCanary(
    targetValue: unknown,
    policyValue: unknown,
  ): Promise<TrustCanaryDecision> {
    const target = validateTarget(targetValue);
    const policy = validatePolicy(policyValue);
    return this.mutate((state, now) => {
      const current = findOrCreateTarget(state, target, now);
      if (current.circuitBreaker === "open")
        throw new TrustLadderError(
          "Canary circuit breaker is open; no further canary action is allowed.",
        );
  ...
  async promote(input: {
    ...
    if (verifiedOwner.principalId !== policy.ownerPrincipalId)
      throw new TrustLadderError(
        "Trust promotion requires the configured verified owner decision.",
      );
    return this.mutate((state, now) => {
      const current = findOrCreateTarget(state, target, now);
      if (current.circuitBreaker === "open")
        throw new TrustLadderError(
          "Trust promotion is blocked by an open circuit breaker.",
        );
      const capability = capabilityFor(this.capabilities, requestedLevel);
      if (capability.status !== "supported")
        throw new TrustLadderError(
          `Trust level ${requestedLevel} is unsupported: ${capability.reason ?? "provider capability is unavailable"}`,
        );
      if (
        requestedLevel !== nextSupportedLevel(this.capabilities, current.level)
      )
        throw new TrustLadderError(
          "Trust promotion must move to the next supported level; automatic promotion is disabled.",
        );
      ...
      if (capability.requiresCanary) {
        ...
        if (!decision.assigned)
          throw new TrustLadderError(
            `Canary scope denies promotion: ${decision.reason}`,
          );
      }
      ...
      appendAudit(
        ...
        previous,
        requestedLevel,
        `verified owner ${verifiedOwner.principalId} approved promotion`,
        now,
      );
  ...
  async compareShadow(input: {
    prediction: unknown;
    actual: unknown;
  }): Promise<TrustComparisonEvidence> {
    const prediction = validateObservation(input.prediction);
    const actual = validateObservation(input.actual);
    if (!sameTarget(prediction.target, actual.target))
      throw new TrustLadderError(
        "Shadow comparison cannot mix synthetic identities.",
      );
    return this.mutate((state, now) => {
      ...
      const evidence = comparisonEvidence(
        prediction.target,
        "shadow",
        observationHash(prediction),
        observationHash(actual),
        now,
      );
      appendAudit(
        state,
        ...
        evidence.matched
          ...
          : "predicted action differs from actual input context",
        now,
      );
      return evidence;
    });
  ...
  async compareBoundedOutcome(input: {
    prediction: unknown;
    actual: unknown;
  }): Promise<TrustComparisonEvidence> {
    ...
    if (!sameTarget(prediction.target, actual.target))
      throw new TrustLadderError(
        "Bounded outcome comparison cannot mix synthetic identities.",
      );
    return this.mutate((state, now) => {
      const current = findOrCreateTarget(state, prediction.target, now);
      ...
      const evidence = comparisonEvidence(
        prediction.target,
        "bounded-auto",
        outcomeHash(prediction),
        outcomeHash(actual),
        now,
      );
      appendAudit(
        state,
        "bounded.outcome_compared",
        current,
        undefined,
        undefined,
        evidence.matched
          ? "predicted and actual bounded outcomes match"
          : "predicted and actual bounded outcomes differ",
        now,
      );
      return evidence;
    });
  ...
  async recordCanaryOutcome(input: {
    target: unknown;
    policy: unknown;
    outcome: TrustCanaryOutcome;
    reason: string;
  }): Promise<TrustTargetState> {
    const target = validateTarget(input.target);
    const policy = validatePolicy(input.policy);
    ...
    return this.mutate((state, now) => {
      ...
      if (current.level !== "bounded-auto")
        throw new TrustLadderError(
          "Canary outcomes require bounded-auto trust.",
        );
      const decision = canaryDecision(target, policy);
      if (!decision.assigned)
        throw new TrustLadderError(
          `Canary scope denies outcome recording: ${decision.reason}`,
        );
      ...
      if (
        ...
        appendAudit(
          ...
          previous,
          "approve",
          "policy violation automatically returned trust to approval mode",
          now,
        );
      }
      if (
        ...
        appendAudit(
          ...
          outcome === "violation" &&
            ...
            : "configured canary stop condition was breached",
          now,
        );
      }
      return clone(current);
  ...
  async rollbackToApproval(input: {
    target: unknown;
    policy: unknown;
    ownerIdentity: unknown;
    reason: string;
    ...
    const owner = validateOwner(
      await this.ownerVerifier.authenticate(input.ownerIdentity),
    );
    const reason = text(input.reason, "Trust rollback reason", 200);
    if (owner.principalId !== policy.ownerPrincipalId)
      throw new TrustLadderError(
        "Trust rollback requires the configured verified owner decision.",
      );
    return this.mutate((state, now) => {
      const current = findOrCreateTarget(state, target, now);
      const previous = current.level;
  ...
  private async read(): Promise<TrustLadderState> {
    ...
    if (!info.isFile() || info.isSymbolicLink())
      throw new TrustLadderError(
        "Trust ladder store must be a regular non-symlink file.",
      );
    const source = await readFile(this.path, "utf8");
    if (Buffer.byteLength(source, "utf8") > MAX_STORE_BYTES)
      throw new TrustLadderError("Trust ladder store exceeds its size limit.");
    try {
  ...
  private async mutate<T>(
    operation: (state: TrustLadderState, now: string) => T,
  ): Promise<T> {
  ...
  private timestamp(): string {
    ...
    if (!(value instanceof Date) || !Number.isFinite(value.getTime()))
      throw new TrustLadderError("Trust ladder clock is invalid.");
    return value.toISOString();
  }
}

export function createTestTrustOwnerVerifier(): {
  verifier: TrustOwnerVerifier;
  issue(
    input: Omit<TrustOwner, "principalId"> & { principalId?: string },
  ): TrustOwner;
} {
  ...
  return {
    verifier: {
      async authenticate(identity: unknown): Promise<TrustOwner> {
        if (
          ...
          throw new TrustLadderError(
            "Trust owner identity is not authenticated.",
          );
    ...
    issue(input): TrustOwner {
      const owner = Object.freeze(
        validateOwner({ ...input, principalId: input.principalId ?? input.id }),
      );
      issued.add(owner);
      return owner;
    },
...
function validateCapabilities(value: unknown): TrustCapabilities {
  ...
  exactKeys(
    capabilities,
    ["schemaVersion", "kind", "provider", "environment", "levels"],
    "Trust capabilities",
  );
  if (
    ...
    throw new TrustLadderError(
      "Only local synthetic trust capabilities are available.",
    );
  const levels = array(
    capabilities.levels,
  ...
  if (
    levels.length !== TRUST_LEVELS.length ||
    levels.some((capability, index) => capability.level !== TRUST_LEVELS[index])
  )
    throw new TrustLadderError(
      "Trust capabilities must define every level in stable order.",
    );
  if (levels.some((capability) => capability.externalSideEffects))
    throw new TrustLadderError(
      "Local trust capabilities cannot enable external side effects.",
    );
  return {
    schemaVersion: 1,
    kind: "ghostapi.trust-capabilities",
...
function validateCapability(value: unknown): TrustLevelCapability {
  ...
  if (capability.externalSideEffects !== false)
    throw new TrustLadderError(
      "Trust capability external side effects are invalid.",
    );
  const requiresActualContext = booleanFlag(
    capability.requiresActualContext,
    "Trust capability flag",
  );
  const requiresOfficialProviderSemantics = booleanFlag(
    capability.requiresOfficialProviderSemantics,
    "Trust capability flag",
  );
  const requiresApproval = booleanFlag(
    capability.requiresApproval,
    "Trust capability flag",
  );
  const requiresCanary = booleanFlag(
    capability.requiresCanary,
    "Trust capability flag",
  );
  const requiresOutcomeComparison = booleanFlag(
    capability.requiresOutcomeComparison,
    "Trust capability flag",
  );
  const reason =
  ...
  if (capability.status === "unsupported" && reason === undefined)
    throw new TrustLadderError(
      "Unsupported trust capability requires a reason.",
    );
  return {
    level,
...
function validatePolicy(value: unknown): TrustPromotionPolicy {
  ...
  exactKeys(
    canary,
    ["tenantIds", "resourceIds", "percentageBps"],
    "Trust canary scope",
  );
  const stopConditions = object(
    policy.stopConditions,
    "Trust canary stop conditions are invalid.",
  );
  exactKeys(
    stopConditions,
    ["maxViolations", "maxErrors"],
    "Trust canary stop conditions",
  );
  if (
    policy.violationResponse !== "demote_to_approve" &&
    policy.violationResponse !== "open_circuit_breaker"
  ...
  return {
    ...
    ownerPrincipalId: identifier(
      ...
      "Trust owner principal id",
    ),
    automaticPromotion: false,
    minimumRuns: positive(policy.minimumRuns, "Trust minimum runs", 1_000_000),
    requiredEvalIds: identifiers(
      policy.requiredEvalIds,
      "Trust required eval ids",
      MAX_EVALS,
    ),
    maxViolationRateBps: bps(
      policy.maxViolationRateBps,
      "Trust maximum violation rate",
    ),
    maxErrorRateBps: bps(policy.maxErrorRateBps, "Trust maximum error rate"),
    evidenceFreshnessMs: positive(
      policy.evidenceFreshnessMs,
      "Trust evidence freshness",
      MAX_FRESHNESS_MS,
    ),
    canary: {
      tenantIds: identifiers(
        canary.tenantIds,
        "Trust canary tenant ids",
        MAX_TARGETS,
        true,
      ),
      resourceIds: identifiers(
        canary.resourceIds,
        "Trust canary resource ids",
        MAX_TARGETS,
        true,
      ),
    ...
    stopConditions: {
      maxViolations: nonNegative(
        stopConditions.maxViolations,
        "Trust canary maximum violations",
        1_000_000,
      ),
      maxErrors: nonNegative(
        stopConditions.maxErrors,
        "Trust canary maximum errors",
        1_000_000,
      ),
    },
...
function validateEvidence(value: unknown): TrustPromotionEvidence {
  ...
  exactKeys(
    evidence,
    ["runCount", "violations", "errors", "evals", "observedAt"],
    "Trust promotion evidence",
  );
  const runCount = nonNegative(
    evidence.runCount,
    "Trust evidence run count",
    1_000_000,
  );
  const violations = nonNegative(
    evidence.violations,
    "Trust evidence violation count",
    runCount,
  );
  const errors = nonNegative(
    evidence.errors,
    "Trust evidence error count",
    runCount,
  );
  const evals: TrustPromotionEvidence["evals"] = array(
    evidence.evals,
    "Trust evidence evals",
    MAX_EVALS,
  ).map((entry) => {
    ...
    return {
      ...
      completedAt: timestamp(
        ...
        "Trust evidence eval completion",
      ),
    };
  });
  unique(
    evals.map((evaluation) => evaluation.id),
    "Trust evidence eval ids",
  );
  return {
    runCount,
    violations,
    ...
    observedAt: timestamp(
      evidence.observedAt,
      "Trust evidence observation time",
    ),
  };
}
...
function validateTarget(value: unknown): TrustTarget {
  ...
  exactKeys(
    target,
    ["provider", "environment", "tenantId", "resourceId"],
    "Trust target",
  );
  if (
    target.provider !== "ghostapi-synthetic" ||
    target.environment !== "synthetic"
  )
    throw new TrustLadderError(
      "Local trust preparation accepts synthetic identities only; production identities are unsupported.",
    );
  return {
    provider: "ghostapi-synthetic",
    environment: "synthetic",
...
function validateObservation(value: unknown): TrustObservation {
  ...
  exactKeys(
    observation,
    ["target", "actionHash", "operation", "contextHash"],
    "Shadow observation",
  );
  return {
...
function validateOutcomeObservation(value: unknown): TrustOutcomeObservation {
  ...
  exactKeys(
    observation,
    ["target", "actionHash", "outcomeHash", "executionReceiptHash"],
    "Bounded outcome observation",
  );
  return {
    target: validateTarget(observation.target),
    actionHash: hash(observation.actionHash, "Bounded action hash"),
    outcomeHash: hash(observation.outcomeHash, "Bounded outcome hash"),
    executionReceiptHash: hash(
      observation.executionReceiptHash,
      "Bounded execution receipt hash",
    ),
  };
}
...
function validateState(value: unknown): TrustLadderState {
  ...
  exactKeys(
    state,
    ["schemaVersion", "targets", "auditAnchor", "audit"],
    "Trust ladder store",
  );
  if (state.schemaVersion !== SCHEMA_VERSION)
    throw new TrustLadderError("Unsupported trust ladder schema version.");
  const targets = array(state.targets, "Trust targets", MAX_TARGETS).map(
    validateTargetState,
  );
  unique(
...
function validateTargetState(value: unknown): TrustTargetState {
  ...
  exactKeys(
    state,
    ["target", "level", "circuitBreaker", "canary", "updatedAt"],
    "Trust target state",
  );
  const canary = object(state.canary, "Trust target canary state is invalid.");
  exactKeys(
    canary,
    ["observed", "violations", "errors"],
    "Trust target canary state",
  );
  const observed = nonNegative(
    canary.observed,
    "Trust canary observed count",
    1_000_000,
  );
  const violations = nonNegative(
    canary.violations,
    "Trust canary violation count",
    observed,
  );
  const errors = nonNegative(
    canary.errors,
    "Trust canary error count",
    observed,
  );
...
function validateAudit(value: unknown): TrustAuditRecord {
  ...
  if (
    hash(record.recordHash, "Trust audit record hash") !==
...
function validateAuditChain(anchor: string, records: TrustAuditRecord[]): void {
  ...
  for (const record of records) {
    if (record.sequence !== sequence || record.previousHash !== previous)
      throw new TrustLadderError("Trust audit chain is invalid.");
...
function findOrCreateTarget(
  state: TrustLadderState,
  target: TrustTarget,
  now: string,
): TrustTargetState {
  const existing = state.targets.find((candidate) =>
  ...
  const created: TrustTargetState = {
    target: clone(target),
    level: "simulate",
...
function assertPromotionEvidence(
  ...
  policy: TrustPromotionPolicy,
  now: string,
): void {
  if (evidence.runCount < policy.minimumRuns)
  ...
  if (
    Date.parse(now) - Date.parse(evidence.observedAt) >
    policy.evidenceFreshnessMs
  )
    throw new TrustLadderError("Trust promotion evidence is stale.");
  for (const required of policy.requiredEvalIds) {
    const evaluation = evidence.evals.find(
      (candidate) => candidate.id === required,
    );
    if (evaluation === undefined || evaluation.status !== "passed")
      throw new TrustLadderError(
        `Trust promotion requires passing eval: ${required}`,
      );
    if (Date.parse(evaluation.completedAt) > Date.parse(now))
      throw new TrustLadderError(
        `Trust promotion eval is from the future: ${required}`,
      );
    if (
      Date.parse(now) - Date.parse(evaluation.completedAt) >
      policy.evidenceFreshnessMs
    )
      throw new TrustLadderError(`Trust promotion eval is stale: ${required}`);
  }
  if (
    rateBps(evidence.violations, evidence.runCount) > policy.maxViolationRateBps
  )
    throw new TrustLadderError(
      "Trust promotion violation rate exceeds policy.",
    );
  if (rateBps(evidence.errors, evidence.runCount) > policy.maxErrorRateBps)
    throw new TrustLadderError("Trust promotion error rate exceeds policy.");
}

function nextSupportedLevel(
  capabilities: TrustCapabilities,
  current: TrustLevel,
): TrustLevel | undefined {
  const index = TRUST_LEVELS.indexOf(current);
  for (const level of TRUST_LEVELS.slice(index + 1))
    if (capabilityFor(capabilities, level).status === "supported") return level;
  return undefined;
}
...
function capabilityFor(
  ...
  const capability = capabilities.levels.find(
    (candidate) => candidate.level === level,
  );
  if (capability === undefined)
    throw new TrustLadderError("Trust capability is missing.");
  return capability;
}
...
function canaryDecision(
  ...
  if (
    policy.canary.tenantIds.length > 0 &&
  ...
  if (
    policy.canary.resourceIds.length > 0 &&
...
function deterministicBucket(policyId: string, target: TrustTarget): number {
  return (
    createHash("sha256")
      .update(
        `ghostapi.trust.canary.v1:${policyId}:${targetKey(target)}`,
        "utf8",
      )
      .digest()
...
function appendAudit(
  state: TrustLadderState,
  event: string,
  target: TrustTargetState,
  fromLevel: TrustLevel | undefined,
  toLevel: TrustLevel | undefined,
  reason: string,
...
function assertActiveLevel(
  state: TrustTargetState,
  level: "shadow" | "bounded-auto",
): void {
  if (state.circuitBreaker === "open")
    throw new TrustLadderError(
      "Canary circuit breaker is open; comparisons cannot continue.",
    );
...
function trustLevel(value: unknown, label: string): TrustLevel {
  if (typeof value !== "string" || !TRUST_LEVELS.includes(value as TrustLevel))
...
function canaryOutcome(value: unknown): TrustCanaryOutcome {
  if (value !== "success" && value !== "violation" && value !== "error")
    throw new TrustLadderError("Trust canary outcome is invalid.");
  return value;
}
...
function identifiers(
  ...
  const values = array(value, label, max).map((entry) =>
    identifier(entry, label),
  );
  if (!allowEmpty && values.length === 0)
    throw new TrustLadderError(`${label} must not be empty.`);
  unique(values, label);
  return values.sort();
}
...
function hash(value: unknown, label: string): string {
  if (typeof value !== "string" || !HASH.test(value))
    throw new TrustLadderError(`${label} must be a SHA-256 hash.`);
  return value;
}
...
function object(value: unknown, message: string): Record<string, unknown> {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new TrustLadderError(message);
