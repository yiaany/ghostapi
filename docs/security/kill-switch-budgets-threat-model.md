...
- The action gateway calls the controller after plan/simulate and before its attempt record. The synthetic world calls the controller's `commit()` while it holds its own world lock, immediately before the atomic world write. The controller persists the final-check audit record before the world write, so audit-capacity failure cannot be reported after a mutation. A kill switch that wins before this final check blocks the local mutation.
...
- Queued actions have a fixed maximum of 100. A full queue returns backpressure. Stopping a matching scope moves queued records to the bounded dead-letter queue; no queued action is auto-retried.
