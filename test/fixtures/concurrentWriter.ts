...
if ((store !== "state" && store !== "behavior" && store !== "world" && store !== "scenario") || !prefix || !Number.isInteger(count) || count < 1) {
  throw new Error("Usage: concurrentWriter.ts state|behavior|world|scenario <prefix> <count>");
}
...
for (let index = 0; index < count; index += 1) {
  if (store === "state") {
    ...
    try {
      await importScenario({ id: `${prefix}-${index}`, title: `${prefix} ${index}`, steps: [{ method: "GET", path: `/${prefix}/${index}`, status: 200, body: {} }] });
    } catch (error) {
      if (!(error instanceof Error) || !error.message.includes("quota was reached")) throw error;
      continue;
    }
  }
  successfulWrites += 1;
