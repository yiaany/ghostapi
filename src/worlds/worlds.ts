...
export async function createWorld(
  ...
  await withFileLock(path, async () => {
    ...
    if (existing !== null)
      throw new SyntheticWorldError(
        `Synthetic world already exists: ${world.manifest.id}`,
      );
...
export async function forkWorld(
  ...
  const source = await readWorld(
    normalizeIdentifier(sourceId, "Source world id"),
  );
  const id = normalizeIdentifier(options.id, "Fork world id");
  const title = normalizeTitle(
    options.title ?? `${source.manifest.title} fork`,
  );
...
): Promise<WorldWorkflowReceipt> {
  const normalizedActionId = normalizeIdentifier(
    actionId,
    "Workflow action id",
  );
  let receipt: WorldWorkflowReceipt | undefined;
  await mutateWorld(
    normalizeIdentifier(id, "World id"),
    (world) => {
      const existing = world.state.receipts.find(
        (candidate) => candidate.actionId === normalizedActionId,
      );
      ...
      if (world.state.receipts.length >= MAX_WORKFLOW_RECEIPTS)
        throw new SyntheticWorldError(
          `Synthetic world permits at most ${MAX_WORKFLOW_RECEIPTS} workflow receipts.`,
        );
...
export function getWorldPath(id: string): string {
  return join(
    getDataPaths().worlds,
    `${normalizeIdentifier(id, "World id")}.world.json`,
  );
}
...
export function validateSyntheticWorld(value: unknown): SyntheticWorld {
  ...
  assertExactKeys(
    root,
    ["schemaVersion", "kind", "manifest", "revision", "baseline", "state"],
    "Synthetic world",
  );
  ...
  if (
    !Number.isInteger(root.revision) ||
    (root.revision as number) < 0 ||
    (root.revision as number) > 1_000_000
  )
...
async function readWorld(id: string): Promise<SyntheticWorld> {
  ...
  if (Buffer.byteLength(source, "utf8") > MAX_WORLD_BYTES)
    throw new SyntheticWorldError(
      `Synthetic world exceeds ${MAX_WORLD_BYTES} bytes.`,
    );
  try {
    const world = validateSyntheticWorld(JSON.parse(source));
    if (world.manifest.id !== id)
      throw new SyntheticWorldError(
        "Synthetic world manifest id does not match its file identity.",
      );
    return world;
  } catch (error) {
...
async function mutateWorld(
  id: string,
  mutation: (world: SyntheticWorld) => SyntheticWorld | Promise<SyntheticWorld>,
  commit?: <T>(operation: () => Promise<T>) => Promise<T>,
): Promise<SyntheticWorld> {
  const path = getWorldPath(id);
  return withFileLock(path, async () => {
    const current = await readWorld(id);
    const next = validateSyntheticWorld(
      await mutation(structuredClone(current)),
    );
...
async function ensureWorldFileIsSafe(
  path: string,
  required: boolean,
): Promise<void> {
  ...
  if (info === null) {
    if (required)
      throw new SyntheticWorldError(
        `Synthetic world was not found: ${path}`,
        "WORLD_NOT_FOUND",
      );
    return;
  }
  if (!info.isFile() || info.isSymbolicLink())
    throw new SyntheticWorldError(
      "Synthetic world must be a regular non-symlink file.",
    );
}
...
function normalizeManifest(value: unknown): SyntheticWorldManifest {
  ...
  if (manifest.version !== WORLD_VERSION)
    throw new SyntheticWorldError(
      `Unsupported synthetic world version: ${String(manifest.version)}`,
    );
  ...
  const providerProjections = normalizeProjections(
    manifest.providerProjections,
    personas[0]!.id,
    organizations[0]!.id,
    accounts,
  );
  ...
  return {
    ...
    version: WORLD_VERSION,
    seed,
    clock: { initialAt: clock.initialAt as string, timezone: "UTC" },
    personas,
    organizations,
    accounts,
    resources,
...
function normalizePersonas(value: unknown): SyntheticWorldManifest["personas"] {
  if (!Array.isArray(value) || value.length !== 1)
    throw new SyntheticWorldError(
      "Synthetic world requires exactly one synthetic persona.",
    );
  ...
  return [
    {
      ...
      displayName: normalizeSafeText(
        persona.displayName,
        "Synthetic persona displayName",
        80,
      ),
...
function normalizeOrganizations(
  value: unknown,
): SyntheticWorldManifest["organizations"] {
  if (!Array.isArray(value) || value.length !== 1)
    throw new SyntheticWorldError(
      "Synthetic world requires exactly one synthetic organization.",
    );
  const organization = readObject(
    value[0],
    "Synthetic organization is invalid.",
  );
  assertExactKeys(
    organization,
    ["id", "name", "slug"],
  ...
  return [
    {
      ...
      name: normalizeSafeText(
        organization.name,
        "Synthetic organization name",
        80,
      ),
      slug: normalizeIdentifier(
        organization.slug,
        "Synthetic organization slug",
      ),
    },
  ];
}
...
function normalizeAccounts(value: unknown): SyntheticWorldManifest["accounts"] {
  ...
  assertExactKeys(
    accounts,
    ["stripe", "github", "email", "genericRest"],
    "Synthetic world accounts",
  );
  const stripe = readObject(
    accounts.stripe,
    "Synthetic Stripe account is invalid.",
  );
  const github = readObject(
    accounts.github,
    "Synthetic GitHub account is invalid.",
  );
  const email = readObject(
    accounts.email,
    "Synthetic email account is invalid.",
  );
  const genericRest = readObject(
    accounts.genericRest,
    "Synthetic generic REST account is invalid.",
  );
  assertExactKeys(
    stripe,
    ["accountId", "customerPrefix"],
    "Synthetic Stripe account",
  );
  assertExactKeys(
    github,
    ["userLogin", "owner", "repository"],
  ...
  assertExactKeys(
    genericRest,
    ["tenantId", "basePath"],
    "Synthetic generic REST account",
  );
  ...
  if (domain !== "ghostapi.invalid")
    throw new SyntheticWorldError(
      "Synthetic world email addresses must use ghostapi.invalid.",
    );
  return {
    stripe: {
      accountId: normalizeIdentifier(
        stripe.accountId,
        "Synthetic Stripe account id",
      ),
      customerPrefix: normalizeIdentifier(
        stripe.customerPrefix,
        "Synthetic Stripe customer prefix",
      ),
    },
    github: {
      userLogin: normalizeIdentifier(
        github.userLogin,
        "Synthetic GitHub login",
      ),
      owner: normalizeIdentifier(github.owner, "Synthetic GitHub owner"),
      repository: normalizeIdentifier(
        github.repository,
...
function normalizeResources(
  value: unknown,
): SyntheticWorldManifest["resources"] {
  const resources = readObject(value, "Synthetic world resources are invalid.");
  assertExactKeys(
    resources,
    ["defaultPriceId", "recoveryIssueLabel"],
    "Synthetic world resources",
  );
  return {
    defaultPriceId: normalizeIdentifier(
      resources.defaultPriceId,
      "Synthetic price id",
    ),
    recoveryIssueLabel: normalizeIdentifier(
      resources.recoveryIssueLabel,
      "Synthetic issue label",
    ),
  };
}
...
function normalizeRelationships(
  ...
  personaId: string,
  organizationId: string,
): SyntheticWorldManifest["relationships"] {
  if (!Array.isArray(value) || value.length !== 1)
    throw new SyntheticWorldError(
      "Synthetic world requires one persona-to-organization relationship.",
    );
  const relationship = readObject(
    value[0],
    "Synthetic world relationship is invalid.",
  );
  assertExactKeys(
    relationship,
    ["personaId", "organizationId", "role"],
    "Synthetic world relationship",
  );
  if (
    ...
    throw new SyntheticWorldError(
      "Synthetic world relationship does not match canonical identity.",
    );
...
function normalizeProjections(
  ...
  personaId: string,
  organizationId: string,
  accounts: SyntheticWorldManifest["accounts"],
): SyntheticWorldManifest["providerProjections"] {
  const projections = readObject(
    value,
    "Synthetic world provider projections are invalid.",
  );
  assertExactKeys(
    projections,
    ["stripe", "github", "email", "genericRest"],
    "Synthetic world provider projections",
  );
  const stripe = readObject(
    projections.stripe,
    "Synthetic Stripe projection is invalid.",
  );
  const github = readObject(
    projections.github,
    "Synthetic GitHub projection is invalid.",
  );
  const email = readObject(
    projections.email,
    "Synthetic email projection is invalid.",
  );
  const genericRest = readObject(
    projections.genericRest,
    "Synthetic generic REST projection is invalid.",
  );
  assertExactKeys(
    stripe,
    ["personaId", "organizationId", "customerPrefix"],
    "Synthetic Stripe projection",
  );
  assertExactKeys(
    github,
    ["personaId", "organizationId", "repository"],
    "Synthetic GitHub projection",
  );
  assertExactKeys(
    email,
    ["personaId", "organizationId", "inboxAddress"],
    "Synthetic email projection",
  );
  assertExactKeys(
    genericRest,
    ["personaId", "organizationId", "tenantId"],
    "Synthetic generic REST projection",
  );
  if (
    stripe.personaId !== personaId ||
    ...
    throw new SyntheticWorldError(
      "Provider projections do not match canonical synthetic identity.",
    );
  }
  return {
    stripe: {
...
function normalizeLineage(
  value: unknown,
): NonNullable<SyntheticWorldManifest["lineage"]> {
  const lineage = readObject(value, "Synthetic world lineage is invalid.");
  assertExactKeys(
    lineage,
    ["parentId", "parentRevision"],
    "Synthetic world lineage",
  );
  if (
    !Number.isInteger(lineage.parentRevision) ||
    (lineage.parentRevision as number) < 0
  )
    throw new SyntheticWorldError(
      "Synthetic world lineage revision is invalid.",
    );
  return {
    parentId: normalizeIdentifier(
      lineage.parentId,
      "Synthetic world parent id",
    ),
    parentRevision: lineage.parentRevision as number,
  };
}
...
function normalizeState(
  ...
  assertExactKeys(
    state,
    ["clockAt", "stripe", "github", "email", "genericRest", "receipts"],
  ...
  const genericRest = readObject(
    state.genericRest,
    "Synthetic generic REST state is invalid.",
  );
  assertExactKeys(
    stripe,
    ["customers", "subscriptions"],
    "Synthetic Stripe state",
  );
  ...
  if (
    customers.length !== subscriptions.length ||
    ...
    throw new SyntheticWorldError(
      "Synthetic world providers have inconsistent workflow state.",
    );
  if (
    new Set(customers.map((customer) => customer.id)).size !==
      customers.length ||
    ...
    throw new SyntheticWorldError(
      "Synthetic world contains duplicate provider resources.",
    );
  }
  ...
  for (const receipt of receipts) {
    ...
    if (
      subscription?.customerId !== receipt.customerId ||
      ...
      throw new SyntheticWorldError(
        "Synthetic workflow receipt does not link one consistent cross-provider transition.",
      );
    }
  }
  return {
    clockAt: state.clockAt as string,
    stripe: { customers, subscriptions },
    github: { issues },
    email: { messages },
    genericRest: { failures },
...
function normalizeCustomers(
  value: unknown,
  manifest: SyntheticWorldManifest,
): SyntheticWorldState["stripe"]["customers"] {
  return readArray(
    value,
    "Synthetic Stripe customers",
    MAX_WORKFLOW_RECEIPTS,
  ).map((entry) => {
    const customer = readObject(entry, "Synthetic Stripe customer is invalid.");
    assertExactKeys(
      customer,
      ["id", "personaId", "organizationId"],
      "Synthetic Stripe customer",
    );
    if (
      customer.personaId !== manifest.personas[0]!.id ||
      customer.organizationId !== manifest.organizations[0]!.id
    )
      throw new SyntheticWorldError(
        "Synthetic Stripe customer identity is inconsistent.",
      );
    return {
...
function normalizeSubscriptions(
  value: unknown,
  customers: SyntheticWorldState["stripe"]["customers"],
  manifest: SyntheticWorldManifest,
): SyntheticWorldState["stripe"]["subscriptions"] {
  const customerIds = new Set(customers.map((customer) => customer.id));
  return readArray(
    value,
    "Synthetic Stripe subscriptions",
    MAX_WORKFLOW_RECEIPTS,
  ).map((entry) => {
    const subscription = readObject(
      entry,
      "Synthetic Stripe subscription is invalid.",
    );
    assertExactKeys(
      subscription,
      ["id", "customerId", "priceId", "status", "failureCode"],
      "Synthetic Stripe subscription",
    );
    if (
      !customerIds.has(subscription.customerId as string) ||
      subscription.priceId !== manifest.resources.defaultPriceId ||
      subscription.status !== "past_due" ||
      subscription.failureCode !== "card_declined"
    )
      throw new SyntheticWorldError(
        "Synthetic Stripe subscription is inconsistent.",
      );
    return {
      id: normalizeIdentifier(
        subscription.id,
        "Synthetic Stripe subscription id",
      ),
      customerId: subscription.customerId as string,
...
function normalizeIssues(
  value: unknown,
  subscriptions: SyntheticWorldState["stripe"]["subscriptions"],
  manifest: SyntheticWorldManifest,
): SyntheticWorldState["github"]["issues"] {
  const subscriptionsById = new Set(
    subscriptions.map((subscription) => subscription.id),
  );
  return readArray(value, "Synthetic GitHub issues", MAX_WORKFLOW_RECEIPTS).map(
    (entry) => {
      const issue = readObject(entry, "Synthetic GitHub issue is invalid.");
      assertExactKeys(
        issue,
        ["number", "repository", "title", "status", "relatedSubscriptionId"],
        "Synthetic GitHub issue",
      );
      if (
        ...
        issue.status !== "open" ||
        !subscriptionsById.has(issue.relatedSubscriptionId as string)
      )
        throw new SyntheticWorldError(
          "Synthetic GitHub issue is inconsistent.",
        );
      return {
        number: issue.number as number,
        repository: manifest.providerProjections.github.repository,
        title: normalizeSafeText(
          issue.title,
          "Synthetic GitHub issue title",
          160,
        ),
        status: "open",
        relatedSubscriptionId: issue.relatedSubscriptionId as string,
      };
    },
  );
}
...
function normalizeMessages(
  ...
  subscriptions: SyntheticWorldState["stripe"]["subscriptions"],
  manifest: SyntheticWorldManifest,
): SyntheticWorldState["email"]["messages"] {
  const subscriptionsById = new Set(
    subscriptions.map((subscription) => subscription.id),
  );
  return readArray(
    value,
    "Synthetic email messages",
    MAX_WORKFLOW_RECEIPTS,
  ).map((entry) => {
    const message = readObject(entry, "Synthetic email message is invalid.");
    assertExactKeys(
      message,
      ["id", "to", "template", "relatedSubscriptionId"],
      "Synthetic email message",
    );
    if (
      message.to !== manifest.accounts.email.inboxAddress ||
      message.template !== "subscription-payment-failed" ||
      !subscriptionsById.has(message.relatedSubscriptionId as string)
    )
    ...
    return {
      ...
      template: "subscription-payment-failed",
      relatedSubscriptionId: message.relatedSubscriptionId as string,
    };
  });
}
...
function normalizeFailures(
  ...
  subscriptions: SyntheticWorldState["stripe"]["subscriptions"],
  manifest: SyntheticWorldManifest,
): SyntheticWorldState["genericRest"]["failures"] {
  const subscriptionsById = new Set(
    subscriptions.map((subscription) => subscription.id),
  );
  return readArray(
    value,
    "Synthetic generic REST failures",
    MAX_WORKFLOW_RECEIPTS,
  ).map((entry) => {
    const failure = readObject(
      entry,
      "Synthetic generic REST failure is invalid.",
    );
    assertExactKeys(
      failure,
      ["id", "tenantId", "code", "relatedSubscriptionId"],
      "Synthetic generic REST failure",
    );
    if (
      failure.tenantId !== manifest.accounts.genericRest.tenantId ||
      failure.code !== "payment_failed" ||
      !subscriptionsById.has(failure.relatedSubscriptionId as string)
    )
      throw new SyntheticWorldError(
        "Synthetic generic REST failure is inconsistent.",
      );
    return {
      id: normalizeIdentifier(failure.id, "Synthetic generic REST failure id"),
      tenantId: manifest.accounts.genericRest.tenantId,
      code: "payment_failed",
      relatedSubscriptionId: failure.relatedSubscriptionId as string,
    };
  });
}
...
function normalizeReceipts(
  ...
  customers: SyntheticWorldState["stripe"]["customers"],
  subscriptions: SyntheticWorldState["stripe"]["subscriptions"],
  messages: SyntheticWorldState["email"]["messages"],
  issues: SyntheticWorldState["github"]["issues"],
  failures: SyntheticWorldState["genericRest"]["failures"],
  manifest: SyntheticWorldManifest,
  ...
  return readArray(
    ...
    const actionId = normalizeIdentifier(
      receipt.actionId,
      "Synthetic workflow action id",
    );
    if (
      actionIds.has(actionId) ||
      !customerIds.has(receipt.customerId as string) ||
      !subscriptionIds.has(receipt.subscriptionId as string) ||
      !messageIds.has(receipt.emailId as string) ||
      !issueNumbers.has(receipt.issueNumber as number) ||
      !failureIds.has(receipt.genericFailureId as string) ||
      !isTimestamp(receipt.completedAt) ||
      (receipt.completedAt as string) < manifest.clock.initialAt
    )
      throw new SyntheticWorldError(
        "Synthetic workflow receipt is inconsistent.",
      );
    actionIds.add(actionId);
    return {
      actionId,
      customerId: receipt.customerId as string,
      subscriptionId: receipt.subscriptionId as string,
      emailId: receipt.emailId as string,
      issueNumber: receipt.issueNumber as number,
      genericFailureId: receipt.genericFailureId as string,
...
function normalizeSeed(value: unknown): string {
  if (
    ...
    throw new SyntheticWorldError(
      "World seed must be a non-secret, non-PII 1-128 character string.",
    );
  return value;
}
...
function normalizeBasePath(value: unknown): string {
  if (typeof value !== "string" || !/^\/tenants\/[a-z0-9_-]+$/.test(value))
    throw new SyntheticWorldError(
      "Synthetic generic REST base path is invalid.",
    );
  return value;
}
...
function normalizeSyntheticInboxAddress(value: unknown): string {
  if (
    ...
    throw new SyntheticWorldError(
      "Synthetic inbox address must use the generated ghostapi.invalid format.",
    );
  return value;
}
...
function assertExactKeys(
  ...
  if (sensitive !== undefined)
    throw new SyntheticWorldError(
      `${label} contains secret-shaped field: ${sensitive}`,
    );
}
...
function deterministicTimestamp(seed: string): string {
  const seconds =
    Number.parseInt(shortHash(seed).slice(0, 8), 16) % (365 * 24 * 60 * 60);
