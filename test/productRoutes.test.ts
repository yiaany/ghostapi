...
describe("product routes", () => {
  ...
  it("serves generated setup and scenario actions", async () => {
    await withServer(async (baseUrl) => {
      ...
      expect(
        setup.files.some(
          (file: { path: string }) => file.path === ".cursor/mcp.json",
        ),
      ).toBe(true);
      ...
      const replayResponse = await fetch(
        `${baseUrl}/api/scenarios/github-issue-create/replay`,
        { method: "POST" },
      );
      expect(replayResponse.status).toBe(200);
      await expect(replayResponse.json()).resolves.toMatchObject({
        applied: [
          { method: "POST", path: "/repos/ghostapi/demo/issues", status: 201 },
        ],
      });
      ...
      const importResponse = await fetch(`${baseUrl}/api/scenarios`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          title: "Route custom",
          steps: [
            { method: "GET", path: "/route", status: 200, body: { ok: true } },
          ],
        }),
      });
      ...
      const testResponse = await fetch(
        `${baseUrl}/api/events/${events.at(-1)!.id}/test`,
      );
      await expect(testResponse.json()).resolves.toMatchObject({
        filename: expect.stringContaining("post-tasks"),
      });

      const saveResponse = await fetch(
        `${baseUrl}/api/scenarios/save-from-traffic`,
        {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({ title: "Saved Smoke" }),
        },
      );
