...
describe("synthetic worlds", () => {
  it("generates an identical initial world from the same manifest input and keeps one canonical identity across providers", () => {
    const first = createSyntheticWorld({
      id: "billing-world",
      seed: "fixed-seed",
      title: "Billing recovery",
    });
    const second = createSyntheticWorld({
      id: "billing-world",
      seed: "fixed-seed",
      title: "Billing recovery",
    });
    ...
    expect(first.manifest.providerProjections.stripe.personaId).toBe(
      first.manifest.personas[0]?.id,
    );
    expect(first.manifest.providerProjections.github.organizationId).toBe(
      first.manifest.organizations[0]?.id,
    );
    expect(first.manifest.providerProjections.email.inboxAddress).toBe(
      first.manifest.accounts.email.inboxAddress,
    );
    expect(first.manifest.accounts.email.inboxAddress).toMatch(
      /@ghostapi\.invalid$/,
    );
  });
  ...
  it("atomically creates linked Stripe, email, GitHub, and generic REST state for a failed subscription workflow", async () => {
    ...
    expect(world.state.stripe.customers).toEqual([
      expect.objectContaining({
        id: receipt.customerId,
        personaId: world.manifest.personas[0]?.id,
      }),
    ]);
    expect(world.state.stripe.subscriptions).toEqual([
      expect.objectContaining({
    ...
    expect(world.state.email.messages).toEqual([
      expect.objectContaining({
        id: receipt.emailId,
        relatedSubscriptionId: receipt.subscriptionId,
        to: world.manifest.accounts.email.inboxAddress,
      }),
    ]);
    expect(world.state.github.issues).toEqual([
      expect.objectContaining({
        number: receipt.issueNumber,
        relatedSubscriptionId: receipt.subscriptionId,
      }),
    ]);
    expect(world.state.genericRest.failures).toEqual([
      expect.objectContaining({
        id: receipt.genericFailureId,
        relatedSubscriptionId: receipt.subscriptionId,
      }),
    ]);

    await expect(
      runSubscriptionFailureWorkflow("workflow-world", "checkout-failed"),
    ).resolves.toEqual(receipt);
    expect((await inspectWorld("workflow-world")).state.receipts).toHaveLength(
      1,
    );
  });
  ...
  it("resets to the deterministic baseline and forks the current snapshot without sharing persisted state", async () => {
    ...
    expect(
      (await inspectWorld("reset-world-fork")).state.receipts,
    ).toHaveLength(2);
  });
  ...
  it("preserves concurrent cross-process world transactions without silent lost updates", async () => {
    ...
    await Promise.all([
      runWorldWriter("left", 20),
      runWorldWriter("right", 20),
    ]);
    ...
    expect(
      new Set(world.state.receipts.map((receipt) => receipt.actionId)),
    ).toHaveLength(40);
  });
  ...
  it("rejects secret-shaped and inconsistent world data before use", async () => {
    ...
    expect(() =>
      validateSyntheticWorld({
        ...world,
        manifest: { ...world.manifest, seed: "sk_live_not_allowed" },
      }),
    ).toThrow(SyntheticWorldError);
    expect(() =>
      createSyntheticWorld({ id: "pii-world", seed: "person@example.com" }),
    ).toThrow("non-PII");
    expect(() =>
      validateSyntheticWorld({
        ...world,
        manifest: {
          ...world.manifest,
          providerProjections: {
            ...world.manifest.providerProjections,
            email: {
              ...world.manifest.providerProjections.email,
              inboxAddress: "person@example.com",
            },
          },
        },
      }),
    ).toThrow("Provider projections");
  ...
  it("executes the documented world CLI lifecycle", async () => {
    ...
    expect(JSON.parse(created.stdout)).toMatchObject({
      manifest: { id: "cli-world", seed: "cli-seed" },
    });
    ...
    expect(JSON.parse(forked.stdout)).toMatchObject({
      manifest: { id: "cli-world-fork", lineage: { parentId: "cli-world" } },
    });
  }, 30_000);
});
...
function runWorldWriter(prefix: string, count: number): Promise<void> {
  return new Promise((resolve, reject) => {
    const child = spawn(
      process.execPath,
      ["--import", "tsx", workerPath, "world", prefix, String(count)],
    ...
    child.once("exit", (code) =>
      code === 0
        ? resolve()
        : reject(new Error(`World writer exited ${code}: ${stderr}`)),
    );
  });
}
