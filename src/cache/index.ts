...
export async function getCachedResponse(
  ...
  try {
    ...
    if (Buffer.byteLength(content, "utf8") > MAX_CACHE_ENTRY_BYTES)
      throw new Error("Cache entry exceeds its size limit.");
...
export async function setCachedResponse(
  ...
  if (
    Buffer.byteLength(JSON.stringify(validated), "utf8") > MAX_CACHE_ENTRY_BYTES
  )
    throw new Error("Cache entry exceeds its size limit.");
...
function validateCacheEntry(value: unknown): CacheEntry {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new Error("Cache entry is invalid.");
  ...
  if (
    ...
    throw new Error("Cache status is invalid.");
...
function cacheFilePath(provider: string, hash: string): string {
  if (!/^[a-z0-9_-]+$/i.test(provider) || !/^[a-z0-9_-]+$/i.test(hash)) {
    throw new Error(
      "Cache provider and hash must contain only letters, numbers, underscores, or hyphens.",
    );
  }
