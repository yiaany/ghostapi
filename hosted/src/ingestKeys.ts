...
export async function createIngestKey(
  ...
  if (
    ...
    input.expiresInDays > MAX_EXPIRY_DAYS
  ) {
    throw new Error(
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    try {
      ...
      if (key === undefined)
        throw new Error("Ingest key insert did not return a key.");

      await writeIngestKeyAuditEvent(
        client,
        input.projectId,
        input.actorId,
        "ci_ingest_key.created",
        key.id,
      );
      return {
        id: key.id,
        ...
        secret,
        expiresAt: new Date(key.expires_at).toISOString(),
      };
    } catch (error) {
...
export async function rotateIngestKey(
  ...
  await client.query(
    "update app.ci_ingest_keys set revoked_at = now() where id = $1 and project_id = $2",
    [input.keyId, input.projectId],
  );
...
export async function revokeIngestKey(
  ...
  await writeIngestKeyAuditEvent(
    ...
    input.projectId,
    input.actorId,
    "ci_ingest_key.revoked",
    key.id,
  );
  return true;
}

async function writeIngestKeyAuditEvent(
  client: PoolClient,
  projectId: string,
  actorId: string,
  action: string,
