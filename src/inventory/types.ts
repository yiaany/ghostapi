...
export const IMPORT_SOURCE_TYPES = ["config", "ci", "gateway", "cloud"] as const;
...
export const SIDE_EFFECT_KINDS = ["read", "create", "update", "communicate", "money_movement", "delete", "permission_change", "deployment"] as const;
...
export const EDGE_RELATIONS = ["owns", "uses", "connects", "exposes", "permits", "requires"] as const;
export type EdgeRelation = (typeof EDGE_RELATIONS)[number];

export const NODE_KINDS = ["agent", "identity", "tool", "provider", "resource", "side_effect", "credential"] as const;
...
export const REMEDIATION_KINDS = ["assign_owner", "reduce_scope", "revoke", "onboard_through_gateway", "create_eval"] as const;
export type RemediationKind = (typeof REMEDIATION_KINDS)[number];

export const REMEDIATION_STATUSES = ["proposed", "applied", "rejected", "expired"] as const;
export type RemediationStatus = (typeof REMEDIATION_STATUSES)[number];

export const FRESHNESS_DEFAULT_DAYS = { edgeStale: 90, credentialStale: 90, evidenceFresh: 30 } as const;
export type InventoryFreshnessDays = { edgeStaleDays?: number; credentialStaleDays?: number; evidenceFreshDays?: number };
...
export type InventoryOperator = { id: string; principalId: string; tenantId: string; permissions: readonly InventoryOperatorPermission[] };
export interface InventoryOperatorAuthorizer { authenticate(identity: unknown): Promise<InventoryOperator>; }

export type RecordProvenance = { sourceId: string; sourceType: ImportSourceType; sourceName: string; importedAt: string; importedBy: string };
...
export type RemediationRecord = {
  ...
  appliedAt?: string;
  result?: { description: string; reducedScopes?: string[]; evalScenarioId?: string; ownerId?: string };
};
...
export type ImportCounters = { preventedEgressAttempts?: number; reviewTimeMinutes?: number; meanTimeToContainMinutes?: number };
...
export type ImportRunRecord = {
  ...
  importedAt: string;
  importedBy: string;
  source: { sourceId: string; sourceType: ImportSourceType; sourceName: string; sourceVersion?: string };
  digest: string;
  expectedPolicyHashes: ExpectedPolicyHash[];
  counters?: ImportCounters;
  recordCounts: { agents: number; tools: number; identities: number; providers: number; resources: number; sideEffects: number; credentials: number; policies: number };
  edgesCreated: number;
...
export type BlastRadiusResource = {
  ...
  sensitive: boolean;
  providerId: string;
  sideEffects: { sideEffectId: string; kind: SideEffectKind; severity: ImpactSeverity; heuristic: boolean }[];
  pathCount: number;
  rules: string[];
};
...
export type RoiCounterRow = { sourceId: string; sourceType: ImportSourceType; sourceName: string; value: number };
...
export type InventoryExport = {
  ...
  tenantId: string;
  exportedAt: string;
  inventory: Omit<InventorySnapshot, "schemaVersion" | "kind" | "edges"> & { edges: GraphEdgeView[] };
  policyRecords: PolicyRecord[];
  scenarioRefs: { evalScenarioId: string; forFindingId: string; createdBy: string }[];
  evidenceMetadata: { agentId: string; lastEvidenceAt: string | null; status: FreshnessStatus }[];
...
export function canonicalPayloadDigest(payload: InventoryImportPayload): string {
...
export function validateImportPayload(value: unknown): InventoryImportPayload {
  ...
  if (payload.schemaVersion !== INVENTORY_SCHEMA_VERSION || payload.kind !== "ghostapi.inventory-import") throw new InventoryError("Unsupported inventory import schema.");
  const source = validateImportSource(payload.source);
  if (source.sourceType === "config" && payload.counters !== undefined) throw new InventoryError("Counters are only allowed for ci, gateway, or cloud imports.");
...
export function validateState(value: unknown): InventoryStoreState {
  ...
  if (state.schemaVersion !== INVENTORY_SCHEMA_VERSION || state.kind !== INVENTORY_KIND) throw new InventoryError("Unsupported inventory store schema.");
...
export function validateOperator(value: unknown): InventoryOperator {
  ...
  const permissions = array(operator.permissions, "inventory operator permissions", 8).map((permission) => {
    if (!isInventoryPermission(permission)) throw new InventoryError("Inventory operator permission is invalid.");
...
export function validateFindingsAndRemediations(value: unknown): { findings: FindingRecord[]; remediations: RemediationRecord[] } {
...
function isInventoryPermission(value: unknown): value is InventoryOperatorPermission {
...
function validateImportSource(value: unknown): InventoryImportPayload["source"] {
  ...
  if (!IMPORT_SOURCE_TYPES.includes(sourceType as ImportSourceType)) throw new InventoryError("Inventory import source type must be config, ci, gateway, or cloud.");
  return {
...
function validateCounters(value: unknown): ImportCounters {
  ...
  for (const key of ["preventedEgressAttempts", "reviewTimeMinutes", "meanTimeToContainMinutes"] as const) {
    if (counters[key] !== undefined) result[key] = nonNegative(counters[key], `Import counter ${key}`, 9_999_999_999);
  }
  return result;
}
...
function validateImportedAgent(value: unknown): ImportedAgent {
  ...
  if (typeof record.gatewayManaged !== "boolean" || typeof record.killSwitchEnabled !== "boolean") throw new InventoryError("Imported agent booleans are invalid.");
  return {
...
function validateImportedTool(value: unknown): ImportedTool {
  ...
  if (!TOOL_KINDS.includes(kind as ToolKind)) throw new InventoryError("Imported tool kind is invalid.");
  return {
...
function validateImportedIdentity(value: unknown): ImportedIdentity {
  ...
  if (!IDENTITY_ROLES.includes(role as IdentityRole)) throw new InventoryError("Imported identity role is invalid.");
  return {
...
function validateImportedProvider(value: unknown): ImportedProvider {
  ...
  if (typeof record.gatewayBound !== "boolean" || typeof record.killSwitchApplied !== "boolean") throw new InventoryError("Imported provider booleans are invalid.");
  return {
...
function validateImportedResource(value: unknown): ImportedResource {
  ...
  if (typeof record.sensitive !== "boolean") throw new InventoryError("Imported resource sensitive flag is invalid.");
  return {
...
function validateImportedSideEffect(value: unknown): ImportedSideEffect {
  ...
  if (!SIDE_EFFECT_KINDS.includes(kind as SideEffectKind)) throw new InventoryError("Imported side effect kind is invalid.");
  if (!IMPACT_SEVERITIES.includes(severity as ImpactSeverity)) throw new InventoryError("Imported side effect severity is invalid.");
  return {
...
function validateImportedCredential(value: unknown): ImportedCredential {
  ...
  if (record.status !== "active" && record.status !== "revoked") throw new InventoryError("Imported credential status is invalid.");
  if (typeof record.gatewayBound !== "boolean") throw new InventoryError("Imported credential gatewayBound flag is invalid.");
  return {
...
function validateSourceRecord(value: unknown): ImportSourceRecord {
  ...
  if (!IMPORT_SOURCE_TYPES.includes(sourceType as ImportSourceType)) throw new InventoryError("Import source type is invalid.");
  return {
...
function validateImportRunRecord(value: unknown): ImportRunRecord {
  ...
  if (record.schemaVersion !== INVENTORY_SCHEMA_VERSION || record.kind !== "ghostapi.inventory-import-run") throw new InventoryError("Unsupported import run schema.");
  if (record.status !== "completed") throw new InventoryError("Import run status is invalid.");
  ...
  const sourceType = source.sourceType;
  if (!IMPORT_SOURCE_TYPES.includes(sourceType as ImportSourceType)) throw new InventoryError("Import run source type is invalid.");
  ...
  if (sourceType === "config" && counters !== undefined) throw new InventoryError("Counters are only allowed for ci, gateway, or cloud imports.");
  return {
    schemaVersion: 1,
    kind: "ghostapi.inventory-import-run",
...
function validateAgentRecord(value: unknown): AgentRecord {
  ...
  if (record.schemaVersion !== INVENTORY_SCHEMA_VERSION || record.kind !== "ghostapi.inventory-agent") throw new InventoryError("Unsupported agent record schema.");
  if (typeof record.gatewayManaged !== "boolean" || typeof record.killSwitchEnabled !== "boolean") throw new InventoryError("Agent record booleans are invalid.");
  return {
    schemaVersion: 1,
    kind: "ghostapi.inventory-agent",
...
function validateToolRecord(value: unknown): ToolRecord {
  ...
  if (record.schemaVersion !== INVENTORY_SCHEMA_VERSION) throw new InventoryError("Unsupported tool record schema.");
  const toolKind = record.kind;
  if (!TOOL_KINDS.includes(toolKind as ToolKind)) throw new InventoryError("Tool record kind is invalid.");
  return {
    schemaVersion: 1,
...
function validateIdentityRecord(value: unknown): IdentityRecord {
  ...
  if (record.schemaVersion !== INVENTORY_SCHEMA_VERSION || record.kind !== "ghostapi.inventory-identity") throw new InventoryError("Unsupported identity record schema.");
  const role = record.role;
  if (!IDENTITY_ROLES.includes(role as IdentityRole)) throw new InventoryError("Identity record role is invalid.");
  return {
    schemaVersion: 1,
    kind: "ghostapi.inventory-identity",
...
function validateProviderRecord(value: unknown): ProviderRecord {
  ...
  if (record.schemaVersion !== INVENTORY_SCHEMA_VERSION || record.kind !== "ghostapi.inventory-provider") throw new InventoryError("Unsupported provider record schema.");
  if (typeof record.gatewayBound !== "boolean" || typeof record.killSwitchApplied !== "boolean") throw new InventoryError("Provider record booleans are invalid.");
  return {
    schemaVersion: 1,
    kind: "ghostapi.inventory-provider",
...
function validateResourceRecord(value: unknown): ResourceRecord {
  ...
  if (record.schemaVersion !== INVENTORY_SCHEMA_VERSION || record.kind !== "ghostapi.inventory-resource") throw new InventoryError("Unsupported resource record schema.");
  if (typeof record.sensitive !== "boolean") throw new InventoryError("Resource record sensitive flag is invalid.");
  return {
    schemaVersion: 1,
    kind: "ghostapi.inventory-resource",
...
function validateSideEffectRecord(value: unknown): SideEffectRecord {
  ...
  if (record.schemaVersion !== INVENTORY_SCHEMA_VERSION) throw new InventoryError("Unsupported side effect record schema.");
  const kind = record.kind;
  const severity = record.severity;
  if (!SIDE_EFFECT_KINDS.includes(kind as SideEffectKind)) throw new InventoryError("Side effect record kind is invalid.");
  if (!IMPACT_SEVERITIES.includes(severity as ImpactSeverity)) throw new InventoryError("Side effect record severity is invalid.");
  return {
    schemaVersion: 1,
...
function validateCredentialRecord(value: unknown): CredentialRecord {
  ...
  if (record.schemaVersion !== INVENTORY_SCHEMA_VERSION || record.kind !== "ghostapi.inventory-credential") throw new InventoryError("Unsupported credential record schema.");
  if (record.status !== "active" && record.status !== "revoked") throw new InventoryError("Credential record status is invalid.");
  if (typeof record.gatewayBound !== "boolean") throw new InventoryError("Credential record gatewayBound flag is invalid.");
  return {
    schemaVersion: 1,
    kind: "ghostapi.inventory-credential",
...
function validatePolicyRecord(value: unknown): PolicyRecord {
  ...
  if (record.schemaVersion !== INVENTORY_SCHEMA_VERSION || record.kind !== "ghostapi.inventory-policy") throw new InventoryError("Unsupported policy record schema.");
  return {
    schemaVersion: 1,
    kind: "ghostapi.inventory-policy",
...
function validateEdge(value: unknown): GraphEdge {
  ...
  if (!NODE_KINDS.includes(record.sourceKind as NodeKind)) throw new InventoryError("Graph edge source kind is invalid.");
  if (!NODE_KINDS.includes(record.targetKind as NodeKind)) throw new InventoryError("Graph edge target kind is invalid.");
  if (!EDGE_RELATIONS.includes(record.relation as EdgeRelation)) throw new InventoryError("Graph edge relation is invalid.");
  return {
...
function validateFindingRecord(value: unknown): FindingRecord {
  ...
  if (record.schemaVersion !== INVENTORY_SCHEMA_VERSION) throw new InventoryError("Unsupported finding record schema.");
  if (!FINDING_KINDS.includes(record.kind as FindingKind)) throw new InventoryError("Finding kind is invalid.");
  if (!FINDING_SEVERITIES.includes(record.severity as FindingSeverity)) throw new InventoryError("Finding severity is invalid.");
  if (!FINDING_BASIS.includes(record.basis as FindingBasis)) throw new InventoryError("Finding basis is invalid.");
  if (record.status !== "open" && record.status !== "resolved") throw new InventoryError("Finding status is invalid.");
  if (record.targetKind !== "agent" && record.targetKind !== "identity" && record.targetKind !== "tool" && record.targetKind !== "provider" && record.targetKind !== "resource" && record.targetKind !== "side_effect" && record.targetKind !== "credential" && record.targetKind !== "environment" && record.targetKind !== "policy") throw new InventoryError("Finding target kind is invalid.");
  return {
    schemaVersion: 1,
...
function validateRemediationRecord(value: unknown): RemediationRecord {
  ...
  if (record.schemaVersion !== INVENTORY_SCHEMA_VERSION) throw new InventoryError("Unsupported remediation record schema.");
  if (!REMEDIATION_KINDS.includes(record.kind as RemediationKind)) throw new InventoryError("Remediation kind is invalid.");
  if (!REMEDIATION_STATUSES.includes(record.status as RemediationStatus)) throw new InventoryError("Remediation status is invalid.");
  if (record.targetKind !== "agent" && record.targetKind !== "identity" && record.targetKind !== "tool" && record.targetKind !== "provider" && record.targetKind !== "resource" && record.targetKind !== "side_effect" && record.targetKind !== "credential" && record.targetKind !== "environment" && record.targetKind !== "policy") throw new InventoryError("Remediation target kind is invalid.");
...
function validateRemediationResult(value: unknown): NonNullable<RemediationRecord["result"]> {
...
function validateProvenance(value: unknown): RecordProvenance {
  ...
  if (!IMPORT_SOURCE_TYPES.includes(sourceType as ImportSourceType)) throw new InventoryError("Record provenance source type is invalid.");
  return {
...
export function buildRecordProvenance(source: InventoryImportPayload["source"], importedAt: string, importedBy: string): RecordProvenance {
...
export function edgeIdFor(sourceKind: NodeKind, sourceId: string, targetKind: NodeKind, targetId: string, relation: EdgeRelation): string {
  return `edge-${sha256Digest(`${sourceKind}:${sourceId}:${targetKind}:${targetId}:${relation}`).slice(0, 40)}`;
}

export function freshnessStatusFor(lastSeenAt: string, now: string, staleDays: number): FreshnessStatus {
...
function array(value: unknown, label: string, max: number): unknown[] {
  if (!Array.isArray(value) || value.length > max) throw new InventoryError(`${label} is invalid.`);
  return value;
}
...
function unique(values: string[], label: string): void {
  if (new Set(values).size !== values.length) throw new InventoryError(`${label} must be unique.`);
}

function identifier(value: unknown, label: string): string {
  if (typeof value !== "string" || !IDENTIFIER.test(value) || sanitizeSecretString(value) !== value) throw new InventoryError(`${label} must be a safe identifier.`);
  return value;
}
...
function scopes(value: unknown, label: string, max: number): string[] {
  return array(value, label, max).map((entry) => {
    if (typeof entry !== "string" || !SCOPE_IDENTIFIER.test(entry) || sanitizeSecretString(entry) !== entry) throw new InventoryError(`${label} must be safe scope identifiers.`);
    return entry;
  });
}
...
function timestamp(value: unknown, label: string): string {
  if (typeof value !== "string" || !TIMESTAMP.test(value) || !Number.isFinite(Date.parse(value))) throw new InventoryError(`${label} must be an ISO UTC timestamp.`);
  return value;
}

function hash(value: unknown, label: string): string {
  if (typeof value !== "string" || !HASH.test(value)) throw new InventoryError(`${label} must be a sha256 hash.`);
  return value;
}

function nonNegative(value: unknown, label: string, max: number): number {
  if (typeof value !== "number" || !Number.isInteger(value) || value < 0 || value > max) throw new InventoryError(`${label} is invalid.`);
  return value;
}
