...
describe("local opt-in product telemetry", () => {
  ...
  it("does not collect before explicit opt-in and records only aggregate counters after opt-in", async () => {
    ...
    expect(await readProductTelemetry()).toMatchObject({
      enabled: false,
      counters: { init_completed: 0 },
      activeWeeks: [],
    });
    ...
    expect(await readProductTelemetry()).toEqual({
      ...
      counters: {
        init_completed: 0,
        enforced_run_completed: 1,
        evidence_generated: 25,
        eval_completed: 0,
      },
  ...
  it("deletes local counters on opt-out and rejects malformed persisted state", async () => {
    ...
    expect(await readProductTelemetry()).toMatchObject({
      enabled: false,
      counters: { eval_completed: 0 },
    });

    await writeFile(
      getDataPaths().productTelemetry,
      JSON.stringify({
        schemaVersion: 1,
        enabled: true,
        counters: {},
    ...
    await expect(readProductTelemetry()).rejects.toBeInstanceOf(
      ProductTelemetryError,
    );
  });
  ...
  it("fails closed on clock rollback and counter overflow", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({
        ...
        enabled: true,
        counters: {
          init_completed: Number.MAX_SAFE_INTEGER,
    ...
    ).rejects.toThrow("counter capacity reached");
  });
});
