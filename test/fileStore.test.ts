...
import { ensurePrivateDirectory, withFileLock } from "../src/storage/fileStore.js";
...
describe("file store locking", () => {
  it("reclaims malformed abandoned lock files", async () => {
    ...
    await expect(withFileLock(filePath, async () => "acquired", { staleLockMs: 0, lockTimeoutMs: 500 })).resolves.toBe("acquired");
  });
});
