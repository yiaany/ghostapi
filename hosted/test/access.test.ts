...
test("locks the organization and re-reads the current role before mutation", async () => {
  ...
  const client = {
    async query(text: string) {
      ...
      if (text.includes("select role from app.organization_memberships")) return { rows: [{ role: "admin" }] };
      return { rows: [] };
    }
  };

  const result = await withAuthorizedMutation(client as never, { organizationId: "org_123" }, "user_123", "admin", async () => {
    calls.push("mutation");
    return "created";
  });
...
test("serializes project mutations on the organization and rejects a revoked role", async () => {
  ...
  const result = await withAuthorizedMutation(client as never, { projectId: "project_123" }, "user_123", "developer", async () => {
    mutated = true;
    return true;
  });
