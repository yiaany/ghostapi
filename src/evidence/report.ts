...
export type EvidenceReport = {
  ...
  run: {
    id: string;
    status:
      "unknown" | "preparing" | "running" | "failed-to-start" | "finished";
    startedAt?: string;
    finishedAt?: string;
    exitCode?: number;
  };
...
export async function generateEvidenceReport(
...
): Promise<{ report: EvidenceReport; path: string }> {
  const projectRoot = await realProjectRoot(
  ...
  const contractDiff = await loadContractDiff(
    options.contractBaselinePath,
    options.contractCandidatePath,
    projectRoot,
  );
  const report = buildEvidenceReport({
    ...
    policyHash:
      loadedPolicy?.hash ?? readString(run?.evidence.policy?.policyHash),
    requiredScenarios:
      loadedPolicy?.policy.requiredScenarios ??
...
export function buildEvidenceReport(input: {
  ...
  const runUsedNamespace =
    input.runEvidence?.backend === "linux-network-namespace" &&
    runStatus === "finished";
  ...
  const providers = uniqueSorted(
    sanitizedEvents.map((event) => event.provider).filter(Boolean),
  );
  ...
  const runOutputSecretMatches =
    readNonNegativeInteger(input.runEvidence?.output?.secretMatches) ?? 0;
  ...
  const retryCount = sanitizedEvents.filter(
    (event) => event.statusCode === 429 || hasRetryAfter(event),
  ).length;
  const productionAttempts = null;
  const contractDrift =
    input.contractDiff === undefined
  ...
  if (sanitizedEvents.length < input.events.length)
    warnings.push(
      `Event evidence was truncated to ${MAX_EVENTS_FOR_REPORT} events.`,
    );
  if (input.runEvidence === null)
    warnings.push(
      "Run evidence is missing; report cannot prove launcher lifecycle or blocked kernel egress attempts.",
    );
  warnings.push(
  ...
  if (secretMatches > 0)
    findings.push({
      ...
      severity: "fail",
      message:
        "Secret-shaped values were detected and summarized without raw values.",
      count: secretMatches,
    });
  if (input.contractDiff !== undefined) {
    ...
    if (contractDrift.uncertain > 0)
      findings.push({
        ...
        severity: "warning",
        message:
          "Contract changes need compatibility review because client tolerance is unknown.",
  ...
  const failCount = reportWithoutHash.findings.filter(
    (finding) => finding.severity === "fail",
  ).length;
  const warningCount =
    reportWithoutHash.findings.filter(
      (finding) => finding.severity === "warning",
    ).length + reportWithoutHash.warnings.length;
...
export async function loadEvidenceReport(
  path: string,
  ...
  if (Buffer.byteLength(source, "utf8") > MAX_REPORT_BYTES)
    throw new EvidenceReportError(
      `Evidence report exceeds ${MAX_REPORT_BYTES} bytes.`,
    );
  let parsed: unknown;
  try {
...
export function validateEvidenceReport(value: unknown): EvidenceReport {
  ...
  if (report.artifact.logicalHash !== expectedHash)
    throw new EvidenceReportError(
      "Evidence report logical hash does not match its contents.",
    );
  return report;
}
...
export function formatEvidenceReport(report: EvidenceReport): string {
  ...
  return [
    ...
    ...report.findings.map(
      (finding) =>
        `  ${finding.severity.toUpperCase()} ${escapeTerminal(finding.id)}: ${escapeTerminal(finding.message)}`,
    ),
    ...report.warnings.map(
      (warning) => `  WARNING evidence: ${escapeTerminal(warning)}`,
    ),
  ].join("\n");
}

export function compareEvidenceReports(
  left: EvidenceReport,
  right: EvidenceReport,
): EvidenceCompareResult {
  const differences: string[] = [];
  if (left.artifact.logicalHash !== right.artifact.logicalHash)
    differences.push("logicalHash differs");
  if (left.summary.passed !== right.summary.passed)
    differences.push("summary.passed differs");
  if (left.summary.failCount !== right.summary.failCount)
    differences.push("summary.failCount differs");
  if (left.egress.productionAttempts !== right.egress.productionAttempts)
  ...
  if (
    stableStringify(left.contractDrift ?? emptyContractDrift()) !==
...
export function formatEvidenceCompare(result: EvidenceCompareResult): string {
  ...
  return [
    ...
    ...result.differences.map(
      (difference) => `  - ${escapeTerminal(difference)}`,
    ),
  ].join("\n");
}
...
async function writeEvidenceReport(
  ...
  const target =
    outPath === undefined
      ? join(
          getDataPaths().reports,
          `${report.run.id === "unknown" ? "latest" : report.run.id}.json`,
        )
  ...
  for (const path of paths.reverse()) {
    ...
    try {
      ...
      if (!info.isFile() || info.isSymbolicLink())
        throw new EvidenceReportError(
          "Event evidence must be a regular non-symlink file.",
        );
    ...
    if (bytes > MAX_JSONL_BYTES)
      throw new EvidenceReportError(
        `Event evidence exceeds ${MAX_JSONL_BYTES} bytes.`,
      );
...
async function readRunEvidence(
  ...
  projectRoot: string,
): Promise<{ path: string; evidence: RunEvidence } | null> {
  ...
  if (Buffer.byteLength(source, "utf8") > MAX_RUN_EVIDENCE_BYTES)
    throw new EvidenceReportError(
      `Run evidence exceeds ${MAX_RUN_EVIDENCE_BYTES} bytes.`,
    );
  try {
...
async function loadContractDiff(
  ...
  if (baselinePath === undefined || candidatePath === undefined)
    throw new EvidenceReportError(
      "Contract drift evidence requires both --contract-baseline and --contract-candidate.",
    );
  try {
...
async function resolveOutputPath(
  ...
  if (!parent.isDirectory() || parent.isSymbolicLink())
    throw new EvidenceReportError(
      "Evidence output parent must be a real directory, not a symlink.",
    );
  ...
  if (existing?.isSymbolicLink())
    throw new EvidenceReportError(
      "Evidence output path must not be a symlink.",
    );
  return target;
}
...
function normalizeEvidenceReport(value: unknown): EvidenceReport {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new EvidenceReportError("Evidence report must be an object.");
  const report = value as EvidenceReport;
  if (report.schemaVersion !== 1)
    throw new EvidenceReportError(
      "Unsupported evidence report schema version.",
    );
  if (
    typeof report.artifact?.logicalHash !== "string" ||
    !/^[a-f0-9]{64}$/.test(report.artifact.logicalHash)
  )
    throw new EvidenceReportError(
      "Evidence report is missing a valid logical hash.",
    );
  if (report.artifact.canonicalization !== "json-stable-sorted-keys-v1")
    throw new EvidenceReportError(
      "Unsupported evidence report canonicalization.",
    );
  if (
    typeof report.run?.id !== "string" ||
...
function detectSecretCategories(value: unknown): {
  ...
  const visit = (entry: unknown, key?: string) => {
    ...
    if (typeof entry === "string") {
      ...
      if (/sk_live_[A-Za-z0-9_-]+/.test(entry))
        categories.add("stripe-live-key");
      if (/ghp_[A-Za-z0-9_]+|github_pat_[A-Za-z0-9_]+/.test(entry))
...
function containsSecretPattern(value: unknown): boolean {
  if (typeof value === "string")
    return /sk_live_[A-Za-z0-9_-]+|ghp_[A-Za-z0-9_]+|github_pat_[A-Za-z0-9_]+|Bearer\s+\S+|SG\.[A-Za-z0-9_.-]+/i.test(
      value,
    );
...
function firstTimestamp(
  events: Array<{ timestamp?: unknown }>,
): string | undefined {
  return events
...
function hasRetryAfter(event: ProxyEvent): boolean {
  return (
    JSON.stringify(event.response).toLowerCase().includes("retry-after") ||
...
function assertPathInsideAllowedRoots(
  ...
  if (!isInside(projectRoot, target) && !isInside(getDataPaths().root, target))
    throw new EvidenceReportError(
      "Evidence path traversal outside the project root or GHOSTAPI_DATA_DIR is not allowed.",
    );
