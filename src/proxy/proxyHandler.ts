...
import {
  corruptStripeWebhookSignature,
  createStripeWebhookSignature,
  isStripeWebhookDeliveryRequest,
  readStripeWebhookDelivery,
} from "../providers/stripeWebhook.js";
...
export async function proxyHandler(
  ...
  const preparedPack =
    pack === null
      ? null
      : prepareProviderPackExecution(pack, normalizedRequest);
  const packExecution: ProviderPackExecution | undefined =
    preparedPack !== null && !("error" in preparedPack)
      ? preparedPack
      : undefined;

  if (packExecution !== undefined) {
    for (const [key, value] of Object.entries(
      getProviderPackHeaders(packExecution),
    ))
      response.setHeader(key, value);
    for (const [key, value] of Object.entries(
      packExecution.pack.createResponseHeaders({
    ...
    pack !== null &&
    preparedPack !== null &&
    "error" in preparedPack
  ) {
    for (const [key, value] of Object.entries(
      pack.createResponseHeaders({
        apiVersion: pack.manifest.apiVersions.default,
  ...
  if (behavior !== null) {
    ...
    if (behavior.delayMs !== undefined && behavior.delayMs > 0) {
      response.setHeader(
        "x-ghostapi-behavior-delay-ms",
        String(behavior.delayMs),
      );
  ...
  if (preparedPack !== null && "error" in preparedPack) {
    ...
    response.setHeader(
      "x-ghostapi-provider-pack",
      `${pack!.name}@${pack!.manifest.packVersion}`,
    );
  ...
  if (packExecution?.pack.stateful) {
    const generatedOption = await transactState((state) => {
      const runtime = createProviderRuntime({
        state: { snapshot: () => structuredClone(state) },
      });
      const execution = prepareProviderPackExecution(
        packExecution.pack,
        normalizedRequest,
        runtime,
      );
      if ("error" in execution)
        throw new Error(
          `Provider pack version changed during execution: ${execution.error.message}`,
        );
      ...
      return {
        state: {
          ...
          ...Object.fromEntries(
            (transition.additionalWrites ?? []).map((write) => [
              write.key,
              write.value,
            ]),
          ),
    ...
    if (normalizedRequest.method === "GET" && generatedOption.status < 400)
      response.setHeader("x-ghostapi-state", "HIT");

    if (
      packExecution.pack.name === "stripe" &&
      isStripeWebhookDeliveryRequest(normalizedRequest)
    ) {
      ...
      response.setHeader(
        "stripe-signature",
        delivery.mode === "invalid_signature"
          ? corruptStripeWebhookSignature(signature)
          : signature,
      );
      ...
      await completeRequest(
        "state",
        generatedOption.status,
        generatedOption.body,
      );
      return;
    }
    ...
    await completeRequest(
      generatedOption.status >= 400 ? "error" : "state",
      generatedOption.status,
      generatedOption.body,
    );
    return;
  ...
  if (stateResolution !== null) {
    ...
    await completeRequest(
      "state",
      stateResolution.status,
      stateResolution.body,
    );
    return;
  }
  ...
  if (cachedResponse !== null) {
    for (const [key, value] of Object.entries(
      sanitizeResponseHeaders(cachedResponse.headers),
    )) {
      response.setHeader(key, value);
    }
  ...
  const generatedOption = await generateAiMock(
    normalizedRequest,
    provider,
    response,
    config,
  ...
  if (packExecution !== undefined) {
    ...
    if (extractedId !== undefined) {
      await saveToStateStore(
        `${provider}:${extractedId}`,
        generatedOption.body,
      );
    }
  }
  ...
  for (const [key, value] of Object.entries(
    sanitizeResponseHeaders(generatedOption.headers),
  )) {
    response.setHeader(key, value);
  }
  ...
  response.status(generatedOption.status).json(generatedOption.body);
  await completeRequest(
    generatedOption.status >= 400
      ? "error"
      : generatedOption.headers["x-ghostapi-generation-source"] ===
          "local-fallback"
        ? "fallback"
        : "ai",
    generatedOption.status,
