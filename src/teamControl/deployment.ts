...
export const TEAM_CONTROL_PLANE_SECURITY_HEADERS: Readonly<
...
export function createTeamControlPlaneSecurityHeaders(): Readonly<
  Record<string, string>
> {
  ...
  return {
    ...
    async authenticate(): Promise<TeamIdentity> {
      throw new TeamControlPlaneError(
        "Enterprise identity provider is not configured.",
      );
    },
  };
}

export type TeamRateLimitOptions = {
  limit?: number;
  windowMs?: number;
  maxKeys?: number;
  now?: () => Date;
...
export class TeamControlPlaneRateLimiter {
  ...
  constructor(options: TeamRateLimitOptions = {}) {
    ...
    if (
      ...
      this.maxKeys < 1 ||
      this.maxKeys > 100_000
    )
      throw new TeamControlPlaneError("Rate limiter options are invalid.");
  }
  ...
  consume(key: string): { remaining: number; resetAt: string } {
    if (
      ...
      key.length > 128 ||
      /[\u0000-\u001f]/.test(key) ||
      sanitizeSecretString(key) !== key
    )
      throw new TeamControlPlaneError("Rate-limit key is invalid.");
    const timestamp = this.now().getTime();
    if (!Number.isFinite(timestamp) || timestamp < 0)
      throw new TeamControlPlaneError("Rate-limit clock is invalid.");
    const current = this.entries.get(key);
    const entry =
      current === undefined || timestamp - current.startedAt >= this.windowMs
        ? { startedAt: timestamp, count: 0 }
        : current;
    if (current === undefined && this.entries.size >= this.maxKeys)
      this.prune(timestamp);
    if (current === undefined && this.entries.size >= this.maxKeys)
      throw new TeamControlPlaneError("Rate-limit key capacity exceeded.");
    entry.count += 1;
    this.entries.set(key, entry);
    const resetAt = new Date(entry.startedAt + this.windowMs).toISOString();
    if (entry.count > this.limit)
      throw new TeamControlPlaneError(
        `Rate limit exceeded. Retry after ${resetAt}.`,
      );
  ...
  private prune(timestamp: number): void {
    for (const [key, entry] of this.entries)
      if (timestamp - entry.startedAt >= this.windowMs)
