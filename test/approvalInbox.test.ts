...
import {
  ApprovalInboxError,
  createLocalApprovalInbox,
  createTestApprovalApproverVerifier,
...
describe("local human approval inbox", () => {
  it("shows normalized impact, requires two independent approvers, and executes an exact action once", async () => {
    ...
    const request = await fixture.inbox.request(
      action("approval-one", "approval-world"),
      criticalPolicy(),
      { confidence: 90, amountMinor: 2500 },
    );
    const alice = fixture.identities.issue({
      id: "alice",
      independenceKey: "alice",
    });
    const bob = fixture.identities.issue({ id: "bob", independenceKey: "bob" });

    expect(request.risk).toBe("update");
    expect(request.display).toMatchObject({
      target: "synthetic-world/approval-world",
      reversibility: "none",
      impact: { amountMinor: 2500, amountKnown: true, irreversible: true },
      simulation: { status: "passed" },
    });
    await expect(
      fixture.inbox.approve(
        request.id,
        fixture.identities.issue({
          id: "agent-one",
          independenceKey: "agent-one",
        }),
      ),
    ...
    await expect(
      fixture.inbox.approve(
        ...
        fixture.identities.issue({
          ...
          independenceKey: "independent-key",
        }),
      ),
    ).rejects.toThrow("cannot approve");
    ...
    await expect(
      fixture.inbox.approve(
        request.id,
        fixture.identities.issue({ id: "alice-alt", independenceKey: "alice" }),
      ),
    ...
    const receipt = await fixture.inbox.execute(
      request.id,
      { actorId: "agent-one", workloadId: "checkout-worker" },
      actionPolicy(),
      criticalPolicy(),
    );
    const replay = await fixture.inbox.execute(
      request.id,
      { actorId: "agent-one", workloadId: "checkout-worker" },
      actionPolicy(),
      criticalPolicy(),
    ...
    expect(state.audit.at(-1)).toMatchObject({
      ...
      actionReceiptHash: receipt.receiptHash,
    });
  });
  ...
  it("rejects stale/action-mismatched approval artifacts and supports edit-and-resubmit", async () => {
    ...
    const original = await fixture.inbox.request(
      action("resubmit-one", "resubmit-a"),
      standardPolicy(),
      { confidence: 99 },
    );
    const reviewer = fixture.identities.issue({
      id: "reviewer",
      independenceKey: "reviewer",
    });
    const updated = await fixture.inbox.editAndResubmit(
      original.id,
      reviewer,
      action("resubmit-two", "resubmit-b"),
      standardPolicy(),
      { confidence: 99 },
    );
  ...
  it("fails closed on timeout, reject, policy drift, and insufficient amount evidence", async () => {
    ...
    const request = await fixture.inbox.request(
      action("timeout-one", "timeout-world"),
      standardPolicy({ approvalTtlMs: 2_000, escalationTimeoutMs: 1_000 }),
      { confidence: 99 },
    );
    ...
    const rejected = await fixture.inbox.request(
      action("reject-one", "timeout-world"),
      standardPolicy(),
      { confidence: 99 },
    );
    const reviewer = fixture.identities.issue({
    ...
    await expect(
      fixture.inbox.request(
        action("amount-one", "timeout-world"),
        standardPolicy({ maxAmountMinor: 100 }),
        { confidence: 99 },
      ),
    ).rejects.toThrow("amount is missing");
  });
  ...
  it("serializes emergency revoke against execution and prevents a revoked artifact from side effects", async () => {
    ...
    const request = await fixture.inbox.request(
      action("revoke-one", "revoke-world"),
      standardPolicy(),
      { confidence: 99 },
    );
    const reviewer = fixture.identities.issue({
  ...
  it("does not let callers bypass inbox artifact consumption through the public action gateway", async () => {
    ...
    await createWorld({
      id: "gateway-bypass-world",
      seed: "gateway-bypass-seed",
    });
    const request = await fixture.inbox.request(
      action("gateway-bypass", "gateway-bypass-world"),
      standardPolicy(),
      { confidence: 99 },
    );
    const approved = await fixture.inbox.approve(
      request.id,
      fixture.identities.issue({ id: "reviewer", independenceKey: "reviewer" }),
    );
    const directGateway = createLocalActionGateway({
      now: () => new Date("2026-08-14T12:00:00.000Z"),
      approvalVerifier: fixture.approvals.verifier,
      pathForAction: (actionId) =>
        join(getDataPaths().root, "direct-actions", `${actionId}.json`),
    });

    await expect(
      directGateway.submit(approved.action, approved.artifact, actionPolicy()),
    ).rejects.toThrow("provenance is not active");
    expect((await fixture.inbox.get(request.id)).status).toBe("approved");
    expect(
      (await inspectWorld("gateway-bypass-world")).state.receipts,
    ).toHaveLength(0);
  });
  ...
  it.each([
    ...
    "recovers repeatedly after a crash at %s without replaying the action",
    async (checkpoint) => {
      ...
      const request = await fixture.inbox.request(
        action(actionId, worldId),
        standardPolicy(),
        { confidence: 99 },
      );
      await fixture.inbox.approve(
        request.id,
        fixture.identities.issue({
          id: "reviewer",
          independenceKey: "reviewer",
      ...
      const recovered = await fixture.inbox.execute(
        request.id,
        executionIdentity(),
        actionPolicy(),
        standardPolicy(),
      );
      const repeated = await fixture.inbox.execute(
        request.id,
        executionIdentity(),
        actionPolicy(),
        standardPolicy(),
      ...
      expect(stored).toMatchObject({
        status: "executed",
        executionReceiptHash: recovered.receiptHash,
      });
      expect((await inspectWorld(worldId)).state.receipts).toHaveLength(1);
      expect(
        gatewayRecord.receipts.filter(
          (receipt) => receipt.status === "verified",
        ),
      ).toHaveLength(1);
      expect(
        (await fixture.inbox.readStateForTesting()).audit.filter(
          (record) => record.event === "artifact.consumed",
        ),
      ).toHaveLength(1);
    },
  );
  ...
  it("reconciles a crash after the gateway durably commits without invoking the provider again", async () => {
    ...
    ).rejects.toThrow("simulated crash after durable commit");
    expect(
      (await fixture.gateway.inspect(actionId)).receipts.map(
        (receipt) => receipt.status,
      ),
    ).toEqual(["requested", "attempted", "committed"]);
    const recovered = await fixture.inbox.execute(
      request.id,
      executionIdentity(),
      actionPolicy(),
    ...
    ).toEqual(["requested", "attempted", "committed", "verified"]);
  });
  ...
  it("re-verifies the signed approval while recovering an executing request", async () => {
    ...
    await createWorld({
      id: "recovery-signature-world",
      seed: "recovery-signature-seed",
    });
    const request = await fixture.inbox.request(
      action("recovery-signature", "recovery-signature-world"),
      standardPolicy(),
      { confidence: 99 },
    );
    await fixture.inbox.approve(
    ...
    const state = await fixture.inbox.readStateForTesting();
    const stored = state.requests.find(
    ...
    ).rejects.toThrow("authenticity could not be verified");
    expect((await fixture.inbox.get(request.id)).status).toBe("executing");
    expect(
      (await inspectWorld("recovery-signature-world")).state.receipts,
    ).toHaveLength(0);
  });
  ...
  it("does not start an expired unattempted action during recovery", async () => {
    ...
    await createWorld({
      id: "recovery-expired-world",
      seed: "recovery-expired-seed",
    });
    const request = await fixture.inbox.request(
      action("recovery-expired", "recovery-expired-world"),
      standardPolicy({ approvalTtlMs: 2_000, escalationTimeoutMs: 1_000 }),
      { confidence: 99 },
    );
    await fixture.inbox.approve(
    ...
    expect((await fixture.inbox.get(request.id)).status).toBe("executing");
    expect(
      (await fixture.gateway.inspect("recovery-expired")).receipts.map(
        (receipt) => receipt.status,
      ),
    ).toEqual(["requested"]);
    expect(
      (await inspectWorld("recovery-expired-world")).state.receipts,
    ).toHaveLength(0);
  });
});
...
function inboxFixture(
  ...
  actionGatewayOptions: NonNullable<
    Parameters<typeof createLocalApprovalInbox>[0]["actionGatewayOptions"]
  > = {},
) {
  ...
  const pathForAction = (actionId: string) =>
    join(root, "approval-actions", `${actionId}.json`);
  const inbox = createLocalApprovalInbox({
    path,
    now,
    approverVerifier: identities.verifier,
    approvalSigner: approvals.signer,
    approvalVerifier: approvals.verifier,
    actionGatewayOptions: { ...actionGatewayOptions, pathForAction },
    testing: { onExecutionCheckpoint },
  });
...
function action(actionId: string, worldId: string) {
  return {
    ...
    project: { id: "checkout-project", environment: "synthetic" as const },
    provider: "ghostapi-synthetic" as const,
    operation: "synthetic.subscription_failure",
    resource: { type: "synthetic-world" as const, id: worldId },
    arguments: { worldId },
    expectedSideEffects: [
...
function criticalPolicy() {
  return standardPolicy({ criticalRisks: ["update"] });
}
function actionPolicy() {
  return { version: 1, hash: POLICY_HASH, allowed: true };
}
function executionIdentity() {
  return { actorId: "agent-one", workloadId: "checkout-worker" };
}
