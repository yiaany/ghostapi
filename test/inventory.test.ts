...
describe("local inventory controller", () => {
  it("denies access without operator authorization and starts empty", async () => {
    ...
    const controller = createLocalInventoryController({
      path: inventoryPath("inv-disabled.json"),
      now: () => new Date(NOW),
      operatorAuthorizer: disabled,
    });
    ...
    const operator = issue({
      ...
      tenantId: "tenant-a",
      permissions: ["inventory.import", "inventory.inspect"],
    });
    const allowed = createLocalInventoryController({
      path: inventoryPath("inv-empty.json"),
      now: () => new Date(NOW),
  ...
  it("imports records with provenance and refreshes instead of duplicating on re-import", async () => {
    const { controller, operator } = setup({
      permissions: ["inventory.import", "inventory.inspect"],
    });
    const summary = await controller.import(operator, tenantAPayload());
    expect(summary.recordCounts).toMatchObject({
      agents: 2,
      tools: 1,
      identities: 2,
      providers: 2,
  ...
  it("isolates tenant data across the shared store (graph, findings, export, attack paths)", async () => {
    ...
    const controllerB = createLocalInventoryController({
      path: controller.path,
      now: () => new Date(NOW),
      operatorAuthorizer: tenantB.authorizer,
    });
    await controllerB.import(tenantB.operator, {
      schemaVersion: 1,
      kind: "ghostapi.inventory-import",
      source: { sourceId: "ci-repo", sourceType: "ci", sourceName: "CI" },
      counters: { preventedEgressAttempts: 2 },
    });
    ...
    expect(
      (await controllerB.attackPaths(tenantB.operator, "agent-order")).paths,
    ).toEqual([]);
  ...
  it("computes complete attack paths and a heuristic-labeled blast radius", async () => {
    ...
    expect(complete!.steps.map((step) => step.kind)).toEqual([
      "agent",
      "identity",
      "tool",
      "provider",
    ...
    const payments = blast.resources.find(
      (resource) => resource.resourceId === "resource-payments",
    );
    expect(payments).toBeDefined();
    expect(
      payments!.sideEffects.every(
        (sideEffect) => sideEffect.heuristic === true,
      ),
    ).toBe(true);
  ...
  it("detects every supported finding kind from the fixture", async () => {
    ...
    const heuristic = snapshot.findings.filter(
      (finding) => finding.basis === "heuristic",
    );
    expect(heuristic.length).toBeGreaterThan(0);
    expect(
      heuristic.every((finding) => finding.kind === "excessive_permissions"),
    ).toBe(true);
  });
  ...
  it("proposes and applies remediations without ever expanding permissions", async () => {
    ...
    await expect(
      controller.proposeRemediation(operator, {
        findingId: excessive!.findingId,
        kind: "reduce_scope",
    ...
    const applied = await controller.applyRemediation(
      operator,
      proposed.remediationId,
    );
    expect(applied.status).toBe("applied");
    const credential = (await controller.inspect(operator)).credentials.find(
      (candidate) => candidate.credentialId === "cred-stripe",
    );
    expect(credential!.grantScopes).toEqual(["read", "charge"]);
    await expect(
      controller.applyRemediation(operator, proposed.remediationId),
    ).rejects.toThrow(/already applied/);

    const orphanFinding = (await controller.inspect(operator)).findings.find(
      (finding) => finding.kind === "orphaned_agents",
    );
    ...
    const legacy = (await controller.inspect(operator)).agents.find(
      (agent) => agent.agentId === "agent-legacy",
    );
  ...
  it("exposes a complete open export and a no-invented-savings ROI report", async () => {
    ...
    await controller.analyze(operator);
    const snapshot = await controller.inspect(operator);
    const evidence = snapshot.findings.find(
      (finding) => finding.kind === "missing_evidence",
    );
    ...
    const legacyEvidence = exported.evidenceMetadata.find(
      (entry) => entry.agentId === "agent-legacy",
    );
    expect(legacyEvidence!.status).toBe("missing");
    expect(exported.scenarioRefs).toEqual([
      {
        evalScenarioId: "eval-safety-baseline",
        forFindingId: evidence!.findingId,
        createdBy: "invop-one",
      },
    ]);
    ...
    expect(exported.roi.notMeasured).toEqual(
      expect.arrayContaining([
        "review_time_minutes",
        "mean_time_to_contain_minutes",
        "prevented_egress_attempts",
      ]),
    );
  });
  ...
  it("uses real counters from gateway/ci imports instead of inventing savings", async () => {
    ...
    await controller.import(operator, {
      ...
      kind: "ghostapi.inventory-import",
      source: {
        sourceId: "gateway-edge",
        sourceType: "gateway",
        sourceName: "Edge gateway",
        sourceVersion: "1.2.0",
      },
      counters: {
        preventedEgressAttempts: 7,
        reviewTimeMinutes: 42,
        meanTimeToContainMinutes: 12,
      },
    });
    ...
    expect(roi.incidents.preventedEgressSources).toEqual([
      expect.objectContaining({ sourceId: "gateway-edge", value: 7 }),
    ]);
  ...
  it("rejects secret-shaped input and invalid payloads", async () => {
    ...
    await expect(
      controller.import(
        ...
        tenantAPayloadWith({
          agents: [
            {
              ...tenantAPayload().agents![0],
              name: "sk_live_secret_key_value",
            },
          ],
        }),
      ),
    ...
    await expect(
      controller.import(operator, {
        ...
        counters: { preventedEgressAttempts: 1 },
      }),
    ).rejects.toThrow(/Counters are only allowed/);
  ...
  it("excludes stale edges from attack paths and prunes them on import", async () => {
    ...
    const controller = createLocalInventoryController({
      path: inventoryPath("inv-stale.json"),
      now: () => clock,
      operatorAuthorizer: authorizer,
      freshnessDays: { edgeStaleDays: 90 },
    });
    await controller.import(operator, tenantAPayload());
    const fresh = await controller.attackPaths(operator, "agent-order");
    expect(
      fresh.paths.some((path) => path.complete && path.edges.length > 0),
    ).toBe(true);
    ...
    expect(
      aged.paths
        ...
        .every((path) => path.edges.length === 0),
    ).toBe(true);
  ...
  it("reopens a finding when an applied remediation stops being effective", async () => {
    ...
    expect(
      (await controller.inspect(operator)).findings.find(
        (finding) => finding.kind === "excessive_permissions",
      )?.status,
    ).toBe("resolved");
    await controller.analyze(operator);
  ...
  it("verifies eval scenarios exist before proposing create_eval remediation", async () => {
    ...
    const controller = createLocalInventoryController({
      path: inventoryPath("inv-eval.json"),
      now: () => new Date(NOW),
      operatorAuthorizer: authorizer,
      evalScenarioExists: (scenarioId) => scenarioId === "eval-safety-baseline",
    });
    await controller.import(operator, tenantAPayload());
    await controller.analyze(operator);
    const evidence = (await controller.inspect(operator)).findings.find(
      (finding) => finding.kind === "missing_evidence",
    );
    await expect(
      controller.proposeRemediation(operator, {
  ...
  it("rotates import runs beyond the per-tenant cap", async () => {
    ...
    const controller = createLocalInventoryController({
      path: inventoryPath("inv-rotation.json"),
      now: () => new Date(NOW),
      operatorAuthorizer: authorizer,
    });
    for (let index = 0; index < INVENTORY_LIMITS.maxImports + 5; index += 1) {
      await controller.import(operator, {
        ...
        source: {
          sourceId: "src-rotate",
          sourceType: "ci",
          sourceName: "CI rotation",
        },
      });
    }
    const snapshot = await controller.inspect(operator);
    expect(snapshot.importRuns.length).toBeLessThanOrEqual(
  ...
  it("accepts an environment remediation target that is actually referenced", async () => {
    ...
    await controller.import(
      ...
      tenantAPayloadWith({
        agents: [
          ...tenantAPayload().agents!,
          {
            agentId: "agent-staging",
            name: "Staging bot",
    ...
    await controller.analyze(operator);
    const snapshot = await controller.inspect(operator);
    const drift = snapshot.findings.find(
      (finding) =>
        finding.kind === "policy_drift" && finding.targetKind === "environment",
    );
...
function setup({
  ...
  permissions: InventoryOperatorPermission[];
}): { controller: LocalInventoryController; operator: InventoryOperator } {
  const { authorizer, issue } = createTestInventoryOperatorAuthorizer();
  const operator = issue({
    id: "invop",
    principalId: "invop-one",
  ...
  const controller = createLocalInventoryController({
    path: inventoryPath("inv-fixture.json"),
    now: () => new Date(NOW),
    operatorAuthorizer: authorizer,
  });
...
function issueOperator(tenantId: string): {
...
function tenantAPayload() {
  return {
    ...
    expectedPolicyHashes: [
      { environmentId: "production", policyHash: "f".repeat(64) },
    ],
    agents: [
      {
        agentId: "agent-order",
    ...
    tools: [
      {
        toolId: "tool-stripe",
        name: "Stripe SDK",
        kind: "sdk_client" as const,
        providerId: "provider-stripe",
        requiredScopes: ["read", "charge"],
        credentialIds: ["cred-stripe"],
      },
    ],
    providers: [
      {
        ...
        ownerId: "sre-one",
        environmentIds: ["production"],
        resourceIds: ["resource-payments"],
        gatewayBound: true,
        killSwitchApplied: false,
      },
      {
        providerId: "provider-saas",
        name: "Legacy SaaS",
        providerType: "billing",
        environmentIds: ["production"],
        resourceIds: ["resource-saas"],
        gatewayBound: false,
        killSwitchApplied: false,
      },
    ],
    resources: [
      {
        ...
        sensitive: true,
        ownerId: "sre-one",
        sideEffectIds: ["side-charge", "side-read"],
      },
      {
        resourceId: "resource-saas",
        providerId: "provider-saas",
        resourceType: "account",
        name: "SaaS accounts",
        environmentIds: ["production"],
        sensitive: false,
        sideEffectIds: ["side-write"],
      },
    ],
    sideEffects: [
      {
        sideEffectId: "side-charge",
        resourceId: "resource-payments",
        kind: "money_movement" as const,
        severity: "critical" as const,
      },
      {
        sideEffectId: "side-read",
        resourceId: "resource-payments",
        kind: "read" as const,
        severity: "low" as const,
      },
      {
        sideEffectId: "side-write",
        resourceId: "resource-saas",
...
function tenantAPayloadWith(overrides: {
  agents: unknown[];
}): ReturnType<typeof tenantAPayload> {
