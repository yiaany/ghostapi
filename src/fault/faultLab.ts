...
export type FaultLabDecision =
  ...
  | {
      type: "error";
      latencyMs: number;
      statusCode: FaultLabConfig["statusCode"];
      retryAfterSeconds: number;
      body: unknown;
    };
...
export async function updateFaultLabConfig(
  input: unknown,
): Promise<FaultLabConfig> {
  ...
  return mutateJsonFile(
    ...
    (current) =>
      validateConfig({
        ...
        latencyMinMs: readInteger(
          record.latencyMinMs,
          current.latencyMinMs,
          0,
          10_000,
        ),
        latencyMaxMs: readInteger(
          record.latencyMaxMs,
...
export async function resetFaultLabForTests(): Promise<void> {
  const faultLabPath = getDataPaths().faultLab;
  await withFileLock(faultLabPath, () =>
    atomicWriteJson(faultLabPath, DEFAULT_CONFIG),
  );
}

export async function decideFault(
  provider: ProviderName,
  random = Math.random,
): Promise<FaultLabDecision> {
  const config = await readPersistedConfig();
  if (!config.enabled) return { type: "off" };

  const shouldInjectChaos =
    config.errorRate > 0 && random() * 100 < config.errorRate;
  if (!shouldInjectChaos) return { type: "off" };

  const action = Math.floor(random() * 3);
  if (action === 2)
    return { type: "delay", latencyMs: pickLatency(config, random) };
...
function createFaultDetails(
  ...
  return {
    status: statusCode,
    message: `Chaos Mode simulated upstream ${statusCode} response.`,
    type: "api_error",
    code: statusCode,
  };
}
...
function sanitizeConfig(value: unknown): FaultLabConfig {
  ...
  try {
    return validateConfig({
      ...
      latencyMs: readInteger(
        record.latencyMs,
        DEFAULT_CONFIG.latencyMs,
        0,
        10_000,
      ),
      latencyMinMs: readInteger(
        record.latencyMinMs,
        DEFAULT_CONFIG.latencyMinMs,
        0,
        10_000,
      ),
      latencyMaxMs: readInteger(
        record.latencyMaxMs,
        DEFAULT_CONFIG.latencyMaxMs,
        0,
        10_000,
      ),
      errorRate: readInteger(
        record.errorRate,
        DEFAULT_CONFIG.errorRate,
        0,
        100,
      ),
...
function validateConfig(config: FaultLabConfig): FaultLabConfig {
  if (config.latencyMaxMs < config.latencyMinMs) {
    throw new Error(
      "Fault Lab latencyMaxMs must be greater than or equal to latencyMinMs.",
    );
  }
  return config;
}
...
function readStatusCode(
  value: unknown,
  fallback: FaultLabConfig["statusCode"],
): FaultLabConfig["statusCode"] {
  if (value === undefined) return fallback;
  if (value === 429 || value === 500 || value === 502 || value === 503)
    return value;
