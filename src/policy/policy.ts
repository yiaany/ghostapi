...
const ENFORCEMENT_MODES = new Set<EnforcementMode>([
...
export async function loadPolicyFile(
  filePath = "ghostapi.policy.yaml",
  projectRoot = process.cwd(),
  required = false,
): Promise<LoadedPolicy | null> {
  const { root, path: resolvedPath } = await resolvePolicyPath(
    filePath,
    projectRoot,
  );
  try {
...
export function parsePolicyYaml(source: string): GhostApiPolicy {
  if (Buffer.byteLength(source, "utf8") > MAX_POLICY_BYTES) {
    throw new PolicyValidationError(
      "policy",
      `exceeds the ${MAX_POLICY_BYTES} byte limit.`,
    );
  }
  ...
  if (document.errors.length > 0)
    throw new PolicyValidationError(
      "policy",
      document.errors[0]!.message.replace(/\n.*/s, ""),
    );
  if (document.warnings.length > 0)
    throw new PolicyValidationError(
      "policy",
      document.warnings[0]!.message.replace(/\n.*/s, ""),
    );
  ...
  switch (event.type) {
    ...
    case "scenario":
      return evaluateScenario(
        policy,
        event.scenarioId,
        event.completedScenarioIds,
      );
    case "enforcement":
      return policy.enforcement.allowedModes.includes(event.mode)
        ? allow(`Enforcement mode ${event.mode} is allowed.`, [
            `enforcement.allowedModes contains ${event.mode}`,
          ])
        : deny(`Enforcement mode ${event.mode} is forbidden.`, [
            `enforcement.allowedModes does not contain ${event.mode}`,
          ]);
    case "report":
...
async function resolvePolicyPath(
  ...
  if (isAbsolute(filePath))
    throw new PolicyValidationError(
      "policy",
      "must be a relative path inside the project root.",
    );
...
function assertPathWithinRoot(root: string, target: string): void {
  ...
  if (
    ...
    throw new PolicyValidationError(
      "policy",
      "path traversal outside the project root is not allowed.",
    );
  }
}
...
function parsePolicy(value: unknown): GhostApiPolicy {
  ...
  assertKeys(networkRecord, "policy.network", [
    "default",
    "allow",
    "deny",
    "productionHosts",
  ]);
  const defaultAction = enumAt(
  ...
  assertKeys(reportsRecord, "policy.reports", [
    "maxProductionEgressAttempts",
    "maxForbiddenCredentialMatches",
    "maxBreakingContractChanges",
  ]);
  ...
  return {
    ...
    network: {
      ...
      productionHosts: stringsAt(
        networkRecord.productionHosts ?? [],
        "policy.network.productionHosts",
        normalizeHost,
      ),
    },
    credentials: {
      forbid: stringsAt(
        credentialsRecord.forbid ?? [],
        "policy.credentials.forbid",
        validateCredentialGlob,
      ),
    },
    requiredScenarios: stringsAt(
      record.requiredScenarios ?? [],
      "policy.requiredScenarios",
      validateScenarioId,
    ),
    enforcement: {
      allowedModes: arrayAt(
        enforcementRecord.allowedModes ?? [],
        "policy.enforcement.allowedModes",
      ).map((entry, index) =>
        validateEnforcementMode(
          entry,
          `policy.enforcement.allowedModes[${index}]`,
        ),
      ),
    },
    reports: {
      maxProductionEgressAttempts: nonNegativeIntegerAt(
        reportsRecord.maxProductionEgressAttempts,
        "policy.reports.maxProductionEgressAttempts",
      ),
      maxForbiddenCredentialMatches: nonNegativeIntegerAt(
        reportsRecord.maxForbiddenCredentialMatches,
        "policy.reports.maxForbiddenCredentialMatches",
      ),
      maxBreakingContractChanges: nonNegativeIntegerAt(
        reportsRecord.maxBreakingContractChanges ?? 0,
        "policy.reports.maxBreakingContractChanges",
      ),
    },
  };
}
...
function evaluateNetwork(
  ...
  const trace = [
    `network host: ${host}`,
    `production classification: ${matchesAnyHost(policy.network.productionHosts, host) ? "production" : "non-production"}`,
  ];
  const deniedRule = policy.network.deny.find((rule) =>
    ruleMatches(rule, host, provider),
  );
  if (deniedRule !== undefined)
    return deny(
      "A matching deny rule takes precedence over every allow rule.",
      [...trace, `matched network.deny ${describeRule(deniedRule)}`],
    );
  const allowedRule = policy.network.allow.find((rule) =>
    ruleMatches(rule, host, provider),
  );
  if (allowedRule !== undefined)
    return allow("A matching allow rule permits the network event.", [
      ...trace,
      `matched network.allow ${describeRule(allowedRule)}`,
    ]);
  return policy.network.default === "allow"
    ? allow("No network rule matched; network.default is allow.", [
        ...trace,
        "no matching allow or deny rule",
        "network.default: allow",
      ])
    : deny("No network rule matched; network.default is deny.", [
        ...trace,
        "no matching allow or deny rule",
...
function evaluateCredential(
  ...
  const match = policy.credentials.forbid.find((pattern) =>
    globMatches(pattern, value),
  );
  return match === undefined
    ? allow("No forbidden credential pattern matched.", [
        "credential input was checked without interpolation",
        "no policy.credentials.forbid pattern matched",
      ])
    : deny("A forbidden credential pattern matched.", [
        `matched policy.credentials.forbid pattern ${match}`,
      ]);
}
...
function evaluateScenario(
  ...
  if (!policy.requiredScenarios.includes(scenarioId))
    return allow("Scenario is not required by this policy.", [
      `${scenarioId} is not in policy.requiredScenarios`,
    ]);
  return completed.includes(scenarioId)
    ? allow("Required scenario is complete.", [
        `${scenarioId} is required and present in completed scenarios`,
      ])
    : deny("Required scenario has not completed.", [
        `${scenarioId} is required but absent from completed scenarios`,
      ]);
}

function evaluateReport(
  policy: GhostApiPolicy,
  event: Extract<PolicyEvent, { type: "report" }>,
): PolicyDecision {
  const trace = [
  ...
  return event.productionEgressAttempts <=
    policy.reports.maxProductionEgressAttempts &&
    event.forbiddenCredentialMatches <=
      policy.reports.maxForbiddenCredentialMatches &&
    (event.breakingContractChanges ?? 0) <=
      policy.reports.maxBreakingContractChanges
...
function rulesAt(value: unknown, path: string): NetworkRule[] {
  ...
  return list.map((entry, index) => {
    ...
    if (hasHost === hasProvider)
      throw new PolicyValidationError(
        rulePath,
        "must contain exactly one of host or provider.",
      );
    return hasHost
      ? { host: normalizeHost(record.host, `${rulePath}.host`) }
      : { provider: providerAt(record.provider, `${rulePath}.provider`) };
  });
}
...
function arrayAt(value: unknown, path: string): unknown[] {
  ...
  if (value.length > MAX_LIST_ITEMS)
    throw new PolicyValidationError(
      path,
      `must contain at most ${MAX_LIST_ITEMS} items.`,
    );
  return value;
}
...
function nonNegativeIntegerAt(value: unknown, path: string): number {
  if (!Number.isInteger(value) || typeof value !== "number" || value < 0)
    throw new PolicyValidationError(path, "must be a non-negative integer.");
  return value;
}
...
function normalizeHost(value: unknown, path: string): string {
  if (typeof value !== "string" || value.length === 0 || value.length > 253)
    throw new PolicyValidationError(
      path,
      "must be a non-empty host up to 253 characters.",
    );
  const host = value.toLowerCase();
  if (
    host.includes("${") ||
    !/^(?:\*\.)?(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)*[a-z0-9][a-z0-9-]{0,61}[a-z0-9]$|^(?:\*\.)?(?:localhost|\d{1,3}(?:\.\d{1,3}){3}|::1)$/.test(
      host,
    )
  ) {
    throw new PolicyValidationError(
      path,
      "must be a lowercase DNS host, loopback address, or leading-wildcard DNS suffix.",
    );
  }
  return host;
}
...
function validateCredentialGlob(value: unknown, path: string): string {
  if (typeof value !== "string" || value.length === 0 || value.length > 128)
    throw new PolicyValidationError(
      path,
      "must be a glob string between 1 and 128 characters.",
    );
  if (value.includes("${") || /[\r\n]/.test(value))
    throw new PolicyValidationError(
      path,
      "must not contain interpolation or newlines.",
    );
  return value;
}
...
function validateScenarioId(value: unknown, path: string): string {
  if (typeof value !== "string" || !/^[a-z0-9][a-z0-9._-]{0,127}$/.test(value))
    throw new PolicyValidationError(
      path,
      "must be a lowercase scenario identifier.",
    );
  return value;
}
...
function rejectInterpolation(value: unknown, path: string): void {
  if (typeof value === "string") {
    if (value.includes("${"))
      throw new PolicyValidationError(
        path,
        "environment interpolation is not supported.",
      );
    return;
  }
  if (Array.isArray(value)) {
    value.forEach((entry, index) =>
      rejectInterpolation(entry, `${path}[${index}]`),
    );
    return;
  }
  if (value !== null && typeof value === "object") {
    for (const [key, entry] of Object.entries(value as Record<string, unknown>))
      rejectInterpolation(entry, `${path}.${key}`);
  }
}
...
function ruleMatches(
  ...
  return rule.host !== undefined
    ? hostMatches(rule.host, host)
    : rule.provider === provider?.toLowerCase();
}
...
function globMatches(pattern: string, value: string): boolean {
  ...
  for (const character of pattern) {
    ...
    for (let index = value.length; index >= 1; index -= 1)
      states[index] = states[index - 1]! && value[index - 1] === character;
    states[0] = false;
  }
