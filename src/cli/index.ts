...
import {
  ...
  formatScenarioSanitizationSummary,
  loadScenarioBundle,
  prepareScenarioRecordingFromFile,
  ScenarioBundleError,
  writeScenarioBundle,
  type ScenarioPiiRules,
...
} from "../productTelemetry/index.js";
import {
  ActionGatewayError,
...
async function startServer(config: ServerConfig, open = false): Promise<void> {
  ...
  server.on("error", (error: NodeJS.ErrnoException) => {
    if (error.code === "EADDRINUSE") {
      ...
      console.error(
        `Reason: Another process is already listening on ${config.host}:${config.port}.`,
      );
      console.error(
  ...
  server.listen(config.port, config.host, () => {
    ...
    console.log(
      `GhostAPI listening on ${protocol}://${config.host}:${config.port}`,
    );
    console.log(`Dashboard: ${dashboardUrl}`);
    console.log(
      `Model: ${config.model}${config.offline ? " (offline)" : config.allowExternalLlm ? " (external LLM opt-in enabled)" : " (local only)"}`,
    );
    if (remote) {
      console.warn(
        "Warning: GhostAPI is bound beyond loopback. Use HTTPS or a secure tunnel because the dashboard token does not encrypt traffic.",
      );
    }
    if (open)
      openUrl(
        remote && config.authToken
...
async function initProject(): Promise<void> {
  ...
  console.log(
    result.created ? `Created ${configPath}.` : `${configPath} already exists.`,
  );
  console.log(`Model: ${result.config.model ?? DEFAULT_MODEL}`);
  console.log(
    `Created setup files: ${setup.result.created.length > 0 ? setup.result.created.join(", ") : "none"}.`,
  );
  console.log(
    `Skipped existing setup files: ${setup.result.skipped.length > 0 ? setup.result.skipped.join(", ") : "none"}.`,
  );
  console.log("Next: ghostapi doctor");
  console.log("Then on supported Linux hosts: ghostapi run -- npm test");
  console.log(
    "Otherwise: ghostapi start --open and point SDKs at http://127.0.0.1:8080.",
  );
...
async function printRepoSetup(write: boolean): Promise<void> {
  ...
  console.log("");
  console.log("Patches:");
  for (const patch of setup.patches)
    console.log(`  ${patch.title} -> ${patch.appliesTo}`);

  if (written !== null) {
    const { result } = written;
    console.log("");
    console.log(
      `Created: ${result.created.length > 0 ? result.created.join(", ") : "none"}`,
    );
    console.log(
      `Skipped existing: ${result.skipped.length > 0 ? result.skipped.join(", ") : "none"}`,
    );
  }
}
...
async function printSafetyReport(): Promise<void> {
  ...
  console.log(
    `GhostAPI provider-risk heuristic scan for ${report.projectRoot}`,
  );
  console.log(
    `Detected SDKs: ${report.detected.length > 0 ? report.detected.join(", ") : "none"}`,
  );
  console.log(
    `Scanned files: ${report.limits.scannedFiles}/${report.limits.maxFiles} (files over ${report.limits.maxFileBytes} bytes are skipped)`,
  );
  console.log("");
  if (report.findings.length === 0) {
    console.log("No matches found in the bounded heuristic scan.");
    console.log(
      "This is not proof that the repository contains no production endpoints, credentials, or egress paths.",
    );
  } else {
...
function listProviders(): void {
  for (const manifest of getProviderManifests()) {
    const version =
      manifest.packVersion === null
        ? manifest.implementation
        : `pack ${manifest.packVersion}`;
    console.log(
      `${manifest.name.padEnd(8)} ${manifest.displayName} (${version})`,
    );
  }
}
...
function inspectProvider(provider: string): void {
  if (!isRegisteredProvider(provider)) {
    throw new CliError(
      ...
      `Run ghostapi providers list to see supported providers.`,
    );
  }
  const adapter = providerRegistry[provider];
  const manifest = getProviderManifests().find(
  ...
  console.log(
    `API versions: ${manifest.apiVersions?.supported.join(", ") ?? "legacy"}`,
  );
  console.log(
...
async function runDoctor(options: {
  ...
  try {
    ...
    checks.push({
      ...
      remediation:
        "Fix GHOSTAPI_* environment values or .ghostapi/config.json, then re-run ghostapi doctor.",
      docs: "docs/usage.md#configuration",
    });
  }
  ...
  if (config !== null) {
    ...
    checks.push({
      ...
      remediation:
        config.allowExternalLlm && !hasApiKey
  ...
  const linuxNamespace = egress.capabilities.find(
    (capability) => capability.id === "linux-network-namespace",
  );
  const enforcementAvailable =
    process.platform === "linux" && linuxNamespace !== undefined;
  checks.push({
  ...
  if (failed > 0)
    throw new CliError(
      `Doctor found ${failed} issue${failed === 1 ? "" : "s"}.`,
      "Fix the failed checks above, then re-run ghostapi doctor.",
    );
}
...
async function recordScenario(options: {
  ...
  if (bundle.sanitization.requiresApproval && !options.approve) {
    throw new CliError(
      "Recording was not saved because sanitization found potentially sensitive traffic.",
      "Review the summary, then re-run the exact command with --approve to save the sanitized bundle.",
    );
  }
...
async function replayBundle(options: {
  ...
  if (replayer.remaining !== 0)
    throw new CliError(
      `Replay ended with ${replayer.remaining} interaction${replayer.remaining === 1 ? "" : "s"} remaining.`,
      "Provide the complete ordered request sequence; GhostAPI never guesses a later match.",
    );
  if (options.json) console.log(JSON.stringify({ responses }, null, 2));
  else {
    console.log(
      `Replay matched ${responses.length} interaction${responses.length === 1 ? "" : "s"}.`,
    );
...
async function importHar(options: {
  ...
  const { bundle, contract } = await importHarContractFromFile(
    options.inputPath,
    {
      title: options.title,
      allowedSandboxHosts: options.allowedSandboxHosts,
  ...
  if (bundle.sanitization.requiresApproval && !options.approve) {
    throw new CliError(
      "HAR import was not saved because sanitization found potentially sensitive traffic.",
      "Review the summary, then re-run the exact command with --approve to save the sanitized bundle and contract.",
    );
  }
...
async function diffContractFiles(options: {
  ...
  if (options.json)
    ...
    if (decision !== null)
      console.log(
        `Policy: ${decision.allowed ? "PASS" : "FAIL"} - ${decision.reason}`,
      );
  }
  if (
    options.ci &&
...
async function forkWorldCommand(
  sourceId: string,
  options: { id: string; title?: string; json?: boolean },
): Promise<void> {
...
async function inspectActionCommand(
  actionId: string,
  json: boolean,
): Promise<void> {
...
async function runTelemetryCommand(
  action: "status" | "enable" | "disable" | "export",
  json: boolean,
): Promise<void> {
  const snapshot =
    action === "enable"
  ...
  if (action === "enable")
    console.log(
      "Only aggregate event counts and up to eight ISO weeks are recorded locally after this command; no network export exists.",
    );
}

async function recordLocalProductTelemetry(
  event:
    | "init_completed"
  ...
  try {
    ...
    console.error(
      "Warning: local opt-in product telemetry was not recorded; no telemetry was sent.",
    );
  }
}

function parsePiiRules(
  value: string | undefined,
): Partial<ScenarioPiiRules> | undefined {
...
async function loadReplayRequests(
  path: string,
): Promise<ScenarioReplayRequest[]> {
  const target = isAbsolute(path)
    ? resolve(path)
    : resolve(process.cwd(), path);
  if (!isInsideAllowedRoots(target))
    throw new CliError(
      "Replay requests path traversal outside the project root or GHOSTAPI_DATA_DIR is not allowed.",
    );
  const info = await lstat(target);
  if (!info.isFile() || info.isSymbolicLink())
    throw new CliError(
      "Replay requests input must be a regular non-symlink file.",
    );
  if (!(await isRealPathInsideAllowedRoots(target)))
    throw new CliError(
      "Replay requests input resolves outside the project root or GHOSTAPI_DATA_DIR through a symlink.",
    );
  ...
  return requests.map((request) => {
    ...
    return {
      ...
      path: candidate.path,
      headers: candidate.headers as ScenarioReplayRequest["headers"],
      body: candidate.body,
    };
  });
}
...
function isInsideAllowedRoots(target: string): boolean {
  return (
    isInside(process.cwd(), target) || isInside(getDataPaths().root, target)
  );
}
