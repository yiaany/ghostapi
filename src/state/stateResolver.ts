      ...
      const list = Object.keys(stateStore)
        .filter(
          (key) =>
            ...
            objectMatchesNamespace(stateStore[key], resourceNamespace),
        )
...
function objectMatchesNamespace(
  obj: unknown,
...
function formatProviderDelete(
  ...
  if (provider === "stripe") {
    return {
      id,
      object:
        isJsonObject(existingObj) && typeof existingObj.object === "string"
          ? existingObj.object
          : "removed_object",
      deleted: true,
    };
