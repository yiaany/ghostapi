...
describe("evidence reports", () => {
  ...
  it("builds a redacted versioned report with policy findings and no raw secrets", () => {
    ...
    expect(report.egress.productionAttempts).toBe(0);
    ...
    expect(report.egress.allowedAttempts[0]?.path).toBe("/v1/payment_intents?api_key=***&safe=1");
  ...
  it("keeps the logical hash deterministic for equivalent normalized events", () => {
    const first = buildEvidenceReport({ events: [eventFixture()], runEvidence: runEvidenceFixture(), generatedAt: "2026-08-07T00:00:00.000Z", ghostApiVersion: "0.0.0-test" });
    const second = buildEvidenceReport({ events: [eventFixture()], runEvidence: runEvidenceFixture(), generatedAt: "2026-08-08T00:00:00.000Z", ghostApiVersion: "0.0.0-test" });

    expect(first.artifact.logicalHash).toBe(second.artifact.logicalHash);
    expect(compareEvidenceReports(first, second)).toMatchObject({ equal: true });
  });

  it("writes, loads, validates, and rejects corrupted artifacts", async () => {
    await addEvent(eventFixture());
    const { report, path } = await generateEvidenceReport({ generatedAt: "2026-08-07T00:00:00.000Z", ghostApiVersion: "0.0.0-test" });

    await expect(loadEvidenceReport(path)).resolves.toMatchObject({ artifact: { logicalHash: report.artifact.logicalHash } });
  ...
  it("rejects output path traversal and strips terminal escapes in the human summary", async () => {
    await expect(generateEvidenceReport({ outPath: "../outside.json" })).rejects.toThrow("path traversal");
    const report = buildEvidenceReport({ events: [{ ...eventFixture(), provider: "stripe\u001b[31m" }], runEvidence: runEvidenceFixture(), generatedAt: "2026-08-07T00:00:00.000Z" });
  ...
  it("sets a non-zero CI exit code when policy findings fail", async () => {
    ...
    const result = await runCli(["evidence", "generate", "--policy", "test/fixtures/evidence-failing.policy.yaml", "--out", outputPath, "--ci", "--json"]);
...
function eventFixture(): ProxyEvent {
  return {
    ...
    source: "state",
    durationMs: 12,
    request: { headers: { authorization: "Bearer live-token", cookie: "session=secret-cookie" }, body: { metadata: { scenario: "stripe.card_declined" }, nested: { client_secret: "sk_live_secret" } } },
...
function runEvidenceFixture() {
  return {
    ...
    status: "finished",
    policy: { policyHash: "a".repeat(64), requiredScenarios: ["stripe.card_declined"] },
    events: [
