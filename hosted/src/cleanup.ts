...
export async function runCleanup(database: ReturnType<typeof createDatabase>, config: ReturnType<typeof loadConfig>): Promise<Record<string, number>> {
  return database.transaction(async (client) => {
    const idempotency = await client.query("delete from app.idempotency_ledger where expires_at < now()");
