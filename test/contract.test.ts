...
describe("contract import and drift", () => {
  it("imports the documented OpenAPI 3.0 JSON subset deterministically without resolving URLs", () => {
    ...
    const first = importOpenApiContract(input, {
      importedAt: "2026-08-07T00:00:00.000Z",
    });
    const second = importOpenApiContract(input, {
      importedAt: "2026-08-07T00:00:00.000Z",
    });
    ...
    expect(first.operations).toMatchObject([
      {
        ...
        responses: { "201": { required: ["id"] } },
      },
    ]);
  });

  it("rejects external refs and recursion-depth schema bombs before any network access", async () => {
    const remoteRef = JSON.parse(
      await readFixture("openapi-malicious-remote-ref.json"),
    );
    const schemaBomb = JSON.parse(
      await readFixture("openapi-schema-bomb.json"),
    );

    expect(() => importOpenApiContract(remoteRef)).toThrow(
      /\$ref.*never fetched/i,
    );
  ...
  it("rejects compressed archives before decompression or parser work", async () => {
    ...
    await expect(importOpenApiContractFromFile(archivePath)).rejects.toThrow(
          ...
          entries: [
            {
              request: {
                ...
                headers: [
                  { name: "authorization", value: "Bearer sk_test_secret" },
                  { name: "content-type", value: "application/json" },
                ],
                postData: {
                  mimeType: "application/json",
                  text: '{"email":"ada@example.com","amount":1}',
                },
              },
              response: {
                status: 201,
                ...
                content: {
                  mimeType: "application/json",
                  text: '{"id":"ord_captured","status":"paid"}',
                },
              },
            },
          ],
  ...
  it("classifies endpoint, required-field, enum, status, type, and provider capability changes deterministically", () => {
      ...
      [
        {
          name: "stripe",
          packVersion: "1.0.0",
          capabilities: { webhooks: true, validation: true },
        },
      ],
    );
      ...
      [
        {
          name: "stripe",
          packVersion: "1.1.0",
          capabilities: { webhooks: false, validation: true },
        },
      ],
    );
    ...
    expect(diff.findings.map((entry) => entry.id)).toEqual(
