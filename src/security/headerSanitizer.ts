...
const SAFE_RESPONSE_HEADERS = new Set(["cache-control", "content-language", "content-type", "etag", "expires", "last-modified", "link", "location", "retry-after", "stripe-signature"]);
const BLOCKED_RESPONSE_HEADERS = new Set(["connection", "content-length", "host", "keep-alive", "proxy-authenticate", "proxy-authorization", "set-cookie", "te", "trailer", "transfer-encoding", "upgrade"]);
...
export function sanitizeResponseHeaders(value: unknown): Record<string, string | string[]> {
  if (value === null || typeof value !== "object" || Array.isArray(value)) return {};
  const result: Record<string, string | string[]> = {};
  for (const [rawName, rawValue] of Object.entries(value as Record<string, unknown>)) {
    const name = rawName.toLowerCase();
    if (!/^[!#$%&'*+.^_`|~0-9a-z-]+$/.test(name) || BLOCKED_RESPONSE_HEADERS.has(name) || (!SAFE_RESPONSE_HEADERS.has(name) && !isSafeResponseExtension(name))) continue;
    const values = typeof rawValue === "string" ? [rawValue] : isStringArray(rawValue) ? rawValue : [];
    if (values.length === 0 || values.length > 16 || values.some((entry) => entry.length > 4096 || /[\r\n\0]/.test(entry))) continue;
    if (name === "location" && (typeof rawValue !== "string" || !isSafeRelativeLocation(rawValue))) continue;
...
function isSafeResponseExtension(name: string): boolean {
  return name === "x-request-id" || name === "x-correlation-id" || name.startsWith("x-ghostapi-") || name.startsWith("x-ratelimit-") || name.startsWith("x-github-") || name.startsWith("x-stripe-");
}
