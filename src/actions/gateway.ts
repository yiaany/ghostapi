...
export type ActionReceiptStatus =
  "requested" | "attempted" | "committed" | "verified" | "failed";
...
export type ActionExecutionReceipt = {
  ...
  failure?: {
    code: "execution_error" | "unknown_outcome" | "verification_failed";
    retrySafe: false;
  };
...
export type ActionApprovalVerification = {
  provenance: "verified";
  approvalInboxRequestId?: string;
};
export interface ActionApprovalVerifier {
  verify(
    action: ActionEnvelope,
    approval: unknown,
    context: ActionApprovalVerificationContext,
  ): Promise<ActionApprovalVerification>;
}
export interface ActionApprovalSigner {
  issue(
    approval: UnsignedActionApproval,
  ): ActionApproval | Promise<ActionApproval>;
}
...
export type ActionExecutionAdapter = {
  ...
  execute(
    action: ActionEnvelope,
    controls?: { commit<T>(operation: () => Promise<T>): Promise<T> },
  ): Promise<
    | {
        outcome: "committed";
        providerRequestId: string;
    ...
    | { outcome: "unknown" }
  >;
  verify(action: ActionEnvelope): Promise<{
    committed: boolean;
    providerRequestId?: string;
    result?: {
      worldId: string;
...
export function validateActionEnvelope(value: unknown): ActionEnvelope {
  ...
  if (
    action.schemaVersion !== ACTION_SCHEMA_VERSION ||
  ...
  if (actor.type !== "agent" && actor.type !== "service")
    throw new ActionGatewayError("Action actor type is invalid.");
  const project = object(action.project, "Action project is invalid.");
  exactKeys(project, ["id", "environment"], "Action project");
  if (project.environment !== "synthetic")
    throw new ActionGatewayError(
      "Only the synthetic action environment is available.",
    );
  if (action.provider !== "ghostapi-synthetic")
    throw new ActionGatewayError("Unknown action provider.");
  const resource = object(action.resource, "Action resource is invalid.");
  exactKeys(resource, ["type", "id"], "Action resource");
  if (resource.type !== "synthetic-world")
    throw new ActionGatewayError("Action resource type is invalid.");
  ...
  const expected: ActionEnvelope["expectedSideEffects"] = [
  ...
  if (
    ...
    throw new ActionGatewayError(
      "Action expected side effects must be a bounded unique list of safe identifiers.",
    );
  if (
    ...
    throw new ActionGatewayError(
      "Action expected side effects are invalid for the synthetic operation.",
    );
  if (
  ...
  if (resource.id !== worldId)
    throw new ActionGatewayError(
      "Action resource and normalized world argument must match.",
    );
  return {
    schemaVersion: 1,
    kind: "ghostapi.action",
...
export function validateActionApproval(value: unknown): ActionApproval {
  ...
  if (Date.parse(expiresAt) <= Date.parse(approvedAt))
    throw new ActionGatewayError(
      "Action approval must expire after it was issued.",
    );
  const provenance = object(
    approval.provenance,
    "Action approval provenance is invalid.",
  );
  exactKeys(
    provenance,
    ["kind", "keyId", "signature"],
    "Action approval provenance",
  );
  if (
    provenance.kind !== "ghostapi.ed25519-signature" ||
...
export function createSyntheticActionAdapter(): ActionExecutionAdapter {
  return {
    ...
    async plan(action) {
      if (action.resource.id !== action.arguments.worldId)
        throw new ActionGatewayError(
          "Synthetic action resource does not match normalized arguments.",
        );
    },
    ...
    async verify(action) {
      ...
      const receipt = world.state.receipts.find(
...
export class LocalActionGateway {
  ...
  async submit(
    ...
    return withFileLock(path, async () => {
      ...
      if (existing !== null) {
        if (
          existing.actionHash === actionDigest &&
          actionApprovalHash(existing.approval) === actionApprovalHash(approval)
        )
          return existing;
        throw new ActionGatewayError(
          "Action id already exists with a different immutable envelope or approval.",
        );
      }
  ...
  async inspect(actionId: string): Promise<StoredAction> {
    return readStoredAction(
      this.pathForAction(identifier(actionId, "Action id")),
      true,
    ) as Promise<StoredAction>;
  }
  ...
  async execute(
    ...
    return withFileLock(path, async () => {
      ...
      if (record === null)
        throw new ActionGatewayError("Action was not submitted.");
      ...
      if (committed !== undefined) {
        ...
        if (
          ...
          throw new ActionGatewayError(
            "Committed action verification is incomplete; execution will not retry without idempotency proof.",
          );
        const verifiedReceipt = createReceipt(
          record,
          "verified",
          current,
      ...
      if (attempted !== undefined) {
        ...
        if (
          ...
          throw new ActionGatewayError(
            "Prior action outcome is unknown; execution will not retry without idempotency proof.",
          );
        appendReceipt(
          record,
          createReceipt(
      ...
      if (Date.parse(record.envelope.expiresAt) <= Date.parse(current))
        throw new ActionGatewayError("Action has expired.");
      ...
      const safety = await this.safetyController.admit(
        safetyAction(record.envelope, record.actionHash),
      );
      if (safety.replay)
        throw new ActionGatewayError(
          "Safety controller replay was reached before a verified action receipt.",
        );
      ...
      try {
        ...
        const failed = createReceipt(
          ...
          undefined,
          undefined,
          { code: "execution_error", retrySafe: false },
        );
        appendReceipt(record, failed);
        await writeStoredAction(path, record);
        throw new ActionGatewayError(
          "Action execution failed; it was not automatically retried.",
        );
      }
      if (execution.outcome === "unknown") {
        ...
        const failed = createReceipt(
          ...
          undefined,
          undefined,
          { code: "unknown_outcome", retrySafe: false },
        );
        appendReceipt(record, failed);
        await writeStoredAction(path, record);
        throw new ActionGatewayError(
          "Action provider outcome is unknown; it was not automatically retried.",
        );
      }
      appendReceipt(
        record,
      ...
      if (
        ...
        const failed = createReceipt(
          ...
          execution.providerRequestId,
          execution.result,
          { code: "verification_failed", retrySafe: false },
        );
        appendReceipt(record, failed);
        await writeStoredAction(path, record);
        throw new ActionGatewayError(
          "Action committed but verification did not prove its result; it was not automatically retried.",
        );
      }
      const verifiedReceipt = createReceipt(
        record,
  ...
  async compensate(actionId: string): Promise<never> {
    ...
    if (
      this.adapter.compensate === undefined ||
      action.envelope.reversibility !== "compensatable"
    )
      throw new ActionGatewayError(
        "Compensation is unsupported for this action; no rollback is claimed.",
      );
    await this.adapter.compensate(action.envelope);
    throw new ActionGatewayError(
      "Compensation adapters must return a durable compensation receipt before this operation can be exposed.",
    );
  }
}
...
export function createLocalActionGateway(
...
export function createDisabledActionApprovalVerifier(): ActionApprovalVerifier {
  return {
    async verify(): Promise<never> {
      throw new ActionGatewayError(
        "Action approval authenticity could not be verified.",
      );
    },
  };
}
...
export function createEd25519ActionApprovalSigner(options: {
  ...
  if (
    ...
    throw new ActionGatewayError(
      "Action approval signing key must be an Ed25519 private key.",
    );
  return {
    issue(value: UnsignedActionApproval): ActionApproval {
      ...
      const signature = sign(
        null,
        Buffer.from(canonicalJson(approval), "utf8"),
        privateKey,
      ).toString("base64url");
      return {
        ...approval,
...
export function createEd25519ActionApprovalVerifier(options: {
  trustedKeys: Readonly<Record<string, string | Buffer | KeyObject>>;
}): ActionApprovalVerifier {
  const keys = new Map(
    Object.entries(options.trustedKeys).map(([keyId, key]) => {
      ...
      if (publicKey.asymmetricKeyType !== "ed25519")
        throw new ActionGatewayError(
          "Action approval verification key must be Ed25519.",
        );
  ...
  return {
    async verify(
      ...
      const valid =
        ...
        verify(
          null,
          Buffer.from(canonicalJson(unsignedActionApproval(parsed)), "utf8"),
          publicKey,
          Buffer.from(parsed.provenance.signature, "base64url"),
        );
...
export function createTestActionApprovalVerifier(): {
  ...
  signer: ActionApprovalSigner;
  issue<T extends UnsignedActionApproval>(approval: T): ActionApproval;
} {
...
export function getActionPath(actionId: string): string {
  return join(
    getDataPaths().actions,
    `${identifier(actionId, "Action id")}.action.json`,
  );
}
...
async function readStoredAction(
  ...
  if (!info.isFile() || info.isSymbolicLink())
    throw new ActionGatewayError(
      "Action record must be a regular non-symlink file.",
    );
  const source = await readFile(path, "utf8");
  if (Buffer.byteLength(source, "utf8") > MAX_ACTION_BYTES)
    throw new ActionGatewayError(
      `Action record exceeds ${MAX_ACTION_BYTES} bytes.`,
    );
  try {
...
export function validateStoredAction(value: unknown): StoredAction {
  ...
  exactKeys(
    record,
    ["schemaVersion", "kind", "envelope", "actionHash", "approval", "receipts"],
    "Action record",
  );
  if (
    record.schemaVersion !== ACTION_SCHEMA_VERSION ||
  ...
  if (digest !== actionHash(envelope))
    throw new ActionGatewayError(
      "Action record hash does not match its envelope.",
    );
  ...
  const receipts = rawReceipts.map((receipt, index) =>
    validateReceipt(
      receipt,
      envelope.actionId,
      digest,
      index === 0
        ...
        : (rawReceipts[index - 1]!.receiptHash as string),
    ),
  );
  if (receipts[0]!.status !== "requested")
    throw new ActionGatewayError(
      "Action record must begin with a requested receipt.",
    );
  return {
    schemaVersion: 1,
    kind: "ghostapi.action-record",
...
function createReceipt(
  ...
  timestamp: string,
  providerRequestId?: string,
  result?: ActionExecutionReceipt["result"],
  failure?: ActionExecutionReceipt["failure"],
...
function appendReceipt(
  ...
  if (record.receipts.length >= MAX_RECEIPTS)
    throw new ActionGatewayError(
      `Action receipt limit of ${MAX_RECEIPTS} was reached.`,
    );
...
function validateReceipt(
  ...
  const result =
    receipt.result === undefined ? undefined : normalizeResult(receipt.result);
  const failure =
    receipt.failure === undefined
      ? undefined
      : normalizeFailure(receipt.failure);
  if (
    (receipt.status === "committed" || receipt.status === "verified") &&
    (providerRequestId === undefined || result === undefined)
  )
    throw new ActionGatewayError(
      "Committed and verified receipts require a provider result.",
    );
  if (receipt.status === "failed" && failure === undefined)
    throw new ActionGatewayError(
      "Failed receipt requires a failure classification.",
    );
  ...
  if (
    hash(receipt.receiptHash, "Action receipt hash") !==
...
function normalizeResult(
  value: unknown,
): NonNullable<ActionExecutionReceipt["result"]> {
  const result = object(value, "Action receipt result is invalid.");
  exactKeys(
    result,
    ["worldId", "subscriptionId", "syntheticReceiptId"],
  ...
  return {
    ...
    subscriptionId: identifier(
      ...
      "Action receipt subscription id",
    ),
    syntheticReceiptId: identifier(
      result.syntheticReceiptId,
      "Action receipt synthetic receipt id",
    ),
  };
}
...
function normalizeFailure(
  value: unknown,
): NonNullable<ActionExecutionReceipt["failure"]> {
...
function verifyApproval(
  ...
  if (approval.actionHash !== actionHash(action))
    throw new ActionGatewayError(
      "Action approval is not bound to the exact canonical action hash.",
    );
  if (
    approval.approvedBy === action.actor.id ||
    approval.approvedBy === action.actor.workloadId
  )
    throw new ActionGatewayError(
      "An action actor cannot approve its own high-risk action.",
    );
  ...
  if (enforceExpiry && Date.parse(approval.expiresAt) <= Date.parse(now))
    throw new ActionGatewayError("Action approval has expired.");
}
...
function verifyPolicy(action: ActionEnvelope, policy: ActionPolicyCheck): void {
  ...
  if (
    policy.version !== action.policy.version ||
    policy.hash !== action.policy.hash
  )
    throw new ActionGatewayError(
      "Current policy reference does not match the approved action.",
    );
}
...
function findLatestReceipt(
  ...
  status: ActionReceiptStatus,
): ActionExecutionReceipt | undefined {
...
function object(value: unknown, message: string): Record<string, unknown> {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new ActionGatewayError(message);
...
function hash(value: unknown, label: string): string {
  if (typeof value !== "string" || !HASH.test(value))
    throw new ActionGatewayError(`${label} must be a SHA-256 hex digest.`);
  return value;
}

function positiveInteger(value: unknown, label: string): number {
  if (!Number.isInteger(value) || typeof value !== "number" || value < 1)
    throw new ActionGatewayError(`${label} must be a positive integer.`);
  return value;
}
...
function canonicalJson(value: unknown): string {
  ...
  if (typeof value === "number") {
    if (!Number.isFinite(value))
      throw new ActionGatewayError("Canonical action values must be finite.");
    return JSON.stringify(value);
  }
...
function requireApprovalInboxVerification(
  approval: ActionApproval,
  verification: ActionApprovalVerification,
): void {
  if (
    approval.approvedBy === "approval-inbox" &&
    verification.approvalInboxRequestId !== approval.approvalId
  )
    throw new ActionGatewayError(
      "Approval inbox provenance is not active for execution.",
    );
}

function validateUnsignedActionApproval(
  value: unknown,
...
function unsignedActionApproval(
  approval: ActionApproval,
): UnsignedActionApproval {
