...
describe("local team control-plane security prototype", () => {
  it("enforces tenant boundaries and the owner/admin/developer/viewer permission matrix", async () => {
    ...
    await plane.bootstrapOrganization({
      organizationId: "roles-a",
      name: "Roles A",
      ownerId: "owner",
    });
    await plane.bootstrapOrganization({
      organizationId: "roles-b",
      name: "Roles B",
      ownerId: "owner-b",
    });
    ...
    await expect(
      plane.addMember(admin, { memberId: "blocked", role: "viewer" }),
    ).rejects.toThrow("Access denied");
    const ownerToken = await plane.issueToken(owner, {
      expiresAt: "2026-08-09T00:00:00.000Z",
    });
    await expect(
      plane.issueToken(admin, {
        memberId: "owner",
        expiresAt: "2026-08-09T00:00:00.000Z",
      }),
    ).rejects.toThrow("Access denied");
    await expect(plane.rotateToken(admin, ownerToken.tokenId)).rejects.toThrow(
      "Access denied",
    );
    await expect(plane.revokeToken(admin, ownerToken.tokenId)).rejects.toThrow(
      "Access denied",
    );
    await expect(
      plane.registerProject(developer, {
        projectId: "blocked",
        name: "Blocked",
      }),
    ).rejects.toThrow("Access denied");
    await expect(
      plane.publishScenario(viewer, scenario("project", "ci", "blocked")),
    ).rejects.toThrow("Access denied");
    await expect(plane.listAudit(developer)).rejects.toThrow("Access denied");
    await expect(
      plane.registerProject(admin, {
        projectId: "admin-project",
        name: "Admin Project",
      }),
    ).resolves.toMatchObject({ id: "admin-project" });
    await expect(
      plane.publishScenario(developer, scenario("project", "ci", "allowed")),
    ).resolves.toMatchObject({ createdBy: "developer" });
    await expect(
      plane.listScenarioVersions(viewer, {
        projectId: "project",
        environmentId: "ci",
        scenarioId: "allowed",
      }),
    ...
    await expect(
      plane.listScenarioVersions(otherOwner, {
        projectId: "project",
        environmentId: "ci",
        scenarioId: "allowed",
      }),
  ...
  it("persists hash-only typed service tokens and enforces their scope on every scoped operation", async () => {
    ...
    const stored = await readFile(
      join(getDataPaths().root, "team-control-service.json"),
      "utf8",
    );
    expect(stored).not.toContain(issued.token);
    expect(stored).toContain("digest");
    await expect(
      plane.publishScenario(
        service,
        scenario("project-a", "ci-a", "service-flow"),
      ),
    ).resolves.toMatchObject({ createdBy: "service:ci-bot" });
    await expect(
      plane.uploadSanitizedEvidence(service, {
        evidenceId: "service-evidence",
        projectId: "project-a",
        environmentId: "ci-a",
        report: evidenceReport(),
      }),
    ).resolves.toMatchObject({ uploadedBy: "service:ci-bot" });
    await expect(
      plane.publishScenario(
        service,
        scenario("project-b", "ci-b", "outside-scope"),
      ),
    ).rejects.toThrow("Access denied");
    await expect(
      plane.registerProject(service, { projectId: "blocked", name: "Blocked" }),
    ).rejects.toThrow("Access denied");
    await expect(plane.getLatestPolicy(service)).rejects.toThrow(
      "Access denied",
    );
    await expect(
      plane.publishScenario(
        ...
        scenario("project-a", "ci-a", "forged-service-actor"),
      ),
    ).rejects.toThrow("Access denied");
  });
  ...
  it("rotates atomically, revokes old credentials, rejects expiry, and blocks service impersonation", async () => {
    ...
    const plane = createLocalTeamControlPlane({
      path: join(getDataPaths().root, "team-control-rotation.json"),
      now: () => now,
    });
    const owner = { organizationId: "rotation-team", memberId: "owner" };
    await plane.bootstrapOrganization({
      organizationId: "rotation-team",
      name: "Rotation Team",
      ownerId: "owner",
    });
    const issued = await plane.issueToken(owner, {
      expiresAt: "2026-08-09T00:00:00.000Z",
    });
    const rotated = await plane.rotateToken(owner, issued.tokenId);
    await expect(plane.authenticateToken(issued.token)).rejects.toThrow(
      "Invalid or expired token",
    );
    await expect(plane.authenticateToken(rotated.token)).resolves.toEqual(
      owner,
    );
    now = new Date("2026-08-10T00:00:00.000Z");
    await expect(plane.authenticateToken(rotated.token)).rejects.toThrow(
      "Invalid or expired token",
    );
    await expect(
      plane.issueToken(
  ...
  it("freezes published role permissions and bounds duplicate token allocation", async () => {
    ...
    const plane = createLocalTeamControlPlane({
      path: join(getDataPaths().root, "team-control-collision.json"),
      now: () => new Date("2026-08-08T00:00:00.000Z"),
      randomTokenBytes: (size) => Buffer.alloc(size, 5),
    });
    const owner = { organizationId: "collision-team", memberId: "owner" };
    await plane.bootstrapOrganization({
      organizationId: "collision-team",
      name: "Collision Team",
      ownerId: "owner",
    });
    await plane.issueToken(owner, { expiresAt: "2026-08-09T00:00:00.000Z" });
    await expect(
      plane.issueToken(owner, { expiresAt: "2026-08-09T00:00:00.000Z" }),
    ).rejects.toThrow("Unable to allocate a unique token");
  });

  it("expires and disables authenticated service accounts before every side effect", async () => {
    let now = new Date("2026-08-08T00:00:00.000Z");
    const plane = createLocalTeamControlPlane({
      path: join(getDataPaths().root, "team-control-disable.json"),
      now: () => now,
    });
    const owner = { organizationId: "disable-team", memberId: "owner" };
    await plane.bootstrapOrganization({
      organizationId: "disable-team",
      name: "Disable Team",
      ownerId: "owner",
    });
    await prepareProject(plane, owner, "project", "ci");
    ...
    now = new Date("2026-08-10T00:00:00.000Z");
    await expect(
      plane.publishScenario(service, scenario("project", "ci", "expired")),
    ).rejects.toThrow("Access denied");

    now = new Date("2026-08-08T12:00:00.000Z");
    const active = await plane.issueServiceToken(owner, {
      serviceAccountId: "ci-bot",
    ...
    await expect(
      plane.publishScenario(
        activeService,
        scenario("project", "ci", "disabled"),
      ),
    ).rejects.toThrow("Access denied");
    await expect(plane.authenticateToken(active.token)).rejects.toThrow(
      "Invalid or expired token",
    );
  });
  ...
  it("does not allow an authenticated service actor to be mutated into a human actor", async () => {
    ...
    const service = await plane.authenticateToken(issued.token);
    const compromised = service as unknown as {
      actorType?: undefined;
      memberId?: string;
    };
    ...
    await expect(plane.deleteProject(service, "project")).rejects.toThrow(
      "Access denied",
    );
  });
  ...
  it("exports a SHA-256 audit chain that detects public-export tampering and retains a safe anchor", async () => {
    ...
    await plane.bootstrapOrganization({
      organizationId: "audit-team",
      name: "Audit Team",
      ownerId: "owner",
    });
    await plane.registerProject(owner, {
      projectId: "project",
      name: "Project",
    });
    ...
    expect(
      verifyAuditExport({
        ...exported,
        records: [
          { ...exported.records[0]!, action: "tampered" },
          ...exported.records.slice(1),
        ],
      }).valid,
    ).toBe(false);
    ...
    await expect(plane.listAudit(owner)).rejects.toThrow(
  ...
  it("deletes only tenant-bound evidence and preserves resource-not-found IDOR semantics", async () => {
    ...
    await plane.bootstrapOrganization({
      organizationId: "delete-a",
      name: "Delete A",
      ownerId: "owner-a",
    });
    await plane.bootstrapOrganization({
      organizationId: "delete-b",
      name: "Delete B",
      ownerId: "owner-b",
    });
    await prepareProject(plane, ownerA, "project", "ci");
    await plane.uploadSanitizedEvidence(ownerA, {
      evidenceId: "evidence",
      projectId: "project",
      environmentId: "ci",
      report: evidenceReport(),
    ...
    await expect(
      plane.deleteEvidence(ownerB, {
        projectId: "project",
        evidenceId: "evidence",
      }),
    ).rejects.toThrow("Resource not found");
    await expect(
      plane.deleteEvidence(ownerA, {
        projectId: "project",
        evidenceId: "evidence",
      }),
    ...
    await expect(
      plane.deleteProject(ownerA, "project"),
    ).resolves.toBeUndefined();
  ...
  it("migrates v1 and v2 safely and validates future transport controls fail closed", async () => {
    ...
    await expect(
      createDisabledIdentityProvider().authenticate(),
    ).rejects.toThrow("not configured");
    expect(
      TEAM_CONTROL_PLANE_SECURITY_HEADERS["content-security-policy"],
    ).toContain("default-src 'none'");
    expect(createTeamControlPlaneSecurityHeaders()).not.toBe(
      TEAM_CONTROL_PLANE_SECURITY_HEADERS,
    );
    let tick = 0;
    const limiter = new TeamControlPlaneRateLimiter({
      limit: 1,
      windowMs: 1_000,
      maxKeys: 1,
      now: () => new Date(tick),
    ...
    expect(() => limiter.consume("sk_live_rate_limit_key")).toThrow(
      "key is invalid",
    );
    tick = 1_001;
    expect(limiter.consume("client").remaining).toBe(0);
    expect(() => new TeamControlPlaneRateLimiter({ limit: 0 })).toThrow(
      TeamControlPlaneError,
    );
    const badClock = new TeamControlPlaneRateLimiter({
...
function teamPlane(name: string) {
  return createLocalTeamControlPlane({
    path: join(getDataPaths().root, `team-control-${name}.json`),
    now: () => new Date("2026-08-08T00:00:00.000Z"),
    randomTokenBytes: (size) => Buffer.alloc(size, 7),
  });
}

async function prepareProject(
  plane: ReturnType<typeof createLocalTeamControlPlane>,
  owner: { organizationId: string; memberId: string },
  projectId: string,
  ...
  await plane.createEnvironment(owner, {
    environmentId,
    projectId,
    name: "CI",
    kind: "ci",
...
function evidenceReport() {
  return buildEvidenceReport({
    ...
    runEvidence: {
      ...
      status: "finished",
      events: [
        { type: "run-created", timestamp: "2026-08-08T00:00:00.000Z" },
        {
          type: "target-exited",
