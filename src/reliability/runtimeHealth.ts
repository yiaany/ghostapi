...
export async function checkRuntimeHealth(
  ...
  for (const store of stores) {
    ...
    if (dataDirStatus !== "unavailable") {
      try {
        const info = await lstat(store.path).catch((error: unknown) =>
          isErrorCode(error, "ENOENT") ? null : Promise.reject(error),
        );
        if (info !== null) {
          if (info.isSymbolicLink())
            throw new RuntimeHealthError(`${store.id} is a symbolic link.`);
          if (store.kind === "file") {
            if (!info.isFile())
              throw new RuntimeHealthError(
                `${store.id} must be a regular file.`,
              );
            const source = await readFile(store.path, "utf8");
            if (Buffer.byteLength(source, "utf8") > MAX_CHECK_FILE_BYTES)
              throw new RuntimeHealthError(
                `${store.id} exceeds its structural size limit.`,
              );
            try {
              JSON.parse(source);
            } catch {
  ...
  if (dataDirStatus === "unavailable") {
    results.unshift({
      ...
      status: "unavailable",
      detail:
        "data directory could not be created or is not a private non-symlink directory",
    });
  }
  const ready =
    dataDirStatus === "healthy" &&
    results.every((store) => store.status === "healthy");
  return {
    schemaVersion: 1,
    kind: "ghostapi.runtime-health",
    checkedAt,
...
export function formatRuntimeHealth(report: RuntimeHealthReport): string {
  const lines = [
    `Runtime health at ${report.checkedAt}: ${report.ready ? "ready" : "NOT READY"}`,
  ];
  for (const store of report.stores) {
    lines.push(
      `  ${store.id} [${store.kind}] ${store.status}${store.detail === undefined ? "" : ` - ${store.detail}`}`,
    );
  }
  return lines.join("\n");
}
...
export async function backupRuntime(
  options: { destinationDir?: string; now?: () => Date } = {},
  ...
  const destinationDir = resolve(
    options.destinationDir ??
      join(getDataPaths().backups, backupName(createdAt)),
  );
  const existing = await lstat(destinationDir).catch((error: unknown) =>
    isErrorCode(error, "ENOENT") ? null : Promise.reject(error),
  );
  if (existing !== null)
    throw new RuntimeHealthError(
  ...
  if (totalBytes > BACKUP_MAX_BYTES) {
    ...
    throw new RuntimeHealthError(
      `Backup exceeds the ${BACKUP_MAX_BYTES} byte local limit.`,
    );
  }
  ...
  for (const file of files) {
    await copyFileWithHash(
      file.sourcePath,
      join(destinationDir, file.relativePath),
      file.size,
      file.sha256,
    );
  }
  await atomicWriteBytes(
    join(destinationDir, MANIFEST_NAME),
...
export async function restoreRuntimeBackup(options: {
  sourceDir: string;
  targetDir: string;
  ...
  if (!manifest.verified)
    throw new RuntimeHealthError(
      "Refusing to restore a backup that did not verify after creation.",
    );
  if (targetDir === sourceDir || targetDir.startsWith(sourceDir + sep))
    throw new RuntimeHealthError(
      "Restore target must not be inside the backup source.",
    );
...
function canonicalStores(): Array<{
  id: string;
  kind: "file" | "dir";
  path: string;
}> {
  ...
  const files = [
    ...
    ["credential-broker", paths.credentialBroker],
    ["trust-ladder", paths.trustLadder],
    ["safety-controller", paths.safetyController],
    ["action-ledger", paths.actionLedger],
    ["team-control-plane", paths.teamControlPlane],
    ["fault-lab", paths.faultLab],
    ["product-telemetry", paths.productTelemetry],
    ["slo", paths.sloStore],
    ["reconciliation", paths.reconciliationStore],
    ["costs", paths.costStore],
...
> {
  ...
  const skipRoots = new Set([
    resolve(getDataPaths().cache),
    resolve(getDataPaths().backups),
  ]);
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        if (
          ...
          skipRoots.has(resolve(fullPath)) ||
          relativeDirectory === "runs"
        )
          continue;
        await walk(fullPath);
        continue;
      ...
      if (entry.isSymbolicLink())
        throw new RuntimeHealthError(
          `Refusing to back up a symbolic link: ${fullPath}`,
        );
      ...
      if (!info.isFile() || info.isSymbolicLink())
        throw new RuntimeHealthError(
          `Refusing to back up an unsafe file: ${fullPath}`,
        );
...
async function copyFileWithHash(
  ...
  if (
    ...
    sha256Buffer(content) !== expectedSha256
  )
    throw new RuntimeHealthError(
      "Backup file content changed during copy; backup is not consistent.",
    );
...
async function verifySourceEntry(
  ...
  if (!info.isFile() || info.isSymbolicLink())
    throw new RuntimeHealthError(
      "Backup source entry is not a regular non-symlink file.",
    );
  const content = await readFile(sourcePath);
  if (
    ...
    throw new RuntimeHealthError(
      "Backup source entry failed integrity verification.",
    );
...
async function verifyBackupFiles(
  ...
  if (
    ...
    parsed.totalBytes !== manifest.totalBytes
  )
    throw new RuntimeHealthError(
      "Backup manifest does not match the copied files.",
    );
}
...
function validateManifest(value: unknown): BackupManifest {
  ...
  for (const key of Object.keys(manifest))
    if (!expected.includes(key))
      throw new RuntimeHealthError(
        `Backup manifest contains unsupported field: ${key}`,
      );
  if (
    manifest.schemaVersion !== MANIFEST_SCHEMA_VERSION ||
    manifest.kind !== MANIFEST_KIND
  )
    throw new RuntimeHealthError("Unsupported backup manifest schema.");
  if (typeof manifest.verified !== "boolean")
    throw new RuntimeHealthError("Backup manifest verified flag is invalid.");
  if (!Array.isArray(manifest.entries) || manifest.entries.length > 100_000)
    throw new RuntimeHealthError("Backup manifest entries are invalid.");
  const entries = manifest.entries.map((entry) => {
    const value = object(entry, "Backup manifest entry is invalid.");
    for (const key of Object.keys(value))
      if (!["path", "size", "sha256"].includes(key))
        throw new RuntimeHealthError(
          `Backup manifest entry contains unsupported field: ${key}`,
        );
    const path = normalizeEntryPath(value.path);
    return {
      path,
      size: nonNegative(value.size, "Backup entry size", BACKUP_MAX_BYTES),
      sha256: hash(value.sha256, "Backup entry hash"),
    };
  });
  return {
    schemaVersion: 1,
    ...
    sourceDataDir: text(
      manifest.sourceDataDir,
      "Backup source data directory",
      4_000,
    ),
...
function normalizeEntryPath(value: unknown): string {
  if (
    ...
    throw new RuntimeHealthError("Backup manifest entry path is invalid.");
...
function assertContained(root: string, candidate: string): void {
  ...
  if (rel === ".." || rel.startsWith(`..${sep}`) || isAbsolute(rel))
    throw new RuntimeHealthError(
      "Backup or restore path escapes its root directory.",
    );
}
...
function identifier(value: unknown, label: string): string {
  if (typeof value !== "string" || !IDENTIFIER.test(value))
    throw new RuntimeHealthError(`${label} must be a safe identifier.`);
  return value;
}

function hash(value: unknown, label: string): string {
  if (typeof value !== "string" || !HASH.test(value))
    throw new RuntimeHealthError(`${label} must be a SHA-256 hash.`);
  return value;
}
...
function nonNegative(value: unknown, label: string, max: number): number {
  if (
    ...
    throw new RuntimeHealthError(`${label} is invalid.`);
  return value;
}
...
function timestamp(value: unknown, label: string): string {
  if (
    ...
    throw new RuntimeHealthError(`${label} must be an ISO UTC timestamp.`);
  return value;
}

function timestampValue(value: Date, label: string): string {
  if (!(value instanceof Date) || !Number.isFinite(value.getTime()))
    throw new RuntimeHealthError(`${label} clock is invalid.`);
...
function object(value: unknown, message: string): Record<string, unknown> {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new RuntimeHealthError(message);
