...
export type CostOperatorPermission =
  "cost.record" | "cost.configure" | "cost.inspect" | "cost.acknowledge";
export type CostOperator = {
  id: string;
  principalId: string;
  permissions: readonly CostOperatorPermission[];
  tenantId?: string;
};
export interface CostOperatorAuthorizer {
  authenticate(identity: unknown): Promise<CostOperator>;
}
...
export function createLocalCostGovernance(
  options: CostControllerOptions = {},
): LocalCostGovernance {
...
export class LocalCostGovernance {
  ...
  constructor(options: CostControllerOptions = {}) {
    ...
    this.operatorAuthorizer =
      options.operatorAuthorizer ?? createDisabledCostOperatorAuthorizer();
  }
    ...
    return this.mutate((state) => {
      if (
        ...
        throw new CostGovernanceError(
          "Cost record already exists for this tenant and action.",
        );
      if (state.records.length >= MAX_RECORDS)
        throw new CostGovernanceError(
          "Cost record limit was reached; export and retention review are required.",
        );
  ...
  async report(input: {
    ...
    const windowMs =
      input.windowMs === undefined
        ? 30 * 24 * 60 * 60 * 1000
        : boundedMs(input.windowMs, "Cost report window");
    const projectionDays =
      input.projectionDays === undefined
        ? 7
        : boundedDays(input.projectionDays, "Cost projection days");
    const now = this.timestamp();
    const runId =
      input.runId === undefined
        ? `cost-${randomUUID().replace(/-/g, "").slice(0, 32)}`
        : identifier(input.runId, "Cost report run id");
    const state = await this.read();
    const inWindow = tenantScoped(state.records, operator.tenantId).filter(
      (record) =>
        Date.parse(record.recordedAt) >= Date.parse(now) - windowMs &&
        Date.parse(record.recordedAt) <= Date.parse(now) + 1_000,
    );
    const totals = sumAmounts(inWindow.map((record) => record.amounts));
    const attribution = aggregateAttribution(inWindow);
    const evaluations = state.budgets.map((budget) =>
      evaluateBudget(budget, inWindow, now),
    );
    ...
    return {
      ...
      windowEnd: now,
      source:
        "Local cost records are synthetic action metadata (counts and token estimates). They are not provider invoices and must be calibrated against real billing during the pilot.",
      totals,
      attribution,
      budgets: evaluations,
      alerts: clone(alerts),
  ...
  async listAlerts(input: { identity: unknown }): Promise<CostAlert[]> {
    ...
    const now = this.timestamp();
    return this.mutate((state) => {
      const inWindow = state.records.filter(
        (record) =>
          Date.parse(record.recordedAt) >= Date.parse(now) - MAX_WINDOW_MS,
      );
      const evaluations = state.budgets.map((budget) =>
        evaluateBudget(budget, inWindow, now),
      );
  ...
  private async authorize(
    identity: unknown,
    permission: CostOperatorPermission,
  ): Promise<CostOperator> {
    ...
    if (!operator.permissions.includes(permission))
      throw new CostGovernanceError(
        `Cost operator lacks required permission: ${permission}.`,
      );
    return operator;
  }
  ...
  private async read(): Promise<CostStoreState> {
    ...
    if (!info.isFile() || info.isSymbolicLink())
      throw new CostGovernanceError(
        "Cost store must be a regular non-symlink file.",
      );
...
export function createDisabledCostOperatorAuthorizer(): CostOperatorAuthorizer {
  return {
    async authenticate(): Promise<never> {
      throw new CostGovernanceError(
        "Cost operator authorization is not configured.",
      );
    },
  };
}
...
export function createTestCostOperatorAuthorizer(): {
  authorizer: CostOperatorAuthorizer;
  issue(input: CostOperator): CostOperator;
} {
  const issued = new WeakSet<object>();
  return {
    authorizer: {
      async authenticate(identity: unknown): Promise<CostOperator> {
        if (
          identity === null ||
          ...
          throw new CostGovernanceError(
            "Cost operator identity is not authenticated.",
          );
        return validateOperator(identity);
      },
    },
    issue(input): CostOperator {
...
export function formatCostReport(report: CostReport): string {
  ...
  lines.push(
    `  totals: requests=${report.totals.requests} messages=${report.totals.messages} mutations=${report.totals.mutations} tokenCost=${report.totals.tokenCost} monetaryAmountMinor=${report.totals.monetaryAmountMinor}`,
  );
  for (const entry of report.attribution) {
    lines.push(
      `  by ${entry.dimension}:${entry.value} requests=${entry.totals.requests} tokenCost=${entry.totals.tokenCost} monetaryAmountMinor=${entry.totals.monetaryAmountMinor}`,
    );
  }
  for (const budget of report.budgets) {
    const exceeded =
      budget.status === "exceeded"
        ? ` EXCEEDED (${budget.exceeded.map((entry) => `${entry.key} ${entry.total}/${entry.limit}`).join(", ")})`
        : "";
    lines.push(
      `  budget ${budget.id} ${budget.scope.dimension}:${budget.scope.value} ${budget.status}${exceeded}`,
    );
  }
  lines.push(
    `  forecast: approximate linear extrapolation over ${report.forecast.sampleDays} day(s), next ${report.forecast.projectionDays} day(s) estimate requests=${report.forecast.nextPeriodEstimate.requests} tokenCost=${report.forecast.nextPeriodEstimate.tokenCost}`,
  );
...
function validateState(value: unknown): CostStoreState {
  ...
  const budgets = array(state.budgets, "Cost budgets", MAX_BUDGETS).map(
    validateBudget,
  );
  unique(
  ...
  const alerts = array(state.alerts, "Cost alerts", MAX_ALERTS).map(
    validateAlert,
  );
  unique(
...
function validateRecord(value: unknown): CostRecord {
  ...
  if (
    ...
    record.kind !== "ghostapi.cost-record"
  )
...
function validateBudget(value: unknown): CostBudget {
  ...
  return {
    id: identifier(budget.id, "Cost budget id"),
    scope: {
      dimension: scope.dimension as CostDimension,
      value: identifier(scope.value, "Cost budget scope value"),
    },
    windowMs: boundedMs(budget.windowMs, "Cost budget window"),
    limits,
...
function validateAlert(value: unknown): CostAlert {
  ...
  const result: CostAlert = {
    ...
    scope: {
      dimension: scope.dimension as CostDimension,
      value: identifier(scope.value, "Cost alert scope value"),
    },
    ...
    ...(alert.acknowledgedAt === undefined
      ...
      : {
          acknowledgedAt: timestamp(
            ...
            "Cost alert acknowledgement time",
          ),
        }),
  };
  if (
    result.status === "acknowledged" &&
    (result.acknowledgedBy === undefined || result.acknowledgedAt === undefined)
  )
    throw new CostGovernanceError(
      "Acknowledged cost alerts require audit metadata.",
    );
  if (result.status === "open" && result.acknowledgedBy !== undefined)
    throw new CostGovernanceError(
      "Open cost alerts cannot carry acknowledgement metadata.",
    );
  return result;
}
...
function validateAttribution(value: unknown): CostAttribution {
  ...
  exactKeys(
    attribution,
    ["agentId", "projectId", "provider", "actionClass", "workflowId"],
    "Cost attribution",
  );
  return {
...
function validateLimits(
  value: unknown,
): Partial<Record<CostAmountKey, number>> {
...
function evaluateBudget(
  ...
  const inWindow = records.filter(
    (record) =>
      ...
      record.attribution[ATTRIBUTION_DIMENSION_KEY[budget.scope.dimension]] ===
        budget.scope.value,
  );
  const totals = sumAmounts(inWindow.map((record) => record.amounts));
...
function deriveAlerts(
  existing: CostAlert[],
  evaluations: CostBudgetEvaluation[],
  budgets: CostBudget[],
  now: string,
): CostAlert[] {
  ...
  for (const evaluation of evaluations) {
    ...
    const current = alerts.find(
      (candidate) =>
        candidate.budgetId === evaluation.id && candidate.status === "open",
    );
    const payload = {
      budgetId: evaluation.id,
      scope: evaluation.scope,
      windowMs: evaluation.windowMs,
      windowStart: new Date(
        Date.parse(now) - evaluation.windowMs,
      ).toISOString(),
      windowEnd: now,
    ...
    if (current !== undefined) {
      ...
      if (alerts.length >= MAX_ALERTS) {
        const removable = alerts.findIndex(
      ...
      alerts.push({
        id: `alert-${randomUUID().replace(/-/g, "").slice(0, 32)}`,
        ...payload,
        created: now,
        status: "open",
      });
...
function aggregateAttribution(
  records: CostRecord[],
): Array<{ dimension: CostDimension; value: string; totals: CostAmounts }> {
  const result: Array<{
    dimension: CostDimension;
    value: string;
    totals: CostAmounts;
...
function buildForecast(
  ...
  const sampleStartMs =
    Date.parse(now) - Math.min(windowMs, 30 * 24 * 60 * 60 * 1000);
  ...
  if (days.length >= 2) {
    ...
    for (const key of COST_AMOUNT_KEYS) {
      ...
      averagePerDay[key] = Math.round(
        points.reduce((sum, point) => sum + point.totals[key], 0) /
          points.length,
      );
    }
  ...
  return {
    ...
    nextPeriodEstimate,
    disclaimer:
      "This forecast is an explainable linear approximation of local synthetic cost metadata. It is not a provider invoice, not a commitment, and not a prediction of real billing; calibrate it against provider billing during the pilot.",
  };
}
...
function validateOperator(value: unknown): CostOperator {
  ...
  exactKeys(
    operator,
    ["id", "principalId", "permissions", "tenantId"],
    "Cost operator",
    ["tenantId"],
  );
  const permissions = array(
    operator.permissions,
    "Cost operator permissions",
    8,
  ).map((permission) => {
    if (
      permission !== "cost.record" &&
      permission !== "cost.configure" &&
      permission !== "cost.inspect" &&
      permission !== "cost.acknowledge"
    )
  ...
  return {
    id: identifier(operator.id, "Cost operator id"),
    principalId: identifier(operator.principalId, "Cost operator principal id"),
    permissions,
    ...(operator.tenantId === undefined
      ? {}
      : { tenantId: identifier(operator.tenantId, "Cost operator tenant id") }),
  };
}
...
function boundedMs(value: unknown, label: string): number {
  if (
    ...
    throw new CostGovernanceError(
      `${label} must be between one hour and 90 days.`,
    );
  return value;
}
