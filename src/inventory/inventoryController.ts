...
export class LocalInventoryController {
  ...
  async import(identityValue: unknown, payloadValue: unknown): Promise<ImportSummary> {
    ...
    return this.mutate((state) => {
      ...
      const counts: ImportRunRecord["recordCounts"] = { agents: 0, tools: 0, identities: 0, providers: 0, resources: 0, sideEffects: 0, credentials: 0, policies: 0 };
  ...
  async analyze(identityValue: unknown): Promise<{ findings: number; resolved: number }> {
    const operator = await this.authorize(identityValue, "inventory.analyze");
    const now = this.timestamp();
    return this.mutate((state) => {
      const previous = new Map<string, InventoryStoreState["findings"][number]>();
      for (const finding of state.findings) {
        if (finding.tenantId === operator.tenantId) previous.set(findingKey(finding), finding);
      }
  ...
  async proposeRemediation(identityValue: unknown, input: unknown): Promise<InventoryStoreState["remediations"][number]> {
    ...
    return this.mutate((state) => {
      ...
      if (finding === undefined) throw new InventoryError("Remediation finding was not found for the tenant.");
      if (finding.status === "resolved") throw new InventoryError("Cannot propose remediation for a resolved finding.");
      if (proposal.targetKind !== finding.targetKind || proposal.targetId !== finding.targetId) throw new InventoryError("Remediation target does not match the finding target.");
      ...
      if (proposal.kind === "reduce_scope") {
        if (proposal.targetKind !== "credential") throw new InventoryError("reduce_scope remediation only supports credential targets.");
        ...
        if (credential === undefined) throw new InventoryError("Remediation target credential does not exist.");
        if (credential.status !== "active") throw new InventoryError("Cannot reduce scopes on a revoked credential.");
  ...
  async applyRemediation(identityValue: unknown, remediationId: string): Promise<InventoryStoreState["remediations"][number]> {
    const operator = await this.authorize(identityValue, "inventory.remediate");
    const now = this.timestamp();
    ...
    return this.mutate((state) => {
      ...
      if (index === -1) throw new InventoryError("Remediation was not found for the tenant.");
      const remediation = state.remediations[index];
      if (remediation.status === "applied") throw new InventoryError("Remediation was already applied.");
      if (remediation.status === "rejected" || remediation.status === "expired") throw new InventoryError(`Remediation is ${remediation.status} and cannot be applied.`);
      applyRemediationEffect(state, operator.tenantId, remediation, now);
      const applied = { ...remediation, status: "applied" as const, appliedAt: now };
  ...
  async rejectRemediation(identityValue: unknown, remediationId: string): Promise<InventoryStoreState["remediations"][number]> {
    const operator = await this.authorize(identityValue, "inventory.remediate");
    const now = this.timestamp();
    const safeRemediationId = safeIdentifier(remediationId, "Remediation id");
    return this.mutate((state) => {
      ...
      if (index === -1) throw new InventoryError("Remediation was not found for the tenant.");
      const remediation = state.remediations[index];
      if (remediation.status === "applied") throw new InventoryError("Remediation is already applied and cannot be rejected.");
  ...
  async listRemediations(identityValue: unknown): Promise<InventoryStoreState["remediations"][number][]> {
    const operator = await this.authorize(identityValue, "inventory.inspect");
    const state = await this.read();
  ...
  async export(identityValue: unknown): Promise<InventoryExport> {
    ...
    const evidenceMetadata = snapshot.agents.map((agent) => ({
      ...
      status: agent.lastEvidenceAt === null ? ("missing" as const) : freshnessStatusFor(agent.lastEvidenceAt, now, normalized.evidenceFreshDays)
    }));
    return {
      schemaVersion: 1,
  ...
  private async authorize(identity: unknown, permission: InventoryOperatorPermission): Promise<InventoryOperator> {
    const operator = validateOperator(await this.operatorAuthorizer.authenticate(identity));
    if (!operator.permissions.includes(permission)) throw new InventoryError(`Inventory operator lacks required permission: ${permission}.`);
    return operator;
  }

  private timestamp(): string {
    const value = this.now();
    if (!(value instanceof Date) || !Number.isFinite(value.getTime())) throw new InventoryError("Inventory clock is invalid.");
    return value.toISOString();
  }

  private upsertSource(state: InventoryStoreState, tenantId: string, payload: InventoryImportPayload, now: string): void {
    const existing = state.sources.find((source) => source.tenantId === tenantId && source.sourceId === payload.source.sourceId);
    if (existing === undefined) {
      if (state.sources.length >= INVENTORY_LIMITS.maxSources) throw new InventoryError("Import source limit was reached.");
      state.sources.push({
        tenantId,
      ...
      if (payload.source.sourceVersion !== undefined) existing.sourceVersion = payload.source.sourceVersion;
    }
  }

  private assertRemediationTargetExists(state: InventoryStoreState, tenantId: string, targetKind: InventoryStoreState["findings"][number]["targetKind"], targetId: string): void {
    const exists =
      targetKind === "agent" ? state.agents.some((record) => record.tenantId === tenantId && record.agentId === targetId)
  ...
  private async read(): Promise<InventoryStoreState> {
    ...
    if (!info.isFile() || info.isSymbolicLink()) throw new InventoryError("Inventory store must be a regular non-symlink file.");
    const source = await readFile(this.path, "utf8");
    if (Buffer.byteLength(source, "utf8") > INVENTORY_LIMITS.maxStoreBytes) throw new InventoryError("Inventory store exceeds its size limit.");
    try {
  ...
  private async mutate<T>(operation: (state: InventoryStoreState) => T): Promise<T> {
...
export function createLocalInventoryController(options: InventoryControllerOptions = {}): LocalInventoryController {
...
export function createDisabledInventoryOperatorAuthorizer(): InventoryOperatorAuthorizer {
  return { async authenticate(): Promise<never> { throw new InventoryError("Inventory operator authorization is not configured."); } };
}

export function createTestInventoryOperatorAuthorizer(): { authorizer: InventoryOperatorAuthorizer; issue(input: InventoryOperator): InventoryOperator } {
  const issued = new WeakSet<object>();
  return {
    authorizer: {
      async authenticate(identity: unknown): Promise<InventoryOperator> {
        if (identity === null || typeof identity !== "object" || !issued.has(identity)) throw new InventoryError("Inventory operator identity is not authenticated.");
...
export function formatBlastRadiusReport(report: BlastRadiusReport): string {
  ...
  lines.push("  Rules:");
  for (const rule of report.rules) lines.push(`    ${rule.ruleId}: ${rule.explanation}`);
...
export function formatRoiReport(report: RoiReport): string {
  ...
  if (report.notMeasured.length > 0) lines.push(`  Not measured: ${report.notMeasured.join(", ")}`);
  return lines.join("\n");
}
...
export function formatRemovalAnalysis(report: RemovalAnalysis): string {
  ...
  if (report.lostWithoutGhostApi.length === 0) lines.push("  (no verifiable value currently depends on GhostAPI in this inventory)");
  for (const statement of report.lostWithoutGhostApi) lines.push(`  - ${statement}`);
  return lines.join("\n");
}
...
class ImportedRecords {
  ...
  constructor(payload: InventoryImportPayload) {
    for (const record of payload.agents ?? []) this.agentIds.add(record.agentId);
    for (const record of payload.tools ?? []) this.toolIds.add(record.toolId);
    for (const record of payload.identities ?? []) this.identityIds.add(record.identityId);
    for (const record of payload.providers ?? []) this.providerIds.add(record.providerId);
    for (const record of payload.resources ?? []) this.resourceIds.add(record.resourceId);
    for (const record of payload.sideEffects ?? []) this.sideEffectIds.add(record.sideEffectId);
    for (const record of payload.credentials ?? []) this.credentialIds.add(record.credentialId);
    for (const record of payload.policies ?? []) this.policyIds.add(record.policyId);
  }
}

function validateCrossReferences(state: InventoryStoreState, tenantId: string, imported: ImportedRecords): void {
  const has = (kind: "agent" | "tool" | "identity" | "provider" | "resource" | "side_effect" | "credential" | "policy", id: string): boolean => {
    if (kind === "agent") return imported.agentIds.has(id) || state.agents.some((record) => record.tenantId === tenantId && record.agentId === id);
    if (kind === "tool") return imported.toolIds.has(id) || state.tools.some((record) => record.tenantId === tenantId && record.toolId === id);
    if (kind === "identity") return imported.identityIds.has(id) || state.identities.some((record) => record.tenantId === tenantId && record.identityId === id);
    if (kind === "provider") return imported.providerIds.has(id) || state.providers.some((record) => record.tenantId === tenantId && record.providerId === id);
    if (kind === "resource") return imported.resourceIds.has(id) || state.resources.some((record) => record.tenantId === tenantId && record.resourceId === id);
    if (kind === "side_effect") return imported.sideEffectIds.has(id) || state.sideEffects.some((record) => record.tenantId === tenantId && record.sideEffectId === id);
    if (kind === "credential") return imported.credentialIds.has(id) || state.credentials.some((record) => record.tenantId === tenantId && record.credentialId === id);
  ...
  for (const agent of importedAgents(state, tenantId)) {
    for (const identityId of agent.identityIds) if (!has("identity", identityId)) throw new InventoryError(`Agent ${agent.agentId} references unknown identity ${identityId}.`);
  }
  for (const identity of importedIdentities(state, tenantId)) {
    for (const toolId of identity.toolIds) if (!has("tool", toolId)) throw new InventoryError(`Identity ${identity.identityId} references unknown tool ${toolId}.`);
  }
  for (const tool of importedTools(state, tenantId)) {
    if (tool.providerId !== null && !has("provider", tool.providerId)) throw new InventoryError(`Tool ${tool.toolId} references unknown provider ${tool.providerId}.`);
    for (const credentialId of tool.credentialIds) if (!has("credential", credentialId)) throw new InventoryError(`Tool ${tool.toolId} references unknown credential ${credentialId}.`);
  }
  for (const provider of importedProviders(state, tenantId)) {
    for (const resourceId of provider.resourceIds) if (!has("resource", resourceId)) throw new InventoryError(`Provider ${provider.providerId} references unknown resource ${resourceId}.`);
  }
  for (const resource of importedResources(state, tenantId)) {
    for (const sideEffectId of resource.sideEffectIds) if (!has("side_effect", sideEffectId)) throw new InventoryError(`Resource ${resource.resourceId} references unknown side effect ${sideEffectId}.`);
  }
  for (const sideEffect of importedSideEffects(state, tenantId)) {
    if (!has("resource", sideEffect.resourceId)) throw new InventoryError(`Side effect ${sideEffect.sideEffectId} references unknown resource ${sideEffect.resourceId}.`);
  }
  for (const credential of importedCredentials(state, tenantId)) {
    if (!has("provider", credential.providerId)) throw new InventoryError(`Credential ${credential.credentialId} references unknown provider ${credential.providerId}.`);
    for (const toolId of credential.toolIds) if (!has("tool", toolId)) throw new InventoryError(`Credential ${credential.credentialId} references unknown tool ${toolId}.`);
  }
}

function importedAgents(state: InventoryStoreState, tenantId: string): { agentId: string; identityIds: string[] }[] {
...
function importedIdentities(state: InventoryStoreState, tenantId: string): { identityId: string; toolIds: string[] }[] {
...
function importedTools(state: InventoryStoreState, tenantId: string): { toolId: string; providerId: string | null; credentialIds: string[] }[] {
...
function importedProviders(state: InventoryStoreState, tenantId: string): { providerId: string; resourceIds: string[] }[] {
...
function importedResources(state: InventoryStoreState, tenantId: string): { resourceId: string; sideEffectIds: string[] }[] {
...
function importedSideEffects(state: InventoryStoreState, tenantId: string): { sideEffectId: string; resourceId: string }[] {
...
function importedCredentials(state: InventoryStoreState, tenantId: string): { credentialId: string; providerId: string; toolIds: string[] }[] {
...
function upsertAgent(state: InventoryStoreState, tenantId: string, input: ImportedAgent, payload: InventoryImportPayload, importedBy: string, now: string): void {
  const record = {
    schemaVersion: 1,
...
function upsertTool(state: InventoryStoreState, tenantId: string, input: ImportedTool, payload: InventoryImportPayload, importedBy: string, now: string): void {
  const record = {
    schemaVersion: 1,
    tenantId,
    toolId: input.toolId,
...
function upsertIdentity(state: InventoryStoreState, tenantId: string, input: ImportedIdentity, payload: InventoryImportPayload, importedBy: string, now: string): void {
  const record = {
    schemaVersion: 1,
...
function upsertProvider(state: InventoryStoreState, tenantId: string, input: ImportedProvider, payload: InventoryImportPayload, importedBy: string, now: string): void {
  const record = {
    schemaVersion: 1,
...
function upsertResource(state: InventoryStoreState, tenantId: string, input: ImportedResource, payload: InventoryImportPayload, importedBy: string, now: string): void {
  const record = {
    schemaVersion: 1,
...
function upsertSideEffect(state: InventoryStoreState, tenantId: string, input: ImportedSideEffect, payload: InventoryImportPayload, importedBy: string, now: string): void {
  const record = {
    schemaVersion: 1,
    tenantId,
...
function upsertCredential(state: InventoryStoreState, tenantId: string, input: ImportedCredential, payload: InventoryImportPayload, importedBy: string, now: string): void {
  const record = {
    schemaVersion: 1,
...
function upsertPolicy(state: InventoryStoreState, tenantId: string, input: ImportedPolicy, payload: InventoryImportPayload, importedBy: string, now: string): void {
  const record = {
    schemaVersion: 1,
...
type InventoryRecordKind = "agent" | "tool" | "identity" | "provider" | "resource" | "side_effect" | "credential" | "policy";

function merge(state: InventoryStoreState, kind: InventoryRecordKind, id: string, tenantId: string, record: Record<string, unknown>, source: InventoryImportPayload["source"], importedBy: string, now: string): void {
  ...
  if (index === -1) {
    if (list.length >= recordLimit(kind)) throw new InventoryError(`${kind} record limit was reached.`);
    list.push(stored);
  } else {
...
function idForKind(kind: InventoryRecordKind, record: Record<string, unknown>): string {
  switch (kind) {
    case "agent":
...
function recordList(state: InventoryStoreState, kind: InventoryRecordKind): Array<Record<string, unknown>> {
  if (kind === "agent") return state.agents as unknown as Array<Record<string, unknown>>;
  if (kind === "tool") return state.tools as unknown as Array<Record<string, unknown>>;
  if (kind === "identity") return state.identities as unknown as Array<Record<string, unknown>>;
  if (kind === "provider") return state.providers as unknown as Array<Record<string, unknown>>;
  if (kind === "resource") return state.resources as unknown as Array<Record<string, unknown>>;
  if (kind === "side_effect") return state.sideEffects as unknown as Array<Record<string, unknown>>;
  if (kind === "credential") return state.credentials as unknown as Array<Record<string, unknown>>;
...
function upsertEdges(
  ...
  tenantId: string,
  upserted: { kind: "agent" | "tool" | "identity" | "provider" | "resource" | "side_effect" | "credential" | "policy"; id: string }[],
  ...
  const apply = (sourceKind: InventoryStoreState["edges"][number]["sourceKind"], sourceId: string, targetKind: InventoryStoreState["edges"][number]["targetKind"], targetId: string, relation: InventoryStoreState["edges"][number]["relation"]): void => {
  ...
  for (const node of upserted) {
    if (node.kind === "agent") {
      ...
      for (const identityId of agent.identityIds) apply("agent", agent.agentId, "identity", identityId, "owns");
      ...
      for (const toolId of identity.toolIds) apply("identity", identity.identityId, "tool", toolId, "uses");
    } else if (node.kind === "tool") {
      const tool = state.tools.find((record) => record.tenantId === tenantId && record.toolId === node.id);
      if (tool === undefined) continue;
      if (tool.providerId !== null) apply("tool", tool.toolId, "provider", tool.providerId, "connects");
      for (const credentialId of tool.credentialIds) apply("tool", tool.toolId, "credential", credentialId, "requires");
      ...
      for (const resourceId of provider.resourceIds) apply("provider", provider.providerId, "resource", resourceId, "exposes");
      ...
      for (const sideEffectId of resource.sideEffectIds) apply("resource", resource.resourceId, "side_effect", sideEffectId, "permits");
    }
  }
...
function snapshotFor(state: InventoryStoreState, tenantId: string, now: string, freshnessDays: InventoryFreshnessDays | undefined): InventorySnapshot {
  return {
    schemaVersion: 1,
...
function applyRemediationEffect(state: InventoryStoreState, tenantId: string, remediation: InventoryStoreState["remediations"][number], now: string): void {
  const result = remediation.result;
  if (remediation.kind === "assign_owner") {
    if (result === undefined || result.ownerId === undefined) throw new InventoryError("assign_owner remediation requires an owner id.");
    if (remediation.targetKind === "agent") {
      const record = state.agents.find((candidate) => candidate.tenantId === tenantId && candidate.agentId === remediation.targetId);
      if (record === undefined) throw new InventoryError("Remediation target agent does not exist.");
      record.ownerId = result.ownerId;
    } else if (remediation.targetKind === "provider") {
      const record = state.providers.find((candidate) => candidate.tenantId === tenantId && candidate.providerId === remediation.targetId);
      if (record === undefined) throw new InventoryError("Remediation target provider does not exist.");
      record.ownerId = result.ownerId;
    } else if (remediation.targetKind === "resource") {
      const record = state.resources.find((candidate) => candidate.tenantId === tenantId && candidate.resourceId === remediation.targetId);
      if (record === undefined) throw new InventoryError("Remediation target resource does not exist.");
      record.ownerId = result.ownerId;
    } else if (remediation.targetKind === "credential") {
      const record = state.credentials.find((candidate) => candidate.tenantId === tenantId && candidate.credentialId === remediation.targetId);
      if (record === undefined) throw new InventoryError("Remediation target credential does not exist.");
      record.ownerId = result.ownerId;
    } else {
    ...
    if (remediation.targetKind !== "credential") throw new InventoryError("reduce_scope remediation only supports credential targets.");
    if (result === undefined || result.reducedScopes === undefined) throw new InventoryError("reduce_scope remediation requires the reduced scope list.");
    const record = state.credentials.find((candidate) => candidate.tenantId === tenantId && candidate.credentialId === remediation.targetId);
    if (record === undefined) throw new InventoryError("Remediation target credential does not exist.");
    if (record.status !== "active") throw new InventoryError("Cannot reduce scopes on a revoked credential.");
    const current = record.grantScopes;
    const reduced = result.reducedScopes;
    for (const scope of reduced) if (!current.includes(scope)) throw new InventoryError("Reduced scopes must be a subset of the current grant scopes; permissions are never expanded.");
    if (reduced.length >= current.length) throw new InventoryError("reduce_scope must remove at least one scope; permissions are never expanded.");
    record.grantScopes = reduced;
  } else if (remediation.kind === "revoke") {
    if (remediation.targetKind !== "credential") throw new InventoryError("revoke remediation only supports credential targets.");
    const record = state.credentials.find((candidate) => candidate.tenantId === tenantId && candidate.credentialId === remediation.targetId);
    if (record === undefined) throw new InventoryError("Remediation target credential does not exist.");
    record.status = "revoked";
  } else if (remediation.kind === "onboard_through_gateway") {
    if (remediation.targetKind !== "agent") throw new InventoryError("onboard_through_gateway remediation only supports agent targets.");
    const record = state.agents.find((candidate) => candidate.tenantId === tenantId && candidate.agentId === remediation.targetId);
    if (record === undefined) throw new InventoryError("Remediation target agent does not exist.");
    record.gatewayManaged = true;
    record.freshness = { ...record.freshness, lastSeenAt: now };
  } else if (remediation.kind === "create_eval") {
    if (remediation.targetKind !== "agent") throw new InventoryError("create_eval remediation only supports agent targets.");
    if (result === undefined || result.evalScenarioId === undefined) throw new InventoryError("create_eval remediation requires an eval scenario id.");
    if (!state.agents.some((record) => record.tenantId === tenantId && record.agentId === remediation.targetId)) throw new InventoryError("Remediation target agent does not exist.");
  }
}

function remediationResult(proposal: { ownerId?: string; reducedScopes?: string[]; evalScenarioId?: string }, proposedBy: string): NonNullable<InventoryStoreState["remediations"][number]["result"]> {
  return {
...
function validateRemediationProposal(value: unknown): { findingId: string; kind: InventoryStoreState["remediations"][number]["kind"]; targetKind: InventoryStoreState["remediations"][number]["targetKind"]; targetId: string; rationale: string; ownerId?: string; reducedScopes?: string[]; evalScenarioId?: string } {
  ...
  if (!["assign_owner", "reduce_scope", "revoke", "onboard_through_gateway", "create_eval"].includes(kind as string)) throw new InventoryError("Remediation kind is invalid.");
  const targetKind = proposal.targetKind;
  if (!["agent", "identity", "tool", "provider", "resource", "side_effect", "credential", "environment", "policy"].includes(targetKind as string)) throw new InventoryError("Remediation target kind is invalid.");
  const result: { findingId: string; kind: InventoryStoreState["remediations"][number]["kind"]; targetKind: InventoryStoreState["remediations"][number]["targetKind"]; targetId: string; rationale: string; ownerId?: string; reducedScopes?: string[]; evalScenarioId?: string } = {
    ...
    targetKind: targetKind as InventoryStoreState["remediations"][number]["targetKind"],
  ...
  if (proposal.ownerId !== undefined) result.ownerId = safeIdentifier(proposal.ownerId, "Remediation owner id");
  if (proposal.reducedScopes !== undefined) result.reducedScopes = safeScopes(proposal.reducedScopes, "Remediation reduced scopes");
  if (proposal.evalScenarioId !== undefined) result.evalScenarioId = safeIdentifier(proposal.evalScenarioId, "Remediation eval scenario id");
  if (result.kind === "assign_owner" && result.ownerId === undefined) throw new InventoryError("assign_owner remediation requires an owner id.");
  if (result.kind === "reduce_scope" && result.reducedScopes === undefined) throw new InventoryError("reduce_scope remediation requires the reduced scope list.");
  if (result.kind === "create_eval" && result.evalScenarioId === undefined) throw new InventoryError("create_eval remediation requires an eval scenario id.");
  return result;
}

function safeIdentifier(value: unknown, label: string): string {
  if (typeof value !== "string" || !/^[a-z0-9][a-z0-9._-]{0,127}$/.test(value) || sanitizeSecretString(value) !== value) throw new InventoryError(`${label} must be a safe identifier.`);
  return value;
}

function safeScopes(value: unknown, label: string): string[] {
  if (!Array.isArray(value) || value.length > INVENTORY_LIMITS.maxScopesPerRecord) throw new InventoryError(`${label} is invalid.`);
  return value.map((entry) => {
    if (typeof entry !== "string" || !/^[a-z0-9][a-z0-9._:\-]{0,127}$/.test(entry) || sanitizeSecretString(entry) !== entry) throw new InventoryError(`${label} must be safe scope identifiers.`);
    return entry;
  });
}

function safeText(value: unknown, label: string, max: number): string {
  if (typeof value !== "string" || value.trim() === "" || value.length > max || /[\u0000-\u001f]/.test(value) || sanitizeSecretString(value) !== value) throw new InventoryError(`${label} is invalid.`);
  return value.trim();
}

function object(value: unknown, message: string): Record<string, unknown> {
  if (value === null || typeof value !== "object" || Array.isArray(value)) throw new InventoryError(message);
  return value as Record<string, unknown>;
}

function exactKeys(value: Record<string, unknown>, keys: string[], label: string, optional: string[] = []): void {
  for (const key of Object.keys(value)) {
    if (!keys.includes(key) || (value[key] === undefined && !optional.includes(key))) throw new InventoryError(`${label} contains unsupported field: ${key}`);
  }
}
