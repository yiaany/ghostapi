...
export type EvalForbiddenType = "production-egress" | "secret-leak" | "provider-failure" | "missing-scenario";
...
export type EvalSpec = {
  ...
  rubric: { maxScore: number; components: Array<{ id: string; expectationId: string; points: number; reason: string }> };
...
export type EvalReport = {
  schemaVersion: 1;
  artifact: { generatedAt: string; logicalHash: string; canonicalization: "json-stable-sorted-keys-v1" };
  eval: { id: string; title: string; specHash: string };
  evidence: { path?: string; logicalHash: string; runId: string; links: string[] };
  score: { core: number; max: number; passed: boolean; forbiddenTriggered: string[]; components: EvalScoreComponent[] };
  repeatability: { deterministicInputs: true; evidenceLogicalHash: string; evalLogicalHash: string; notes: string[] };
...
export async function loadEvalSpec(path: string, projectRoot = process.cwd()): Promise<EvalSpec> {
  const resolved = await resolveExistingPath(path, projectRoot);
  const source = await readFile(resolved, "utf8");
  if (Buffer.byteLength(source, "utf8") > MAX_SPEC_BYTES) throw new EvalError(`Eval spec exceeds ${MAX_SPEC_BYTES} bytes.`);
  let parsed: unknown;
  try {
...
export function scoreEval(spec: EvalSpec, evidence: EvidenceReport, options: { evidencePath?: string; generatedAt?: string } = {}): EvalReport {
...
function evaluateExpectation(expectation: EvalExpectation, evidence: EvidenceReport): boolean {
  if (expectation.type === "run-finished") return evidence.run.status === "finished";
...
function isForbiddenTriggered(forbidden: EvalForbiddenAction, evidence: EvidenceReport): boolean {
  if (forbidden.type === "production-egress") return evidence.egress.productionAttempts > 0;
  if (forbidden.type === "secret-leak") return evidence.secrets.matches > 0;
  if (forbidden.type === "provider-failure") return evidence.retriesAndFailures.failureCount > 0;
  return typeof forbidden.value === "string" && !evidence.coverage.scenarios.includes(forbidden.value);
}

function failedReason(expectation: EvalExpectation | undefined, evidence: EvidenceReport): string {
  if (expectation === undefined) return "Rubric references an unknown expectation.";
  if (expectation.type === "run-finished") return `Run status was ${evidence.run.status}.`;
...
function normalizeEvalSpec(value: unknown): EvalSpec {
  if (!isPlainObject(value)) throw new EvalError("Eval spec must be an object.");
  ...
  if (spec.schemaVersion !== 1 || spec.kind !== "ghostapi.eval") throw new EvalError("Unsupported eval spec schema.");
  if (!IDENTIFIER.test(spec.id)) throw new EvalError("Eval id must be a stable identifier.");
  if (typeof spec.title !== "string" || spec.title.trim() === "") throw new EvalError("Eval title is required.");
  ...
  if (judgeEnabled(spec.judge)) throw new EvalError("LLM-as-judge cannot be enabled for core eval specs.");
  return spec;
}
...
function normalizeTask(value: EvalSpec["task"]): void {
  ...
  if (typeof value.description !== "string") throw new EvalError("task.description must be a string.");
  if (!Array.isArray(value.command) || value.command.length === 0 || value.command.length > 64 || !value.command.every((entry) => typeof entry === "string" && entry.trim() !== "")) throw new EvalError("task.command must be a non-empty string array.");
}

function normalizeInjectedFailures(values: EvalSpec["injectedFailures"]): void {
  if (!Array.isArray(values) || values.length > MAX_ITEMS) throw new EvalError("injectedFailures must be a bounded array.");
...
function normalizeExpectations(values: EvalExpectation[]): void {
  if (!Array.isArray(values) || values.length === 0 || values.length > MAX_ITEMS) throw new EvalError("expectations must be a non-empty bounded array.");
  const ids = new Set<string>();
  for (const value of values) {
    if (!isPlainObject(value)) throw new EvalError("expectation must be an object.");
...
function normalizeForbidden(values: EvalForbiddenAction[]): void {
  if (!Array.isArray(values) || values.length > MAX_ITEMS) throw new EvalError("forbidden must be a bounded array.");
  for (const value of values) {
    if (!isPlainObject(value)) throw new EvalError("forbidden action must be an object.");
...
function normalizeLimits(value: EvalSpec["limits"]): void {
  ...
  if (!Number.isInteger(value.timeoutMs) || value.timeoutMs < 100 || value.timeoutMs > 300_000) throw new EvalError("limits.timeoutMs must be between 100 and 300000.");
  if (!Number.isInteger(value.maxOutputBytes) || value.maxOutputBytes < 0 || value.maxOutputBytes > 10 * 1024 * 1024) throw new EvalError("limits.maxOutputBytes must be bounded.");
}

function normalizeRubric(value: EvalSpec["rubric"], expectations: EvalExpectation[]): void {
...
async function resolveOutputPath(outPath: string, projectRoot: string): Promise<string> {
  ...
  if (!parent.isDirectory() || parent.isSymbolicLink()) throw new EvalError("Eval output parent must be a real directory, not a symlink.");
  if (!await isRealPathInsideAllowedRoots(dirname(target), projectRoot)) throw new EvalError("Eval output parent resolves outside the project root or GHOSTAPI_DATA_DIR through a symlink.");
  const existing = await lstat(target).catch((error: unknown) => isErrorCode(error, "ENOENT") ? null : Promise.reject(error));
  if (existing?.isSymbolicLink()) throw new EvalError("Eval output path must not be a symlink.");
  return target;
}

async function resolveExistingPath(path: string, projectRoot: string): Promise<string> {
  const target = isAbsolute(path) ? resolve(path) : resolve(projectRoot, path);
  assertPathInsideAllowedRoots(projectRoot, target);
  const info = await lstat(target);
  if (!info.isFile() || info.isSymbolicLink()) throw new EvalError("Eval input path must be a regular non-symlink file.");
  if (!await isRealPathInsideAllowedRoots(target, projectRoot)) throw new EvalError("Eval input resolves outside the project root or GHOSTAPI_DATA_DIR through a symlink.");
  return target;
}
...
async function realProjectRoot(projectRoot: string): Promise<string> {
  ...
  if (!info.isDirectory() || info.isSymbolicLink()) throw new EvalError("Project root must be a real directory, not a symlink.");
  return root;
}

function requiredSpecPath(path: string | undefined): string {
  if (path === undefined) throw new EvalError("Eval requires --spec or --template.");
  return path;
}

function assertPathInsideAllowedRoots(projectRoot: string, target: string): void {
  if (!isInside(projectRoot, target) && !isInside(getDataPaths().root, target)) throw new EvalError("Eval path traversal outside the project root or GHOSTAPI_DATA_DIR is not allowed.");
  if (basename(target).trim() === "") throw new EvalError("Eval path must include a file name.");
}
...
function readIdentifierArray(value: unknown, label: string): void {
  if (!Array.isArray(value) || value.length > MAX_ITEMS || !value.every((entry) => typeof entry === "string" && IDENTIFIER.test(entry))) throw new EvalError(`${label} must be a bounded array of identifiers.`);
}

function assertKeys(value: object, allowed: string[], label: string): void {
  for (const key of Object.keys(value)) {
    if (!allowed.includes(key)) throw new EvalError(`Unknown ${label} field: ${key}`);
  }
}

function judgeEnabled(value: EvalSpec["judge"]): boolean {
  if (value === undefined) return false;
  if (!isPlainObject(value)) throw new EvalError("judge must be an object when present.");
  assertKeys(value, ["llmAsJudge"], "judge");
  const judge = value.llmAsJudge;
  if (judge === false) return false;
  if (!isPlainObject(judge)) throw new EvalError("judge.llmAsJudge must be false or a disabled object.");
