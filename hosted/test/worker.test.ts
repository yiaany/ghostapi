...
test("evaluates only matching latest scenario definitions", () => {
  ...
  assert.equal(
    evaluateScenario(definition, { request: { method: "GET" } }),
    null,
  );
  assert.deepEqual(
    evaluateScenario(definition, {
      request: { method: "POST" },
      response: { status: 201, id: "x" },
    }),
    {
      passed: true,
      assertions: [
        { path: "response.status", passed: true },
        { path: "response.id", passed: true },
      ],
    },
  );
  assert.equal(
...
test("rejects malformed scenario definitions as processing failures", () => {
  assert.throws(
    () =>
      evaluateScenario(
        { assertions: [{ path: "bad path", exists: true }] },
        {},
      ),
...
test("validates bounded scenario definitions at creation", () => {
  assert.doesNotThrow(() =>
    validateScenarioDefinition({
      assertions: [{ path: "response.status", equals: 200 }],
    }),
  );
  assert.throws(
    () =>
      validateScenarioDefinition({
        assertions: [{ path: "response.status", equals: 200, exists: true }],
      }),
    /invalid_scenario_definition/,
  );
  assert.throws(
    () =>
      validateScenarioDefinition({
        assertions: Array.from({ length: 101 }, () => ({
...
test("publishes enough QStash deliveries to reach WORKER_MAX_ATTEMPTS", async () => {
  ...
  const event = {
    ...
    payload: {
      schemaVersion: 1,
      eventId: "5a8ad6ae-d872-4c9d-b3ea-5bfeacb93f17",
      reportId: "8594f47f-9016-490d-ae9d-d6f158c1ffbb",
    },
    attempts: 1,
  };
  const client = {
  ...
  const database = {
    primary: client,
    transaction: async (
      operation: (transactionClient: typeof client) => Promise<unknown>,
    ) => operation(client),
  };
  const queue = {
    client: {
  ...
  await dispatchOutbox(
    database as never,
    queue as never,
    "https://api.example.test/internal/jobs/process-report",
    10,
    5,
  );
