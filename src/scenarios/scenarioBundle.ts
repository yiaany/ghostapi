...
type RawInteraction = {
  request: { method: string; url: string; headers: Record<string, string>; body: unknown; binary: boolean; multipart: boolean };
  response: { status: number; headers: Record<string, string>; body: unknown; binary: boolean; multipart: boolean };
};
...
export class ScenarioReplayer {
  ...
  replay(request: ScenarioReplayRequest): ScenarioReplayResult {
    ...
    this.bindings.clear();
    for (const [name, value] of candidateBindings) this.bindings.set(name, value);
    this.cursor += 1;
    return {
      index: this.cursor,
...
export function prepareScenarioRecording(input: unknown, options: ScenarioRecordingOptions): ScenarioBundle {
  if (serializedBytes(input, "Recording input") > MAX_CAPTURE_BYTES) throw new ScenarioBundleError(`Recording input exceeds ${MAX_CAPTURE_BYTES} bytes.`);
  const allowedHosts = normalizeAllowedSandboxHosts(options.allowedSandboxHosts);
  const rawInteractions = parseCapture(input);
  if (rawInteractions.length === 0) throw new ScenarioBundleError("Recording input does not contain any interactions.");
  if (rawInteractions.length > MAX_INTERACTIONS) throw new ScenarioBundleError(`Recording input exceeds ${MAX_INTERACTIONS} interactions.`);
...
export async function prepareScenarioRecordingFromFile(path: string, options: ScenarioRecordingOptions, projectRoot = process.cwd()): Promise<ScenarioBundle> {
...
export async function writeScenarioBundle(bundle: ScenarioBundle, outPath?: string, projectRoot = process.cwd()): Promise<string> {
  ...
  if (Buffer.byteLength(serialized, "utf8") > MAX_BUNDLE_BYTES) throw new ScenarioBundleError(`Scenario bundle exceeds ${MAX_BUNDLE_BYTES} bytes.`);
  await atomicWriteJson(target, valid);
  return target;
}

export async function loadScenarioBundle(path: string, projectRoot = process.cwd()): Promise<ScenarioBundle> {
  return validateScenarioBundle(await readBoundedJson(path, projectRoot, MAX_BUNDLE_BYTES, "Scenario bundle"));
}

export function createScenarioReplayer(bundle: ScenarioBundle): ScenarioReplayer {
  return new ScenarioReplayer(validateScenarioBundle(bundle));
}

export function formatScenarioSanitizationSummary(summary: ScenarioSanitizationSummary): string {
...
export function validateScenarioBundle(input: unknown): ScenarioBundle {
  ...
  if (root.schemaVersion !== 1) throw new ScenarioBundleError("Unsupported scenario bundle schema version.");
  if (root.kind !== "ghostapi.scenario-bundle") throw new ScenarioBundleError("Scenario bundle kind is invalid.");
  ...
  if (!isTimestamp(metadata.recordedAt)) throw new ScenarioBundleError("Scenario bundle recordedAt must be an ISO timestamp.");
  ...
  if (!Number.isInteger(sanitization.replacements) || (sanitization.replacements as number) < 0 || typeof sanitization.requiresApproval !== "boolean") {
    throw new ScenarioBundleError("Scenario bundle sanitization summary is invalid.");
  }

  if (!Array.isArray(root.variables) || root.variables.length > MAX_VARIABLES) throw new ScenarioBundleError(`Scenario bundle variables must contain at most ${MAX_VARIABLES} entries.`);
  ...
  const variables = root.variables.map((entry) => {
    ...
    if (names.has(variable.name)) throw new ScenarioBundleError(`Scenario bundle variable is duplicated: ${variable.name}`);
  ...
  return {
    schemaVersion: 1,
    kind: "ghostapi.scenario-bundle",
    metadata: { title, recordedAt: metadata.recordedAt as string, sandboxHosts },
    sanitization: { categories: [...new Set(categories)].sort(), replacements: sanitization.replacements as number, requiresApproval: sanitization.requiresApproval as boolean },
    variables,
    interactions
  };
}
...
export function migrateScenarioBundle(input: unknown): unknown {
  ...
  if (source.schemaVersion !== 0) throw new ScenarioBundleError("Unsupported scenario bundle schema version; no migration is registered.");
...
function normalizeBundleInteraction(input: unknown, variableNames: Set<string>): ScenarioBundleInteraction {
...
function normalizeBundleRequest(input: unknown, variableNames: Set<string>): ScenarioBundleInteraction["request"] {
...
function normalizeBundleResponse(input: unknown, variableNames: Set<string>): ScenarioBundleInteraction["response"] {
  ...
  if (!Number.isInteger(response.status) || (response.status as number) < 100 || (response.status as number) > 599) throw new ScenarioBundleError("Scenario bundle response status must be 100-599.");
  ...
  return { status: response.status as number, headers, body: structuredClone(response.body) };
}

function parseCapture(input: unknown): RawInteraction[] {
  const capture = readObject(input, "Recording input must be an object.");
  if (Array.isArray(capture.interactions)) return capture.interactions.map(parseDirectInteraction);
  const log = readObject(capture.log, "Recording input must contain interactions or HAR log.entries.");
  if (!Array.isArray(log.entries)) throw new ScenarioBundleError("HAR recording input must contain log.entries.");
...
function sanitizeHeaders(headers: Record<string, string>, allowed: Set<string>, state: SanitizerState, response: boolean): Record<string, string> {
...
function sanitizeBody(value: unknown, binary: boolean, state: SanitizerState): unknown {
  if (binary) {
...
function sanitizeValue(value: unknown, state: SanitizerState, key?: string, depth = 0): unknown {
  if (depth > MAX_VALUE_DEPTH) throw new ScenarioBundleError(`Recording value exceeds maximum depth of ${MAX_VALUE_DEPTH}.`);
  if (Array.isArray(value)) return value.map((entry) => sanitizeValue(entry, state, key, depth + 1));
  if (value !== null && typeof value === "object") {
    const entries = Object.entries(value as Record<string, unknown>);
    if (entries.length > MAX_VALUE_KEYS) throw new ScenarioBundleError(`Recording object exceeds ${MAX_VALUE_KEYS} keys.`);
  ...
  if (isTimestamp(result)) return variableReference(getVariable(state, result, "timestamp", deterministicTimestamp(state.variables.size + 1)));
  return replaceUnstableIdentifiers(result, state, key);
}

function replaceUnstableIdentifiers(value: string, state: SanitizerState, key: string | undefined): string {
...
function getVariable(state: SanitizerState, raw: string, hint: string, value: string): { name: string; value: string } {
  const existing = state.variables.get(raw);
  if (existing !== undefined) return existing;
  if (state.variables.size >= MAX_VARIABLES) throw new ScenarioBundleError(`Recording exceeds ${MAX_VARIABLES} stateful variables.`);
...
function parseSandboxUrl(value: string, allowedHosts: Set<string>, headers: Record<string, string>): URL {
  let url: URL;
  try {
    url = new URL(value);
  } catch {
  ...
  if (url.protocol !== "https:") throw new ScenarioBundleError("Recording permits HTTPS sandbox traffic only.");
  if (!allowedHosts.has(url.hostname)) throw new ScenarioBundleError(`Recording host is not explicitly allowed as a sandbox host: ${url.hostname}`);
  if (!looksLikeSandboxHost(url.hostname, headers)) throw new ScenarioBundleError(`Recording host does not pass the sandbox safety check: ${url.hostname}`);
  return url;
}

function looksLikeSandboxHost(host: string, headers: Record<string, string>): boolean {
  if (host === "api.stripe.com") return /\b(?:sk|rk)_test_[A-Za-z0-9_-]+/i.test(headers.authorization ?? "");
...
function normalizeAllowedSandboxHosts(hosts: string[]): Set<string> {
  if (hosts.length === 0) throw new ScenarioBundleError("Recording requires at least one explicit --allow-sandbox-host.");
...
function normalizeReplayRequest(input: ScenarioReplayRequest): ScenarioBundleInteraction["request"] {
  return {
...
function normalizeReplayHeaders(headers: ScenarioReplayRequest["headers"]): Record<string, string> {
  if (headers === undefined) return {};
  const result: Record<string, string> = {};
  for (const [name, value] of Object.entries(headers)) {
    const lower = name.toLowerCase();
    if (SAFE_REQUEST_HEADERS.has(lower)) result[lower] = Array.isArray(value) ? value.join(", ") : value;
  }
  return result;
}

function compareRequest(expected: ScenarioBundleInteraction["request"], actual: ScenarioBundleInteraction["request"], bindings: Map<string, string>, variables: ScenarioBundle["variables"]): string[] {
  const mismatches: string[] = [];
  if (expected.method !== actual.method) mismatches.push(`method expected ${expected.method}, received ${actual.method}`);
  if (!matchTemplate(expected.path, actual.path, bindings, variables)) mismatches.push(`path expected ${expected.path}, received ${actual.path}`);
  if (!matchValue(expected.headers, actual.headers, bindings, variables)) mismatches.push("safe request headers differ");
  if (!matchValue(expected.body, actual.body, bindings, variables)) mismatches.push("request body differs");
  return mismatches;
}

function resolveHeaders(headers: Record<string, string>, variables: ScenarioBundle["variables"], bindings: Map<string, string>): Record<string, string> {
  return Object.fromEntries(Object.entries(headers).map(([key, value]) => [key, resolveTemplate(value, variables, bindings)]));
}

function resolveVariables(value: unknown, variables: ScenarioBundle["variables"], bindings: Map<string, string>): unknown {
  if (Array.isArray(value)) return value.map((entry) => resolveVariables(entry, variables, bindings));
  if (value !== null && typeof value === "object") return Object.fromEntries(Object.entries(value as Record<string, unknown>).map(([key, entry]) => [key, resolveVariables(entry, variables, bindings)]));
  return typeof value === "string" ? resolveTemplate(value, variables, bindings) : value;
}

function resolveTemplate(value: string, variables: ScenarioBundle["variables"], bindings: Map<string, string>): string {
...
function matchValue(expected: unknown, actual: unknown, bindings: Map<string, string>, variables: ScenarioBundle["variables"]): boolean {
  if (typeof expected === "string" && typeof actual === "string") return matchTemplate(expected, actual, bindings, variables);
  if (expected === null || actual === null || typeof expected !== "object" || typeof actual !== "object") return expected === actual;
...
function matchTemplate(expected: string, actual: string, bindings: Map<string, string>, variables: ScenarioBundle["variables"]): boolean {
...
function validateHeaders(input: unknown, allowed: Set<string>, variableNames: Set<string>, response: boolean): Record<string, string> {
...
function validateValue(value: unknown, variableNames: Set<string>, depth = 0): void {
  if (depth > MAX_VALUE_DEPTH) throw new ScenarioBundleError(`Scenario bundle value exceeds maximum depth of ${MAX_VALUE_DEPTH}.`);
  if (typeof value === "string") {
    if (value.length > 64 * 1024) throw new ScenarioBundleError("Scenario bundle string exceeds 65536 characters.");
    if (sanitizeSecretString(value) !== value) throw new ScenarioBundleError("Scenario bundle contains a secret-shaped value.");
    validateTemplate(value, variableNames);
    return;
  }
  ...
  if (typeof value === "object") {
    const entries = Object.entries(value as Record<string, unknown>);
    if (entries.length > MAX_VALUE_KEYS) throw new ScenarioBundleError(`Scenario bundle object exceeds ${MAX_VALUE_KEYS} keys.`);
    for (const [key, entry] of entries) {
      if (isSecretFieldName(key)) throw new ScenarioBundleError(`Scenario bundle contains a secret-shaped field: ${key}`);
...
function validateTemplate(value: string, variableNames: Set<string>): void {
  for (const match of value.matchAll(VARIABLE_PATTERN)) {
    if (!variableNames.has(match[1]!)) throw new ScenarioBundleError(`Scenario bundle references an undeclared variable: ${match[1]}`);
  }
  if (value.includes("{{ghostapi.var.")) {
    const stripped = value.replace(VARIABLE_PATTERN, "");
    if (stripped.includes("{{ghostapi.var.")) throw new ScenarioBundleError("Scenario bundle contains an invalid variable reference.");
  }
}

async function readBoundedJson(path: string, projectRoot: string, limit: number, label: string): Promise<unknown> {
  const target = await resolveExistingPath(path, projectRoot, label);
  const source = await readFile(target, "utf8");
  if (Buffer.byteLength(source, "utf8") > limit) throw new ScenarioBundleError(`${label} exceeds ${limit} bytes.`);
  try {
...
async function resolveExistingPath(path: string, projectRoot: string, label: string): Promise<string> {
  ...
  if (!info.isFile() || info.isSymbolicLink()) throw new ScenarioBundleError(`${label} must be a regular non-symlink file.`);
  if (!await isRealPathInsideAllowedRoots(target, projectRoot)) throw new ScenarioBundleError(`${label} resolves outside the project root or GHOSTAPI_DATA_DIR through a symlink.`);
  return target;
}
...
async function resolveOutputPath(path: string, projectRoot: string): Promise<string> {
  ...
  if (!parent.isDirectory() || parent.isSymbolicLink()) throw new ScenarioBundleError("Scenario bundle output parent must be a real directory, not a symlink.");
  if (!await isRealPathInsideAllowedRoots(dirname(target), projectRoot)) throw new ScenarioBundleError("Scenario bundle output parent resolves outside the project root or GHOSTAPI_DATA_DIR through a symlink.");
  const existing = await lstat(target).catch((error: unknown) => isErrorCode(error, "ENOENT") ? null : Promise.reject(error));
  if (existing?.isSymbolicLink()) throw new ScenarioBundleError("Scenario bundle output path must not be a symlink.");
  return target;
}

function assertPathInsideAllowedRoots(projectRoot: string, target: string, label: string): void {
  if (!isInside(projectRoot, target) && !isInside(getDataPaths().root, target)) throw new ScenarioBundleError(`${label} path traversal outside the project root or GHOSTAPI_DATA_DIR is not allowed.`);
  if (basename(target).trim() === "") throw new ScenarioBundleError(`${label} path must include a file name.`);
}
...
function readObject(value: unknown, message: string): Record<string, unknown> {
  if (value === null || typeof value !== "object" || Array.isArray(value)) throw new ScenarioBundleError(message);
  return value as Record<string, unknown>;
}

function assertExactKeys(value: Record<string, unknown>, expected: string[], label: string): void {
  const unknown = Object.keys(value).filter((key) => !expected.includes(key));
  if (unknown.length > 0) throw new ScenarioBundleError(`${label} contains unknown field: ${unknown[0]}`);
}
...
function readHeaderInput(value: unknown): Record<string, string> {
  ...
  if (Array.isArray(value)) {
    return Object.fromEntries(value.map((entry) => {
      ...
      if (typeof header.name !== "string" || typeof header.value !== "string") throw new ScenarioBundleError("Capture header is invalid.");
  ...
  return Object.fromEntries(Object.entries(record).map(([name, headerValue]) => {
    if (typeof headerValue !== "string") throw new ScenarioBundleError("Capture header values must be strings.");
    return [name.toLowerCase(), headerValue];
  }));
}

function normalizeMethod(value: unknown): string {
  if (typeof value !== "string" || !METHODS.has(value.toUpperCase())) throw new ScenarioBundleError("HTTP method is invalid.");
  return value.toUpperCase();
}

function readUrl(value: unknown): string {
  if (typeof value !== "string" || value.length > 8_000) throw new ScenarioBundleError("Recording request URL is invalid.");
  return value;
}

function readStatus(value: unknown): number {
  if (!Number.isInteger(value) || (value as number) < 100 || (value as number) > 599) throw new ScenarioBundleError("Capture response status must be 100-599.");
...
function readHostArray(value: unknown): string[] {
  ...
  if (hosts.length === 0 || hosts.some((host) => !/^[a-z0-9.-]+$/.test(host))) throw new ScenarioBundleError("Scenario bundle sandbox hosts are invalid.");
  return [...new Set(hosts)].sort();
}

function readStringArray(value: unknown, message: string): string[] {
  if (!Array.isArray(value) || value.some((entry) => typeof entry !== "string" || entry.length > 256)) throw new ScenarioBundleError(message);
...
function normalizeTitle(value: unknown): string {
  if (typeof value !== "string" || value.trim() === "" || value.trim().length > 120) throw new ScenarioBundleError("Scenario bundle title must be 1-120 characters.");
  return value.trim();
}
