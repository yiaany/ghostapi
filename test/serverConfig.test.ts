...
describe("server configuration security defaults", () => {
  ...
  it("requires both explicit opt-in and the GhostAPI-specific key", () => {
    const withoutOptIn = loadServerConfig(
      { GHOSTAPI_LLM_API_KEY: "ghost-secret" },
      [],
      {},
    );
    const withOptIn = loadServerConfig(
      {
        GHOSTAPI_ALLOW_EXTERNAL_LLM: "true",
        GHOSTAPI_LLM_API_KEY: "ghost-secret",
      },
      [],
      {},
