...
export function createProviderRuntime(
  overrides: Partial<ProviderRuntimeCapabilities> = {},
): ProviderRuntime {
  ...
  const capabilities: ProviderRuntimeCapabilities = {
    ...
    idGenerator: overrides.idGenerator ?? {
      create: (prefix) =>
        `${prefix}_${clock.now().getTime().toString(36)}${(defaultIdSequence += 1).toString(36).padStart(4, "0")}`,
    },
...
export function prepareProviderPackExecution(
  pack: ProviderPack,
  request: NormalizedRequest,
  runtime: ProviderRuntime = defaultProviderRuntime,
): ProviderPackExecution | { error: ProviderErrorDetails } {
  ...
  if (!pack.manifest.apiVersions.supported.includes(version.version)) {
    throw new Error(
      `Provider pack ${pack.name} selected undeclared API version: ${version.version}`,
    );
  }
...
export function getProviderPackHeaders(
...
export function assertProviderStateTransition(
  pack: ProviderPack,
  transition: ProviderStateTransition,
): void {
  for (const write of [transition, ...(transition.additionalWrites ?? [])]) {
    if (
      !write.key.startsWith(`${pack.name}:`) ||
      write.key.length <= pack.name.length + 1
    ) {
      throw new Error(
        `Provider pack ${pack.name} returned an invalid state transition key.`,
      );
    }
  }
}
