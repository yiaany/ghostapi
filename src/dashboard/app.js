...
async function fetchProviderManifests() {
  try {
    ...
    for (const manifest of manifests) {
      ...
      badge.textContent = manifest.packVersion
        ? `${manifest.packVersion} / ${manifest.apiVersions.default}`
...
window.setFilter = (filter) => {
  ...
  document
    .querySelectorAll(".nav-scroll-area .nav-item")
  ...
  if (filter === "all") {
    ...
    document.querySelector(".breadcrumb-active").textContent =
      activeEl.querySelector(".nav-item-title").textContent + " Routing";
  }
...
window.clearStorage = async (target, event) => {
  try {
    ...
    if (!res.ok)
      throw new Error(payload?.error?.message || `Unable to clear ${target}`);
...
window.toggleFaultLab = async (event) => {
  ...
  try {
    ...
    if (!res.ok)
      throw new Error(payload?.error?.message || "Unable to toggle Chaos Mode");
...
window.generateAiRules = async (event) => {
  ...
  try {
    ...
    const payload = await res.json();
    if (!res.ok)
      throw new Error(payload?.error?.message || "Unable to generate AI rules");
    els.rulesOutput.textContent = payload.content;
    els.rulesSubtitle.textContent =
      payload.detected.length > 0
        ? `Detected: ${payload.detected.join(", ")}`
        : "No known provider SDKs detected";
  } catch (e) {
...
window.copyAgentPrompt = async (event) => {
  ...
  try {
    ...
    const payload = await res.json();
    if (!res.ok)
      throw new Error(payload?.error?.message || "Unable to generate prompt");
...
window.openSafetyReport = async (event) => {
  ...
  els.rulesModal.classList.add("open");
  els.rulesTitle.textContent = "Provider-Risk Heuristic Scan";
  ...
  try {
    ...
    if (!res.ok)
      throw new Error(
        payload?.error?.message || "Unable to generate safety report",
      );
...
window.generateRepoSetup = async (event) => {
  ...
  els.setupOutput.textContent =
    "Scanning package.json and building Cursor/Cline/Claude setup...";
  ...
  try {
    ...
    if (!res.ok)
      throw new Error(
        setupPayload?.error?.message || "Unable to generate setup",
      );
...
function renderSetup(setup) {
  els.setupSubtitle.textContent =
    setup.detected.length > 0
      ? `Detected SDKs: ${setup.detected.join(", ")}`
  ...
  for (const file of setup.files)
    addSetupItem(file.path, file.content, file.description);
  for (const patch of setup.patches)
    addSetupItem(patch.title, patch.content, patch.appliesTo);
...
async function copySetupOutput() {
  const copied = await copyText(
    selectedSetupText || els.setupOutput.textContent,
  );
...
window.openScenarios = async (event) => {
  ...
  try {
    ...
    const payload = await res.json();
    if (!res.ok)
      throw new Error(payload?.error?.message || "Unable to load scenarios");
    scenarios = payload;
    renderScenarios();
  } catch (e) {
...
function renderScenarios() {
  ...
  saveDescription.textContent =
    "Turn the latest dashboard traffic into a reusable custom scenario.";
  ...
  for (const scenario of scenarios) {
    ...
    actions.append(
      scenarioButton("Arm", () => window.armScenario(scenario.id), "Armed"),
      scenarioButton(
        "Export",
        () => window.exportScenario(scenario.id),
        "Copied",
      ),
      scenarioButton(
        "Share",
        () => window.shareScenario(scenario.id),
        "Copied",
      ),
    );
...
window.armScenario = async (id) => {
  const payload = await fetchScenarioAction(
    `/api/scenarios/${encodeURIComponent(id)}/replay`,
    { method: "POST" },
  );
...
window.exportScenario = async (id) => {
  const payload = await fetchScenarioAction(
    `/api/scenarios/${encodeURIComponent(id)}/export`,
  );
  const text = JSON.stringify(payload, null, 2);
  els.scenarioOutput.textContent = text;
  await copyText(text);
...
async function saveTrafficScenario() {
  const payload = await fetchScenarioAction(
    "/api/scenarios/save-from-traffic",
    {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        title: `Saved traffic ${new Date().toLocaleString()}`,
      }),
    },
  );
  const text = JSON.stringify(payload, null, 2);
  ...
  const res = await fetch("/api/scenarios");
  if (!res.ok)
    throw new Error("Saved scenario, but could not refresh the scenario list.");
...
async function fetchScenarioAction(url, options) {
  ...
  const payload = await res.json();
  if (!res.ok)
    throw new Error(payload?.error?.message || "Scenario action failed");
  return payload;
}
...
function renderList() {
  ...
  const list = events.filter((e) => {
    ...
    const matchesFilter =
      ...
      provider.startsWith(`${currentFilter}:`);
...
window.generateSelectedTest = async () => {
  ...
  try {
    ...
    const payload = await res.json();
    if (!res.ok)
      throw new Error(payload?.error?.message || "Unable to generate test");
    els.rulesModal.classList.add("open");
  ...
  const findings =
    report.findings.length > 0
      ? report.findings
          ...
          .join("\n")
      : "- No matches found in the bounded heuristic scan. This is not proof that the repository contains no production endpoints, credentials, or egress paths.";
  return [
    "# GhostAPI Provider-Risk Heuristic Scan",
    "",
    `Project: ${report.projectRoot}`,
    `Detected SDKs: ${report.detected.length > 0 ? report.detected.join(", ") : "none"}`,
    `Scanned files: ${report.limits.scannedFiles}/${report.limits.maxFiles}`,
    `Maximum file size: ${report.limits.maxFileBytes} bytes`,
    `Extensions: ${report.limits.extensions.join(", ")}`,
    "",
    "## Findings",
    findings,
    "",
