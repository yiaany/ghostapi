...
describe("egress capability detection", () => {
  ...
  it("maps Windows to a non-green AppContainer backend", () => {
    const report = detectEgressCapabilities({ platform: "win32", arch: "x64", nodeVersion: "20.20.2", nodeNetworkPermission: false });
  ...
  it("keeps unsupported platforms out of an isolated status", () => {
    const report = detectEgressCapabilities({ platform: "freebsd", arch: "x64", nodeVersion: "26.4.0", nodeNetworkPermission: true });
  ...
  it("keeps the JSON and human output contracts explicit about no isolation", () => {
    const report = detectEgressCapabilities({ platform: "darwin", arch: "arm64", nodeVersion: "26.4.0", nodeNetworkPermission: true });
