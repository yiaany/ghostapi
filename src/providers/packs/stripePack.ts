...
const stripeScenarios: readonly ProviderScenario[] = [
  {
    ...
    provider: "stripe",
    description:
      "A confirmed Payment Intent using pm_card_chargeDeclined returns a Stripe card_error.",
    steps: [
      {
  ...
  },
  {
    ...
    provider: "stripe",
    description:
      "A renewal with pm_card_chargeDeclined leaves the invoice open and subscription past_due.",
    steps: [
      {
...
export const stripePack: ProviderPack = {
  ...
  detection: {
    priority: 400,
    matches: ({ path }) =>
      /^\/v1\/(?:customers|products|prices|subscriptions|invoices|events|payment_intents|payment_methods|checkout\/sessions|refunds)(?:\/|$)/.test(
        path.toLowerCase(),
      ),
  },
  ...
  selectApiVersion(request) {
    const requested =
      firstHeaderValue(request.headers["stripe-version"]) ?? STRIPE_API_VERSION;
  ...
  validate(parsedRequest, request) {
    ...
    if (request.method === "POST" && request.path === "/v1/prices") {
      if (!hasPositiveInteger(parsed.params.unit_amount))
        return invalidParamError(
          "unit_amount",
          "Invalid positive integer: unit_amount.",
        );
      if (!hasCurrency(parsed.params.currency)) return missingParam("currency");
    }
    if (request.method === "POST" && request.path === "/v1/subscriptions") {
      ...
      if (
        ...
        return invalidParamError(
          "trial_period_days",
          "Invalid non-negative integer: trial_period_days.",
        );
    }
    if (
      request.method === "POST" &&
  ...
  handleDeterministic({ request, parsedRequest, runtime }): ProviderResponse {
    ...
    const now = Math.floor(
      runtime.requireCapability("clock").now().getTime() / 1000,
    );
    ...
    const response = dispatchStripeRequest(
      request,
      parsed.params,
      snapshot,
      runtime,
...
function dispatchStripeRequest(
  request: NormalizedRequest,
  params: Record<string, unknown>,
  state: Readonly<Record<string, unknown>>,
  runtime: Parameters<ProviderPack["handleDeterministic"]>[0]["runtime"],
  now: number,
): ProviderResponse {
...
function handleSubscriptions(
  ...
  if (method === "GET" && id === undefined)
    return stripeList(
      "/v1/subscriptions",
      listResources(state, "subscription"),
      query,
    );
  if (method === "POST" && id === undefined)
    return createSubscription(params, state, runtime, now);
  const subscription =
    id === undefined ? null : getResource(state, id, "subscription");
  ...
  if (method === "POST" && action === undefined) {
    if (params.cancel_at_period_end === true)
      return responseWithWebhookWrites(
        { ...subscription, cancel_at_period_end: true, updated: now },
...
function handleInvoices(
  ...
  if (method === "POST" && action === "void")
    return responseWithWebhookWrites(
      { ...invoice, status: "void", paid: false, updated: now },
      ["invoice.voided"],
      runtime,
      now,
    );
...
function createSubscription(
  ...
  if (!isJsonObject(price.recurring))
    return stripeFailure(
      400,
      "invalid_request_error",
      "Subscription prices require recurring configuration.",
      "items[0][price]",
  ...
  if (trialDays === 0) {
    ...
    const response = responseWithWebhookWrites(
      createdSubscription,
      [
      ...
      runtime,
      now,
      { "invoice.paid": invoice, "invoice.payment_succeeded": invoice },
    );
    response.stateWrites = [
      ...(response.stateWrites ?? []),
      {
...
function renewSubscription(
  ...
  const price =
    priceId === undefined ? null : getResource(state, priceId, "price");
  if (price === null)
    return stripeFailure(
      500,
      "api_error",
      "Subscription has no valid recurring price.",
      undefined,
  ...
  const declined =
    ...
    isDeclined({
      payment_method:
        params.payment_method ?? subscription.default_payment_method,
    });
...
function responseWithWebhookWrites(
  resource: Record<string, unknown>,
  eventTypes: string[],
  runtime: Parameters<ProviderPack["handleDeterministic"]>[0]["runtime"],
  now: number,
  eventObjects: Readonly<Record<string, Record<string, unknown>>> = {},
): ProviderResponse {
  return {
    status: 200,
    headers: {},
    body: resource,
    stateWrites: createWebhookWrites(
      eventTypes,
      resource,
      runtime,
      now,
...
function createStripeResponseHeaders(
  runtime: Parameters<ProviderPack["createResponseHeaders"]>[0]["runtime"],
...
function handlePaymentMethods(
  ...
  if (method === "GET" && id === undefined)
    return stripeList(
      "/v1/payment_methods",
      listResources(state, "payment_method"),
      query,
    );
  if (method === "POST" && id === undefined) {
    ...
    if (type !== "card")
      return stripeFailure(
        400,
        "invalid_request_error",
        "Only payment method type 'card' is supported by this GhostAPI Stripe pack.",
        "type",
        "parameter_invalid_enum",
      );
    return stripeObject({
...
function handlePaymentIntents(
  ...
  if (method === "GET" && id === undefined)
    return stripeList(
      "/v1/payment_intents",
      listResources(state, "payment_intent"),
      query,
    );
  if (method === "POST" && id === undefined) {
    ...
    const paymentIntent = createPaymentIntent(
      ...
      runtime,
      now,
      Boolean(params.confirm),
    );
    return params.confirm === true
      ? responseWithWebhookWrites(
          paymentIntent,
  ...
  const paymentIntent =
    id === undefined ? null : getResource(state, id, "payment_intent");
...
function handleCheckoutSessions(
  method: string,
  id: string | undefined,
  action: string | undefined,
  params: Record<string, unknown>,
  ...
  if (method === "GET" && id === undefined)
    return stripeList(
      ...
      listResources(state, "checkout.session"),
      query,
    );
  if (method === "POST" && id === undefined) {
  ...
  const session =
    id === undefined ? null : getResource(state, id, "checkout.session");
      ...
      response.stateWrites = [
        ...(response.stateWrites ?? []),
        {
          ...
          value: {
            ...paymentIntent,
            amount_refunded:
              numberOr(paymentIntent.amount_refunded, 0) + amount,
            status: "succeeded",
          },
        },
      ];
...
function stripeList(
  ...
  const limit = Math.min(
    Math.max(readPositiveInteger(query.limit) ?? 10, 1),
    100,
  );
  const startingAfter =
    typeof query.starting_after === "string" ? query.starting_after : undefined;
  const endingBefore =
    typeof query.ending_before === "string" ? query.ending_before : undefined;
  let start = 0;
...
function stripeError(details: ProviderErrorDetails): {
  error: {
    type: string;
    message: string;
    code?: string | number;
...
function unsupportedEndpoint(request: NormalizedRequest): ProviderResponse {
  return stripeFailure(
    404,
    "invalid_request_error",
    `Unsupported Stripe endpoint: ${request.method} ${request.path}. This GhostAPI Stripe pack supports Customers, Payment Intents, Payment Methods, Checkout Sessions, and Refunds.`,
    undefined,
    "resource_missing",
  );
}

function unsupportedEndpointFor(
  resource: string,
  method: string,
): ProviderResponse {
  return stripeFailure(
    405,
    "invalid_request_error",
    `Unsupported Stripe operation: ${method} /v1/${resource}.`,
    undefined,
    "method_not_allowed",
  );
}
...
function parseRecurring(value: unknown): {
  ...
  if (!isJsonObject(value) || typeof value.interval !== "string")
    return {
      value: null,
      error:
        "Recurring prices require recurring[interval] to be day, week, month, or year.",
    };
...
function isDeclined(params: Record<string, unknown>): boolean {
  return (
    params.payment_method === "pm_card_chargeDeclined" ||
