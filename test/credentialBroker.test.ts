...
import { createCredentialBroker, createTestActionReceiptVerifier, createTestBreakGlassAuthorizer, createTestCredentialExecutor, createTestCredentialVault, createTestWorkloadIdentityProvider } from "../src/credentials/index.js";
...
describe("credential broker and workload identity", () => {
  it("executes server-side without returning or persisting the upstream secret", async () => {
    ...
    const grant = await fixture.broker.issueGrant({ identity: workload, request: request(), expiresAt: "2026-08-14T12:10:00.000Z" });
    const receipt = await fixture.broker.executeServerSide({ identity: workload, grantId: grant.id, request: request() });
    const persisted = await readFile(join(getDataPaths().root, "credential-broker-server-side.json"), "utf8");

    expect(receipt).toMatchObject({ status: "executed", action: action() });
    expect(fixture.executor.executions).toEqual([{ grantId: grant.id, secretLength: secret.byteLength }]);
  ...
  it("rejects expired, revoked, and rotated grants before vault or executor use", async () => {
    ...
    await fixture.broker.registerCredential(credential());
    const workload = fixture.identities.issue(identity());
    const expired = await fixture.broker.issueGrant({ identity: workload, request: request(), expiresAt: "2026-08-14T12:01:00.000Z" });
    now = new Date("2026-08-14T12:02:00.000Z");
    await expect(fixture.broker.executeServerSide({ identity: workload, grantId: expired.id, request: request() })).rejects.toThrow("expired or revoked");

    const revokable = await fixture.broker.issueGrant({ identity: workload, request: request(), expiresAt: "2026-08-14T12:10:00.000Z" });
    await fixture.broker.revokeCredential("tenant-a", "stripe-write");
    await expect(fixture.broker.executeServerSide({ identity: workload, grantId: revokable.id, request: request() })).rejects.toThrow("expired or revoked");
    ...
    const oldGrant = await rotated.broker.issueGrant({ identity: rotatedWorkload, request: request(), expiresAt: "2026-08-14T12:10:00.000Z" });
    await rotated.broker.rotateCredential({ tenantId: "tenant-a", credentialId: "stripe-write", vaultRef: "test-vault/checkout-v2", expiresAt: "2026-08-15T12:00:00.000Z" });
    await expect(rotated.broker.executeServerSide({ identity: rotatedWorkload, grantId: oldGrant.id, request: request() })).rejects.toThrow("expired or revoked");
  ...
  it("enforces tenant, workload, scope, audience, action receipt, and break-glass boundaries", async () => {
    ...
    const crossTenant = fixture.identities.issue({ ...identity(), tenantId: "tenant-b", workloadId: "tenant-b-worker", subjectId: "tenant-b-agent", runId: "run-b" });
    const ci = fixture.identities.issue({ ...identity(), workloadKind: "ci_job", workloadId: "ci-worker", subjectId: "ci-bot", runId: "ci-run" });

    await expect(fixture.broker.issueGrant({ identity: crossTenant, request: request(), expiresAt: "2026-08-14T12:10:00.000Z" })).rejects.toThrow("not found");
    await expect(fixture.broker.issueGrant({ identity: owner, request: { ...request(), scopes: ["test-other"] }, expiresAt: "2026-08-14T12:10:00.000Z" })).rejects.toThrow("scope");
    await expect(fixture.broker.issueGrant({ identity: owner, request: { ...request(), audience: "not-server" as "ghostapi-server" }, expiresAt: "2026-08-14T12:10:00.000Z" })).rejects.toThrow("audience");
    await expect(fixture.broker.issueGrant({ identity: ci, request: request(), expiresAt: "2026-08-14T12:10:00.000Z" })).rejects.toThrow("Break-glass authorization is not configured");

    const breakGlass = createTestBreakGlassAuthorizer({ now: fixture.now });
    const broker = createCredentialBroker({ ...fixture.options, breakGlassAuthorizer: breakGlass.authorizer });
    const approval = breakGlass.issue({ approvalId: "approval-one", action: action(), approvedBy: "reviewer-one", reason: "incident-recovery", issuedAt: "2026-08-14T12:00:00.000Z", expiresAt: "2026-08-14T12:04:00.000Z" });
    const approvedGrant = await broker.issueGrant({ identity: ci, request: request(), expiresAt: "2026-08-14T12:04:00.000Z", breakGlassApproval: approval });
    await expect(broker.executeServerSide({ identity: ci, grantId: approvedGrant.id, request: { ...request(), action: { ...action(), actionReceiptHash: "b".repeat(64) } } })).rejects.toThrow("differs from the granted action");
    await expect(broker.executeServerSide({ identity: ci, grantId: approvedGrant.id, request: request() })).resolves.toMatchObject({ status: "executed" });

    const executorBoundary = brokerFixture("executor-scope");
    executorBoundary.vault.put("test-vault/checkout", testSecretBytes());
    await executorBoundary.broker.registerCredential({ ...credential(), allowedScopes: ["test.execute", "test-extra"] });
    const executorWorkload = executorBoundary.identities.issue(identity());
    const executorGrant = await executorBoundary.broker.issueGrant({ identity: executorWorkload, request: { ...request(), scopes: ["test-extra"] }, expiresAt: "2026-08-14T12:10:00.000Z" });
  ...
  it("records failed server-side use, returns idempotent executed receipts, and detects orphaned workloads", async () => {
    ...
    const workload = fixture.identities.issue(identity());
    const grant = await fixture.broker.issueGrant({ identity: workload, request: request(), expiresAt: "2026-08-14T12:10:00.000Z" });
    const first = await fixture.broker.executeServerSide({ identity: workload, grantId: grant.id, request: request() });
    const replay = await fixture.broker.executeServerSide({ identity: workload, grantId: grant.id, request: request() });
...
function brokerFixture(name: string, now = () => new Date("2026-08-14T12:00:00.000Z")) {
  ...
  const options = { path: join(getDataPaths().root, `credential-broker-${name}.json`), now, vault: vault.vault, executor: executor.executor, workloadVerifier: identities.verifier, actionReceiptVerifier: actionReceipt.verifier };
  return { broker: createCredentialBroker(options), vault, executor, identities, options, now };
}

function credential() {
  return { id: "stripe-write", tenantId: "tenant-a", projectId: "checkout-project", environment: "production", provider: "test-provider", ownerWorkloadId: "checkout-worker", ownerWorkloadKind: "agent_run" as const, vaultRef: "test-vault/checkout", allowedScopes: ["test.execute"], expiresAt: "2026-08-15T12:00:00.000Z" };
}

function identity() {
  return { tenantId: "tenant-a", projectId: "checkout-project", environment: "production", workloadId: "checkout-worker", subjectId: "agent-one", workloadKind: "agent_run" as const, runId: "run-one", issuedAt: "2026-08-14T12:00:00.000Z", expiresAt: "2026-08-14T12:15:00.000Z" };
}

function action() {
  return { actionId: "action-one", actionHash: "a".repeat(64), actionReceiptHash: "c".repeat(64) };
}

function request() {
  return { credentialId: "stripe-write", provider: "test-provider", scopes: ["test.execute"], audience: "ghostapi-server" as const, action: action() };
}
