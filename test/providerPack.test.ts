...
describe("ProviderPack", () => {
  it("runs the same conformance harness with deterministic capabilities", () => {
    expect(
      runProviderPackConformance(resendPack, deterministicRuntime),
    ).toEqual({
      provider: "resend",
      apiVersion: "v1",
      fixtures: 1,
    ...
    expect(
      runProviderPackConformance(stripePack, deterministicRuntime),
    ).toEqual({
      provider: "stripe",
    ...
    expect(
      deterministicRuntime.requireCapability("clock").now().toISOString(),
    ).toBe("2026-08-06T12:00:00.000Z");
  });

  it("rejects unknown runtime capabilities explicitly", () => {
    expect(() =>
      deterministicRuntime.requireCapability("network" as never),
    ).toThrow("Unknown provider capability: network");
  });
  ...
  it("catches malformed provider responses", () => {
    ...
    expect(() =>
      runProviderPackConformance(malformedPack, deterministicRuntime),
    ).toThrow(/response.*id/i);
  });
  ...
  it("catches invalid state transitions", () => {
    ...
    expect(() =>
      runProviderPackConformance(invalidTransitionPack, deterministicRuntime),
    ).toThrow(/state transition.*key/i);
  });
  ...
  it("fails closed when a pack selects an undeclared API version", () => {
    ...
    expect(() =>
      runProviderPackConformance(undeclaredVersionPack, deterministicRuntime),
    ).toThrow("selected undeclared API version: v2");
  });
  ...
  it("publishes a versioned capability manifest and keeps legacy providers available", () => {
    ...
    expect(getProviderManifests()).toEqual(
  ...
  it("keeps pack modules behind the no-ambient-secret and no-network boundary", async () => {
    ...
    const files = (await readdir(packsDir)).filter((file) =>
      file.endsWith(".ts"),
    );
    ...
    for (const file of files) {
      ...
      expect(source, file).not.toMatch(
        /node:(?:fs|http|https|net|tls|dns)|process\.env|globalThis\.fetch|\bfetch\s*\(/,
      );
    }
  });
});
