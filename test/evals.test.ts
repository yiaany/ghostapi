...
describe("agent evals", () => {
  it("scores deterministic evidence with a stable logical hash", () => {
    ...
    const first = scoreEval(builtinEvalTemplates["retry-after"], evidence, {
      generatedAt: "2026-08-07T00:00:00.000Z",
      evidencePath: ".ghostapi/reports/evidence.json",
    });
    const second = scoreEval(builtinEvalTemplates["retry-after"], evidence, {
      generatedAt: "2026-08-08T00:00:00.000Z",
      evidencePath: ".ghostapi/reports/evidence.json",
    });

    expect(first.score.core).toBe(80);
    expect(first.score.passed).toBe(false);
    expect(first.artifact.logicalHash).toBe(second.artifact.logicalHash);
    expect(first.repeatability.evidenceLogicalHash).toBe(
      evidence.artifact.logicalHash,
    );
  });
  ...
  it("gives forbidden actions priority over cosmetic success", () => {
    const evidence = {
      ...evidenceFixture(),
      egress: { ...evidenceFixture().egress, productionAttempts: 1 },
    };
    const report = scoreEval(
      builtinEvalTemplates["no-production-bypass"],
      evidence,
      { generatedAt: "2026-08-07T00:00:00.000Z" },
    );
  ...
  it("does not credit zero production egress without a completed enforced run", () => {
    ...
    expect(report.score.core).toBe(0);
    expect(report.score.components).toContainEqual(
      expect.objectContaining({ id: "no-production-egress", passed: false }),
    );
  });
  ...
  it("does not include raw secrets in eval reports", () => {
      ...
      events: [
        {
          ...
          request: {
            ...
            body: { scenario: "security.no_secret_logs" },
          },
        },
      ],
    ...
    const report = scoreEval(builtinEvalTemplates["no-secret-logs"], evidence, {
      generatedAt: "2026-08-07T00:00:00.000Z",
    });
  ...
  it("loads strict data-only JSON specs and rejects unknown executable fields", async () => {
    ...
    await expect(loadEvalSpec(specPath)).resolves.toMatchObject({
      id: "custom.retry",
    });
    ...
    await writeFile(
      ...
      JSON.stringify({
        ...customSpec(),
        hooks: { afterRun: "curl https://example.com" },
      }),
      "utf8",
    );
    await expect(loadEvalSpec(maliciousPath)).rejects.toThrow(
      "Unknown eval spec field",
    );
    ...
    await writeFile(
      ...
      JSON.stringify({
        ...customSpec(),
        judge: { llmAsJudge: { enabled: true } },
      }),
      "utf8",
    );
    await expect(loadEvalSpec(judgePath)).rejects.toThrow(
      "LLM-as-judge cannot be enabled",
    );

    const duplicateRubricPath = join(
      getDataPaths().root,
      "duplicate-rubric.eval.json",
    );
    await writeFile(
      duplicateRubricPath,
      JSON.stringify({
          ...
          components: [
            {
              ...
              expectationId: "retry",
              points: 50,
              reason: "Retry observed.",
            },
            {
              id: "retry-again",
              expectationId: "retry",
              points: 50,
              reason: "Retry observed again.",
            },
          ],
        },
      }),
    ...
    await expect(loadEvalSpec(duplicateRubricPath)).rejects.toThrow(
    ...
    await writeFile(
      ...
      JSON.stringify({
        ...customSpec(),
        syntheticWorld: {
          ...customSpec().syntheticWorld,
          world: {
            id: "billing-world",
            version: "1.0.0",
            seed: "sk_live_not_allowed",
          },
        },
      }),
      "utf8",
    );
    await expect(loadEvalSpec(unsafeWorldPath)).rejects.toThrow(
      "world must reference a safe world id",
    );
  });
  ...
  it("rejects corrupt supplied evidence instead of silently replacing it", async () => {
    ...
    const parsed = JSON.parse(await readFile(evidencePath, "utf8")) as Record<
      string,
      unknown
    >;
    ...
    await expect(
      runEval({ template: "no-production-bypass", evidencePath }),
    ).rejects.toThrow("logical hash");
  });

  it("fails closed before running a spec whose declared failures cannot be applied", async () => {
    await expect(runEval({ template: "retry-after" })).rejects.toThrow(
      "injectedFailures require --evidence",
    );
  });
  ...
  it("rejects unknown templates from the public API", async () => {
    await expect(
      runEval({
        ...
        evidencePath: join(getDataPaths().reports, "eval-evidence.json"),
      }),
    ).rejects.toThrow("Unknown eval template");
  });
...
function customSpec(): EvalSpec {
  return {
    ...
    syntheticWorld: {
      world: { id: "billing-world", version: "1.0.0", seed: "fixed-seed" },
      providers: ["stripe"],
