...
const ranks: Record<MemberRole, number> = { viewer: 0, developer: 1, admin: 2, owner: 3 };
...
export async function withAuthorizedMutation<T>(
  ...
  operation: (authorization: { organizationId: string; role: MemberRole }) => Promise<T>
): Promise<T | null> {
...
export async function findProjectRole(database: Pool, projectId: string, userId: string): Promise<MemberRole | null> {
...
export async function findOrganizationRole(database: Pool, organizationId: string, userId: string): Promise<MemberRole | null> {
  const result = await database.query<{ role: MemberRole }>(
    "select role from app.organization_memberships where organization_id = $1 and user_id = $2",
    [organizationId, userId]
  );
