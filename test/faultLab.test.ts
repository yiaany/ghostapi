...
import {
  ...
  updateFaultLabConfig,
} from "../src/fault/faultLab.js";
...
describe("Fault Lab", () => {
  ...
  it("keeps a safe disabled default", async () => {
    await expect(getFaultLabConfig()).resolves.toMatchObject({
      enabled: false,
      latencyMinMs: 2000,
      latencyMaxMs: 5000,
      errorRate: 15,
  ...
  it("returns provider-formatted 429 faults when Chaos Mode selects rate limiting", async () => {
    ...
    const decision = await decideFault(
      "stripe",
  ...
  it("injects 2-5 second latency when Chaos Mode selects delay", async () => {
    ...
    await expect(
      decideFault("generic", () => randomValues.shift() ?? 0),
    ).resolves.toEqual({ type: "delay", latencyMs: 3500 });
  });

  it("does nothing for the other 85 percent of traffic", async () => {
    await updateFaultLabConfig({ enabled: true, errorRate: 15 });
    await expect(decideFault("generic", () => 0.2)).resolves.toEqual({
      type: "off",
    });
  });

  it("validates config boundaries", async () => {
    await expect(updateFaultLabConfig({ errorRate: 101 })).rejects.toThrow(
      "Expected integer between 0 and 100",
    );
    await expect(updateFaultLabConfig({ latencyMs: -1 })).rejects.toThrow(
      "Expected integer between 0 and 10000",
    );
    await expect(
      updateFaultLabConfig({ latencyMinMs: 5000, latencyMaxMs: 2000 }),
    ).rejects.toThrow("latencyMaxMs");
    await expect(updateFaultLabConfig({ statusCode: 418 })).rejects.toThrow(
