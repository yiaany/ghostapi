...
const remoteConfig = { host: "0.0.0.0", port: 8080, model: "gpt-4o-mini", authToken: token } satisfies ServerConfig;
...
describe("dashboard access control", () => {
  it("fails closed when a remote bind has no strong token", async () => {
    await expect(createServer({ host: "0.0.0.0", port: 8080, model: "gpt-4o-mini" })).rejects.toThrow("requires GHOSTAPI_AUTH_TOKEN");
    await expect(createServer({ host: "0.0.0.0", port: 8080, model: "gpt-4o-mini", authToken: "short" })).rejects.toThrow("at least 24 characters");
  });

  it("requires a token for dashboard APIs and SSE on remote binds", async () => {
    await withServer(remoteConfig, async (baseUrl) => {
      for (const path of ["/dashboard", "/dashboard/app.js", "/api/events", "/events", "/API/events/"]) {
        const response = await fetch(`${baseUrl}${path}`, { redirect: "manual" });
        expect(response.status, path).toBe(401);
      }

      const authorized = await fetch(`${baseUrl}/api/events`, { headers: { authorization: `Bearer ${token}` } });
  ...
  it("establishes an HttpOnly cookie only through the dashboard query-token bootstrap", async () => {
    await withServer(remoteConfig, async (baseUrl) => {
      const bootstrap = await fetch(`${baseUrl}/dashboard?token=${encodeURIComponent(token)}`, { redirect: "manual" });
      ...
      const dashboard = await fetch(`${baseUrl}/dashboard`, { headers: { cookie: cookie!.split(";")[0]! } });
      expect(dashboard.status).toBe(200);

      const queryTokenOnApi = await fetch(`${baseUrl}/api/events?token=${encodeURIComponent(token)}`);
  ...
  it("rejects DNS-rebinding style origins on loopback", async () => {
    await withServer({ host: "127.0.0.1", port: 8080, model: "gpt-4o-mini" }, async (baseUrl) => {
