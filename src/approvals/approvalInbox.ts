...
export interface ApprovalApproverVerifier {
  authenticate(identity: unknown): Promise<ApprovalApprover>;
}
...
export type ApprovalDisplay = {
  ...
  normalizedArgumentDiff: {
    ...
    after: ActionEnvelope["arguments"];
  };
...
export type ApprovalInboxOptions = {
  ...
  testing?: {
    onExecutionCheckpoint?: (
      checkpoint: "inbox-executing" | "gateway-submitted" | "gateway-verified",
    ) => void | Promise<void>;
  };
};
...
export function createLocalApprovalInbox(
  options: ApprovalInboxOptions,
): LocalApprovalInbox {
  return new LocalApprovalInbox(options);
}

export function createApprovalInboxExecutionFlow(
  options: ApprovalInboxOptions,
): LocalApprovalInbox {
  return new LocalApprovalInbox(options);
}
...
export class LocalApprovalInbox {
  ...
  private readonly onExecutionCheckpoint?: NonNullable<
    ApprovalInboxOptions["testing"]
  >["onExecutionCheckpoint"];
  ...
  constructor(options: ApprovalInboxOptions) {
    ...
    const inboxVerifier: ActionApprovalVerifier = {
      verify: async (action, approvalValue, context) => {
        const verified = await options.approvalVerifier.verify(
          action,
          approvalValue,
          context,
        );
        ...
        const request = state.requests.find(
          (candidate) => candidate.id === approval.approvalId,
        );
        const active =
  ...
  async request(
    ...
    if (Date.parse(action.expiresAt) <= Date.parse(now))
      throw new ApprovalInboxError("Action has expired.");
    ...
    return this.mutate((state, timestamp) => {
      ...
      const existing = state.requests.find(
        (request) =>
          request.actionHash === actionDigest &&
      ...
      if (!decision.allowed)
        throw new ApprovalInboxError(
          `Approval policy denies this action: ${decision.reasons.join("; ")}`,
        );
      const expiresAt = new Date(
        Math.min(
          Date.parse(action.expiresAt),
          Date.parse(timestamp) + policy.approvalTtlMs,
        ),
      ).toISOString();
      const request: ApprovalRequest = {
        id: identifier(
          `approval-${randomUUID().replace(/-/g, "")}`,
          "Approval request id",
        ),
        action,
        ...
        escalationAt: new Date(
          Date.parse(timestamp) + policy.escalationTimeoutMs,
        ).toISOString(),
        decisions: [],
      };
  ...
  async approve(
    ...
    return this.mutate(async (state, now) => {
      ...
      if (
        ...
        throw new ApprovalInboxError(
          "An independent approver can approve this action only once.",
        );
      const decision: ApprovalDecision = {
        id: identifier(
          `decision-${randomUUID().replace(/-/g, "")}`,
      ...
      if (
        request.decisions.filter((entry) => entry.kind === "approve").length >=
        request.policy.requiredApprovals
      ) {
        request.status = "approved";
        request.artifact = await createArtifact(
          request,
          now,
          this.approvalSigner,
        );
  ...
  async editAndResubmit(
    ...
    const old = await this.mutate((state, now) => {
      ...
      if (
        ...
        throw new ApprovalInboxError(
          "Executed or executing approvals cannot be edited.",
        );
      request.status = "superseded";
    ...
    if (next.actionHash === old.actionHash)
      throw new ApprovalInboxError(
        "Edit-and-resubmit requires changed normalized action arguments.",
      );
  ...
  async execute(
    ...
    actionPolicy: ActionPolicyCheck,
    policyValue: unknown,
    ...
    return withFileLock(this.path, async () => {
      ...
      if (
        ...
        throw new ApprovalInboxError(
          "Current approval policy no longer authorizes execution.",
        );
      if (
        identity.actorId !== request.action.actor.id ||
      ...
      if (request.status === "executed") {
        const receipt = await this.actionGateway.execute(
          request.action,
          identity,
          actionPolicy,
        );
        if (
          ...
          throw new ApprovalInboxError(
            "Executed approval does not match its durable action receipt.",
          );
        return receipt;
      }
      if (request.status === "approved") {
        if (request.consumedAt !== undefined)
          throw new ApprovalInboxError(
            "Approval consumption state is invalid.",
          );
      ...
      try {
        ...
        await this.actionGateway.submit(
          request.action,
          request.artifact,
          actionPolicy,
        );
        ...
        const failedReceipt = await this.latestDurableFailedReceipt(
  ...
  async readStateForTesting(): Promise<ApprovalInboxState> {
    return this.read();
  }
  ...
  private async decideTerminal(
    ...
    return this.mutate((state, now) => {
      ...
      if (
        ...
        throw new ApprovalInboxError(
          "Execution has already started; approval cannot be revoked.",
        );
      ...
      appendAudit(
        state,
        kind === "reject" ? "request.rejected" : "request.revoked",
        request,
        approver.id,
        now,
      );
  ...
  private async read(): Promise<ApprovalInboxState> {
    ...
    if (!info.isFile() || info.isSymbolicLink())
      throw new ApprovalInboxError(
        "Approval inbox store must be a regular non-symlink file.",
      );
    const source = await readFile(this.path, "utf8");
    if (Buffer.byteLength(source, "utf8") > MAX_STORE_BYTES)
      throw new ApprovalInboxError(
        "Approval inbox store exceeds its size limit.",
      );
    try {
      return validateState(JSON.parse(source));
    } catch (error) {
      if (error instanceof ApprovalInboxError) throw error;
      throw new ApprovalInboxError("Approval inbox store is not valid JSON.");
    }
  }

  private async mutate<T>(
    operation: (state: ApprovalInboxState, now: string) => T | Promise<T>,
  ): Promise<T> {
    return withFileLock(this.path, async () => {
      const state = await this.read();
  ...
  private async write(state: ApprovalInboxState): Promise<void> {
    await atomicWriteJson(this.path, validateState(state));
  }
  private async latestDurableFailedReceipt(
    actionId: string,
  ): Promise<ActionExecutionReceipt | undefined> {
    try {
...
export function createTestApprovalApproverVerifier(): {
  verifier: ApprovalApproverVerifier;
  issue(
    input: Omit<ApprovalApprover, "principalId"> & { principalId?: string },
  ): ApprovalApprover;
} {
  const issued = new WeakSet<object>();
  return {
    verifier: {
      async authenticate(identity: unknown): Promise<ApprovalApprover> {
        if (
          identity === null ||
          ...
          throw new ApprovalInboxError(
            "Approver identity is not authenticated.",
          );
        return validateApprover(identity);
      },
    },
    issue(input): ApprovalApprover {
...
export function approvalPolicyHash(value: unknown): string {
  return policyHash(validatePolicy(value));
}

function emptyState(): ApprovalInboxState {
  return {
    schemaVersion: 1,
    requests: [],
...
function validateState(value: unknown): ApprovalInboxState {
  ...
  exactKeys(
    state,
    ["schemaVersion", "requests", "auditAnchor", "audit"],
    "Approval inbox store",
  );
  if (state.schemaVersion !== 1)
    throw new ApprovalInboxError("Unsupported approval inbox schema version.");
  const requests = array(state.requests, "Approval requests", MAX_REQUESTS).map(
    validateRequest,
  );
  unique(
  ...
  unique(
    requests.map((request) => request.actionHash),
    "Approval action hashes",
  );
  const anchor = hash(state.auditAnchor, "Approval audit anchor");
  const audit = array(state.audit, "Approval audit", MAX_AUDIT).map(
    validateAudit,
  );
...
function validateRequest(value: unknown): ApprovalRequest {
  ...
  const displayValue = displayRecord(request.display, action, context);
  const decisions = array(
    request.decisions,
    "Approval decisions",
    MAX_DECISIONS,
  ).map(validateDecision);
  ...
  const escalationAt = timestamp(
    request.escalationAt,
    "Approval escalation time",
  );
  if (
  ...
  if (
    ...
    throw new ApprovalInboxError(
      "Approved requests require an approval artifact.",
    );
  const consumedAt =
    request.consumedAt === undefined
      ? undefined
  ...
  if (
    ...
    throw new ApprovalInboxError(
      "Executed approvals require consumption metadata.",
    );
  const executionReceiptHash =
    request.executionReceiptHash === undefined
      ? undefined
  ...
  if (request.status === "executed" && executionReceiptHash === undefined)
    throw new ApprovalInboxError(
      "Executed approvals require an action receipt hash.",
    );
  return {
    ...
    ...(request.supersedesRequestId === undefined
      ? {}
      : {
          supersedesRequestId: identifier(
            request.supersedesRequestId,
            "Superseded approval request id",
          ),
        }),
  };
}
function validateAudit(value: unknown): ApprovalAuditRecord {
  ...
  const base = {
    ...
    ...(record.actionReceiptHash === undefined
      ? {}
      : {
          actionReceiptHash: hash(
            record.actionReceiptHash,
            "Approval audit receipt hash",
          ),
        }),
  ...
  if (
    hash(record.recordHash, "Approval audit record hash") !==
...
function validatePolicy(value: unknown): ApprovalPolicy {
  ...
  exactKeys(
    ...
    "Approval policy",
    ["allowedActors", "allowedResources", "maxAmountMinor"],
  );
  ...
  const result: ApprovalPolicy = {
    ...
    escalationTimeoutMs: boundedMs(
      policy.escalationTimeoutMs,
      "Approval escalation timeout",
    ),
  };
  if (result.escalationTimeoutMs > result.approvalTtlMs)
    throw new ApprovalInboxError(
      "Approval escalation timeout cannot exceed approval TTL.",
    );
  return result;
}
function evaluate(
  ...
  now: string,
  currentId?: string,
): { allowed: boolean; requiredApprovals: number; reasons: string[] } {
  ...
  if (
    history.filter(
      (request) =>
        ...
        request.status !== "superseded",
    ).length >= policy.velocity.maxActions
  )
    reasons.push("action velocity limit was reached");
  const requiredApprovals =
    policy.criticalRisks.includes(actionRisk) ||
...
function displayRecord(
  ...
  const generated = display(
    action,
    context,
    array(candidate.policyReason, "Approval policy reason", 16).map((item) =>
      text(item, "Approval policy reason", 200),
    ),
  );
  if (canonical(candidate) !== canonical(generated))
    throw new ApprovalInboxError(
      "Approval display does not match normalized action data.",
    );
  return generated;
}
...
function validateArtifact(
  ...
  const provenance = object(
    artifact.provenance,
    "Approval artifact provenance is invalid.",
  );
  exactKeys(
    provenance,
...
function policyRecord(value: unknown): ApprovalRequest["policy"] {
  const policy = object(value, "Approval policy record is invalid.");
  exactKeys(
    policy,
    ["id", "version", "hash", "requiredApprovals"],
...
function validateContext(value: unknown): ApprovalContext {
...
function validateApprover(value: unknown): ApprovalApprover {
  const approver = object(value, "Approver identity is invalid.");
  exactKeys(
    approver,
    ["id", "principalId", "independenceKey"],
    "Approver identity",
  );
  return {
    ...
    independenceKey: identifier(
      approver.independenceKey,
      "Approver independence key",
    ),
  };
}
function validateDecision(value: unknown): ApprovalDecision {
...
function refresh(state: ApprovalInboxState, now: string): void {
...
function classify(action: ActionEnvelope): ApprovalRisk {
  if (
    action.provider === "ghostapi-synthetic" &&
    action.operation === "synthetic.subscription_failure"
  )
    return "update";
  throw new ApprovalInboxError(
    "No approval risk taxonomy rule exists for this action.",
  );
}
function risk(value: unknown): ApprovalRisk {
  if (
    value !== "read" &&
    value !== "create" &&
    value !== "update" &&
...
function risks(value: unknown): ApprovalRisk[] {
...
function assertIndependent(
  ...
  if (
    approver.id === request.action.actor.id ||
...
function validStatus(value: unknown): value is ApprovalRequestStatus {
  return (
    value === "pending" ||
    value === "approved" ||
    value === "rejected" ||
...
function policyHash(policy: ApprovalPolicy): string {
...
function identifiers(value: unknown, label: string): string[] {
  const result = array(value, label, 100).map((entry) =>
