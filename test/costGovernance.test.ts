...
import { createLocalCostGovernance, createTestCostOperatorAuthorizer } from "../src/reliability/index.js";
...
describe("local cost governance", () => {
  it("attributes costs, evaluates budgets, and raises acknowledgeable alerts", async () => {
    ...
    const operator = issue({ id: "finops", principalId: "finops-one", permissions: ["cost.record", "cost.configure", "cost.inspect", "cost.acknowledge"] });
    const controller = createLocalCostGovernance({ path: costPath("costs.json"), now: () => new Date("2029-01-01T00:00:00.000Z"), operatorAuthorizer: authorizer });
    ...
    await controller.recordCost({ identity: operator, record: { tenantId: "tenant-a", runId: "run-1", actionId: "action-1", attribution, amounts: { monetaryAmountMinor: 1_250, requests: 10, messages: 20, mutations: 5, deletes: 0, tokenCost: 4_000 } } });
    await controller.recordCost({ identity: operator, record: { tenantId: "tenant-a", runId: "run-2", actionId: "action-2", attribution, amounts: { monetaryAmountMinor: 2_750, requests: 30, messages: 40, mutations: 10, deletes: 1, tokenCost: 8_000 } } });

    await expect(controller.recordCost({ identity: {}, record: { tenantId: "tenant-a", runId: "run-3", actionId: "action-3", attribution, amounts: { monetaryAmountMinor: 0, requests: 1, messages: 0, mutations: 0, deletes: 0, tokenCost: 0 } } })).rejects.toThrow("not authenticated");
    ...
    await expect(controller.configureBudget({ identity: operator, budget: { id: "budget.bad", scope: { dimension: "project", value: "checkout-project" }, windowMs: 24 * 60 * 60 * 1000, limits: { requests: 1_000, tokenCost: 99_999 }, alertOnExceed: true } })).resolves.toBeTruthy();

    const report = await controller.report({ identity: operator });
    expect(report.totals).toEqual({ monetaryAmountMinor: 4_000, requests: 40, messages: 60, mutations: 15, deletes: 1, tokenCost: 12_000 });
    expect(report.attribution).toContainEqual({ dimension: "agent", value: "agent-one", totals: report.totals });
    expect(report.attribution).toContainEqual({ dimension: "provider", value: "ghostapi-synthetic", totals: report.totals });
    expect(report.attribution).toContainEqual({ dimension: "workflow", value: "recovery-workflow", totals: report.totals });

    const budget = report.budgets.find((candidate) => candidate.id === "budget.requests")!;
    expect(budget.status).toBe("exceeded");
    expect(budget.exceeded).toEqual([{ key: "requests", limit: 35, total: 40 }]);
    expect(report.forecast).toMatchObject({ method: "linear-extrapolation", approximation: true, sampleDays: 1, disclaimer: expect.stringContaining("not a provider invoice") });

    const alerts = await controller.listAlerts({ identity: operator });
    expect(alerts).toHaveLength(1);
    expect(alerts[0]).toMatchObject({ budgetId: "budget.requests", status: "open", exceeded: [{ key: "requests", limit: 35, total: 40 }] });
    const acknowledged = await controller.acknowledgeAlert({ identity: operator, alertId: alerts[0]!.id });
    expect(acknowledged).toMatchObject({ status: "acknowledged", acknowledgedBy: "finops-one" });
    await expect(controller.acknowledgeAlert({ identity: operator, alertId: alerts[0]!.id })).rejects.toThrow("already acknowledged");
  });
  ...
  it("removes budgets and reports within budgets as within", async () => {
    ...
    const controller = createLocalCostGovernance({ path: costPath("costs-within.json"), now: () => new Date("2029-01-01T00:00:00.000Z"), operatorAuthorizer: authorizer });
    await controller.recordCost({ identity: operator, record: { tenantId: "tenant-a", runId: "run-1", actionId: "action-1", attribution: { agentId: "agent-one", projectId: "checkout-project", provider: "ghostapi-synthetic", actionClass: "subscription_failure", workflowId: "recovery-workflow" }, amounts: { monetaryAmountMinor: 100, requests: 5, messages: 5, mutations: 1, deletes: 0, tokenCost: 500 } } });

    await controller.configureBudget({ identity: operator, budget: { id: "budget.requests", scope: { dimension: "project", value: "checkout-project" }, windowMs: 24 * 60 * 60 * 1000, limits: { requests: 100 }, alertOnExceed: false } });
    const report = await controller.report({ identity: operator });
