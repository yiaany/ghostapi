...
export function loadConfig(env: Record<string, string | undefined> = process.env): HostedConfig {
  ...
  const allowedOrigins = required(env, "HOSTED_ALLOWED_ORIGINS").split(",").map((value) => secureOrigin(value.trim(), "HOSTED_ALLOWED_ORIGINS"));
  ...
  return {
    ...
    ciIngestLimitPerMinute: integer(env.CI_INGEST_LIMIT_PER_MINUTE ?? "60", "CI_INGEST_LIMIT_PER_MINUTE", 1, 10_000),
    organizationCreateLimitPerHour: integer(env.ORGANIZATION_CREATE_LIMIT_PER_HOUR ?? "10", "ORGANIZATION_CREATE_LIMIT_PER_HOUR", 1, 1_000),
    ...
    outboxMaxAttempts: integer(env.OUTBOX_MAX_ATTEMPTS ?? "10", "OUTBOX_MAX_ATTEMPTS", 1, 100),
    workerMaxAttempts: integer(env.WORKER_MAX_ATTEMPTS ?? "5", "WORKER_MAX_ATTEMPTS", 1, 20),
    reportRetentionDays: integer(env.REPORT_RETENTION_DAYS ?? "90", "REPORT_RETENTION_DAYS", 1, 3650),
    auditRetentionDays: integer(env.AUDIT_RETENTION_DAYS ?? "365", "AUDIT_RETENTION_DAYS", 1, 3650)
  };
}
...
function secureOrigin(value: string, name: string): string {
  const url = new URL(value);
  if (url.protocol !== "https:" || url.pathname !== "/" || url.search !== "" || url.hash !== "") throw new Error(`${name} entries must be HTTPS origins.`);
  return url.origin;
}
