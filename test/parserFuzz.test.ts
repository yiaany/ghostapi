...
import { ContractError, importOpenApiContract } from "../src/contracts/index.js";
import { parsePolicyYaml, PolicyValidationError } from "../src/policy/index.js";
import { ScenarioBundleError, validateScenarioBundle } from "../src/scenarios/scenarioBundle.js";
...
function* generatedStrings(count: number, maxLength: number): Generator<string> {
  const random = createRandom(0x51a7e);
  const alphabet = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789:{}[]&*#${\\\"'\n\r\t ";
  for (let index = 0; index < count; index += 1) {
    const length = Math.floor(random() * maxLength);
    let value = "";
    for (let character = 0; character < length; character += 1) value += alphabet[Math.floor(random() * alphabet.length)]!;
    yield value;
  }
}
...
function* generatedValues(count: number): Generator<unknown> {
  ...
  for (let index = 0; index < count; index += 1) yield generatedValue(random, 0);
}
...
function generatedValue(random: () => number, depth: number): unknown {
  ...
  if (kind === 4) return Array.from({ length: Math.floor(random() * 6) }, () => generatedValue(random, depth + 1));
  const record: Record<string, unknown> = {};
  for (let index = 0; index < Math.floor(random() * 6); index += 1) record[`field_${index}_${Math.floor(random() * 10)}`] = generatedValue(random, depth + 1);
  return record;
}
