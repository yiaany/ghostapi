...
describe("detectProvider", () => {
  ...
  it("uses explicit provider header override", () => {
    expect(
      detectProvider("/anything", { "x-ghostapi-provider": "stripe" }),
    ).toBe("stripe");
  });
  ...
  it("detects Stripe", () => {
    ...
    expect(
      detectProvider("/anything", { "stripe-version": "2024-06-20" }),
    ).toBe("stripe");
    expect(
      detectProvider("/anything", { authorization: `Bearer ${stripeTestKey}` }),
    ).toBe("stripe");
  });

  it("detects Twilio", () => {
    expect(detectProvider("/2010-04-01/Accounts/AC123/Messages.json")).toBe(
      "twilio",
    );
  ...
  it("detects GitHub", () => {
    ...
    expect(
      detectProvider("/anything", { "x-github-api-version": "2022-11-28" }),
    ).toBe("github");
  });
  ...
  it("detects Discord", () => {
    ...
    expect(
      detectProvider({ path: "/anything", query: { guild_id: "guild_123" } }),
    ).toBe("discord");
    expect(
      detectProvider({
        path: "/anything",
        body: { nested: { channel_id: "channel_123" } },
      }),
    ).toBe("discord");
  });
  ...
  it("detects OpenAI", () => {
    ...
    expect(
      detectProvider("/anything", { "openai-organization": "org_mock" }),
    ).toBe("openai");
  });
