...
export type SafetyBudgetLimits = Partial<{
  monetaryAmountMinor: number;
  requests: number;
  messages: number;
  mutations: number;
...
export type SafetyBudget = {
  ...
  limits: SafetyBudgetLimits;
};
export type SafetyCircuit = {
  id: string;
  scope: SafetyScope;
  windowMs: number;
  ...
  maxLatencyMs: number;
  maxReconciliationMismatches: number;
  state: "closed" | "open";
  openedAt?: string;
  reason?: string;
};
export type SafetyOperatorPermission =
  "safety.stop" | "safety.reenable" | "safety.configure" | "safety.inspect";
export type SafetyOperator = {
  id: string;
  principalId: string;
  permissions: SafetyOperatorPermission[];
};
export interface SafetyEmergencyAuthorizer {
  authenticate(identity: unknown): Promise<SafetyOperator>;
}
...
export type SafetyDeadLetter = SafetyQueueItem & {
  reason: string;
...
export type SafetyControllerOptions = {
  path?: string;
  now?: () => Date;
  emergencyAuthorizer?: SafetyEmergencyAuthorizer;
  leaseTtlMs?: number;
};
...
export function createLocalSafetyController(
  options: SafetyControllerOptions = {},
...
export class LocalSafetyController {
  ...
  async configureBudget(input: {
    ...
    return this.mutate((state, now) => {
      ...
      if (existing === undefined) {
        if (state.budgets.length >= MAX_CONTROLS)
          throw new SafetyControllerError("Safety budget limit was reached.");
      ...
      appendAudit(
        ...
        "budget.configured",
        budget.id,
        `configured ${scopeKey(budget.scope)}`,
        now,
  ...
  async configureCircuit(input: {
    identity: unknown;
    circuit: unknown;
  }): Promise<SafetyCircuit> {
    const operator = await this.authorize(input.identity, "safety.configure");
    const circuit = validateCircuit(input.circuit);
    return this.mutate((state, now) => {
      const existing = state.circuits.find(
        (candidate) => candidate.id === circuit.id,
      );
      if (existing === undefined) {
        if (state.circuits.length >= MAX_CONTROLS)
          throw new SafetyControllerError("Safety circuit limit was reached.");
      ...
      appendAudit(
        ...
        "circuit.configured",
        circuit.id,
        `configured ${scopeKey(circuit.scope)}`,
        now,
        operator.principalId,
      );
  ...
  async reenable(input: {
    ...
    await this.mutate((state, now) => {
      const control = state.switches.find((candidate) =>
        sameScope(candidate.scope, scope),
      );
      if (control === undefined || !control.enabled)
        throw new SafetyControllerError(
          "Kill switch is not active for this scope.",
        );
  ...
  async admit(actionValue: unknown): Promise<SafetyLease> {
    ...
    const replay = await this.mutate((state, current) => {
      ...
      const sameIdempotency = state.ledger.find(
        (candidate) =>
          candidate.action.idempotencyKey === action.idempotencyKey,
      );
      if (
        sameIdempotency !== undefined &&
        sameIdempotency.action.actionHash !== action.actionHash
      )
        throw new SafetyControllerError(
          "Safety idempotency key collides with a different action hash.",
        );
    ...
    return {
      ...
      assertActive: async () => {
        ...
        await this.mutate((state, current) => {
          ...
          if (this.leaseExpired(entry, current))
            throw new SafetyControllerError("Safety lease has expired.");
          if (entry.status !== "reserved")
            throw new SafetyControllerError(
              "Safety lease is no longer active.",
            );
          assertActionAllowed(state, action, current);
          appendAudit(
            state,
            "action.final_check",
            key,
            "kill switch, budgets, and circuits remain active",
            current,
          );
        });
      },
      commit: async <T>(operation: () => Promise<T>): Promise<T> => {
        if (replay)
          throw new SafetyControllerError(
            "Safety replay lease cannot commit a side effect.",
          );
        await this.mutate((state, current) => {
          const entry = findLedger(state, key);
          if (this.leaseExpired(entry, current))
        ...
        await this.mutate((state, current) => {
          ...
          appendAudit(
            ...
            "action.final_check",
            key,
            "kill switch, budgets, and circuits remained active through commit",
            current,
          );
        });
        return result;
      ...
      complete: async (outcomeValue) => {
        ...
        await this.mutate((state, current) => {
          ...
          appendAudit(
            state,
            outcome.success ? "action.completed" : "action.unknown",
            key,
            outcome.reason ??
              (outcome.success
                ? "action completed"
  ...
  async enqueue(actionValue: unknown): Promise<SafetyQueueItem> {
    ...
    return this.mutate((state, now) => {
      ...
      const existing = state.queue.find(
        (item) => ledgerKey(item.action) === ledgerKey(action),
      );
      if (existing !== undefined) return clone(existing);
      if (state.queue.length >= MAX_QUEUE)
        throw new SafetyControllerError(
          "Safety queue is full; backpressure is active.",
        );
      const item = {
      ...
      appendAudit(
        ...
        "queue.enqueued",
        item.id,
        "action is queued without execution",
        now,
      );
      return clone(item);
    });
  ...
  async dequeue(): Promise<SafetyQueueItem | null> {
    return this.mutate((state, now) => {
      ...
      appendAudit(
        ...
        "queue.dequeued",
        item.id,
        "dequeued for separate re-admission",
        now,
      );
      return clone(item);
    });
  ...
  async runScheduledGameDay(input: {
    stopIdentity: unknown;
    reenableIdentity: unknown;
    action: unknown;
    reason: string;
  }): Promise<{ passed: true; stoppedAt: string; reenabledAt: string }> {
    ...
    if (
      before.switches.some(
        (control) => sameScope(control.scope, globalScope) && control.enabled,
      )
    )
      throw new SafetyControllerError(
        "Game-day test requires an inactive global kill switch.",
      );
    await this.stop({
    ...
    await this.admit(action).then(
      () => {
        throw new SafetyControllerError(
          "Game-day kill switch did not block the action.",
        );
      },
      (error: unknown) => {
        if (
          !(error instanceof SafetyControllerError) ||
    ...
    await this.mutate((state, now) =>
      appendAudit(state, "game_day.passed", ledgerKey(action), reason, now),
    );
    return { passed: true, stoppedAt, reenabledAt };
  }

  async inspect(): Promise<SafetyControllerState> {
    return this.read();
  }
  async readStateForTesting(): Promise<SafetyControllerState> {
    return this.read();
  }

  private async authorize(
    identity: unknown,
    permission: SafetyOperatorPermission,
  ): Promise<SafetyOperator> {
    const operator = validateOperator(
      await this.emergencyAuthorizer.authenticate(identity),
    );
    if (!operator.permissions.includes(permission))
      throw new SafetyControllerError(
        `Emergency operator lacks required permission: ${permission}.`,
      );
    return operator;
  }
  ...
  private async read(): Promise<SafetyControllerState> {
    ...
    if (!info.isFile() || info.isSymbolicLink())
      throw new SafetyControllerError(
        "Safety controller store must be a regular non-symlink file.",
      );
    const source = await readFile(this.path, "utf8");
    if (Buffer.byteLength(source, "utf8") > MAX_STORE_BYTES)
      throw new SafetyControllerError(
        "Safety controller store exceeds its size limit.",
      );
    try {
      return validateState(JSON.parse(source));
    } catch (error) {
      if (error instanceof SafetyControllerError) throw error;
      throw new SafetyControllerError(
        "Safety controller store is not valid JSON.",
      );
    }
  }

  private async mutate<T>(
    operation: (state: SafetyControllerState, now: string) => T,
  ): Promise<T> {
  ...
  private timestamp(): string {
    ...
    if (!(value instanceof Date) || !Number.isFinite(value.getTime()))
      throw new SafetyControllerError("Safety controller clock is invalid.");
    return value.toISOString();
  }

  private leaseExpired(
    entry: SafetyControllerState["ledger"][number],
    now: string,
  ): boolean {
    return (
...
export function createDisabledSafetyEmergencyAuthorizer(): SafetyEmergencyAuthorizer {
  return {
    async authenticate(): Promise<never> {
      throw new SafetyControllerError(
        "Emergency authorization is not configured.",
      );
    },
  };
}
...
export function createTestSafetyEmergencyAuthorizer(): {
  authorizer: SafetyEmergencyAuthorizer;
  issue(input: SafetyOperator): SafetyOperator;
} {
  const issued = new WeakSet<object>();
  return {
    authorizer: {
      async authenticate(identity: unknown): Promise<SafetyOperator> {
        if (
          identity === null ||
          ...
          throw new SafetyControllerError(
            "Emergency operator identity is not authenticated.",
          );
        return validateOperator(identity);
      },
    },
    issue(input): SafetyOperator {
...
function emptyState(): SafetyControllerState {
  return {
    schemaVersion: 1,
    switches: [],
    budgets: [],
...
function validateState(value: unknown): SafetyControllerState {
  ...
  if (state.schemaVersion !== SCHEMA_VERSION)
    throw new SafetyControllerError(
      "Unsupported safety controller schema version.",
    );
  const switches = array(state.switches, "Safety switches", MAX_CONTROLS).map(
    validateSwitch,
  );
  unique(
    switches.map((control) => scopeKey(control.scope)),
    "Safety switch scopes",
  );
  const budgets = array(state.budgets, "Safety budgets", MAX_CONTROLS).map(
    validateBudget,
  );
  unique(
  ...
  const circuits = array(state.circuits, "Safety circuits", MAX_CONTROLS).map(
    validateCircuit,
  );
  unique(
  ...
  const ledger = array(state.ledger, "Safety ledger", MAX_LEDGER).map(
    validateLedger,
  );
  unique(
  ...
  unique(
    ledger.map((entry) => entry.action.idempotencyKey),
    "Safety ledger idempotency keys",
  );
  const queue = array(state.queue, "Safety queue", MAX_QUEUE).map(
    validateQueueItem,
  );
  unique(
  ...
  const deadLetters = array(
    state.deadLetters,
    "Safety dead-letter queue",
    MAX_DEAD_LETTERS,
  ).map(validateDeadLetter);
  unique(
    deadLetters.map((entry) => entry.id),
    "Safety dead-letter ids",
  );
  const auditAnchor = hash(state.auditAnchor, "Safety audit anchor");
  const audit = array(state.audit, "Safety audit", MAX_AUDIT).map(
    validateAudit,
  );
...
function validateSwitch(
  value: unknown,
): SafetyControllerState["switches"][number] {
  const control = object(value, "Safety switch is invalid.");
  exactKeys(
    control,
    ["scope", "enabled", "changedAt", "reason", "changedBy"],
    "Safety switch",
  );
...
function validateBudget(value: unknown): SafetyBudget {
  ...
  exactKeys(
    budget,
    ["id", "scope", "windowMs", "velocityWindowMs", "limits"],
    "Safety budget",
  );
  ...
  if (Object.keys(parsed).length === 0)
    throw new SafetyControllerError(
      "Safety budget requires at least one limit.",
    );
  return {
    ...
    velocityWindowMs: boundedMs(
      budget.velocityWindowMs,
      "Safety velocity window",
    ),
    limits: parsed,
  };
}
function validateCircuit(value: unknown): SafetyCircuit {
  ...
  const result: SafetyCircuit = {
    ...
    minimumSamples: positive(
      circuit.minimumSamples,
      "Safety circuit minimum samples",
      MAX_LEDGER,
    ),
    maxFailureRateBps: bps(
      circuit.maxFailureRateBps,
      "Safety circuit failure rate",
    ),
    maxPolicyViolations: nonNegative(
      circuit.maxPolicyViolations,
      "Safety circuit policy violations",
      MAX_LEDGER,
    ),
    maxLatencyMs: nonNegative(
      circuit.maxLatencyMs,
      "Safety circuit latency",
      24 * 60 * 60 * 1000,
    ),
    maxReconciliationMismatches: nonNegative(
      circuit.maxReconciliationMismatches,
      "Safety circuit reconciliation mismatches",
      MAX_LEDGER,
    ),
    state: circuit.state,
  };
  if (result.state === "open") {
    if (circuit.openedAt === undefined || circuit.reason === undefined)
      throw new SafetyControllerError(
        "Open safety circuit requires audit metadata.",
      );
    result.openedAt = timestamp(circuit.openedAt, "Safety circuit open time");
    result.reason = text(circuit.reason, "Safety circuit reason", 200);
  }
  return result;
}
function validateAction(value: unknown): SafetyAction {
  ...
  return {
    ...
    costs: {
      monetaryAmountMinor: nonNegative(
        costs.monetaryAmountMinor,
        "Safety monetary amount",
        Number.MAX_SAFE_INTEGER,
      ),
      requests: nonNegative(
        costs.requests,
        "Safety request count",
        Number.MAX_SAFE_INTEGER,
      ),
      messages: nonNegative(
        costs.messages,
        "Safety message count",
        Number.MAX_SAFE_INTEGER,
      ),
      mutations: nonNegative(
        costs.mutations,
        "Safety mutation count",
        Number.MAX_SAFE_INTEGER,
      ),
      deletes: nonNegative(
        costs.deletes,
...
function validateScope(value: unknown): SafetyScope {
  const scope = object(value, "Safety scope is invalid.");
  exactKeys(scope, ["kind", "id"], "Safety scope", ["id"]);
  if (!SAFETY_SCOPE_KINDS.includes(scope.kind as SafetyScopeKind))
  ...
  return {
    kind: scope.kind as Exclude<SafetyScopeKind, "global">,
    id: identifier(scope.id, "Safety scope id"),
  };
}
function validateOperator(value: unknown): SafetyOperator {
  ...
  const permissions = array(
    ...
    if (
      permission !== "safety.stop" &&
      permission !== "safety.reenable" &&
      permission !== "safety.configure" &&
      permission !== "safety.inspect"
    )
      throw new SafetyControllerError(
        "Emergency operator permission is invalid.",
      );
    return permission as SafetyOperatorPermission;
  });
  ...
  return {
    ...
    principalId: identifier(
      operator.principalId,
      "Emergency operator principal id",
    ),
    permissions,
  };
}
function validateLedger(
  value: unknown,
): SafetyControllerState["ledger"][number] {
  const entry = object(value, "Safety ledger entry is invalid.");
  exactKeys(
    entry,
    ["key", "action", "status", "admittedAt", "completedAt", "outcome"],
    "Safety ledger entry",
    ["completedAt", "outcome"],
  );
  ...
  const outcome =
    entry.outcome === undefined ? undefined : validateOutcome(entry.outcome);
  if (
    entry.status === "completed" &&
    (completedAt === undefined || outcome === undefined)
  )
    throw new SafetyControllerError(
      "Completed safety ledger entry is invalid.",
    );
  if (
    entry.status === "reserved" &&
    (completedAt !== undefined || outcome !== undefined)
  ...
  if (key !== ledgerKey(action))
    throw new SafetyControllerError(
      "Safety ledger key does not match its action.",
    );
  return {
    key,
    action,
...
function validateOutcome(
  value: unknown,
): NonNullable<SafetyControllerState["ledger"][number]["outcome"]> {
  ...
  exactKeys(
    ...
    "Safety action outcome",
    ["policyViolation", "reconciliationMismatch", "reason"],
  );
  if (
...
function validateQueueItem(value: unknown): SafetyQueueItem {
...
function validateDeadLetter(value: unknown): SafetyDeadLetter {
  const item = object(value, "Safety dead-letter item is invalid.");
  exactKeys(
    item,
    ["id", "action", "queuedAt", "reason", "deadLetteredAt"],
    "Safety dead-letter item",
  );
  return {
    id: identifier(item.id, "Safety queue id"),
    action: validateAction(item.action),
...
function validateAudit(value: unknown): SafetyAuditRecord {
  ...
  if (
    hash(record.recordHash, "Safety audit record hash") !==
...
function validateAuditChain(anchor: string, audit: SafetyAuditRecord[]): void {
  let previous = anchor;
  let sequence = 1;
...
function setSwitch(
  ...
  const existing = state.switches.find((candidate) =>
    sameScope(candidate.scope, scope),
  );
  if (existing === undefined) {
...
function assertActionAllowed(
  ...
  const stopped = state.switches.find(
    (control) => control.enabled && scopeMatches(control.scope, action),
  );
  if (stopped !== undefined)
    throw new SafetyControllerError(
      `Kill switch is active for ${scopeKey(stopped.scope)}.`,
      KILL_SWITCH_ERROR_CODE,
    );
  const circuit = state.circuits.find(
    (candidate) =>
      candidate.state === "open" && scopeMatches(candidate.scope, action),
  );
  if (circuit !== undefined)
    throw new SafetyControllerError(
      `Safety circuit breaker is open: ${circuit.id}.`,
    );
  for (const circuitCandidate of state.circuits)
    if (
      circuitCandidate.state === "closed" &&
      scopeMatches(circuitCandidate.scope, action)
    )
      evaluateCircuit(state, circuitCandidate, now);
  const opened = state.circuits.find(
    (candidate) =>
      candidate.state === "open" && scopeMatches(candidate.scope, action),
  );
  if (opened !== undefined)
    throw new SafetyControllerError(
      `Safety circuit breaker is open: ${opened.id}.`,
    );
}
function assertBudgets(
  ...
  for (const budget of state.budgets) {
    if (!scopeMatches(budget.scope, action)) continue;
    const windowEntries = state.ledger.filter(
      (entry) =>
        scopeMatches(budget.scope, entry.action) &&
        Date.parse(entry.admittedAt) >= Date.parse(now) - budget.windowMs,
    );
    const velocityEntries = state.ledger.filter(
      (entry) =>
        scopeMatches(budget.scope, entry.action) &&
    ...
    const velocity = sumCosts(
      velocityEntries.map((entry) => entry.action.costs),
    ).requests;
    const concurrent = windowEntries.filter(
    ...
    for (const key of [
      ...
      if (
        budget.limits[key] !== undefined &&
        totals[key] + action.costs[key] > budget.limits[key]!
      )
        throw new SafetyControllerError(
          `Safety budget exceeded: ${budget.id} ${key}.`,
        );
    if (
      budget.limits.concurrency !== undefined &&
      concurrent + 1 > budget.limits.concurrency
    )
      throw new SafetyControllerError(
        `Safety budget exceeded: ${budget.id} concurrency.`,
      );
    if (
      budget.limits.velocity !== undefined &&
      velocity + action.costs.requests > budget.limits.velocity
    )
      throw new SafetyControllerError(
        `Safety velocity limit exceeded: ${budget.id}.`,
      );
  }
}
...
function evaluateCircuit(
  ...
  if (entries.length < circuit.minimumSamples) return;
  const failures = entries.filter((entry) => !entry.outcome!.success).length;
  const violations = entries.filter(
    (entry) => entry.outcome!.policyViolation,
  ).length;
  const mismatches = entries.filter(
    (entry) => entry.outcome!.reconciliationMismatch,
  ).length;
  const latencyExceeded = entries.some(
    (entry) => entry.outcome!.latencyMs > circuit.maxLatencyMs,
  );
  const reason =
    rateBps(failures, entries.length) > circuit.maxFailureRateBps
...
function deadLetterMatchingQueued(
  state: SafetyControllerState,
  scope: SafetyScope,
  reason: string,
  now: string,
): void {
  const remaining: SafetyQueueItem[] = [];
  for (const item of state.queue) {
    if (scopeMatches(scope, item.action)) {
      ...
      appendAudit(
        ...
        "queue.dead_lettered",
        item.id,
        "kill switch stopped queued action",
        now,
      );
...
function addDeadLetter(
  ...
  if (state.deadLetters.length >= MAX_DEAD_LETTERS) state.deadLetters.shift();
...
function pruneExpiredLeases(
  ...
  state.ledger = state.ledger.filter(
    (entry) =>
      entry.status === "completed" || Date.parse(entry.admittedAt) >= cutoff,
  );
}
function findLedger(
  state: SafetyControllerState,
...
function scopeMatches(scope: SafetyScope, action: SafetyAction): boolean {
  return scope.kind === "global" || scope.id === scopeValue(scope.kind, action);
}
function scopeValue(
  kind: Exclude<SafetyScopeKind, "global">,
  action: SafetyAction,
): string {
  switch (kind) {
    case "organization":
...
function scopeKey(scope: SafetyScope): string {
  return scope.kind === "global" ? "global" : `${scope.kind}:${scope.id}`;
}
function sameScope(left: SafetyScope, right: SafetyScope): boolean {
  return left.kind === right.kind && left.id === right.id;
}
function ledgerKey(action: SafetyAction): string {
  return `${action.idempotencyKey}:${action.actionHash}`;
}
function sumCosts(values: SafetyAction["costs"][]): SafetyAction["costs"] {
  return values.reduce(
    (total, value) => ({
      monetaryAmountMinor:
        total.monetaryAmountMinor + value.monetaryAmountMinor,
