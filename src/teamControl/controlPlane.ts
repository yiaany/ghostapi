...
export type TeamRole =
  "owner" | "admin" | "developer" | "viewer" | "service_account";
...
export type TeamPermission =
  ...
  | "scenario.publish"
  | "evidence.upload";
export type TeamScopedPermission = Extract<
  TeamPermission,
  | "project.read"
  | "environment.read"
  | "scenario.read"
  ...
  | "evidence.upload"
>;
export type TeamTokenScope = {
  projectId: string;
  environmentId: string;
  permissions: TeamScopedPermission[];
};
...
export type TeamMember = {
  id: string;
  organizationId: string;
  role: Exclude<TeamRole, "service_account">;
  createdAt: string;
};
export type TeamServiceAccount = {
  id: string;
  organizationId: string;
  name: string;
  createdBy: string;
...
export type TeamEnvironment = {
  ...
  projectId: string;
  name: string;
  kind: TeamEnvironmentKind;
  createdAt: string;
};
export type TeamScenarioVersion = {
  organizationId: string;
  projectId: string;
  environmentId: string;
  scenarioId: string;
...
export type TeamEvidence = {
  ...
  scenarios: string[];
  enforcement: Pick<
    EvidenceReport["enforcement"],
    "mode" | "isolated" | "degraded"
  >;
  uploadedBy: string;
  uploadedAt: string;
  expiresAt: string;
...
export type TeamAuditExport = {
  organizationId: string;
  anchor: TeamAuditAnchor;
  records: TeamAuditRecord[];
...
type TeamToken = {
  ...
  issuedAt: string;
  expiresAt: string;
  scope?: TeamTokenScope[];
  revokedAt?: string;
};
export type TeamControlPlaneState = {
  schemaVersion: 3;
  ...
  tokens: TeamToken[];
  scenarios: TeamScenarioVersion[];
  evidence: TeamEvidence[];
  policies: TeamPolicyVersion[];
  auditAnchors: TeamAuditAnchor[];
  audit: TeamAuditRecord[];
};
export type LocalTeamControlPlaneOptions = {
  path?: string;
  now?: () => Date;
  randomTokenBytes?: (size: number) => Buffer;
};

const READ_PERMISSIONS: TeamScopedPermission[] = [
  "project.read",
  "environment.read",
  "scenario.read",
  "evidence.read",
...
const PUBLISH_PERMISSIONS: TeamScopedPermission[] = [
  "scenario.publish",
  "evidence.upload",
];
...
const ADMIN_PERMISSIONS: TeamPermission[] = OWNER_PERMISSIONS.filter(
  (permission) => permission !== "member.manage",
);
const DEVELOPER_PERMISSIONS: TeamPermission[] = [
  ...READ_PERMISSIONS,
  ...PUBLISH_PERMISSIONS,
];
const VIEWER_PERMISSIONS: TeamPermission[] = [...READ_PERMISSIONS];
const SERVICE_ACCOUNT_PERMISSIONS: TeamPermission[] = [
  ...DEVELOPER_PERMISSIONS.filter((permission) => permission !== "policy.read"),
];
export const TEAM_PERMISSION_MATRIX: Readonly<
...
export function createLocalTeamControlPlane(
  options: LocalTeamControlPlaneOptions = {},
...
export class LocalTeamControlPlane {
  ...
  async addMember(
    ...
    input: {
      memberId: string;
      role: Exclude<TeamRole, "owner" | "service_account">;
    },
  ): Promise<TeamMember> {
  ...
  async createServiceAccount(
    actor: TeamActor,
    input: { serviceAccountId: string; name: string },
  ): Promise<TeamServiceAccount> {
    const serviceAccountId = identifier(
  ...
  async disableServiceAccount(
    ...
    await this.mutate((state, now) => {
      requirePermission(state, actor, "service_account.manage");
      const account = state.serviceAccounts.find(
        (candidate) =>
      ...
      if (account.disabledAt === undefined) {
        ...
        for (const token of state.tokens) {
          if (
            token.organizationId === actor.organizationId &&
            token.type === "service" &&
            token.subjectId === id &&
  ...
  async issueServiceToken(
    ...
    return this.mutate((state, now) => {
      ...
      const scope = validateTokenScope(
        input.scope,
        state,
        actor.organizationId,
      );
  ...
  async rotateToken(
    ...
    return this.mutate((state, now) => {
      requirePermission(state, actor, "token.manage");
      const oldToken = state.tokens.find(
        (candidate) =>
          candidate.id === id &&
          candidate.organizationId === actor.organizationId,
      );
      ...
      if (
        oldToken.type === "user" &&
        !canManageMemberToken(
          actorRole(state, actor),
          requireMember(state, {
            organizationId: actor.organizationId,
      ...
      if (new Date(expiresAt).getTime() <= new Date(now).getTime())
        throw new TeamControlPlaneError(
          "Token expiry must be within the next 90 days.",
        );
  ...
  async authenticateToken(token: string): Promise<TeamActor> {
    ...
    const record = state.tokens.find((candidate) =>
      equalHash(candidate.digest, digest),
    );
    if (
      record === undefined ||
    ...
    const actor: TeamActor = Object.freeze({
      organizationId: record.organizationId,
    ...
    return this.mutate((state, now) => {
      ...
      if (
        state.scenarios.some(
          (scenario) =>
            scenario.organizationId === actor.organizationId &&
  ...
  async deleteEvidence(
    actor: TeamActor,
    input: { projectId: string; evidenceId: string },
  ): Promise<void> {
    const projectId = identifier(input.projectId, "Project id");
    const evidenceId = identifier(input.evidenceId, "Evidence id");
    await this.mutate((state, now) => {
      ...
      .filter(
        (project) =>
          ...
          (!isServiceActor(actor) ||
            serviceTokenScopes(state, actor).some(
              (scope) =>
  ...
  async listScenarioVersions(
    actor: TeamActor,
    input: { projectId: string; environmentId: string; scenarioId: string },
  ): Promise<TeamScenarioVersion[]> {
    const projectId = identifier(input.projectId, "Project id");
    const environmentId = identifier(input.environmentId, "Environment id");
    const scenarioId = identifier(input.scenarioId, "Scenario id");
  ...
  async exportAudit(actor: TeamActor): Promise<TeamAuditExport> {
    ...
    const anchor = state.auditAnchors.find(
  ...
  async pruneRetention(
    actor: TeamActor,
  ): Promise<{ evidenceRemoved: number; auditRemoved: number }> {
    return this.mutate((state, now) => {
  ...
  async deleteProject(actor: TeamActor, projectId: string): Promise<void> {
    ...
    await this.mutate((state, now) => {
      ...
      state.projects = state.projects.filter(
        (project) =>
          !(
            project.organizationId === actor.organizationId && project.id === id
          ),
      );
  ...
  private async read(): Promise<TeamControlPlaneState> {
    ...
    if (!info.isFile() || info.isSymbolicLink())
      throw new TeamControlPlaneError(
        "Team control-plane store must be a regular non-symlink file.",
      );
    const source = await readFile(this.path, "utf8");
    if (Buffer.byteLength(source, "utf8") > MAX_STORE_BYTES)
      throw new TeamControlPlaneError(
        `Team control-plane store exceeds ${MAX_STORE_BYTES} bytes.`,
      );
    try {
      ...
      for (const organization of state.organizations) {
        ...
        if (
          ...
          !verifyAuditChain(
            ...
            anchor,
            state.audit.filter(
              (record) => record.organizationId === organization.id,
            ),
          ).valid
        )
          throw new TeamControlPlaneError(
      ...
      throw new TeamControlPlaneError(
        "Team control-plane store is not valid JSON.",
      );
    }
  }

  private async mutate<T>(
    operation: (state: TeamControlPlaneState, now: string) => T,
    autoRetain = true,
  ): Promise<T> {
...
export function verifyAuditExport(value: unknown): {
  ...
  try {
    ...
    exactKeys(
      exportValue,
      ["organizationId", "anchor", "records", "integrity"],
      "Audit export",
    );
    const organizationId = identifier(
      exportValue.organizationId,
      "Audit export organization id",
    );
    const anchor = auditAnchor(exportValue.anchor, new Set([organizationId]));
    const records = array(
      exportValue.records,
      "Audit export records",
      MAX_AUDIT_PER_ORGANIZATION,
    ).map((record) => auditRecord(record, new Set([organizationId]), [], []));
    return verifyAuditChain(organizationId, anchor, records);
  } catch (error) {
    return {
      valid: false,
      reason:
        error instanceof Error ? error.message : "Audit export is invalid.",
    };
  }
}
...
function migrateV2State(value: Record<string, unknown>): TeamControlPlaneState {
  ...
  if (value.schemaVersion !== 2)
    throw new TeamControlPlaneError(
      "Unsupported team control-plane schema version.",
    );
  ...
  const validatedBase = validateState({
    ...base,
    auditAnchors: array(value.organizations, "Organizations", 100).map(
      (raw) => ({
        organizationId: identifier(
          object(raw, "Organization is invalid.").id,
          "Organization id",
        ),
        sequence: 0,
  ...
  const legacyAudit = array(value.audit, "Audit records", 100_000).map((raw) =>
    legacyAuditRecord(
      raw,
      new Set(
        validatedBase.organizations.map((organization) => organization.id),
      ),
      validatedBase.members,
    ),
  );
...
function validateState(value: unknown): TeamControlPlaneState {
  ...
  const organizations = array(state.organizations, "Organizations", 100).map(
    (entry) => {
  ...
  const members = array(state.members, "Members", 1_000).map((entry) =>
    member(entry, organizationIds),
  );
  unique(
    members.map((entry) => scopedKey(entry.organizationId, entry.id)),
  ...
  const serviceAccounts = array(
    ...
    "Service accounts",
    1_000,
  ).map((entry) => serviceAccount(entry, organizationIds, members));
  unique(
    serviceAccounts.map((entry) => scopedKey(entry.organizationId, entry.id)),
    "Organization service account ids",
  );
  const actorIds = actorReferenceIds(members, serviceAccounts);
  const projects = array(state.projects, "Projects", 1_000).map((entry) =>
    project(entry, organizationIds),
  );
  unique(
    projects.map((entry) => scopedKey(entry.organizationId, entry.id)),
    "Organization project ids",
  );
  const projectOwners = new Set(
    projects.map((entry) => scopedKey(entry.organizationId, entry.id)),
  );
  const environments = array(state.environments, "Environments", 2_000).map(
    (entry) => environment(entry, organizationIds, projectOwners),
  );
  unique(
  ...
  const tokens = array(state.tokens, "Tokens", 10_000).map((entry) =>
    token(
      entry,
      organizationIds,
      members,
  ...
  unique(
    tokens.map((entry) => entry.id),
    "Token ids",
  );
  const scenarios = array(state.scenarios, "Scenarios", 10_000).map((entry) =>
    scenario(entry, organizationIds, projectOwners, environmentsById, actorIds),
  );
  unique(
    scenarios.map((entry) =>
      scopedKey(
  ...
  const evidence = array(state.evidence, "Evidence", 10_000).map((entry) =>
    evidenceRecord(
      entry,
      organizationIds,
      projectOwners,
  ...
  unique(
    evidence.map((entry) => scopedKey(entry.organizationId, entry.id)),
    "Organization evidence ids",
  );
  const policies = array(state.policies, "Policies", 10_000).map((entry) =>
    policyVersion(entry, organizationIds, actorIds),
  );
  unique(
    policies.map((entry) =>
      scopedKey(entry.organizationId, String(entry.version)),
    ),
    "Policy versions",
  );
  const auditAnchors = array(state.auditAnchors, "Audit anchors", 100).map(
    (entry) => auditAnchor(entry, organizationIds),
  );
  unique(
    auditAnchors.map((entry) => entry.organizationId),
    "Audit anchor organization ids",
  );
  if (
    auditAnchors.length !== organizations.length ||
    auditAnchors.some((anchor) => !organizationIds.has(anchor.organizationId))
  )
    throw new TeamControlPlaneError(
      "Each organization requires an audit anchor.",
    );
  const auditRecords = array(state.audit, "Audit records", 100_000).map(
    (entry) => auditRecord(entry, organizationIds, members, serviceAccounts),
  );
  for (const organization of organizations) {
    const anchor = auditAnchors.find(
      (candidate) => candidate.organizationId === organization.id,
    );
    if (
      anchor === undefined ||
...
function serviceAccount(
  ...
  exactKeys(
    entry,
    ["id", "organizationId", "name", "createdBy", "createdAt", "disabledAt"],
    "Service account",
    ["disabledAt"],
  );
  const organizationId = identifier(
    entry.organizationId,
    "Service account organization id",
  );
  if (
    !organizations.has(organizationId) ||
    !members.some(
...
function environment(
  ...
  exactKeys(
    entry,
    ["id", "organizationId", "projectId", "name", "kind", "createdAt"],
    "Environment",
  );
  const organizationId = identifier(
    entry.organizationId,
...
function token(
  ...
  if (
    ...
    !members.some(
      (member) =>
        member.organizationId === organizationId && member.id === subjectId,
    )
  )
    throw new TeamControlPlaneError("Token is invalid.");
...
function scenario(
  ...
  organizations: Set<string>,
  projects: Set<string>,
  environments: Map<string, TeamEnvironment>,
  actors: Set<string>,
): TeamScenarioVersion {
  ...
  if (
    ...
    !actors.has(
      scopedKey(
        organizationId,
        actorReference(entry.createdBy, "Scenario creator id"),
      ),
    )
  )
...
function evidenceRecord(
  ...
  const environmentId = identifier(
    entry.environmentId,
    "Evidence environment id",
  );
  const enforcement = object(
    entry.enforcement,
    "Evidence enforcement is invalid.",
  );
  exactKeys(
    enforcement,
    ["mode", "isolated", "degraded"],
  ...
  if (
    ...
    !actors.has(
      scopedKey(
        organizationId,
        actorReference(entry.uploadedBy, "Evidence uploader id"),
      ),
    ) ||
...
function policyVersion(
  ...
  if (
    ...
    !actors.has(
      scopedKey(
        organizationId,
        actorReference(entry.distributedBy, "Policy distributor id"),
      ),
    ) ||
...
function legacyAuditRecord(
  ...
  exactKeys(
    entry,
    ["organizationId", "actorId", "action", "resource", "timestamp"],
    "Audit record",
  );
  const organizationId = identifier(
    entry.organizationId,
  ...
  if (
    ...
    !members.some(
      (member) =>
        member.organizationId === organizationId && member.id === actorIdValue,
    )
  )
    throw new TeamControlPlaneError("Audit record is invalid.");
  return {
...
const MEMBER_TOKEN_MANAGEMENT_MATRIX: Readonly<
  Record<
    Exclude<TeamRole, "service_account">,
    readonly Exclude<TeamRole, "service_account">[]
  >
> = Object.freeze({
...
function canManageMemberToken(
  actor: TeamRole,
  subject: Exclude<TeamRole, "service_account">,
): boolean {
  return (
...
function actorRole(state: TeamControlPlaneState, actor: TeamActor): TeamRole {
  ...
  const member = state.members.find(
    (candidate) =>
      candidate.organizationId === organizationId && candidate.id === memberId,
  );
...
function requireMember(
  state: TeamControlPlaneState,
  actor: { organizationId: string; memberId: string },
): TeamMember {
  const organizationId = identifier(actor.organizationId, "Organization id");
  const memberId = identifier(actor.memberId, "Member id");
  const member = state.members.find(
...
function issueTokenRecord(
...
): { tokenId: string; token: string; expiresAt: string } {
  for (let attempts = 0; attempts < 8; attempts += 1) {
    ...
    if (
      state.tokens.some(
        (candidate) =>
          candidate.id === tokenId || equalHash(candidate.digest, digest),
      )
    )
      continue;
    state.tokens.push({
...
function appendAudit(
  ...
  const records = state.audit.filter(
    (record) => record.organizationId === organizationId,
  );
  const previous = records.at(-1);
  const sequence =
    previous === undefined ? anchor.sequence + 1 : previous.sequence + 1;
  const previousHash =
    previous === undefined ? anchor.hash : previous.recordHash;
...
function retain(state: TeamControlPlaneState, now: string): void {
  ...
  for (const organization of state.organizations) {
    state.evidence = newest(
      state.evidence,
      organization.id,
      MAX_EVIDENCE_PER_ORGANIZATION,
      (entry) => entry.uploadedAt,
    );
...
function pruneAudit(
  ...
  state.audit = state.audit.filter(
    (entry) =>
      entry.organizationId !== organizationId || retained.includes(entry),
  );
}

function newest<T extends { organizationId: string }>(
  entries: T[],
  organizationId: string,
  max: number,
  key: (entry: T) => string,
  ...
  return entries.filter(
    (entry) => entry.organizationId !== organizationId || allowed.has(entry),
  );
}

function validateTokenScope(
  value: unknown,
  state: Pick<TeamControlPlaneState, "projects" | "environments">,
  organizationId: string,
): TeamTokenScope[] {
  const scopes = array(
    value,
    "Service token scope",
    MAX_SERVICE_TOKEN_SCOPES,
  ).map((raw) => {
    const entry = object(raw, "Service token scope is invalid.");
    exactKeys(
      entry,
      ["projectId", "environmentId", "permissions"],
      "Service token scope",
    );
    const projectId = identifier(
      entry.projectId,
      "Service token scope project id",
    );
    const environmentId = identifier(
      entry.environmentId,
      "Service token scope environment id",
    );
    if (
      !state.projects.some(
        (project) =>
    ...
    const permissions = array(
      ...
      SERVICE_ACCOUNT_PERMISSIONS.length,
    ).map((permission) => {
      if (
        ...
        throw new TeamControlPlaneError(
          "Service token permissions are invalid.",
        );
      return permission;
    });
  ...
  if (scopes.length === 0)
    throw new TeamControlPlaneError("Service token scope is invalid.");
  unique(
    scopes.map((scope) => scopedKey(scope.projectId, scope.environmentId)),
    "Service token scope",
  );
  return scopes.sort((left, right) =>
    scopedKey(left.projectId, left.environmentId).localeCompare(
      scopedKey(right.projectId, right.environmentId),
    ),
  );
}
...
function canReadEvidence(
  ...
  return (
    ...
    serviceTokenScopes(state, actor).some(
      (scope) =>
        scope.projectId === evidence.projectId &&
...
function isServiceActor(
  actor: TeamActor,
): actor is Extract<TeamActor, { actorType: "service_account" }> {
  return actor.actorType === "service_account";
}

function serviceTokenScopes(
  state: TeamControlPlaneState,
  actor: Extract<TeamActor, { actorType: "service_account" }>,
): TeamTokenScope[] {
...
function userActorId(actor: TeamActor): string {
  if (isServiceActor(actor))
    throw new TeamControlPlaneError(
      "Service accounts cannot impersonate a human member.",
    );
...
function actorReferenceIds(
  ...
  return new Set([
    ...
    ...serviceAccounts.map((account) =>
      scopedKey(account.organizationId, `service:${account.id}`),
    ),
  ]);
}

function actorReferenceExists(
  organizationId: string,
  ...
  return (
    members.some(
      (member) => member.organizationId === organizationId && member.id === id,
    ) ||
...
function rejectRawEvidence(report: EvidenceReport): void {
  ...
  const visit = (value: unknown, key?: string): void => {
    if (key !== undefined && prohibited.has(key.toLowerCase()))
      throw new TeamControlPlaneError(
        "Evidence artifact contains an unsafe raw field.",
      );
    if (typeof value === "string" && sanitizeSecretString(value) !== value)
      throw new TeamControlPlaneError(
        "Evidence artifact contains a secret-shaped value.",
      );
...
function inspectJson(
  ...
  if (typeof value === "string") {
    if (value.length > 2_000 || sanitizeSecretString(value) !== value)
      throw new TeamControlPlaneError(
        `${label} contains an unsafe string value.`,
      );
    return;
  }
  ...
  if (Array.isArray(value)) {
    ...
    for (const entry of value) inspectJson(entry, label, depth + 1, key);
    return;
  }
...
function validatePolicy(policy: GhostApiPolicy): void {
  ...
  if (
    ...
    throw new TeamControlPlaneError(
      "Team policy does not match GhostAPI policy schema v1.",
    );
}
...
function auditGenesisHash(organizationId: string): string {
...
function identifier(value: unknown, label: string): string {
  if (typeof value !== "string" || !IDENTIFIER.test(value))
    throw new TeamControlPlaneError(
      `${label} must be 1-64 lowercase letters, numbers, underscores, or hyphens.`,
    );
  return value;
}
...
function futureTimestamp(value: unknown, now: Date): string {
...
function object(value: unknown, message: string): Record<string, unknown> {
  if (!plainObject(value)) throw new TeamControlPlaneError(message);
  return value;
}
...
function runStatus(value: unknown): value is TeamEvidence["status"] {
  return (
    value === "unknown" ||
    value === "preparing" ||
    value === "running" ||
...
function evidenceMode(
  value: unknown,
): value is TeamEvidence["enforcement"]["mode"] {
  return (
    value === "linux-network-namespace" ||
    value === "proxy-guidance" ||
    value === "unknown"
  );
}
function isScopedPermission(value: string): value is TeamScopedPermission {
  return (
    READ_PERMISSIONS.includes(value as TeamScopedPermission) ||
    PUBLISH_PERMISSIONS.includes(value as TeamScopedPermission)
  );
}
