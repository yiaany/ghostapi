...
export type WorkloadBinding = Pick<
  WorkloadIdentity,
  "tenantId" | "projectId" | "environment" | "workloadId" | "workloadKind"
>;
export type CredentialActionReference = {
  actionId: string;
  actionHash: string;
...
export interface ActionReceiptVerifier {
  verify(
    reference: CredentialActionReference,
    workload: WorkloadIdentity,
  ): Promise<void>;
}
...
export interface BreakGlassAuthorizer {
  ...
  ): Promise<{ approvalId: string; approvedBy: string; reason: string }>;
}
...
export class CredentialBroker {
  ...
  async registerCredential(
    input: Omit<
      CredentialMetadata,
      "version" | "createdAt" | "rotatedAt" | "revokedAt" | "lastUsedAt"
    >,
    ...
    return this.mutate((state) => {
      if (
        state.credentials.some(
          (credential) =>
            credential.tenantId === candidate.tenantId &&
  ...
  async issueGrant(input: {
    ...
    return this.mutate(async (state, now) => {
      ...
      const normalOwnerMatch =
        credential.ownerWorkloadId === workload.workloadId &&
        credential.ownerWorkloadKind === workload.workloadKind;
      const breakGlass = normalOwnerMatch
        ? undefined
        : await this.breakGlassAuthorizer.authorize(
            input.breakGlassApproval,
            workload,
            request,
          );
      if (
        ...
        throw new CredentialBrokerError(
          "Break-glass grants must expire within five minutes.",
        );
      if (Date.parse(expiresAt) - Date.parse(now) > MAX_GRANT_TTL_MS)
        throw new CredentialBrokerError(
          "Credential grants must expire within fifteen minutes.",
        );
      if (
        Date.parse(expiresAt) > Date.parse(credential.expiresAt) ||
        Date.parse(expiresAt) > Date.parse(workload.expiresAt)
      )
        throw new CredentialBrokerError(
          "Credential grant expiry exceeds its credential or workload lifetime.",
        );
      const grant: CredentialGrant = {
        id: identifier(
          `grant-${randomUUID().replace(/-/g, "")}`,
          "Credential grant id",
        ),
  ...
  async executeServerSide(input: {
    ...
    grantId: string;
    request: CredentialAccessRequest;
  }): Promise<CredentialUseReceipt> {
    const workload = await this.authenticateWorkload(input.identity);
    const grantId = identifier(input.grantId, "Credential grant id");
    const request = validateAccessRequest(input.request);
    const started: CredentialExecutionStart | CredentialUseReceipt =
      await this.mutate(async (state, now) => {
        ...
        if (credential.version !== grant.credentialVersion)
          throw new CredentialBrokerError(
            "Credential grant was invalidated by rotation.",
          );
        if (
          ...
          throw new CredentialBrokerError(
            "Credential executor denies the requested scope.",
          );
        const prior = state.receipts.find(
          (receipt) => receipt.grantId === grant.id,
        );
        if (prior !== undefined) {
          if (prior.status === "executed") return clone(prior);
          throw new CredentialBrokerError(
            "Prior credential execution has no retry-safe outcome and will not retry automatically.",
          );
        }
        await this.actionReceiptVerifier.verify(request.action, workload);
        const receipt: CredentialUseReceipt = {
          id: identifier(
            `credential-use-${randomUUID().replace(/-/g, "")}`,
            "Credential use receipt id",
          ),
          grantId: grant.id,
          credentialId: credential.id,
          credentialVersion: credential.version,
    ...
    try {
      ...
      if (
        ...
        throw new CredentialBrokerError(
          "Credential vault returned invalid secret material.",
        );
    ...
    try {
      const result = await this.executor.execute({
        ...
        assertActive: async () =>
          this.assertExecutionActive(started.grant, input.identity),
      });
      ...
      throw new CredentialBrokerError(
        "Credential execution outcome is unknown and will not retry automatically.",
      );
    } finally {
      secret?.fill(0);
    }
  ...
  private async assertExecutionActive(
    ...
    const verifiedWorkload = validateWorkloadIdentity(
      await this.workloadVerifier.authenticate(identity),
      this.timestamp(),
    );
    ...
    await withFileLock(this.path, async () => {
      ...
      const credential = findCredential(
        state,
        verifiedWorkload.tenantId,
        current.credentialId,
      );
      assertCredentialUsable(credential, this.timestamp());
      if (
        credential.version !== current.credentialVersion ||
        credential.provider !== this.executor.provider ||
        !current.scopes.every((scope) => this.executor.supportsScope(scope))
      )
        throw new CredentialBrokerError(
          "Credential grant is no longer executable.",
        );
      if (
        state.receipts.find((receipt) => receipt.grantId === current.id)
          ?.status !== "executing"
      )
        throw new CredentialBrokerError(
          "Credential execution is no longer active.",
        );
    });
  }
  ...
  private async completeExecution(
    ...
    return this.mutate((state, now) => {
      ...
      if (receipt === undefined || receipt.status !== "executing")
        throw new CredentialBrokerError(
          "Credential execution receipt is not active.",
        );
      receipt.status = status;
      receipt.executedAt = now;
      if (status === "executed") {
        receipt.providerRequestId = identifier(
          providerRequestId,
          "Provider request id",
        );
        findCredential(
  ...
  async listOrphanedCredentials(): Promise<CredentialMetadata[]> {
    ...
    const results = await Promise.all(
      candidates.map(async (credential) => {
        const active =
          this.workloadVerifier.isActive === undefined
            ? false
    ...
    return results.filter(
      (credential): credential is CredentialMetadata =>
  ...
  async inspectCredential(
    ...
    const credential = findCredential(
      await this.read(),
      identifier(tenantIdValue, "Credential tenant id"),
      identifier(credentialIdValue, "Credential id"),
    );
    return clone(credential);
  }
  ...
  private async authenticateWorkload(
    identity: unknown,
  ...
  private async read(): Promise<CredentialBrokerState> {
    ...
    if (!info.isFile() || info.isSymbolicLink())
      throw new CredentialBrokerError(
        "Credential broker store must be a regular non-symlink file.",
      );
    const source = await readFile(this.path, "utf8");
    if (Buffer.byteLength(source, "utf8") > MAX_STORE_BYTES)
      throw new CredentialBrokerError(
        `Credential broker store exceeds ${MAX_STORE_BYTES} bytes.`,
      );
    try {
      ...
      throw new CredentialBrokerError(
        "Credential broker store is not valid JSON.",
      );
    }
  }

  private async mutate<T>(
    operation: (state: CredentialBrokerState, now: string) => T | Promise<T>,
  ): Promise<T> {
...
export function createTestActionReceiptVerifier(
  reference: CredentialActionReference,
): { verifier: ActionReceiptVerifier; calls: number } {
  let calls = 0;
  return {
    verifier: {
      async verify(candidate: CredentialActionReference): Promise<void> {
        ...
        if (!sameAction(reference, candidate))
          throw new CredentialBrokerError(
            "Action receipt does not authorize this credential use.",
          );
      },
    },
    get calls() {
    ...
    vault: {
      ...
      async readSecret(vaultRef: string): Promise<Uint8Array> {
        const value = values.get(
          reference(vaultRef, "Credential vault reference"),
        );
        if (value === undefined)
          throw new CredentialBrokerError(
            "Credential vault reference was not found.",
          );
...
export function createTestCredentialExecutor(): {
  executor: CredentialExecutor;
  executions: Array<{ grantId: string; secretLength: number }>;
} {
  ...
  return {
    executor: {
      ...
      async execute(input): Promise<{ providerRequestId: string }> {
        if (
          input.grant.audience !== "ghostapi-server" ||
          !input.grant.scopes.every((scope) => scope === "test.execute") ||
          input.secret.byteLength === 0
        )
          throw new CredentialBrokerError(
            "Test provider rejected credential execution.",
          );
...
export function createTestWorkloadIdentityProvider(
  options: { now?: () => Date } = {},
): {
  verifier: WorkloadIdentityVerifier;
  issue(
    input: Omit<WorkloadIdentity, "schemaVersion" | "kind">,
  ): WorkloadIdentity;
  ...
  return {
    verifier: {
      async authenticate(identity: unknown): Promise<WorkloadIdentity> {
        if (
          ...
          throw new CredentialBrokerError(
            "Workload identity is not authenticated.",
          );
    ...
    issue(input): WorkloadIdentity {
      const workload = validateWorkloadIdentity(
        { schemaVersion: 1, kind: "ghostapi.workload-identity", ...input },
        timestampFrom(now),
      );
    ...
    deactivate(workloadId: string): void {
      ...
      for (const key of [...active])
        if (key.endsWith(`:${id}`)) active.delete(key);
    },
  };
}

export function createTestBreakGlassAuthorizer(
  options: { now?: () => Date } = {},
): {
  authorizer: BreakGlassAuthorizer;
  issue(
    input: Omit<BreakGlassApproval, "schemaVersion" | "kind">,
  ): BreakGlassApproval;
} {
  const now = options.now ?? (() => new Date());
  const issued = new WeakSet<object>();
  return {
    ...
    issue(input): BreakGlassApproval {
      const approval = validateBreakGlassApproval(
        { schemaVersion: 1, kind: "ghostapi.break-glass-approval", ...input },
        timestampFrom(now),
      );
...
function validateState(value: unknown): CredentialBrokerState {
  ...
  exactKeys(
    state,
    ["schemaVersion", "credentials", "grants", "receipts"],
    "Credential broker store",
  );
  if (state.schemaVersion !== SCHEMA_VERSION)
    throw new CredentialBrokerError(
      "Unsupported credential broker schema version.",
    );
  const credentials = array(
    state.credentials,
    "Credentials",
    MAX_CREDENTIALS,
  ).map(validateCredentialMetadata);
  unique(
    credentials.map((credential) => `${credential.tenantId}:${credential.id}`),
    "Credential ids",
  );
  const grants = array(state.grants, "Credential grants", MAX_GRANTS).map(
    validateGrant,
  );
  unique(
    grants.map((grant) => grant.id),
    "Credential grant ids",
  );
  for (const grant of grants) {
    ...
    if (
      ...
      throw new CredentialBrokerError(
        "Credential grant references an invalid credential.",
      );
  }
  ...
  unique(
    ...
    "Credential use receipt ids",
  );
  unique(
    receipts.map((receipt) => receipt.grantId),
    "Credential use receipt grants",
  );
  for (const receipt of receipts) {
    ...
    if (
      ...
      throw new CredentialBrokerError(
        "Credential use receipt references an invalid grant.",
      );
  }
  return { schemaVersion: 1, credentials, grants, receipts };
}
...
function validateCredentialMetadata(value: unknown): CredentialMetadata {
  ...
  return {
    ...
    ownerWorkloadId: identifier(
      metadata.ownerWorkloadId,
      "Credential owner workload id",
    ),
  ...
  return {
    ...
    ...(grant.revokedAt === undefined
      ...
      : {
          revokedAt: timestamp(
            grant.revokedAt,
            "Credential grant revocation time",
          ),
        }),
  };
}
...
function validateReceipt(value: unknown): CredentialUseReceipt {
  ...
  exactKeys(
    ...
    "Credential use receipt",
    ["providerRequestId", "failureCode"],
  );
  if (
    ...
    throw new CredentialBrokerError(
      "Credential use receipt status is invalid.",
    );
  if (
    receipt.status === "executing" &&
  ...
  return {
    ...
    status: receipt.status as CredentialUseReceipt["status"],
    ...(receipt.providerRequestId === undefined
      ? {}
      : {
    ...
    ...(receipt.failureCode === undefined
      ? {}
      : {
          failureCode: receipt.failureCode as
            "execution_not_started" | "unknown_outcome",
        }),
...
function validateAccessRequest(value: unknown): CredentialAccessRequest {
  ...
  exactKeys(
    request,
    ["credentialId", "provider", "scopes", "audience", "action"],
    "Credential access request",
  );
  if (request.audience !== "ghostapi-server")
    throw new CredentialBrokerError(
      "Credential access request audience is invalid.",
    );
  return {
...
function validateBreakGlassApproval(
  value: unknown,
  now: string,
): BreakGlassApproval {
  ...
  if (
    ...
    throw new CredentialBrokerError(
      "Break-glass approval is expired or exceeds five minutes.",
    );
  return {
    schemaVersion: 1,
    kind: "ghostapi.break-glass-approval",
...
function breakGlassRecord(value: unknown): {
  approvalId: string;
  approvedBy: string;
  reason: string;
} {
...
function actionReference(value: unknown): CredentialActionReference {
  ...
  exactKeys(
    action,
    ["actionId", "actionHash", "actionReceiptHash"],
    "Credential action reference",
  );
  return {
    ...
    actionReceiptHash: hash(
      action.actionReceiptHash,
      "Credential action receipt hash",
    ),
  };
}

function assertCredentialUsable(
  credential: CredentialMetadata,
  now: string,
): void {
  if (credential.revokedAt !== undefined)
...
function assertRequestMatchesCredential(
  request: CredentialAccessRequest,
  credential: CredentialMetadata,
  workload: WorkloadIdentity,
): void {
  if (
    ...
    throw new CredentialBrokerError(
      "Credential request crosses a tenant, project, environment, or provider boundary.",
    );
  if (
    !request.scopes.every((scope) => credential.allowedScopes.includes(scope))
  )
    throw new CredentialBrokerError(
      "Credential request exceeds the credential scope.",
    );
}
...
function assertGrantUsable(
  ...
  if (
    grant.tenantId !== workload.tenantId ||
    ...
    throw new CredentialBrokerError(
      "Credential grant does not belong to this workload subject and run.",
    );
  if (
    grant.credentialId !== request.credentialId ||
    ...
    throw new CredentialBrokerError(
      "Credential request differs from the granted action.",
    );
}
...
function scopes(value: unknown, label: string): string[] {
  if (!Array.isArray(value) || value.length < 1 || value.length > MAX_SCOPES)
    throw new CredentialBrokerError(
      `${label} must be a bounded non-empty list.`,
    );
...
function sameAction(
  left: CredentialActionReference,
  right: CredentialActionReference,
): boolean {
  return (
